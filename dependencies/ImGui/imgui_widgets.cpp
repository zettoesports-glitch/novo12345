...
//-------------------------------------------------------------------------
bool ImGui::ColorEdit3(const char* label, unsigned long* col, ImGuiColorEditFlags flags)
{
...
bool ImGui::ColorEdit4(const char* label, unsigned long* CLRREFF, ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    float col[4];
    col[0] = ((*CLRREFF) & 0xff) / 255.f;
    col[1] = (((*CLRREFF) >> 8) & 0xff) / 255.f;
    col[2] = (((*CLRREFF) >> 16) & 0xff) / 255.f;
    col[3] = (((*CLRREFF) >> 24) & 0xff) / 255.f;
    ...
    unsigned char colr = static_cast<unsigned char>(col[0] * 255.f);

    unsigned char colg = static_cast<unsigned char>(col[1] * 255.f);

    unsigned char colb = static_cast<unsigned char>(col[2] * 255.f);

    *CLRREFF = (unsigned long)((255 << 24) | (colr) | (colg << 8) | (colb << 16));
...
bool ImGui::SelectableFunc(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, void (*func)(ImVec2, ImVec2, const char*))
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
