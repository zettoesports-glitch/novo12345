...
CLoginWin::CLoginWin()
{
	...
	_RenderFrameY = 0;
	m_PendingSave = false;
	memset(m_PendingID, 0, sizeof(m_PendingID));
	memset(m_PendingPass, 0, sizeof(m_PendingPass));
...
#ifdef SAVE_ACCOUNT_SYSTEM
					// Do NOT save here — wait for server success (OnLoginSuccess).
					// Wrong password / blocked / not exist must never enter the list.
					ClearPendingSave();
					// Save param = checkbox "remember" was on when pressing OK
					if (Save && gmProtect->remember_account)
					{
						m_PendingSave = true;
						strncpy_s(m_PendingID, szID, MAX_ID_SIZE);
						strncpy_s(m_PendingPass, szPass, MAX_PASSWORD_SIZE);
					}
#endif // SAVE_ACCOUNT_SYSTEM
...
void CLoginWin::ClearPendingSave()
{
	m_PendingSave = false;
	memset(m_PendingID, 0, sizeof(m_PendingID));
	memset(m_PendingPass, 0, sizeof(m_PendingPass));
...
void CLoginWin::OnLoginSuccess()
{
	// Only real accounts (server accepted login) and only if user asked to remember
	if (!gmProtect->remember_account || !m_PendingSave)
	{
		ClearPendingSave();
		return;
	}

	if (m_PendingID[0] == '\0' || m_PendingPass[0] == '\0')
	{
		ClearPendingSave();
		return;
	}

	SaveAccount(m_PendingID, m_PendingPass);
	ClearPendingSave();
}
...
void CLoginWin::SaveAccount(std::string NameId, std::string PassID)
{
	...
	if (NameId.empty() || PassID.empty())
		return;

	// Trim — no empty / space-only accounts
	while (!NameId.empty() && (NameId.back() == ' ' || NameId.back() == '\t'))
		NameId.pop_back();
	while (!NameId.empty() && (NameId.front() == ' ' || NameId.front() == '\t'))
		NameId.erase(NameId.begin());

	if (NameId.empty())
		return;

	// Already saved? Update password once (same slot) — never duplicate
	for (auto& account : stAccountMacro)
	{
		if (_stricmp(account.NameId.c_str(), NameId.c_str()) == 0)
		{
			if (account.PassID != PassID)
			{
				account.PassID = PassID;
				ReloadAccount(false, true, true);
			}
			// Already in list → "apenas uma vez" (no second entry)
			return;
		}
	}

	// New real account — only if free slot
