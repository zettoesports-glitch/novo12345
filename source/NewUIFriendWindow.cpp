...
SEASON3B::CNewUIFriendWindow::CNewUIFriendWindow()
	: m_pNewUIMng(NULL), m_pFriendWindowMgr(NULL)
{
}
...
bool SEASON3B::CNewUIFriendWindow::UpdateMouseEvent()
{
	if (m_pFriendWindowMgr)
	{
		...
		if (pMainWnd)
		{
			...
			if (pMainWnd->GetPosition_y() > Position_In_The_Down(221))
			{
				pMainWnd->SetPosition(pMainWnd->GetPosition_x(), Position_In_The_Down(221));
			}
