...
void CMixItem::SetItem(ITEM* pItem, DWORD dwMixValue)
{
	...
	if (m_bCanStack != TRUE && GMProtect->CheckItemStack(pItem->Type, m_iLevel))
	{
		m_bCanStack = TRUE;
	}
...
BOOL CMixRecipes::IsMixSource(ITEM* pItem)
{
	...
	{
		for (int j = 0; j < (*iter)->m_iNumMixSources; ++j)
		{
...
BOOL CMixRecipes::CheckRecipeSub(std::vector<MIX_RECIPE*>::iterator iter, int iNumMixItems, CMixItem* pMixItems)
{
	...
	WORD iMixID = (*iter)->m_iMixID;

	if (g_MixRecipeMgr.GetMixInventoryType() == 0 && ((iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL1 && gmProtect->max_item_level_on < 10)
		|| (iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL2 && gmProtect->max_item_level_on < 11)
		|| (iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL3 && gmProtect->max_item_level_on < 12)
		|| (iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL4 && gmProtect->max_item_level_on < 13)
		|| (iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL5 && gmProtect->max_item_level_on < 14)
		|| (iMixID == CHAOS_MIX_PLUS_ITEM_LEVEL6 && gmProtect->max_item_level_on < 15)
#ifdef SHUTDOWN_LEVEL_WING3
		|| ((iMixID == CHAOS_MIX_FEATHER_OF_CONDOR || iMixID == CHAOS_MIX_WING4) && gmProtect->shutdown_level_wing3 != 0)
...
int CMixRecipes::CheckRecipeSimilarity(int iNumMixItems, CMixItem* pMixItems)
{
	if (iNumMixItems == 0 && m_Recipes.size() == 1)
	{
		...
		for (int i = 0; i < (*m_Recipes.begin())->m_iNumMixSources; ++i)
		{
	...
	for (std::vector<MIX_RECIPE*>::iterator iter = m_Recipes.begin(); iter != m_Recipes.end(); ++iter)
	{
		...
		memset(m_iMixSourceTest, 0, sizeof(int) * MAX_MIX_SOURCES);
		for (int i = 0; i < (*iter)->m_iNumMixSources; ++i)
		...
		if (iSimilarityPoint > iMostSimiliarityPoint || (iSimilarityPoint == iMostSimiliarityPoint && iSimilarityPoint > 0 && m_iCurMixIndex == (*iter)->m_iMixIndex + 1))
		{
...
int CMixRecipes::GetSourceName(int iItemNum, unicode::t_char* pszNameOut, int iNumMixItems, CMixItem* pMixItems)
{
	...
	if (GetMostSimilarRecipe() == NULL)
		return MIX_SOURCE_ERROR;
	if (iItemNum >= GetMostSimilarRecipe()->m_iNumMixSources)
		return MIX_SOURCE_ERROR;
...
void CMixRecipes::CalcMixRate(int iNumMixItems, CMixItem* pMixItems)
{
	...
	if (gCB_GetMixRate)
	{
		if (gCB_GetMixRate->ShowRateInfo)
		{
...
#if(CB_GETMIXRATE)
	if (g_MixRecipeMgr.RealZen != -1) m_dwRequiredZen = g_MixRecipeMgr.RealZen;
#endif
}
