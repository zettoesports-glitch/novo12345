...
int TurnAngle(int iTheta, int iHeading, int maxTURN)
{
	...
	if (iTheta > iHeading)
	{
		if (Delta < abs((iHeading + 360) - iTheta))
			iChange = -min(maxTURN, Delta);
		else
			iChange = min(maxTURN, Delta);
	...
	if (iTheta < iHeading)
	{
		if (Delta < abs((iTheta + 360) - iHeading))
			iChange = min(maxTURN, Delta);
		else
			iChange = -min(maxTURN, Delta);
		//else iTheta = iHeading;
...
float TurnAngle2(float angle, float a, float d)
{
	...
	if (angle < 180.f)
	{
		...
		if (a >= angle + d && (a < angle + 180.f))
			angle += d;
		else if (aa >= 0 && (a >= angle + 180.f || a < aa))
			angle -= d;
	...
	else
	{
		...
		if (a < angle - d && (a >= angle - 180.f))
			angle -= d;
		else if (aa < 360.f && (a < angle - 180.f || a >= aa))
			angle += d;
...
float FarAngle(float angle1, float angle2, bool abs)
{
	...
	if (angle1 < 180.f)
	{
		if (angle2 >= angle1 + 180.f)
			d = (360.f - angle2 + angle1);
	}
	else
	{
		if (angle2 < angle1 - 180.f)
			d = (360.f - angle1 + angle2);
	}
	if (abs == true)
		if (d < 0.f)
			d = -d;
...
float MoveHumming(vec3_t Position, vec3_t Angle, vec3_t TargetPosition, float Turn, bool Fvg)
{
	vec3_t Range;
	float TargetAngle, Distance;

	TargetAngle = CreateAngle(Position[0], Position[1], TargetPosition[0], TargetPosition[1]);

	if (Fvg == true)
		Angle[2] = TurnAngle2(Angle[2], TargetAngle, timefac(Turn));
	else
	...
	if (Fvg == true)
		Angle[0] = TurnAngle2(Angle[0], TargetAngle, timefac(Turn));
	else
...
void MovePosition(vec3_t Position, vec3_t Angle, vec3_t Speed, bool Fvg)
{
	float Matrix[3][4];
	...
	if (Fvg == true)
	{
		Position[0] += timefac(Velocity[0]);
		Position[1] += timefac(Velocity[1]);
		Position[2] += timefac(Velocity[2]);
	}
	else
	{
...
void SetAction_Fenrir_Run(CHARACTER* c, OBJECT* o)
{
	...
	{
		if (c->GetBaseClass() == CLASS_ELF)
			SetAction(o, PLAYER_FENRIR_RUN_TWO_SWORD_ELF);
		else if (c->GetBaseClass() == CLASS_DARK)
...
#endif //PBG_ADD_NEWCHAR_MONK_ANI
		if (c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1 && c->Weapon[0].Type != MODEL_BOW + 15 && c->Weapon[1].Type != MODEL_BOW + 7)
			SetAction(o, PLAYER_FENRIR_WALK_WEAPON_TWO);
		else if (c->Weapon[0].Type != -1 && c->Weapon[1].Type == -1)
			SetAction(o, PLAYER_FENRIR_WALK_WEAPON_RIGHT);
		else if (c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1 && c->Weapon[1].Type == MODEL_BOW + 7)
			SetAction(o, PLAYER_FENRIR_WALK_WEAPON_RIGHT);
		else if (c->Weapon[0].Type == -1 && c->Weapon[1].Type != -1)
			SetAction(o, PLAYER_FENRIR_WALK_WEAPON_LEFT);
		else if (c->Weapon[0].Type != -1 && c->Weapon[1].Type != -1 && c->Weapon[0].Type == MODEL_BOW + 15)
			SetAction(o, PLAYER_FENRIR_WALK_WEAPON_LEFT);
		else
...
void MoveHead(CHARACTER* c)
{
	...
	if (o->CurrentAction != MONSTER01_DIE)
	{
		...
		{
			LookAtTarget(o, gmCharacters->GetCharacter(c->TargetCharacter));
		}
		else
		{
			o->HeadTargetAngle[0] = 0.f;
...
bool PathFinding2(int sx, int sy, int tx, int ty, PATH_t* a, float fDistance, int iDefaultWall)
{
	...
	{
		CHARACTER* pCharacter = gmCharacters->GetCharacter(TargetNpc);

		if (pCharacter)
		{
			if ((pCharacter->Object.Type >= MODEL_CRYWOLF_ALTAR1 && pCharacter->Object.Type <= MODEL_CRYWOLF_ALTAR5))
			{
				Value = true;
			}
		}
...
void CalcFPS()
{
	...
	if (!timeinit)
	{
		...
		start = timeGetTime();
		timeinit = 1;
		last = timeGetTime(); // found in winmm.
	}
	...
	double dif = (double)((double)current - (double)start) / CLOCKS_PER_SEC;
