...
namespace SEASON3B
{
	...
	private:
		void RenderFrame();
		void RenderTextInfo();
		void RenderButton(float posx, float posy, float width, float height, const char* psText);
	private:
		int					m_TargetIndex;
		int					m_SourceIndex;
	...
	inline void CNewUIMyShopInventory::ChangeTitle( unicode::t_string& titletext )
	{
		if(m_EditBox)
