...
// ---- Debug de culling (log em arquivo, estilo perf_debug.log) ----
...
inline bool MapFileDecrypt(std::vector<BYTE>& buff)
{
	if (buff.size() == 0)
	{
		return false;
	...
	BYTE* block = buff.data();

	//memcpy(block, buff.data(), data_size);

	for (int i = 0; i < Size; ++i)
	{
		BYTE _block = block[i];
		block[i] = (block[i] ^ byMapXorKey[i % 16]) - (BYTE)wMapKey;
		wMapKey = _block + 0x3D;
		wMapKey = wMapKey & 0xFF;
	}
	return true;
