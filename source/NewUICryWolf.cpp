...
bool SEASON3B::CNewUICryWolf::Render()
{
	...
	g_pCryWolfInterface->Render(frameX + Main[0], frameY + Main[1], Main[2], Main[3], 0.f, 0.f, Main[4], Main[5], 3);
	...
	for (int ia = 0; ia < 5; ia++)
	{
		...
		if (Use == CRYWOLF_ALTAR_STATE_CONTRACTED)
		{
			if (State == 1)
			{
				g_pCryWolfInterface->Render(frameX + Number[ia][0], frameY + Number[ia][1], Number[ia][2], Number[ia][3], 0.f, 0.f, Number[ia][4], Number[ia][5], 23);
			}
			else if (State == 2)
			{
				g_pCryWolfInterface->Render(frameX + Number[ia][0], frameY + Number[ia][1], Number[ia][2], Number[ia][3], 0.f, 0.f, Number[ia][4], Number[ia][5], 24);
			}
			else
			{
				g_pCryWolfInterface->Render(frameX + Number[ia][0], frameY + Number[ia][1], Number[ia][2], Number[ia][3], 0.f, 0.f, Number[ia][4], Number[ia][5], 25);
			}
		}
		else
		{
			if (State == 1)
			{
				g_pCryWolfInterface->Render(frameX + Number[ia][0], frameY + Number[ia][1], Number[ia][2], Number[ia][3], 0.f, 0.f, Number[ia][4], Number[ia][5], 7);
			}
			else if (State == 2)
			{
				g_pCryWolfInterface->Render(frameX + Number[ia][0], frameY + Number[ia][1], Number[ia][2], Number[ia][3], 0.f, 0.f, Number[ia][4], Number[ia][5], 8);
			}
		}
	}
	...
	if (Dark_elf_Num == 0)
	{
		g_pCryWolfInterface->Render(frameX + Dark_Elf_Icon[0], frameY + Dark_Elf_Icon[1], Dark_Elf_Icon[2], Dark_Elf_Icon[3], 0.f, 0.f, Dark_Elf_Icon[4], Dark_Elf_Icon[5], 6);
	}
	else
	{
		g_pCryWolfInterface->Render(frameX + Dark_Elf_Icon[0], frameY + Dark_Elf_Icon[1], Dark_Elf_Icon[2], Dark_Elf_Icon[3], 0.f, 0.f, Dark_Elf_Icon[4], Dark_Elf_Icon[5], 5);
	}

	g_pCryWolfInterface->Render(frameX + 538, frameY + 392, 104, 37, 0.f, 0.f, 104.f / 128.f, 36.f / 64.f, 12);
	...
	g_pRenderText->RenderText(frameX + 582, frameY + 359, Text, 0, 0, RT3_WRITE_CENTER);
	...
	if (View_Bal == true)
	{
		...
		{
			g_pCryWolfInterface->Render(frameX + Val_Icon[0], frameY + Val_Icon[1], Val_Icon[2], Val_Icon[3], 0.f, 0.f, Val_Icon[4], Val_Icon[5], 4);

			wsprintf(Text, GlobalText[1949]);

			g_pRenderText->RenderText(frameX + 600, frameY + 380, Text, 0, 0, RT3_WRITE_CENTER);

			float Hp = ((67.f / 100.f) * (float)Val_Hp);
			float nx = ((68.f / 100.f) * (float)Val_Hp);

			g_pCryWolfInterface->Render(frameX + 548, frameY + 388, nx, 8, 0.f, 0.f, Hp / 128.f, 8.f / 8.f, 1);
		}
	}
	if (View_Bal == false)
	{
	...
	if (m_bTimeStart == true && m_CrywolfState == CRYWOLF_STATE_START)
	{
		...
		if (m_iMinute < 10)
		{
			RenderNumber2D(frameX + 510 + 60, frameY + 384 + 18, 0, 14, 14);
		}
		RenderNumber2D(frameX + 510 + 70, frameY + 384 + 18, m_iMinute, 14, 14);
		if (m_iSecond / 1000 < 10)
		{
			RenderNumber2D(frameX + 520 + 77, frameY + 384 + 18, 0, 14, 14);
		}
		RenderNumber2D(frameX + 520 + 87, frameY + 384 + 18, m_iSecond / 1000, 14, 14);
	...
	{
		RenderNumber2D(frameX + 510 + 60, frameY + 384 + 18, 0, 14, 14);
		RenderNumber2D(frameX + 510 + 70, frameY + 384 + 18, 0, 14, 14);
		RenderNumber2D(frameX + 520 + 77, frameY + 384 + 18, 0, 14, 14);
		RenderNumber2D(frameX + 520 + 87, frameY + 384 + 18, 0, 14, 14);
	}

	int HpS = 100 - m_StatueHP;
	float Hp = ((88.f / 100.f) * (float)HpS);
	float nx = ((89.f / 100.f) * (float)HpS);
	RenderImage(IMAGE_MVP_INTERFACE + 9, frameX + 548 + nx, frameY + 323, 89.f - nx, 30, Hp / 128.f, 0.f, ((((88.f / 100.f) * (float)(100.f - HpS))) / 128.f), 29.f / 32.f);
	...
	if (Message_Box == 1)
	{
		...
		if (SEASON3B::CheckMouseIn(No, 250, 54, 30))
		{
			if (Button_Down == 1)
			{
		...
		if (SEASON3B::CheckMouseIn(Yes, 250, 54, 30))
		{
			if (Button_Down == 2)
			{
	...
	else if (Message_Box == 2)
	{
		...
		if (SEASON3B::CheckMouseIn(290, 250, 54, 30))
		{
			if (Button_Down == 3)
			{
