...
namespace
{
	bool s_bPhotoModeHideHud = false;
	const int kPhotoKeepMax = 8;
	CNewUIObj* s_pPhotoModeKeepUI[kPhotoKeepMax] = {};
	int s_nPhotoModeKeep = 0;

	bool IsPhotoModeKeepUI(CNewUIObj* p)
	{
		if (!p) return false;
		for (int i = 0; i < s_nPhotoModeKeep; ++i)
		{
			if (s_pPhotoModeKeepUI[i] == p)
				return true;
		}
		return false;
	}
...
bool SEASON3B::IsPhotoModeHideHud()
{
	return s_bPhotoModeHideHud;
}

void SEASON3B::SetPhotoModeHideHud(bool bHide)
{
	s_bPhotoModeHideHud = bHide;
}

void SEASON3B::RegisterPhotoModeKeepUI(CNewUIObj* pUI)
{
	if (!pUI) return;
	for (int i = 0; i < s_nPhotoModeKeep; ++i)
	{
		if (s_pPhotoModeKeepUI[i] == pUI)
			return;
	}
	if (s_nPhotoModeKeep < kPhotoKeepMax)
		s_pPhotoModeKeepUI[s_nPhotoModeKeep++] = pUI;
}
...
bool SEASON3B::CNewUIManager::UpdateMouseEvent()
{
	...
	for(; vi != m_vecUI.end(); vi++)
	{
		if((*vi)->IsVisible()) 
		{
			// Photo mode: only registered keep UIs receive mouse (Options / ESC menu)
			if (s_bPhotoModeHideHud && !IsPhotoModeKeepUI(*vi))
				continue;
...
bool SEASON3B::CNewUIManager::Render()
{
	...
	for(; vi != m_vecUI.end(); vi++)
	{
		if((*vi)->IsVisible()) 
		{
			// Photo mode: hide entire HUD — only keep UIs draw (Options / ESC menu)
			if (s_bPhotoModeHideHud && !IsPhotoModeKeepUI(*vi))
				continue;
			(*vi)->Render();
		}
