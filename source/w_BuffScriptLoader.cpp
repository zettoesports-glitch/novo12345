...
BuffScriptLoader::BuffScriptLoader()
{
	...
	std::string filename = "data\\local\\BuffEffect.bmd";
#else
	std::string filename = "data\\local\\" + g_strSelectedML + "\\BuffEffect.bmd";
...
bool BuffScriptLoader::Load(const std::string& pchFileName)
{
	...
	if(fp != NULL)
	{
		...
		if ( dwCheckSum != GenerateCheckSum2( Buffer, structsize*listsize, 0xE2F1))
		{
			...
			gwinhandle->SendWindowMessage(Text, false);
	...
	{
		char Text[256];
		sprintf_s( Text, "%s - File not exist.", pchFileName.c_str() );
		g_ErrorReport.Write( Text );
