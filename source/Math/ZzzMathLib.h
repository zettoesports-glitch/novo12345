...
inline void Vector(float a, float b, float c, vec3_t vOut)
{
	vOut[0] = a;
	vOut[1] = b;
	vOut[2] = c;
...
inline void VectorAdd(const vec3_t a, const vec3_t b, vec3_t vOut)
{
	if (a && b && vOut)
	{
		vOut[0] = (a[0] + b[0]);
...
inline void VectorSubtract(const vec3_t a, const vec3_t b, vec3_t vOut)
{
	if (a && b &&vOut)
	{
		vOut[0] = (a[0] - b[0]);
...
inline void VectorScale(const vec3_t vIn, float b, vec3_t vOut)
{
	if (vIn && vOut)
	{
		vOut[0] = (b * vIn[0]);
...
void VectorCopy(const vec3_t vIn, vec3_t vOut, int size = 3);
void interpolatesum(vec3_t v_out, vec3_t b);
void interpolatesum2(vec3_t a, vec3_t b, vec3_t v_out);
