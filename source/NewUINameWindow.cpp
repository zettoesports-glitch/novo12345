...
{
	// Show all player names only when Options → Nome is on (default off).
	// Players only — no monsters. Hover/select still shows the focused target.
	const bool showPlayerNames =
		(g_pOption && g_pOption->IsShowPlayerName()) || g_bGMObservation;
	...
	// Always show name of current mouse target (player / monster / item)
