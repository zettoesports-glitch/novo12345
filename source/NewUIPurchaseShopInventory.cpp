...
bool SEASON3B::CNewUIPurchaseShopInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
...
#ifdef test_main_dev
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, m_Pos.x + 22, m_Pos.y + 90, 8, 4))
	{
	...
	m_Button->ChangeButtonImgState(true, BITMAP_btn_exit_00, false);
	m_Button->ChangeButtonInfo(m_Pos.x + 22, m_Pos.y + 384, 20, 20);
	m_Button->CurSize(69.f, 72.f);
#else
	...
	if (gmProtect->checkold_school())
	{
		...
		SetButtonInterface(m_Button, m_Pos.x, m_Pos.y, 1);
	}
#endif // test_main_dev
...
void SEASON3B::CNewUIPurchaseShopInventory::RenderTextInfo()
{
	...
	if (gmProtect->checkold_school())
	{
		...
		RenderText(m_TitleText.c_str(), m_Pos.x + 20.0, m_Pos.y + 65.0, 150.0, 24.0, RGBA(0, 255, 0, 255), 0x00000000, RT3_SORT_CENTER, g_hFontBold);
	}
	else
	{
		RenderText(GlobalText[1102], m_Pos.x, m_Pos.y + 15, 190, 0, 0xFF49B0FF, 0x00000000, RT3_SORT_CENTER);
...
void SEASON3B::CNewUIPurchaseShopInventory::EventOrderWindows(double WindowsX, double WindowsY)
{
