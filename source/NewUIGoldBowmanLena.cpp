...
void CNewUIGoldBowmanLena::OpenningProcess()
{
}
...
bool CNewUIGoldBowmanLena::UpdateMouseEvent()
{
	...
	{
		int registerItem = g_pMyInventory->GetItemCount(ITEM_POTION + 21, 0);

		if (registerItem != 0)
		{
			int index = g_pMyInventory->FindItemIndex(ITEM_POTION + 21, 0);
...
void CNewUIGoldBowmanLena::Render3D()
{
    ...
    float x = 640.f-120.f;
    float y = 200.f;
	float Width  = (float)GMItemMng->find(Type)->Width  * INVENTORY_SCALE;
	float Height = (float)GMItemMng->find(Type)->Height * INVENTORY_SCALE;
