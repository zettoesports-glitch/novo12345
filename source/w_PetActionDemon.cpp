...
{
	float fRad = (3.14f / 10000.0f) * fmodf(tick , 10000);
...
bool PetActionDemon::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	if (b != NULL)
	{
		...
		vec3_t Light, Light2;
		float fRad = ((3.14f / 2500.0f) * fmodf(tick , 25000));
		...
		for (int i = 0; i < 9; i++)
		{
			...
			case 4:
			case 5:
				CreateParticleSync(BITMAP_CLUD64, Position, obj->Angle, Light, 11, 0.5f);
				break;
			case 6:
			case 7:
				...
				break;
			case 8:
				CreateParticleSync(BITMAP_FIRE_HIK3, Position, obj->Angle, Light, 1, 0.4f);
