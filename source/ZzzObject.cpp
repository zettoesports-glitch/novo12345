...
void RenderObjects()
{
	...
	static float s_objMaxDist = -1.f;
	if (s_objMaxDist < 0.f)
	{
		s_objMaxDist = (float)GetPrivateProfileIntA("Performance2", "ObjectMaxDistance", 2500, ".\\config.ini");
		if (s_objMaxDist < 500.f) s_objMaxDist = 500.f;
		if (s_objMaxDist > 7000.f) s_objMaxDist = 7000.f;
	}

	static float s_renderDist = -1.f;
	if (s_renderDist < 0.f)
	{
		s_renderDist = (float)GetPrivateProfileIntA("Performance2", "RenderDistance", 2800, ".\\config.ini");
	}
	...
	for (int i = 0; i < 16; i++)
	{
		for (int j = 0; j < 16; j++)
		{
			...
			if (o != NULL)
			{
				...
				{
					CullingDebugObjectTest(ob->Visible);
					bool inDist = true;
					if (SceneFlag != LOG_IN_SCENE && SceneFlag != CHARACTER_SCENE)
					{
						float dx = o->Position[0] - CameraPosition[0];
						float dy = o->Position[1] - CameraPosition[1];
						float distSq = dx * dx + dy * dy;
						inDist = distSq <= s_objMaxDist * s_objMaxDist;
					}

					if (FrustrumEnable && inDist)
					{
						...
						if (ob->Visible || CameraTopViewEnable)
						{
							if (o->Live)
							{
								o->Visible = TestFrustrum2D((o->Position[0] * 0.01f), (o->Position[1] * 0.01), minf(o->CollisionRange + range, FrustrumRange));
							}
						}
					}
					else if (inDist && (ob->Visible || CameraTopViewEnable))
					{
						if (o->Live)
						{
							o->Visible = TestFrustrum2D((o->Position[0] * 0.01f), (o->Position[1] * 0.01), minf(o->CollisionRange + range, FrustrumRange));
				...
				else
				{
					...
					if (o != NULL)
					{
						...
						for (FrustrumEnable = ForceRenderAfterCharacter(o); ; FrustrumEnable = ForceRenderAfterCharacter(o))
						{
							if (FrustrumEnable)
							{
								o->Visible = TestFrustrum2D(o->Position[0] * 0.01f, o->Position[1] * 0.01f, minf(o->CollisionRange + range, FrustrumRange));
								RenderObject_AfterCharacter(o);
							}
...
void MoveObjects()
{
	...
	// Action countdown in reference-frames (25Hz units). Accumulate fractions
	// so high FPS still ticks (timefac(1)≈0.2 at 120 → needs ~5 frames for -1).
	static double s_actionTimeAccum = 0.0;
	s_actionTimeAccum += timefac(1.0);
	const int actionDec = static_cast<int>(s_actionTimeAccum);
	if (actionDec > 0)
	{
		g_iActionTime -= actionDec;
		s_actionTimeAccum -= actionDec;
		if (g_iActionTime < 0)
			g_iActionTime = 0;
	}
}
