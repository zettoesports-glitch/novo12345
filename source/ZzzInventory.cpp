...
void SEASON3B::RenderInventoryInterface(int StartX, int StartY, int Flag)
{
	...
	// Look-2.5: Item_Back HD — UV da textura inteira (output_width/height), nao o recorte classico /256
	if (gmProtect->LookAndFeel == 6)
	{
		BITMAP_t* pBack1 = &Bitmaps[BITMAP_INVENTORY];
		BITMAP_t* pBack2 = &Bitmaps[BITMAP_INVENTORY + 1];
		RenderBitmap(BITMAP_INVENTORY, x, y, Width, Height, 0.f, 0.f,
			pBack1->output_width / pBack1->Width, pBack1->output_height / pBack1->Height);
		Width = 190.f; Height = 177.f; y = (float)StartY + 256;
		RenderBitmap(BITMAP_INVENTORY + 1, x, y, Width, Height, 0.f, 0.f,
			pBack2->output_width / pBack2->Width, pBack2->output_height / pBack2->Height);
	}
	else
	{
		RenderBitmap(BITMAP_INVENTORY, x, y, Width, Height, 0.f, 0.f, Width / 256.f, Height / 256.f);
