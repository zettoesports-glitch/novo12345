					...
					if ((signed int)MagicNumber > Skill_Fire_Burst_Strengthener)
					{
						...
						LABEL_321:
							...
							}
							{
								...
								Vector(0.5f, 0.5f, 0.5f, Light);
								// Chaotic Diseier — 5 shiny + 8 rastros, spawn LOCAL (reto no yaw)
								for (int i = 0; i < 5; ++i)
								{
								...
								{
									float matGhost[3][4];
									vec3_t vLocal, vWorld, posGhost;
									AngleMatrix(o->Angle, matGhost);
									for (int i = 0; i < 8; i++)
									{
										float fSide = (float)(rand() % 240) - 119.f;
										float fFront = (float)(rand() % 50);
										Vector(fSide, -fFront, 0.f, vLocal);
										VectorRotate(vLocal, matGhost, vWorld);
										posGhost[0] = o->Position[0] + vWorld[0];
										posGhost[1] = o->Position[1] + vWorld[1];
										posGhost[2] = o->Position[2] + 49.f + (float)(rand() % 60);
										CreateJoint(BITMAP_2LINE_GHOST, posGhost, o->Position, o->Angle, 0, o, 20.f, o->PKKey, 0, o->m_bySkillSerialNum);
									}
								}
						...
						LABEL_43:
...

							{
								...
								for (int i = 0; i < 8; i++)
								{
									...
									Vector(fSide, -fFront, 0.f, vLocal);
									VectorRotate(vLocal, matGhost, vWorld);
									posGhost[0] = so->Position[0] + vWorld[0];
									posGhost[1] = so->Position[1] + vWorld[1];
									posGhost[2] = so->Position[2] + 49.f + (float)(rand() % 60);
									CreateJoint(BITMAP_2LINE_GHOST, posGhost, so->Position, so->Angle, 0, so, 20.f, so->PKKey, 0, so->m_bySkillSerialNum);
								}
							}
							{
								CHARACTER* st = gmCharacters->GetCharacter(SelectedCharacter);
...
BOOL ReceiveDieExp(BYTE* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (pCharacter)
	{
		...
		if (Exp > 0 && g_pChatListBox)
		{
			const bool bMaster = (gCharacterManager.IsMasterLevel(Hero->Class) == true
				&& CharacterAttribute->Level >= CharacterAttribute->MaxCharacterLevel);
			g_pChatListBox->AddExperience((DWORD)Exp, bMaster);
		}
...
BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	...
	case 0xF1:
	{
		...
		case 0x01: //receive log in
			...
			case 0x20:
				...
				// Real login OK → save account once (if user checked remember)
				CUIMng::Instance().m_LoginWin.OnLoginSuccess();
#endif
				break;
			case 0x00:
				CUIMng::Instance().PopUpMsgWin(RECEIVE_LOG_IN_FAIL_PASSWORD);
#ifdef SAVE_ACCOUNT_SYSTEM
				CUIMng::Instance().m_LoginWin.ClearPendingSave();
#endif
				break;
			case 0x01:
				CurrentProtocolState = RECEIVE_LOG_IN_SUCCESS;
	...
	case 0xF3:
	{
		...
		case 0xE4:
			break;
		case 0xE5:
			// Ranking preview 3D (backup se ProtocolCoreEx nao pegar)
			GCRankingPreviewRecv((PMSG_RANKING_PREVIEW_RECV*)ReceiveBuffer);
			return 1;
		case 0xE6:
			if (gNewUIRankingTop)
