...
namespace SEASON3B
{
	// Photo mode (Options → Hide HUD): skip drawing most UI
	bool IsPhotoModeHideHud();
	void SetPhotoModeHideHud(bool bHide);
	// UIs that stay visible/interactive while photo mode is on (Options, ESC menu, …)
	void RegisterPhotoModeKeepUI(CNewUIObj* pUI);
	...
	public:
		...
		void UpdateInterface(DWORD dwKey, double WindowsX, double WindowsY);
#ifdef PBG_MOD_STAMINA_UI
		int GetShowUICnt();
#endif //PBG_MOD_STAMINA_UI
