...
SEASON3B::CNewUI3DCamera::CNewUI3DCamera()
{
	...
	m_uiWidth = 1024;
	m_uiHeight = 768;
	m_fZOrder = INVENTORY_CAMERA_Z_ORDER;
}
...
void SEASON3B::CNewUI3DCamera::resize(int width, int height)
{
	m_uiWidth = width;
	m_uiHeight = height;
}
...
bool SEASON3B::CNewUI3DCamera::Render()
{
	...
	if (m_fZOrder != TOOLTIP_CAMERA_Z_ORDER)
	{
		EndBitmap();
		...
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
		// BeginBitmap limpa VAO/shader → glColor das potions (numero stack) volta a funcionar
		BeginBitmap();
	}
...
#ifdef SYSTEM_OPTION_RENDER
void SEASON3B::CNewUI3DRenderMng::Reload3DEffectObject(int width, int height)
{
	type_list_camera::iterator li = m_listCamera.begin();
	for (; li != m_listCamera.end(); li++)
	{
		(*li)->resize(width, height);
	}
}
#endif // SYSTEM_OPTION_RENDER
