...
typedef struct
{
	...
	WORD GetSize()
	{
		return ((WORD)(this->SizeH) << 8) + this->SizeL;
	}
...
typedef struct
{
	...
	BYTE  m_byPosX;
	BYTE  m_byPosY;
	DWORD m_byEquipment[EQUIPMENT_LENGTH];
	BYTE  s_BuffCount;
	BYTE  s_BuffEffectState[MAX_BUFF_SLOT_INDEX];
...
extern void ReceiveCharacterList(BYTE* ReceiveBuffer);
extern void ReceiveMovePosition(BYTE* ReceiveBuffer);
extern void ReceiveMoveCharacter(BYTE* ReceiveBuffer);
extern BOOL TranslateProtocol(int HeadCode, BYTE* ReceiveBuffer, int Size, BOOL bEncrypted);
extern void DataSend(BYTE* byData, DWORD size); // OK
