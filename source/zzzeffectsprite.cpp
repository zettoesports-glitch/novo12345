...
{
	// Upgrade 1: sprites are also skill/light FX — gate spawn + soft cap.
	if (!EffectBudget::Enabled())
		return false;
	if (EffectBudget::IsFull(Sprites, MAX_SPRITES, EffectBudget::CapSprites()))
		return false;
...
void RenderSprites(BYTE byRenderOneMore)
{
	if (!EffectBudget::Enabled())
	{
		// Do not leave Live sprites stuck when FX are off (render used to only skip draw).
		EffectBudget::ClearAll(Sprites, MAX_SPRITES);
		return;
	}
	for (int i = 0; i < MAX_SPRITES; i++)
	{
