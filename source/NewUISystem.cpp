...
bool SEASON3B::CNewUISystem::LoadMainSceneInterface()
{
	...
	// Created centered; OpenningProcess re-centers each open
	if (m_pNewUIInventoryJewel->Create(m_pNewUIMng,
		(int)PositionX_The_Mid((float)SEASON3B::CNewUIInventoryJewel::WINDOW_W),
		(int)PositionY_The_Mid((float)SEASON3B::CNewUIInventoryJewel::WINDOW_H)) == false)
		return false;
	...
	// Ranking: Menu Custom UI, larger window (500x400) — re-centered on open
	if (m_NewUIRankTop->Create(m_pNewUIMng, PositionX_The_Mid((float)CNewUIRankingTop::WINDOW_W), PositionY_The_Mid((float)CNewUIRankingTop::WINDOW_H)) == false)
	{
		return false;
	}
	...
	// Custom menu 3x2: joias, vip, eventos, ranking, travar, senha
	if (m_NewUIMenuUser->Create(m_pNewUIMng, PositionX_The_Mid((float)CNewUIMenuUser::WINDOW_W), PositionY_The_Mid((float)CNewUIMenuUser::WINDOW_H)) == false)
	{
		return false;
	}
	...
	// VIP: Menu Custom UI, larger window (320x360) — re-centered on open
	if (m_NewUIMemberShip->Create(m_pNewUIMng, PositionX_The_Mid((float)CNewUIPurcharseVip::WINDOW_W), PositionY_The_Mid((float)CNewUIPurcharseVip::WINDOW_H)) == false)
	{
		return false;
	}

	m_NewUISecurity = new CNewUISecurityPanel;
	if (m_NewUISecurity->Create(m_pNewUIMng, PositionX_The_Mid((float)CNewUISecurityPanel::WINDOW_W), PositionY_The_Mid((float)CNewUISecurityPanel::WINDOW_H)) == false)
	{
		return false;
	}
...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if (m_pNewUIMng)
	{
		...
		case SEASON3B::INTERFACE_INVENTORY_JEWEL:
			// Standalone Jewel Bank — centered alone, NO inventory side panel
			HideAllGroupA();
		...
		case SEASON3B::INTERFACE_SHOW_VIP:
			...
			break;
		case SEASON3B::INTERFACE_ACCOUNT_SECURITY:
			if (m_NewUISecurity)
				m_NewUISecurity->OpenningProcess();
			HideGroupOpenInterface(INTERFACE_ACCOUNT_SECURITY);
			break;
		}
			...
			if (dwKey != SEASON3B::INTERFACE_MYSHOP_INVENTORY && dwKey != SEASON3B::INTERFACE_PURCHASESHOP_INVENTORY)
			{
				...
				default:
					if (dwKey != SEASON3B::INTERFACE_OPTION)
					{
						...
						case SEASON3B::INTERFACE_ACCOUNT_SECURITY:
							if (m_NewUISecurity)
								m_NewUISecurity->ClosingProcess();
							break;
						case SEASON3B::INTERFACE_INVENTORY_CORE:
							m_pNewUIInventoryCore->ClosingProcess();
							break;
...
void SEASON3B::CNewUISystem::HideGroupOpenInterface(DWORD dwKey)
{
	DWORD dwGroupC[] =
	{
		...
		SEASON3B::INTERFACE_ACCOUNT_SECURITY,
		0,
	};
...
CNewUISecurityPanel* SEASON3B::CNewUISystem::GetUI_NewUISecurity() const
{
	return m_NewUISecurity;
}
