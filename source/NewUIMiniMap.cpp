...
{
	// Auto-move only while the full TAB map is open (corner radar removed)
	if (!this->IsVisible())
		return false;
	...
	if (SEASON3B::CheckMouseIn(this->m_RenderFrameX, this->m_RenderFrameY, this->m_RenderSizeX, this->m_RenderSizeY, false))
	{
		...
		// LMB on TAB map: set destination / cancel current auto-path
		// (Ctrl no longer required — corner map no longer steals clicks)
		if (SEASON3B::IsRelease(VK_LBUTTON))
		{
			...
			else
			{
				...
				if (this->runtime_calc_target(mouseTx, mouseTy, &Tx, &Ty))
				{
					...
					if (this->FindPath(Hero->PositionX, Hero->PositionY, this->m_MoveX, this->m_MoveY))
					{
						gfxNotice::addLongMovementText(gTextClien.TextClien_Khac[2], 20, 150, 2000); // "Auto-Moving"
					}
					else
					{
						this->m_MoveX = -1;
						this->m_MoveY = -1;
						gfxNotice::addLongMovementText("Cannot reach", 255, 80, 2000);
					}
				}
			}
		}
...
void SEASON3B::CNewUIMiniMap::runtime_render_map(bool rendername)
{
	...
	// Auto-move path: original blue icon (type 6), fewer dots along the way + clear destination
	if (this->IsMoving())
		runtime_render_path(rot);
...
void SEASON3B::CNewUIMiniMap::runtime_render_path(const vec3_t angles)
{
	// Same icon as before (type 6 = blue waypoint on minimap atlas).
	// Old code drew every waypoint → crowded. Now:
	//  - path: 1 icon every 4 waypoints
	//  - always draw the final destination (last waypoint / m_MoveX,Y)
	if (m_WayPoint.empty())
		return;

	const int total = (int)m_WayPoint.size();
	int step = 0;
	for (std::deque<PAIR>::const_iterator it = m_WayPoint.begin(); it != m_WayPoint.end(); ++it, ++step)
	{
		const bool isLast = (step == total - 1);
		if (!isLast && (step % 4) != 0)
			continue;

		runtime_render_objet(-1, it->first, it->second, 1, 6, 0.f, NULL, angles);
	}

	// If final dest was set and still valid, draw it again when not already last waypoint
	// (covers case where m_Move is final tile not yet matching remaining front)
	if (m_MoveX >= 0 && m_MoveY >= 0)
	{
		const PAIR& last = m_WayPoint.back();
		if (last.first != m_MoveX || last.second != m_MoveY)
			runtime_render_objet(-1, m_MoveX, m_MoveY, 1, 6, 0.f, NULL, angles);
	}
}
