...
{
	return (gMapManager->currentMap == WD_40AREA_FOR_GM) ? true : false;
}
...
{
	return (gMapManager->currentMap == WD_37KANTURU_1ST) ? true : false;
}
...
bool M37Kanturu1st::MoveKanturu1stObject(OBJECT* pObject)
{
	...
	case 97:
		pObject->HiddenMesh = -2;
		pObject->Timer += timefac(0.1f);
		if (pObject->Timer > 10.f)
			pObject->Timer = 0.f;
		if (pObject->Timer > 5.f)
			CreateParticleSync(BITMAP_BUBBLE, pObject->Position, pObject->Angle, pObject->Light, 5);
		break;
	case 98:
...
bool M37Kanturu1st::RenderKanturu1stObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 37:
	{
		...
		if (time >= 0 && time < 10)
		{
			...
			CreateEffectSync(MODEL_BUTTERFLY01, pObject->Position, pObject->Angle, Light, 3, pObject);
		}
	...
	case 59:
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticleSync(BITMAP_SMOKE, pObject->Position, pObject->Angle, Light, 21, pObject->Scale);
		}
		break;
	case 61:
		if (rand() % 3 == 0)
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticleSync(BITMAP_TRUE_FIRE, pObject->Position, pObject->Angle, Light, 0, pObject->Scale);
		}
		break;
	case 62:
	...
	case 81:
		Vector(1.f, 1.f, 1.f, Light);
		if (rand() % 2 == 0)
			CreateParticleSync(BITMAP_WATERFALL_1, pObject->Position, pObject->Angle, Light, 2, pObject->Scale);
		break;
	case 82:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_WATERFALL_3, pObject->Position, pObject->Angle, Light, 4, pObject->Scale);
		break;
	case 83:
		Vector(1.f, 1.f, 1.f, Light);
		if (rand() % 3 == 0)
			CreateParticleSync(BITMAP_WATERFALL_2, pObject->Position, pObject->Angle, Light, 2, pObject->Scale);
		break;
	case 85:
		pModel->BeginRender(pObject->Alpha);
		pModel->RenderMesh(3, RENDER_TEXTURE | RENDER_BRIGHT, pObject->Alpha, pObject->BlendMesh, pObject->BlendMeshLight, pObject->BlendMeshTexCoordU, pObject->BlendMeshTexCoordV);
		pModel->RenderMesh(3, RENDER_CHROME | RENDER_DARK, pObject->Alpha, pObject->BlendMesh, pObject->BlendMeshLight, pObject->BlendMeshTexCoordU, pObject->BlendMeshTexCoordV, BITMAP_CHROME);
		pModel->EndRender();
		break;
	case 92:
	{
		...
		if (rand() % 30 == 1)
			CreateJointSync(BITMAP_JOINT_THUNDER, StartPos, EndPos, pObject->Angle, 18, NULL, 50.f);
	}
	break;
	case 96:
		pModel->StreamMesh = 0;
		glAlphaFunc(GL_GREATER, 0.0f);
		pModel->RenderMesh(0, RENDER_TEXTURE, 1.0f, pObject->BlendMesh, pObject->BlendMeshLight, pObject->BlendMeshTexCoordU, -(int)WorldTime % 20000 * 0.00005f);
	...
	case 98:
		if (rand() % 3 == 0)
		{
			...
			CreateParticleSync(BITMAP_TWINTAIL_WATER, Position, pObject->Angle, Light, 2);
		}
		break;
	case 105:
	{
		...
		CreateParticleSync(BITMAP_ENERGY, Position, pObject->Angle, Light, 0, 1.5f);
		...
		EndPos[2] += 10.f;
		if (rand() % 20 == 0)
			CreateJointSync(BITMAP_JOINT_THUNDER, StartPos, EndPos, pObject->Angle, 8, NULL, 40.f);
	}
	break;
	case 107:
		if (pObject->HiddenMesh != -2)
	...
	case 108:
		if (pObject->HiddenMesh != -2)
		{
			...
			for (int i = 0; i < 20; ++i)
				CreateParticle(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 7, pObject->Scale, pObject);
		}
		break;
	case 110:
	{
...
void M37Kanturu1st::RenderKanturu1stAfterObjectMesh(OBJECT* o, BMD* b)
{
	...
	case 76:
		...
		b->BodyLight[2] = 0.52f;
		b->StreamMesh = 0;
		b->RenderMesh(0, RENDER_TEXTURE | RENDER_BRIGHT, o->Alpha, o->BlendMesh, o->BlendMeshLight, -(int)WorldTime % 100000 * 0.00001f, o->BlendMeshTexCoordV);
		b->StreamMesh = -1;
		break;
...
bool M37Kanturu1st::AttackEffectKanturu1stMonster(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 108:
	{
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 10, 4.0f);
		}
	...
	case MODEL_MONSTER01 + 111:
	{
		if (o->CurrentAction == MONSTER01_ATTACK1 || o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, index, Width);
...
bool M37Kanturu1st::RenderKanturu1stMonsterObjectMesh(OBJECT* o, BMD* b, int ExtraMon)
{
	...
	case MODEL_MONSTER01 + 106:
	{
		...
		b->RenderMesh(0, RENDER_TEXTURE | RENDER_BRIGHT, o->Alpha, 0, fLumi, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, BITMAP_BERSERK_EFFECT);
		b->RenderMesh(1, RENDER_TEXTURE | RENDER_BRIGHT, o->Alpha, 1, fLumi, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, BITMAP_BERSERK_WP_EFFECT);
...
bool M37Kanturu1st::RenderKanturu1stMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 106:
	{
		...
		CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, o->Light, 42, o->Scale);
	...
	case MODEL_MONSTER01 + 113:
	{
		...
		if (o->CurrentAction == MONSTER01_WALK)
		{
			...
			CreateParticleSync(BITMAP_SMOKE + 1, vPos, o->Angle, o->Light, 1, o->Scale);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, vPos, o->Angle, o->Light);
		}
		return true;
	}
	break;
	...
	case MODEL_MONSTER01 + 108:
	{
		...
		else
		{
			...
			CreateParticleSync(BITMAP_SMOKE + 3, vPos, o->Angle, vLight, 3, 2.0f);
			CreateParticleSync(BITMAP_SMOKE + 3, vPos, o->Angle, vLight, 4, 1.0f);
			if (o->SubType == FALSE)
			{
				o->SubType = TRUE;
	...
	case MODEL_MONSTER01 + 110:
	{
		if (gMapManager->currentMap == WD_39KANTURU_3RD && g_Direction.m_CKanturu.m_iKanturuState == KANTURU_STATE_MAYA_BATTLE)
		{
			vec3_t Light;
			float Scale, Angle;

			o->Distance += timefac(0.05f);
			Scale = o->Distance;
	...
	case MODEL_MONSTER01 + 111:
	{
		...
		if (o->CurrentAction == MONSTER01_DIE)
		{
			...
			CreateParticleSync(BITMAP_SMOKE + 3, vPos, o->Angle, o->Light, 3, Scale);
			BoneManager::GetBonePosition(o, "KENTAUROS_BIP_TAIL_1", vRelative, vPos);
			CreateParticleSync(BITMAP_SMOKE + 1, vPos, o->Angle, o->Light, 1, Scale);
			...
			{
				if (rand() % 10 == 0) {
					CreateParticleSync(BITMAP_SMOKE + 1, o->Position, o->Angle, o->Light);
				}
			}
	}
	break;
