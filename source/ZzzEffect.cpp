...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 11; ++i)
			{
				fScale = (float)(rand() % 80 + 32) * 0.01f * 1.0f;
				Vector(o->Position[0] + (rand() % 70 - 35) * 1.0f, o->Position[1] + (rand() % 70 - 35) * 1.0f, o->Position[2] + (rand() % 70 - 35) * 1.0f, vPos);
				CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, pObject->Angle, vLight, 0, fScale);	// 전기
			}
			...
			for (int i = 0; i < 6; ++i)
			{
				...
				CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPosition, pObject->Angle, vLight, 0, fScale);	// 전기
			}
	...
	case MODEL_PIERCING:
		...
		if (o->SubType == 1)
		{
			...
			CreateJointSync(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, 20.f, 7); //  전기 
