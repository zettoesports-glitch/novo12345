...
bool AddRangeAttack(vec3_t Position, float Range, short PKKey, int DamageKey[], int* piCount, int iMaxKey)
{
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		float Distance = sqrtf(dx * dx + dy * dy);
		if (o->Live && o->Visible && c != Hero && c->Dead == 0 && Distance <= Range &&
...
bool AttackCharacterRange(int Index, vec3_t Position, float Range, BYTE Serial, short PKKey, WORD SkillSerialNum)
{
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		if (o->Live == false || o->Visible == false || c == Hero || c->Dead > 0)
		{
			continue;
		}
		...
		if (Distance <= Range && (o->Kind == KIND_MONSTER || (o->Kind == KIND_PLAYER && (c->Key == PKKey || DamageChr))))
		{
			if (Skill == AT_SKILL_STORM || Skill == Skill_Evil_Spirit
				|| Skill == Skill_Evil_Spirit_Strengthener
				|| Skill == Skill_Evil_Spirit_Strengthener2)
			{
...
void EffectDestructor(OBJECT* o)
{
	...
	case MODEL_SKILL_WHEEL1:
		if (o->Owner)
		{
			if (o->Owner->runtime_whelskill > 0)
				o->Owner->runtime_whelskill--;
		}
		break;
	}
	...
	if (to != NULL && to->Live && to->Visible)
	{
		...
		if (Range <= 100.f)
		{
			...
			case MODEL_FIRE:
				...
				break;
			case BITMAP_ENERGY:
				CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 1, 6.f);
				break;
			case MODEL_LIGHTNING_ORB:
			{
				CreateEffectSync(MODEL_LIGHTNING_ORB, o->Position, o->Angle, o->Light, 1);
			}
			break;
			case MODEL_SNOW1:
			{
				for (int j = 0; j < 2; j++)
				{
					CreateEffectSync(MODEL_SNOW2 + rand() % 2, o->Position, o->Angle, o->Light);
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light);
				}
			...
			case MODEL_WOOSISTONE:
				if (Range <= 30.f)
				{
					for (int j = 0; j < 20; j++)
					{
						CreateEffectSync(MODEL_WOOSISTONE, o->Position, o->Angle, o->Light, 1);
						CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, o->Light, 0, 1, o);
					}
					PlayBuffer(SOUND_BREAK01);
				}
				else
				{
					o->LifeTime = 100;
					o->life_time_work = standlimit(100);
				}
				break;
			}
		}
...
void CreateEffectSync(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT* Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	if (checkNormalizer)
	{
...
{
	// Upgrade 1: do not allocate effect slots when FX are off or over soft cap.
	if (!EffectBudget::Enabled())
		return;
	if (EffectBudget::IsFull(Effects, MAX_EFFECTS, EffectBudget::CapEffects()))
		return;
...
void CreateEffect(OBJECT* o, int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT* Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	float Matrix[3][4];
	vec3_t p1, p2;
	...
	case MODEL_DEATH_SPI_SKILL:
		...
		else if (o->SubType == 1)
		{
			...
			o->Scale = 0.2f;
			o->BlendMesh = -1;
			o->Alpha = (float)((20 - o->LifeTime._runvalue) / 5.f);
	...
	case MODEL_PIER_PART:
		...
		else if (o->SubType == 1)
		{
			...
			o->HiddenMesh = 0;
			o->Scale = 0.5f;
			o->Alpha = (float)((20 - (float)o->LifeTime) / 5.f);
			Vector(0.f, 0.f, 0.f, o->Direction);
			CreateParticle(BITMAP_FIRE + 1, o->Position, o->Angle, o->Light, 0, 1.f, o);
		}
		else if (o->SubType == 2)
	...
	case MODEL_FENRIR_THUNDER:
	{
		if (o->SubType == 0)
		{
			...
			BMD* p_b = gmClientModels->GetModel(o->Owner->Type);
			int irandom = rand() % 30;
	...
	case MODEL_PROTECTGUILD:
	{
		...
		BMD* b = gmClientModels->GetModel(o->Owner->Type);
	...
	case MODEL_BLOW_OF_DESTRUCTION:
	{
		...
		else if (o->SubType == 2)
		{
			...
			if (o->Owner->m_sTargetIndex < 0)
				break;
			CHARACTER* pCharacter = gmCharacters->GetCharacter(o->Owner->m_sTargetIndex);
			if (pCharacter)
			{
				VectorCopy(pCharacter->Object.Position, o->StartPosition);
			}
			o->Scale = 0.5f;
		}
#endif //PBG_ADD_NEWCHAR_MONK_SKILL
	...
	case MODEL_EFFECT_FLAME_STRIKE:
		if (o->SubType == 0)
		{
			o->Alpha = 0;
			o->LifeTime = 35;
			o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_FLAMESTRIKE].PlaySpeed;
			o->AI = 0;
			o->m_iAnimation = rand();
		}
		break;
	...
	case MODEL_WOLF_HEAD_EFFECT:
	{
		...
		{
			o->Scale = 1.0f;
			o->LifeTime = 10.0f / gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_THRUST].PlaySpeed;
		}
		else if (o->SubType == 1)
		{
			o->Velocity = 0.1f;
	...
	{
		VectorCopy(Position, o->Position);
		o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed * 2.0f;
		o->LifeTime = 100;
		gmClientModels->GetModel(MODEL_DOWN_ATTACK_DUMMY_L)->Actions[0].PlaySpeed = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed;
		gmClientModels->GetModel(MODEL_DOWN_ATTACK_DUMMY_R)->Actions[0].PlaySpeed = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed;
	}
	break;
	...
	case MODEL_SHOCKWAVE01:
	{
		...
		if (o->SubType == 1)
		{
			o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONLORE].PlaySpeed;
		...
		{
			o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_DARKSIDE_READY].PlaySpeed * 2.0f;
			o->LifeTime = 5.0f / o->Velocity;
			o->Scale = Scale;
			vec3_t vPos, vDir;
			VectorCopy(o->Position, vPos);
		...
		{
			o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed * 2.0f;
			o->LifeTime = 5.0f / o->Velocity;
	...
	case MODEL_SHOCKWAVE_SPIN01:
	{
		o->Scale = Scale;
		VectorCopy(Light, o->Light);
		o->Velocity = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed;
	...
	case MODEL_WOLF_HEAD_EFFECT2:
	{
		...
		if (b)
		{
			...
			if (o->SubType == 3 || o->SubType == 5)
			{
				...
				o->LifeTime = 4.0f / b->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed;
			}
			else if (o->SubType == 4)
			{
				o->Velocity = 0.1f;
	...
	case MODEL_DRAGON_KICK_DUMMY:
	{
		...
		// ani keyframe 10 : 19
		o->LifeTime = 200;
		gmClientModels->GetModel(MODEL_DRAGON_KICK_DUMMY)->Actions[0].PlaySpeed = gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONKICK].PlaySpeed;
		o->Velocity = gmClientModels->GetModel(MODEL_DRAGON_KICK_DUMMY)->Actions[0].PlaySpeed * 2.0f;
		VectorCopy(Light, o->Light);
	}
	break;
	case BITMAP_LAVA:
	...
	case MODEL_DRAGON_LOWER_DUMMY:
	{
		...
		gmClientModels->GetModel(o->Type)->Actions[o->CurrentAction].PlaySpeed = 0.03f;
		o->m_iAnimation = 0;
	}
	break;
	...
	case MODEL_VOLCANO_OF_MONK:
	{
		if (o->SubType == 1)
		{
			...
			VectorCopy(Position, o->Position);
			o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
			gmClientModels->GetModel(o->Type)->Actions[0].PlaySpeed = 0.1f;
			o->LifeTime = 100;
			o->Velocity = 0.6f;
			vec3_t vLight;
			Vector(1.0f, 1.0f, 1.0f, vLight);
	...
	break;
	case MODEL_PHOENIX_SHOT_EFFECT:
		if (o)
		{
			if (o->Owner->Type >= 0)
			{
				o->LifeTime = 70;
				Action_t* PlayerAction = gmClientModels->GetModelAction(MODEL_PHOENIX_SHOT_EFFECT, 0);

				if (PlayerAction)
				{
					PlayerAction->PlaySpeed = gmClientModels->GetPlayerModelAction(PLAYER_SKILL_PHOENIX_SHOT)->PlaySpeed * 3;
					o->Velocity = PlayerAction->PlaySpeed;
					o->Scale = Scale;
					o->Angle[2] -= 90.0;
					o->m_sTargetIndex = o->Owner->m_sTargetIndex;
				}
			}
			else
			{
		...
		break;
	case MODEL_WIND_SPIN03:
	{
		o->Scale = Scale;
		o->m_sTargetIndex = (int)(Light[2]);
		CHARACTER* pCharacter = gmCharacters->GetCharacter(o->m_sTargetIndex);
		...
		if (pCharacter)
		{
			...
			vPos[0] = pCharacter->Object.Position[0] - o->Position[0];
			vPos[1] = pCharacter->Object.Position[1] - o->Position[1];
			vPos[2] = pCharacter->Object.Position[2] - o->Position[2];
			VectorNormalize(vPos);
			o->Direction[0] = vPos[0];
			o->Direction[1] = vPos[1];
			o->Direction[2] = vPos[2];
	...
	break;
	case MODEL_WIND_SPIN02:
	{
		Action_t* pAction = gmClientModels->GetModelAction(MODEL_WIND_SPIN02, 0);
		if (pAction != NULL)
		{
			pAction->PlaySpeed = 0.2;
			o->Velocity = pAction->PlaySpeed;
			o->Scale = Scale;
			o->Light[0] = Light[0];
			o->Light[1] = Light[1];
			o->Light[2] = Light[2];
...
void MoveParticle(OBJECT* o, int Turn)
{
	if (Turn)
	{
		...
		interpolatesum2(o->Position, Position, o->Position);
	}
	else
	{
		interpolatesum2(o->Position, o->Direction, o->Position);
	}
}
...
bool MoveJump(OBJECT* o)
{
	...
	o->Gravity -= timefac(1.f);
	float Height = RequestTerrainHeight(o->Position[0], o->Position[1]);
	if (o->Position[2] < Height)
	{
		o->Position[2] = Height;
		o->Gravity = -o->Gravity * timepow(0.4f);
		o->Direction[1] *= timepow(0.4f);
		return true;
	}
	return false;
}
...
void CreateBomb(vec3_t p, bool Exp, int SubType)
{
	...
	if (SubType != 5)
	{
		for (int j = 0; j < 12; j++)
		{
			...
			if (SubType == 2)
			{
				...
				CreateParticleSync(BITMAP_SPARK, Position, Angle, Light, 8);
			}
			else if (SubType == 3)
			{
			...
			else if (SubType == 6)
			{
				...
				CreateParticleSync(BITMAP_SPARK, Position, Angle, Light, 10);
			}
			else
			{
				CreateParticleSync(BITMAP_SPARK, Position, Angle, Light, 2);
			}
		}
	}
	...
	if (Exp)
	{
		if (SubType == 2 || SubType == 5 || SubType == 6)
		{
			...
			CreateParticleSync(BITMAP_EXPLOTION_MONO, Position, Angle, Light);
		}
		else if (SubType == 3)
		{
		...
		else if (SubType == 4)
		{
			...
			CreateParticleSync(BITMAP_EXPLOTION_MONO, Position, Angle, Light, 0, 0.5f);
			Vector(1.0f, 0.6, 0.2f, Light);
			CreateParticleSync(BITMAP_EXPLOTION_MONO, Position, Angle, Light, 0, 0.3f);
		}
		else
		{
			CreateParticleSync(BITMAP_EXPLOTION, Position, Angle, Light);
		}
	}
}
...
void CreateBomb2(vec3_t p, bool Exp, int SubType, float Scale)
{
	...
	if (Exp)
	{
		CreateParticleSync(BITMAP_EXPLOTION_MONO, Position, Angle, Light, 0, 4.f + Scale);
	}
	else
	{
		CreateParticleSync(BITMAP_EXPLOTION + 1, Position, Angle, Light, 0, 4.f);
	}
}
...
void CreateBomb3(vec3_t vPos, int iSubType, float fScale)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		if (fScale != 1.0f)
			CreateParticleSync(BITMAP_SUMMON_SAHAMUTT_EXPLOSION, vBombPos, vAngle, vLight, 0, fScale);
		else
			CreateParticleSync(BITMAP_SUMMON_SAHAMUTT_EXPLOSION, vBombPos, vAngle, vLight, 0, 0.15f * (rand() % 10));
	}

	Vector(1.0f, 0.5f, 0.2f, vLight);

	if (fScale != 1.0f)
		CreateParticleSync(BITMAP_MAGIC + 1, vBombPos, vAngle, vLight, 0, 2.2f * fScale);
	else
		CreateParticleSync(BITMAP_MAGIC + 1, vBombPos, vAngle, vLight, 0, 1.0f);
	...
	{
		if (fScale != 1.0f)
			CreateEffectSync(MODEL_STONE1 + rand() % 2, vBombPos, vAngle, vLight, 14, NULL, -1, 0, 0, 0, 2.2f * fScale);
		else
			CreateEffectSync(MODEL_STONE1 + rand() % 2, vBombPos, vAngle, vLight, 13);
	}
	Vector(1.0f, 1.0f, 1.0f, vLight);
	...
	{
		CreateParticleSync(BITMAP_SPARK, vBombPos, vAngle, vLight, 2);
	}
}
...
void CreateInferno(vec3_t Position, int SubType)
{
	...
	for (int j = 0; j < 8; j++)
	{
		...
		if (SubType == 1)
		{
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
		}
		else if (SubType == 2 || SubType == 3)
		{
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 10);
		}
		else if (SubType == 5)
		{
			...
			for (int i = 0; i < 3; ++i)
				CreateEffectSync(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, p2, Angle, Light, 0);
		}
		else
		{
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 1);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, p2, Angle, Light, 0);
		}
	}
}
...
void CheckClientArrow(OBJECT* o)
{
	if (battleCastle::IsBattleCastleStart())
	{
		...
		if ((att & TW_NOATTACKZONE) == TW_NOATTACKZONE)
		{
			o->Velocity = 0.f;
			Vector(0.f, 0.f, 0.f, o->Direction);
			o->LifeTime /= timepow(2.f);
			o->life_time_work /= 2.f;
			return;
		}
	}
	...
	if (!o->Kind || o->SubType == 99)
	{
		if (CheckCharacterRange(o, 100.f, o->PKKey))
		{
			...
			case AT_SKILL_PIERCING:
				if (o->SubType == 2)
				{
					CreateJointSync(BITMAP_FLARE, o->Position, o->Position, o->Angle, 6, o, 30.0f);
				}
				else
				{
					o->Live = false;
	...
	else
	{
		if (rand() % 2 == 0)
		{
			if (o->Owner == &Hero->Object)
			{
				...
				if ((Skill == AT_SKILL_PIERCING || Skill == Skill_Fire_Scream) && o->AttackPoint[0] > 0)
				{
					range = 200.f;
					if (Skill == Skill_Fire_Scream)
						range = 100.f;
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	if (o->LifeTime < 5)
	{
		Luminosity -= ((5 - o->LifeTime._runvalue) * 0.2f);
		if (Luminosity < 0.f)
			Luminosity = 0.f;
	}
	...
	case MODEL_DRAGON:
	{
		...
		if (fTemp <= 800.0f && fTemp >= -800.0f)
		{
			o->Kind = 2;
			o->Distance = 0.0f;
			o->Angle[2] += (0.5f * o->Distance / 2.0f) * timefac(1.0);
			o->CollisionRange = true;
		}
		else
		{
			...
			{
				o->Angle[2] += (0.5f * o->Gravity * 3.0f) * timefac(1.0);
				o->Kind = 1;
			}
		...
		{
			o->LifeTime = 1000;
			o->life_time_work = standlimit(1000);
		}
	...
	case MODEL_ARROW_AUTOLOAD:
	{
		...
		{
			OBJECT* pOwn = o->Owner;
			if (o->life_time_work % standlimit(10))
			{
		...
		else if (o->SubType == 1)
		{
			...
			if (o->LifeTime < 20)
			{
				float flumi = (float)(o->LifeTime._runvalue - 20) / 20.f;
			...
			interpolatesum2(o->Position, pOwner->Position, o->Position);
			VectorCopy(pOwner->Angle, o->Angle);
			o->Angle[0] += timefac(85.f);
			o->Angle[1] += timefac(-17.f);
			o->Angle[2] += timefac(20.f);
		}
	}
	break;
	case MODEL_INFINITY_ARROW:
	{
		...
		if (o->SubType == 1)
		{
			o->Light[0] *= timepow(0.95f);
			o->Light[1] *= timepow(0.95f);
			o->Light[2] *= timepow(0.95f);
		}
	...
	case MODEL_INFINITY_ARROW3:
	{
		...
		{
			o->Light[0] *= timepow(0.98f);
			o->Light[1] *= timepow(0.98f);
			o->Light[2] *= timepow(0.98f);
			if (o->LifeTime == (40))
	...
	case MODEL_INFINITY_ARROW4:
	{
		vec3_t tmp = { 0.f, 0.f, 0.f };
		OBJECT* pOwner = o->Owner;
		o->Scale *= timepow(1.2f);
		o->Light[0] *= timepow(0.9f);
		o->Light[1] *= timepow(0.9f);
		o->Light[2] *= timepow(0.9f);

		VectorTransform(tmp, pOwner->GetBoneTransform(29), o->Position);
		VectorScale(o->Position, pOwner->Scale, o->Position);
		VectorAdd(o->Position, pOwner->Position, o->Position);
		VectorCopy(pOwner->Angle, o->Angle);
	}
	...
	case MODEL_SHIELD_CRASH:
	{
		...
		{
			ftmp = (float)o->LifeTime._runvalue / 8;
			VectorScale(o->Direction, ftmp, o->Light);
		}

		if (o->LifeTime >= 8 && o->LifeTime < 24)
		{
			ftmp = 1.f - ((float)(o->LifeTime._runvalue - 24) / 16);
			VectorScale(o->Direction, ftmp, o->Light);
		}
		...
		if (o->LifeTime == (23))
		{
			if (o->SubType == 1)
			{
				...
				CreateEffect(BITMAP_SHOCK_WAVE, o->Owner->Position, o->Owner->Angle, nLight, 9, o->Owner, -1, 0, 0, 0);
			}
			else if (o->SubType == 2)
			{
				vec3_t vShockColor = { 0.8f, 0.3f, 0.3f };
				CreateEffect(BITMAP_SHOCK_WAVE, o->Owner->Position, o->Owner->Angle, vShockColor, 9, o->Owner, -1, 0, 0, 0);
			}
			else
			{
	...
	case MODEL_IRON_RIDER_ARROW:
	{
		o->Scale *= timepow(1.05f);
		if (o->LifeTime < 5)
		{
			o->Light[0] *= timepow(0.7f);
			o->Light[1] *= timepow(0.7f);
			o->Light[2] *= timepow(0.7f);
		}
		vec3_t vPos;
		VectorScale(o->Direction, timepow(o->Velocity), vPos);
	...
	break;
	case MODEL_MULTI_SHOT1:
		o->Scale += timefac(0.2f);
		o->BlendMeshLight = (float)o->LifeTime._runvalue / 18.f;
		o->Alpha = o->BlendMeshLight;
		break;
	case MODEL_MULTI_SHOT2:
		o->Scale += timefac(0.3f);
		o->BlendMeshLight = (float)o->LifeTime._runvalue / 18.f;
		o->Alpha = o->BlendMeshLight;
		break;
	case MODEL_MULTI_SHOT3:
		o->Scale += timefac(0.25f);
		o->BlendMeshLight = (float)o->LifeTime._runvalue / 18.f;
		o->Alpha = o->BlendMeshLight;
		break;
	case MODEL_BLADE_SKILL:
	{
		...
		{
			o->BlendMesh = -2;
			o->BlendMeshLight = (float)o->LifeTime._runvalue / 14.f;
			o->Alpha = o->BlendMeshLight;

			vec3_t p, Angle, Light;
			OBJECT* to = NULL;

			if (o->LifeTime >= 3 && o->LifeTime.duration(2))
			{
				VectorCopy(o->Owner->Angle, Angle);
				int Ran = -1;
				CHARACTER* pChar = gmCharacters->GetCharacter(o->PKKey);
				...
				if (pChar)
				{
					...
					CreateArrow(pChar, o->Owner, to, FindHotKey((o->Skill)), 1, 0);
	...
	case MODEL_KENTAUROS_ARROW:
	{
		...
		if (o->LifeTime <= 24)
		{
			...
			VectorScale(o->Direction, timepow(o->Velocity), vPos);
			VectorAdd(o->Position, vPos, o->Position);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 26, 0.2f);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 26, 0.2f);
		}
		o->Alpha += timefac(0.001f);
	}
	break;
	case MODEL_WARP3:
	case MODEL_WARP2:
	...
	case MODEL_BUTTERFLY01:
	{
		...
		else if (o->SubType == 3)
		{
			...
			if (rand() % 2 == 0)
			{
				...
				CreateParticleSync(BITMAP_SPARK, Position, o->Angle, o->Light, 7);
			}
		}
	...
	case 9:
		{
			// Scale step by frame time so projectile/effect path is stable at high FPS
			vec3_t stepDir;
			VectorScale(o->Direction, static_cast<float>(timefac(o->Velocity)), stepDir);
			VectorAdd(o->Position, stepDir, o->Position);

			o->Velocity += timefac(3.f);
			o->Angle[0] += static_cast<float>(timefac(5.f * o->Velocity));
		}
		...
		else
		{
			...
			else
			{
				...
				case 3:
				{
					for (int i = 0; i < 3; ++i)
					{
						float fParam = (float)i * Q_PI * 2 / 3.0f + o->LifeTime._runvalue * 0.17f;
						if (o->SubType == 3)
						{
							fParam = -fParam;
						}
				...
				case 5:
				{
					for (int i = 0; i < 3; ++i)
					{
						...
						CreateParticleSync(BITMAP_LIGHT, Position, o->Angle, LightFlame, 5, 0.7f);
					}
				}
				break;
				case 2:
				{
					vec3_t Light;
					Light[0] = Light[1] = Light[2] = 0.1f * (float)o->LifeTime._runvalue;
	...
	case MODEL_MAGIC_CAPSULE2:
		...
		if (o->LifeTime < 10)
		{
			o->BlendMeshLight = (float)o->LifeTime._runvalue * 0.1f;
		}
		break;
	case MODEL_SPEAR:
		if (o->SubType == 1)
		{
			CreateJointSync(BITMAP_FLARE, o->Position, o->Position, o->Angle, 12, o, 100.0f);
		}
		else if (o->SubType == 0)
		{
			CreateJointSync(BITMAP_FLARE, o->Position, o->Position, o->Angle, 4, o, 50.0f);
		}
		break;
	case MODEL_SPEAR + 1:
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			CreateJointSync(BITMAP_JOINT_HEALING, Position, o->Position, Angle, 6, NULL, 5.f);
		}
	}
	break;
	...
	case MODEL_HALLOWEEN_CANDY_STAR:
		if (o->SubType == 0 || o->SubType == 1)
		{
			o->Angle[o->m_iAnimation] += timefac(20.f);
			o->Position[2] += (o->Gravity * 0.5f);
			o->Gravity -= timefac(1.5f);
			...
			if (o->Position[2] < Height)
			{
				...
				o->LifeTime -= timefac(2);
			}
			VectorAdd(o->Position, o->Direction, o->Position);

			if (o->Type == MODEL_HALLOWEEN_CANDY_HOBAK && (rand() % 3 == 0))
			{
				CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, o->Light, 5, 0.4f + rand() % 10 * 0.01f);
			}
		}
		break;
	case MODEL_HALLOWEEN_EX:
	...
	case MODEL_NEWYEARSDAY_EVENT_PIG:
	case MODEL_NEWYEARSDAY_EVENT_YUT:
		o->Angle[o->m_iAnimation] += timefac(10.f);
		o->Position[0] += timefac((o->Direction[0] * 1.2f));
		o->Position[1] += timefac((o->Direction[1] * 1.2f));
		...
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * timepow(0.3f);
			o->LifeTime -= timefac(2);
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		break;
	case MODEL_MOONHARVEST_MOON:
		if (o->SubType == 0)
		{
			o->Angle[2] += timefac(5.0f);
			if (o->LifeTime < 25)
			{
				o->Light[0] -= timefac(0.05f);
				o->Light[1] -= timefac(0.06f);
		...
		else if (o->SubType == 1)
		{
			...
			CreateParticleSync(BITMAP_SMOKELINE1 + rand() % 3, o->Position, o->Angle, o->Light, 0, 1.0f);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 11, 1.0f);


			if (rand() % 2 == 0)
			{
				CreateParticleSync(BITMAP_WATERFALL_3, o->Position, o->Angle, o->Light, 3, 3.f);
			}
		}
		else if (o->SubType == 2)
		{
			o->Scale += timefac(0.01f);
		}
		break;
	case MODEL_MOONHARVEST_GAM:
	case MODEL_MOONHARVEST_SONGPUEN1:
	case MODEL_MOONHARVEST_SONGPUEN2:
		o->Angle[0] += timefac(10.f);
		o->Position[0] += timefac((o->Direction[0] * 2.2f));
		o->Position[1] += timefac((o->Direction[1] * 2.2f));
		o->Position[2] += (o->Gravity * 1.5f);
		o->Gravity -= timefac(1.5f);
	...
	case BITMAP_FIRE_CURSEDLICH:
		if (o->SubType == 0)
		{
			...
			if (pModel)
			{
				...
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 3, 1, pObject);
			}
		}
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 1; ++i)
			{
				...
				{
				case 0:
					CreateParticleSync(BITMAP_FIRE_HIK1, vFirePosition, o->Angle, o->Light, 1, fScale);
					break;
				case 1:
					CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vFirePosition, o->Angle, o->Light, 5, fScale);
					break;
				case 2:
					CreateParticleSync(BITMAP_FIRE_HIK3, vFirePosition, o->Angle, o->Light, 1, fScale);
					break;
				}
			}
		}
		...
		else if (o->SubType == 2)
		{
			...
			BMD* pModel = gmClientModels->GetModel(o->Owner->Type);
			...
			if (pModel)
			{
				...
				for (int i = 0; i < 8; ++i)
				{
					...
					pModel->TransformByObjectBone(vFirePosition, o->Owner, rand() % iNumBones);
					...
					{
					case 0:
						CreateParticleSync(BITMAP_FIRE_HIK1, vFirePosition, o->Angle, vLight, 0, fScale);
						break;
					case 1:
						CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vFirePosition, o->Angle, vLight, 4, fScale);
						break;
					case 2:
						CreateParticleSync(BITMAP_FIRE_HIK3, vFirePosition, o->Angle, vLight, 0, fScale);
						break;
					}
				}
			}
		...
		else if (o->SubType == 3)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				{
				case 0:
					CreateParticleSync(BITMAP_FIRE_HIK1, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				case 1:
					CreateParticleSync(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, o->Light, 4, fScale);
					break;
				case 2:
					CreateParticleSync(BITMAP_FIRE_HIK3, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				}
			}
		}
		else if (o->SubType == 4)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				{
				case 0:
					CreateParticleSync(BITMAP_FIRE_HIK1_MONO, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				case 1:
					CreateParticleSync(BITMAP_FIRE_HIK2_MONO, o->Position, o->Angle, o->Light, 4, fScale);
					break;
				case 2:
					CreateParticleSync(BITMAP_FIRE_HIK3_MONO, o->Position, o->Angle, o->Light, 0, fScale);
					break;
				}
			}
		}
	...
	case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT:
		if (o->SubType == 0)
		{
			...
			{
				float fAngle = CreateAngle(o->StartPosition[0], o->StartPosition[1], o->Position[0] - pObject->Position[0], o->Position[1] - pObject->Position[1]);
				o->Angle[2] = fAngle + 0;
			}
			...
			vec3_t vLight;
			Vector(o->Alpha * 0.3f, o->Alpha * 0.3f, o->Alpha * 0.3f, vLight);
			CreateParticleSync(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 1, 1, pObject);
		}
		else if (o->SubType == 1)
		{
			if (rand() % 2 == 0)
			{
				...
				{
					int iNumBones = pModel->NumBones;
					CreateEffectSync(BITMAP_FIRE_CURSEDLICH, pObject->Position, pObject->Angle, pObject->Light, 0, pObject, -1, rand() % iNumBones);
				}
			}
		}
		break;
	case MODEL_SUMMONER_EQUIP_HEAD_NEIL:
		if (o->SubType == 0)
		{
			...
			CreateParticleSync(BITMAP_LIGHT + 2, o->Position, o->Angle, o->Light, 3, 0.30f, pObject);
			if (rand() % 2 == 0)
				CreateParticleSync(BITMAP_LIGHT + 2, o->Position, o->Angle, o->Light, 3, 0.30f, pObject);
		}
		else if (o->SubType == 1)
		{
			...
			CreateParticleSync(BITMAP_LIGHT + 2, pObject->Position, pObject->Angle, vColor, 5, 0.1f, pObject);
		}
		break;
	case MODEL_SUMMONER_EQUIP_HEAD_LAGUL:
	{
		...
		CreateParticleSync(BITMAP_CLUD64, o->Position, o->Angle, vLight, 10, 1.f, pObject);
	}
	case MODEL_SUMMONER_CASTING_EFFECT1:
	case MODEL_SUMMONER_CASTING_EFFECT11:
	case MODEL_SUMMONER_CASTING_EFFECT111:
	...
	{
		if (o->LifeTime < 20)
			o->BlendMeshLight -= timefac(0.03f);
		else if (o->BlendMeshLight < 0.5f)
			o->BlendMeshLight += timefac(0.05f);

		switch (o->Type)
		{
		case MODEL_SUMMONER_CASTING_EFFECT1:
			o->Angle[2] -= timefac(3.0f);
			break;
		case MODEL_SUMMONER_CASTING_EFFECT11:
			o->Angle[2] += timefac(3.0f);
			break;
	...
	break;
	{
		...
		o->BlendMeshTexCoordV += timefac(0.05f);
	}
	break;
	case MODEL_SUMMONER_SUMMON_SAHAMUTT:
	{
		...
		if (pModel)
		{
			...
			for (int i = 0; i < 8; ++i)
			{
				...
				Vector(o->Alpha * 0.3f, o->Alpha * 0.3f, o->Alpha * 0.3f, vLight);
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 5, o);
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 4, o);
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 2, 3, o);
			}
		}
	}
	break;
	case MODEL_SUMMONER_SUMMON_NEIL:
	{
		if (o->LifeTime < 20)
			...
			o->Alpha += timefac(0.04f);

		if (o->AnimationFrame > 8 && o->Skill == 0)
		{
			o->Skill = 1;
			CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_NIFE1, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 1)
				CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_NIFE2, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 2)
				CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_NIFE3, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
		}
		if (o->AnimationFrame > 10 && o->Skill == 1)
		{
			...
			CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_GROUND1, vPosition, o->Angle, o->Light, o->SubType, o);
			CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_GROUND1, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);

			if (o->SubType >= 1)
				CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_GROUND2, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
			if (o->SubType >= 2)
				CreateEffectSync(MODEL_SUMMONER_SUMMON_NEIL_GROUND3, o->HeadTargetAngle, o->Angle, o->Light, o->SubType);
	...
	case MODEL_SUMMONER_SUMMON_LAGUL:
	{
		if (o->SubType == 0)
		{
			...
			if (rand() % anEffectVolume[o->PKKey] == 0)
			{
				...
				Vector(1.0f, 1.0f, 1.0f, vLight);
				CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 57, 3.5f);

				VectorCopy(o->HeadTargetAngle, vPos);
				vPos[0] += ((float)(rand() % 400 - 200));
				vPos[1] += ((float)(rand() % 400 - 200));
				Vector(0.6f, 0.1f, 1.f, vLight);
				CreateEffectSync(BITMAP_CLOUD, vPos, o->Angle, vLight, 0, NULL, -1, 0, 0, 0, 2.0f);

				Vector(0.6f, 0.1f, 1.f, vLight);
				CreateParticleSync(BITMAP_TWINTAIL_WATER, vPos, o->Angle, vLight, 1);
			}
		}
		else if (o->SubType == 1)
		{
			...
			if (o->LifeTime < 30)
				o->Alpha = o->LifeTime._runvalue / 40.f;
			else if (o->LifeTime > 144)
				o->Alpha = (160 - o->LifeTime._runvalue) / 20.f;
		}
	}
	break;
	case BITMAP_FIRE:
		CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 9, 1.f, o->Owner);
		break;
	case BITMAP_FIRE + 1:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_FIRE + 1, o->Position, o->Angle, Light, 1);
	...
	case BITMAP_MAGIC:
		if (o->SubType == 0)
		{
			CreateEffectSync(BITMAP_MAGIC, o->Position, o->Angle, o->Light, 1);
		...
		else if (o->SubType == 2 || o->SubType == 3 || o->SubType == 7)
		{
			...
			{
				if (o->SubType == 3)
					CreateParticleSync(BITMAP_FLARE, o->Position, o->Angle, o->Light, 10, 0.19f, o);
				else
					CreateParticleSync(BITMAP_FLARE, o->Position, o->Angle, o->Light, 0, 0.19f, o);
			}
		}
		...
		{
			CreateParticleSync(BITMAP_FLARE, o->Position, o->Angle, o->Light, 12, 0.19f, o);
		}
		else if (o->SubType == 8)
		{
			o->Scale += timefac(1.8f);
		}
		else if (o->SubType == 9)
		{
			if (o->LifeTime < 20)
			...
			o->HeadAngle[0] += timefac(4.0f);
			o->HeadAngle[1] -= timefac(8.0f);
			o->HeadAngle[2] += timefac(4.0f);
		}
		else if (o->SubType == 10)
		{
			if (o->LifeTime < 20)
		...
		{
			o->HeadAngle[0] += timefac(2.0f);
			o->HeadAngle[1] -= timefac(2.0f);
			o->HeadAngle[2] += timefac(2.0f);
		...
		else if (o->SubType == 12)
		{
			...
			o->Scale += timefac(o->Alpha);
			if (o->Scale > 5.0f)
			{
				o->Light[0] *= timepow(0.5f);
				o->Light[1] *= timepow(0.5f);
				o->Light[2] *= timepow(0.5f);
			}
		}
		else if (o->SubType == 13)
		{
			o->Scale *= timepow(1.1f);
			VectorScale(o->Light, timepow(0.95f), o->Light);
			if (o->Scale > 8)
			{
				o->Live = false;
			}
			if (o->Scale > 4)
			{
				VectorScale(o->Light, timepow(0.5f), o->Light);
			}
		}
		else if (o->SubType == 14)
		{
	...
	case BITMAP_MAGIC + 2:
		...
		else if (o->SubType == 6)
		{
			...
			{
				Vector(1, 1, 1, Light);
				VectorCopy(o->StartPosition, pModel->BodyOrigin);
				o->BlendMeshLight = (float)(rand() % 10) * 0.1f;

				int iIndex;
				for (int i = 0; i < o->LifeTime._runvalue; i++)
				{
					iIndex = rand() % pModel->NumBones;
					if (pModel->Bones[iIndex].Dummy)
						continue;
					Vector(-50 + rand() % 100, -10 + rand() % 20, -10 + rand() % 20, p);
					pModel->TransformPosition(o->Owner->BoneTransform[iIndex], p, Position, true);
					CreateParticleSync(BITMAP_LIGHT, Position, o->Owner->Angle, Light, 8);
				}
			}
			o->Owner->Alpha = o->LifeTime._runvalue / 80.0f;
		}
		else if (o->SubType == 9)
		{
			o->Scale += (timefac(0.3f));
			o->Light[0] /= (timepow(1.1f));
			o->Light[1] /= (timepow(1.1f));
			o->Light[2] /= (timepow(1.1f));
		}
		break;
	case BITMAP_OUR_INFLUENCE_GROUND:
	case BITMAP_ENEMY_INFLUENCE_GROUND:
	...
	case BITMAP_SHINY + 6:
		...
		case 0:
			...
			CreateParticleSync(o->Type, Position, o->Angle, o->Light, 0, o->Scale);
			break;
		case 1:
		case 2:
			if (o->Owner == NULL || o->Owner->Live == false)
			...
			else
			{
				...
				if (pModel)
				{
					...
					if (!pModel->Bones[iBone].Dummy)
					{
						...
						{
							vWorldPos[2] -= (20.f);
							CreateParticleSync(o->Type, vWorldPos, o->Angle, o->Light, 0, o->Scale);
						}
					}
				}
			}
	...
	case MODEL_DESAIR:
	{
		...
		if (oj->Live == true)
		{
			...
			if (o->life_time_work % standlimit(10) == 0)
			{
	...
	case BITMAP_PIN_LIGHT:
		...
		case 0:
			...
			{
				if (o->SubType == 3)
					CreateParticleSync(o->Type, Position, o->Angle, o->Light, 1, o->Scale);
				else
					CreateParticleSync(o->Type, Position, o->Angle, o->Light, 0, o->Scale);
			}
			break;
	...
	case BITMAP_ORORA:
		...
		else
		{
			...
			{
				o->Live = false;
				CreateEffectSync(o->Type, o->Position, o->Angle, o->Light, o->SubType, o->Owner);
			}
		}
		break;
	case BITMAP_SPARK + 2:
	...
	case BITMAP_GATHERING:
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			if (o->SubType == 1)
			{
				if (o->LifeTime.duration(2))
				{
					CreateJoint(BITMAP_JOINT_THUNDER, Position, o->Position, Angle, 3, NULL, 10.f, 10, 10);
				}
				else
				{
					CreateParticleSync(BITMAP_SPARK + 1, Position, Angle, Light, 2, (rand() % 50 + 10) / 100.f, o);
				}
			...
			{
				CreateParticleSync(BITMAP_SPARK + 1, Position, Angle, o->Light, 26, (rand() % 10 + 5) / 25.f, o);
			}
			else
			{
	...
	case BITMAP_JOINT_THUNDER:
	{
		...
		if (rand() % 10 > 4)
		{
			if (o->SubType == 1)
			{
				CreateJointSync(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 33, NULL, fScale);
			}
			else
			{
				CreateJointSync(BITMAP_JOINT_THUNDER, vStartPosition, vPosition, o->Angle, 16, NULL, fScale);
			}
		}
		...
		{
			if (rand() % 2 == 0)
				CreateEffectSync(BITMAP_MAGIC + 1, vPosition, o->Angle, vLight, 11, o);
		}

		if (rand() % 4 == 0)
			CreateParticleSync(BITMAP_SMOKE, vPosition, o->Angle, vLight, 54, 2.8f);

		if (rand() % 4 == 0)
		{
			CreateEffectSync(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, vLight, 13);
		}
		...
		if (o->LifeTime > 5)
		{
			...
			if (rand() % 5 == 0)
				CreateEffectSync(BITMAP_CHROME_ENERGY2, vPosition, o->Angle, vLight, 0);
		}

		if (o->Owner != NULL && rand() % 5 == 0 && o->Owner->BoneTransform != NULL)
		{
			BMD* pTargetModel = gmClientModels->GetModel(o->Owner->Type);
			...
			for (int i = 0; i < iNumBones; ++i)
			{
				...
				CreateParticleSync(BITMAP_LIGHT, vPos, vAngle, vLight, 5, fRandom);
			}
		}
	}
	break;
	...
	case BITMAP_FLAME:
		if (o->SubType == 0)
		{
			for (int j = 0; j < 6; j++)
			{
				...
				VectorAdd(Position, o->Position, Position);
				CreateParticleSync(BITMAP_FLAME, Position, o->Angle, Light);
			}
			if (rand() % 8 == 0)
			{
				CreateEffectSync(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light);
			...
			if (o->Owner == &Hero->Object && o->life_time_work % standlimit(20))
			{
				o->LifeTime = (o->LifeTime._runvalue / 20) * 19.9f;
				o->life_time_work = (o->life_time_work / 20.f) * 19.9;
		...
		else if (o->SubType == 1 || o->SubType == 2)
		{
			for (int j = 0; j < 18; j++)
			{
				...
				if (o->SubType == 1)
					CreateParticleSync(BITMAP_FLAME, Position, o->Angle, Light, 0, 1.2f);
				else if (o->SubType == 2)
					CreateParticleSync(BITMAP_FIRE + 3, Position, o->Angle, Light, 13, 2.5f);
			}
		}
		else if (o->SubType == 3)
		{
			for (int j = 0; j < 3; j++)
			{
				CreateParticleSync(BITMAP_FLAME, o->Position, o->Angle, Light, 6);
				Vector((float)(rand() % 10 - 5), (float)(rand() % 10 - 5), 40.f, Position);
				VectorAdd(Position, o->Position, Position);
				CreateParticleSync(BITMAP_TRUE_FIRE, Position, o->Angle, Light, 0, 2.8f);
			}
			Vector((float)(rand() % 10 - 5), (float)(rand() % 10 - 5), -40.f, Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 21, 0.8f);

			CreateEffectSync(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 12);
		...
		else if (o->SubType == 5)
		{
			...
			VectorAdd(Position, o->Position, Position);
			CreateParticleSync(BITMAP_FLAME, Position, o->Angle, Light, 0, o->Scale);
		}
		else if (o->SubType == 6)
		{
			Vector((float)(rand() % 32 - 16), (float)(rand() % 32 - 16), 0.f, Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticleSync(BITMAP_FLAME, Position, o->Angle, o->Light, 12, o->Scale);
		}
		break;
	case MODEL_RAKLION_BOSS_CRACKEFFECT:
		if (o->SubType == 0)
	...
	case BITMAP_FIRE_HIK2_MONO:
		if (o->SubType == 0)
		{
			for (int i = 0; i < 2; ++i)
			{
				...
				if (rand() % 3 != 0)
				{
					CreateParticleSync(BITMAP_FLAME, Position, o->Angle, o->Light, 11, 1.4f);
				}

				if (rand() % 8 == 0)
				{
					CreateEffectSync(MODEL_ICE_SMALL, Position, o->Angle, o->Light, 0);
				}
			}
		}
		else if (o->SubType == 1)
	...
	case BITMAP_CLOUD:
		if (o->SubType == 0)
		{
			o->Light[0] /= timepow(1.05f);
			o->Light[1] /= timepow(1.05f);
			o->Light[2] /= timepow(1.05f);
			o->Scale += timefac(0.03f);
		}
		break;
	case BITMAP_FIRE_RED:
		Vector((float)(rand() % 32 - 16), (float)(rand() % 32 - 16), 0.f, Position);
		...
		CreateParticleSync(BITMAP_FIRE_RED, Position, o->Angle, Light, 0, o->Scale);
		break;
	case BITMAP_SPARK + 1:
	{
		vec3_t Position;
		VectorCopy(o->Position, Position);
		float Luminosity = (double)o->LifeTime * 0.1f;
		...
		for (int j = 0; j < 18; j++)
		{
			...
			if (j == 0)
				CreateParticleSync(BITMAP_SPARK + 1, Position, o->Angle, Light, 1, Scale * 2.f);
			else
				CreateParticleSync(BITMAP_SPARK + 1, Position, o->Angle, Light, 1, Scale);
		}
		break;
	}
	case BITMAP_ENERGY:
		if (o->SubType == 0)
		{
			Luminosity = (double)o->LifeTime * 0.2f;
			Vector(Luminosity, Luminosity, Luminosity, Light);
			CreateParticleSync(BITMAP_ENERGY, o->Position, o->Angle, Light);
			CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 0, 4.f);
	...
	case MODEL_LIGHTNING_ORB:
	{
		...
		if (o->SubType == 0)
		{
			...
			CreateParticleSync(BITMAP_MAGIC, o->Position, o->Angle, vLight, 0, 1.f);
			for (int i = 0; i < 3; ++i)
			{
				CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 13, 1.0f);
			}
			//CheckTargetRange(o);
		}
		else if (o->SubType == 1)
		{
			...
			if (o->LifeTime >= 15)
			{
				for (int i = 0; i < 5; ++i)
				{
					CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 20, 1.0f);
				}
			}
			// shockwave
			if (o->LifeTime >= 14)
			{
				...
				CreateParticleSync(BITMAP_SHOCK_WAVE, o->Position, o->Angle, vLight, 0, 0.3f);
				CreateParticleSync(BITMAP_SHOCK_WAVE, o->Position, o->Angle, vLight, 0, 0.3f);
			}
			...
			for (int i = 0; i < 2; ++i)
			{
				CreateEffectSync(MODEL_FENRIR_THUNDER, o->Position, o->Angle, vLight, 3, o);
			}
			...
			if (o->LifeTime <= 5)
			{
				for (int i = 0; i < 2; ++i)
				{
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 40);
				}
			}
			o->Light[0] /= timepow(1.08f);
			o->Light[1] /= timepow(1.08f);
			o->Light[2] /= timepow(1.08f);
		}
	}
	break;
	// ChainLighting
	case MODEL_CHAIN_LIGHTNING:
	{
		...
		case 2:
		{
			...
			CHARACTER* pTargetChar = gmCharacters->GetCharacter(FindCharacterIndex(o->m_sTargetIndex));

			if (!pSourceObj || !pSourceObj->Live)
			{
				o->Live = false;
				o->LifeTime = 0;
				return;
			...
			if (!pTargetChar || pTargetChar->Dead > 0)
			{
				o->Live = false;
				o->LifeTime = 0;
				return;
			...
			//f (pSourceObj == NULL || pSourceObj->Live == false || pTargetObj == NULL || pTargetObj->Live == false || pTargetChar->Dead > 0)
			//
			//	o->LifeTime = 0;
			//	o->Live = false;
			//	break;
			...
			BMD* pSourceModel = gmClientModels->GetModel(pSourceObj->Type);
			BMD* pTargetModel = gmClientModels->GetModel(pTargetObj->Type);
			...
			if (pSourceModel && pTargetModel)
			{
				...
				if (o->SubType == 0)
				{
					...
					CreateJointSync(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, vAngle, 0, pTargetObj, 50.f, -1, 0, 0, -1, vLight);
					CreateJointSync(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, vAngle, 0, pTargetObj, 10.f, -1, 0, 0, -1, vLight);
				...
				else if (o->SubType == 1 || o->SubType == 2)
				{
					...
					vTargetPos[2] += (80.0f);
					CreateJointSync(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, pTargetObj->Angle, 0, pTargetObj, 50.f, -1, 0, 0, -1, vLight);
					CreateJointSync(BITMAP_JOINT_THUNDER, vPos, pTargetObj->Position, pTargetObj->Angle, 0, pTargetObj, 10.f, -1, 0, 0, -1, vLight);
				}
	...
	case MODEL_ALICE_DRAIN_LIFE:
	{
		...
		if (pSourceModel && pTargetModel)
		{
			...
			for (int i = 0; i < iCnt; i++)
			{
				...
				CreateParticleSync(BITMAP_LIGHT + 2, vSourcePos, pSourceObj->Angle, vLight, 7, 1.8f);
			}
			...
			if (o->LifeTime <= 60)
			{
				for (int i = 0; i < iCnt; i++)
				{
					...
					Vector(1.0f, 0.2f, 0.2f, vLight);
					CreateParticleSync(BITMAP_LIGHT + 2, vTargetPos, pTargetObj->Angle, vLight, 7, 1.8f);
				}
			}

			if (o->LifeTime >= 66 && o->LifeTime <= 70)
			{
				int iRandom = rand() % 10;
				int iCnt2 = 0;
				switch (iRandom)
				...
				for (int i = 0; i < iCnt2; i++)
				{
					...
					CreateJointSync(BITMAP_DRAIN_LIFE_GHOST, vSourcePos, vTargetPos, o->Angle, 0, pSourceObj, 40.f, 0, 0, 0, -1, vLight);
				}
			}
	...
	case MODEL_ALICE_BUFFSKILL_EFFECT2:
	{
		if (o->SubType == 0 || o->SubType == 1 || o->SubType == 2)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				if (o->SubType == 0 || o->SubType == 2)
				{
					CreateJointSync(BITMAP_JOINT_HEALING, vWorldPos, o->Position, vAngle, 15, o, 5.f, 0, 0, 0, 0, vLight);
				}
				else if (o->SubType == 1)
				{
					CreateJointSync(BITMAP_JOINT_HEALING, vWorldPos, o->Position, vAngle, 16, o, 5.f, 0, 0, 0, 0, vLight);
				}
			}
		}
		else if (o->SubType == 3 || o->SubType == 4)
		{
			...
			else
			{
				...
				if (!pModel->Bones[iBone].Dummy)
				{
					...
					if (o->SubType == 3)
					{
						CreateParticleSync(BITMAP_LIGHT + 2, vWorldPos, o->Angle, o->Light, 6, o->Scale);
					...
					else if (o->SubType == 4)
					{
						...
						{
							vWorldPos[2] -= (20.f);
							CreateParticleSync(BITMAP_TWINTAIL_WATER, vWorldPos, o->Angle, o->Light, 2);
						}
					}
				}
			}
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		if (o->SubType == 0)
		{
			if (o->Owner->AnimationFrame > 6.0f || o->LifeTime < 15)
			{
				...
				o->Gravity += timefac(0.1f);
				o->Velocity += timefac(o->Gravity);
			}

			OBJECT* pObject = o;
			OBJECT* pOwner = pObject->Owner;
			BMD* pOwnerModel = gmClientModels->GetModel(pOwner->Type);
			...
			if (pOwnerModel)
			{
				...
				CreateParticleSync(BITMAP_MAGIC, o->Position, o->Angle, vLight, 0, 1.f * fScale);
				...
				for (int i = 0; i < 11; ++i)
				{
					...
					if (rand() % 3 > 0)
						continue;
					CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, pObject->Angle, vLight, 0, fScale);
				}
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 6; ++i)
			{
				...
				CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPosition, pObject->Angle, vLight, 0, fScale);	// ����
			}
			...
			/*for (int i = 0; i < 2; i++)
				CreateParticleSync(BITMAP_SMOKE, vPosition, o->Angle, vLight, 58);
			*/
			if (rand_fps_check(2))
			{
				Vector(1.0f, 0.0f, 0.0f, vLight);
			...
			CreateEffectSync(MODEL_STONE1 + rand() % 2, vPosition, o->Angle, vLight, 13, o);
		}
		else if (o->SubType == 2)
		{
			if (o->Owner != NULL && o->Owner->BoneTransform != NULL)
			{
				...
				for (int i = 0; i < iNumBones; i++)
				{
					...
					if (rand_fps_check(5))
					{
						...
						CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, vAngle, vLight, 0, fRandom);
					}
				}

				Vector(1.0f, 0.0f, 0.0f, vLight);
				CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 58);

				if (rand_fps_check(4))
				{
					Vector(1.0f, 0.0f, 0.0f, vLight);
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 54, 2.8f);
				}
			}
		}
	}
	...
	case BITMAP_LIGHT:
		...
		{
			VectorAdd(o->Position, o->Direction, o->Position);
			o->Direction[2] -= timefac(0.01f);

			CreateParticleSync(BITMAP_LIGHT, o->Position, o->Angle, o->Light, 1, o->Scale);
		...
		else if (o->SubType == 1)
		{
			...
			CreateParticleSync(BITMAP_LIGHT, o->Position, Angle, Light, 4, Luminosity, o->Owner);
		...
		else if (o->SubType == 2)
		{
			...
			if (((int)(o->Velocity) % 50) == 0)
			{
				if (o->Owner != NULL)
				{
					...
					CreateJointSync(BITMAP_JOINT_HEALING, o->Position, o->Position, o->Angle, 9, o->Owner, 15.f);
				}
			}
		}
		else if (o->SubType == 3)
		{
			...
			if (o->LifeTime == (15) || o->LifeTime == (30))
			{
				if (o->Owner != NULL)
				{
					vec3_t Position;
	...
	case MODEL_SKILL_BLAST:
		...
		if (o->Position[2] < Height)
		{
			...
			if (o->Owner == &Hero->Object && checkNormalizer)
				AttackCharacterRange(o->Skill, o->Position, 150.f, o->Weapon, o->PKKey);
			o->Live = false;
		}
		VectorCopy(o->Position, o->EyeLeft);
	...
	case MODEL_WAVE:
		...
		{
			o->Scale += timefac(0.1f);
			o->Position[0] -= timefac(1.f);
			o->Position[2] -= timefac(1.5f);
			o->BlendMeshLight = o->LifeTime._runvalue / 30.f;
		}
		else
		{
			o->Scale += timefac(1.2f);
			o->Position[0] -= timefac(1.2f);
			o->Position[2] -= timefac(1.8f);
		}
	...
	case MODEL_TAIL:
		o->Position[2] -= timefac(o->Gravity);
		o->Gravity += timefac(60.f);
		o->BlendMeshLight = o->LifeTime._runvalue / 20.f;
		break;
	case MODEL_WAVE_FORCE:
		o->BlendMeshLight = o->LifeTime._runvalue / 20.f;
		Vector(-Luminosity * 0.5f, -Luminosity * 0.5f, -Luminosity * 0.5f, Light);
	...
	case MODEL_SKILL_INFERNO:
		...
		else if (o->SubType == 8)
		{
			...
			if (o->LifeTime < 10)
			{
				o->Light[0] *= (timepow(0.8f));
				o->Light[1] *= (timepow(0.8f));
				o->Light[2] *= (timepow(0.8f));
			}
		}
		else if (o->SubType == 3)
		{
		...
		else if (o->SubType == 4)
		{
			...
			o->Scale += timefac(0.003f);
			o->Gravity += timefac(0.8f * o->Scale * 30.f);
			o->Position[2] += timefac(o->Gravity);
		}
		else if (o->SubType == 5)
		{
			o->Position[2] += timefac(2.f);
			o->Angle[2] += timefac(20.f);
			o->BlendMeshLight = (double)o->LifeTime / 20.f;
		...
		{
			o->Scale += (timefac(0.01f));
			o->BlendMeshLight = (float)o->LifeTime / 5.f * (0.1f);
		...
		{
			o->BlendMeshLight = o->LifeTime._runvalue / 80.f;
			o->Position[2] += (timefac(o->Gravity));
			o->Light[0] /= (timepow(1.04f));
			o->Light[2] /= (timepow(1.04f));
			o->Alpha -= (timefac(0.01f));
		}
		else if (o->SubType == 10)
		{
	...
	case MODEL_BIG_METEO3:
		...
		if ((TerrainWall[Index] & TW_NOGROUND) != TW_NOGROUND)
		{
			...
			if (o->Position[2] < Height)
			{
				...
				for (int j = 0; j < 5; j++)
					CreateEffectSync(MODEL_METEO1 + rand() % 2, o->Position, o->Angle, o->Light, 0);
			}
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		o->Angle[0] += timefac(10.f / o->Scale);
		CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 3);
		return;
	case MODEL_PROTECT:
		...
		break;
	case MODEL_POISON:
		o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
		o->Alpha = o->LifeTime._runvalue * 0.1f;
		Vector(Luminosity * 0.3f, Luminosity * 1.f, Luminosity * 0.6f, Light);
		AddTerrainLight(o->Position[0], o->Position[1], Light, 2, PrimaryTerrainLight);
		break;
	case MODEL_SAW:
		o->Angle[2] -= timefac(30.f);
		break;
	case MODEL_LASER:
		if (o->SubType != 0 && o->SubType != 3)
		{
	...
	case MODEL_MAGIC1:
		VectorCopy(o->Owner->Position, o->Position);
		o->Position[2] += (100.f);
		o->Angle[1] += timefac(20.f);
		o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
		break;
	case MODEL_SKILL_WHEEL1:
		CreateEffectSync(MODEL_SKILL_WHEEL2, o->Position, o->Angle, o->Light, (4) - (int)o->LifeTime, o->Owner, o->PKKey, o->Skill, o->Kind);
		break;
	case MODEL_SKILL_WHEEL2:
		switch (o->SubType)
		{
		...
		o->Angle[2] -= (18) * timefac(1.0);//

		CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 93);
		...
		if (gMapManager->InHellas())
		{
			...
			CreateParticleSync(BITMAP_WATERFALL_5, Position, Angle, Light, 3);
		}
		else
		{
			for (int j = 0; j < 3; j++)
		...
		if (gMapManager->InHellas() && o->life_time_work % standlimit(4))
		{
			int PositionX = (int)(o->Position[0] / TERRAIN_SCALE);
			int PositionY = (int)(o->Position[1] / TERRAIN_SCALE);
	...
	case MODEL_SKILL_FISSURE:
	{
		if (o->LifeTime == 8)
		{
			for (int i = 0; i < 16; ++i)
			{
				...
				CreateJointSync(BITMAP_FLARE, Position, Position, Angle, 24, NULL, 90);
			}
		}
	...
	case MODEL_FISSURE:
	{
		double fLevel = sinf(Q_PI * double(o->LifeTime._runvalue) / 120.f) * -0.5f;
		vec3_t Light;
		...
		if (rand() % 2 == 0)
		{
			...
			VectorRotate(p, Matrix, Position);
			VectorAdd(Position, o->Position, Position);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 0);
		}
		...
		if (pModel)
		{
			...
			for (int i = 0; i < pModel->NumBones; i++) {
				if (!pModel->Bones[i].Dummy && rand() % 12 == 0)
				{
					...
					CreateParticleSync(BITMAP_TRUE_FIRE, Position, o->Angle, Light, 3, 4.f, o);
					CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 21, 1.f);
				}
			}
		}
	}
	...
	case MODEL_SKILL_FURY_STRIKE:
	{
		...
		{
			float count = (float)o->LifeTime._runvalue;
			float addAngle = 15.f;
	...
	break;
	case MODEL_SKILL_FURY_STRIKE + 1:
		o->BlendMeshLight = (o->LifeTime._runvalue * 0.1f) / 3.f;
		if (o->LifeTime < 10)
		{
			o->Position[2] -= timefac(0.5f);
		}
		if (o->LifeTime > 15 && (o->life_time_work % standlimit(3)) == 0)
		{
			EarthQuake = (float)(rand() % 8 - 4) * 0.1f;
		}
		break;
	case MODEL_SKILL_FURY_STRIKE + 3:
		o->BlendMeshLight = (o->LifeTime._runvalue * 0.1f) / 10.f;
		if (o->LifeTime < 13)
		{
			o->Position[2] -= timefac(0.5f);
		}
	...
	case MODEL_SKILL_FURY_STRIKE + 2:
		if (o->Scale > 50)
			o->LifeTime = 0;
		if (o->LifeTime >= 10)
			o->BlendMeshLight = ((20 - o->LifeTime._runvalue) * 0.1f);
		else
		{
			if (o->LifeTime < 5)
			{
		...
		o->BlendMeshTexCoordU = -(float)o->LifeTime._runvalue * 0.01f;
	...
	case MODEL_SKILL_FURY_STRIKE + 5:
		if (o->LifeTime >= 30)
			o->BlendMeshLight = ((40 - o->LifeTime._runvalue) * 0.1f);
		else
		{
			if (o->LifeTime < 15)
			{
	...
	case MODEL_CHANGE_UP_EFF:
	{
		...
		if (rand() % STwo == 0)
		{
			...
			if (o->SubType == 0 || o->SubType == 2)
			{
				CreateJointSync(BITMAP_FLARE, Loc, Loc, o->Angle, 50, NULL, 40);
			}

			if (o->LifeTime > 40)
				CreateEffectSync(BITMAP_MAGIC, o->Position, o->Angle, Light, 4, o, 4.0f);
	...
	case MODEL_CHANGE_UP_NASA:
	{
		...
		{
			o->Scale += timefac(0.02f);
			o->BlendMeshLight = o->LifeTime._runvalue * 0.005f;
		}
		else if (o->SubType == 0)
			o->BlendMeshLight = o->LifeTime._runvalue * 0.01f;
	}
	break;
	case MODEL_CHANGE_UP_CYLINDER:
	{
		...
		if (o->SubType == 1)
		{
			o->BlendMeshLight = o->LifeTime._runvalue * 0.015f;
			o->Scale += timefac(0.08f);
		}
	}
	break;
	case MODEL_DARK_ELF_SKILL:
	{
		...
		if (o->Scale < 0.8f)
			...
			o->Scale -= timefac(0.4f);
		o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
		o->BlendMeshTexCoordU = -o->LifeTime._runvalue * 0.2f;
		...
		for (int j = 0; j < 6; j++)
		{
			CreateParticleSync(BITMAP_SMOKE, Position, Angle, o->Light, 25);
		}
		Vector(Luminosity * 0.3f, Luminosity * 0.6f, Luminosity, Light);
	...
	case MODEL_MAGIC2:
		...
		if (o->SubType == 2)
		{
			...
			VectorCopy(o->Position, Position);
			Angle[2] += (rand() % 10 - 5);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 11, (float)(rand() % 32 + 80) * 0.015f);
	...
	case MODEL_STORM:
		...
		case 0:
			o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
			o->BlendMeshTexCoordU = -o->LifeTime._runvalue * 0.1f;
			...
			if (rand() % 2 == 0)
			{
				...
				CreateJointSync(BITMAP_JOINT_THUNDER, Position, o->Position, Angle, 0, o, 10.f);
			}
			if (rand() % 2 == 0)
			{
				Vector(o->Position[0] + 200.f, o->Position[1], o->Position[2] + 700.f, Position);
			...
			o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
			if (rand() % 4 == 0)
				CreateEffectSync(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 2);
			...
			if (battleCastle::IsBattleCastleStart())
			{
				...
				if ((att & TW_NOATTACKZONE) == TW_NOATTACKZONE)
				{
					o->Velocity = 0.f;
					Vector(0.f, 0.f, 0.f, o->Direction);
					o->LifeTime._runvalue /= timepow(5.f);
					o->life_time_work /= 5.f;

					break;
				}
			}
			if (o->life_time_work % standlimit(15))
				if (o->Owner == &Hero->Object)
					AttackCharacterRange(o->Skill, o->Position, 150.f, o->Weapon, o->PKKey);
			break;
		...
		case 1:
			o->BlendMeshLight = o->LifeTime._runvalue * 0.01f;
			o->BlendMeshTexCoordU = -(float)o->LifeTime._runvalue * 0.1f;
			o->Angle[2] += (rand() % 30 + 30.f);

			VectorCopy(o->Position, Position);
			Position[2] += (100.f);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, o->Light, 3);
			CreateParticleSync(BITMAP_BUBBLE, Position, o->Angle, o->Light, 3, 0.1f);
		...
		case 7:
			EarthQuake = (float)(rand() % 8 - 4) * 0.1f;
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 28);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 29);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 30);
			if (rand() % 2 == 0)
				CreateEffectSync(MODEL_STONE1 + rand() % 2, o->Position, o->Angle, o->Light, 2);
			break;
		case 8:
	...
	case MODEL_SUMMON:
	{
		if (o->LifeTime >= 30)
		{
			o->Light[0] *= timepow(1.03f);
			o->Light[1] *= timepow(1.03f);
			o->Light[2] *= timepow(1.03f);
		}
		else
		{
			o->Light[0] /= timepow(1.08f);
	...
	case MODEL_STORM2:
	{
		if (o->SubType == 0)
		{
			o->Angle[1] += timefac(40.0f);

			if (o->LifeTime <= 5)
			{
				o->Light[0] /= timepow(1.01f);
				o->Light[1] /= timepow(1.01f);
				o->Light[2] /= timepow(1.01f);
			}
		}
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				CreateParticleSync(BITMAP_CLUD64, Position, o->Angle, Light, 1, 1.0f);
			}
			...
			CreateJointSync(BITMAP_JOINT_THUNDER, Pos1, Pos2, Angle, 20, o, 20.f);

			Pos2[0] = Pos1[0] + rand() % 100 - 50.0f;
			Pos2[1] = Pos1[1] + rand() % 100 - 50.0f;
			Pos2[2] = Pos1[2];
			CreateEffectSync(MODEL_STONE1 + rand() % 2, Pos2, o->Angle, Light, 2);
	...
	case MODEL_MAYASTAR:
	{
		if (o->LifeTime <= 20)
		{
			o->Light[0] /= timepow(1.2f);
			o->Light[1] /= timepow(1.2f);
			o->Light[2] /= timepow(1.2f);
		}
		o->Scale += (sinf(WorldTime * 0.005f) * 2.0f);
		o->Angle[1] += timefac(10.0f);
	...
	case MODEL_MAYASTONE3:
	{
		...
		if (success)
		{
			...
			Vector(0.f, 0.f, 0.f, Angle);
			CreateEffectSync(MODEL_SKILL_INFERNO, o->Position, Angle, Light, 2, o, 30, 0);
			...
			for (int i = 0; i < smokeNum; ++i)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 11, (float)(rand() % 32 + 80) * 0.025f);
			}

			for (int j = 0; j < 6; j++)
				CreateEffectSync(MODEL_MAYASTONE4 + rand() % 2, o->Position, o->Angle, Light);

			CreateParticleSync(BITMAP_EXPLOTION, Position, o->Angle, Light, 0, 4.0f);
			o->Live = false;
		}
	}
	break;
	...
	case MODEL_MAYASTONE5:
	{
		...
		o->Position[2] += (o->Gravity);
		o->Gravity -= timefac(3.f);
		Height = RequestTerrainHeight(o->Position[0], o->Position[1]);
		if (o->Position[2] < Height)
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * timepow(0.5f);
			o->LifeTime -= timefac(4);
			o->Angle[0] -= timefac(o->Scale * 128.f);
		}
		else
			o->Angle[0] -= timefac(o->Scale * 32.f);

		if (rand() % 10 == 0)
			CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, o->Light, 15);
	}
	break;
	case MODEL_MAYASTONEFIRE:
	{
	...
	case MODEL_MAYAHANDSKILL:
	{
		if (o->SubType == 0)
		{
			o->Light[0] /= timepow(1.17f);
			o->Light[1] /= timepow(1.17f);
			o->Light[2] /= timepow(1.17f);

			vec3_t vPos;
			VectorCopy(o->Position, vPos);
			vPos[2] += (100.f);
			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, o->StartPosition, 17, 10.0f);
			...
			CreateEffectSync(MODEL_MAYAHANDSKILL, o->Position, o->Angle, o->StartPosition, 1, NULL, -1, 0, 0, 0, o->Scale + 0.2f);
	...
	case MODEL_CIRCLE:
		...
		if (o->SubType == 1)
		{
			if (o->LifeTime > (44 - o->Owner->m_bySkillCount))
			{
				...
				for (int i = 0; i < iCount; ++i)
				{
					...
					VectorCopy(o->Position, Position);
					Position[2] += (100.f);
					CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 6, o, 60.f, 0, 0);
		...
		else if (o->SubType == 2)
		{
			if (o->LifeTime > 240)
			{
				o->BlendMeshLight = (250 - o->LifeTime._runvalue) * 0.1f;
			}
			else
			{
				o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
		...
		else if (o->SubType == 3)
		{
			if (o->LifeTime > 10)
			{
				o->BlendMeshLight = (30 - o->LifeTime._runvalue) * 0.1f;
			}
			else
			{
				o->BlendMeshLight = o->LifeTime._runvalue * 0.1f;
		...
		else if (o->SubType == 4)
		{
			if (o->LifeTime > (44 - o->Owner->m_bySkillCount))
			{
				...
				for (int i = 0; i < 36; ++i)
				{
					...
					VectorCopy(o->Position, Position);
					Position[2] += (100.f);
					CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 22, o, 2.0f, 0, 0);
					CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 23, o, 1.75f, 0, 0);
				}
			}
		}
		break;
	case MODEL_CIRCLE_LIGHT:
		if (o->SubType != 3 && o->SubType != 4)
		{
			...
			if (rand() % value == 0)
			{
				...
				VectorRotate(p, Matrix, Position);
				VectorAdd(Position, o->Position, Position);
				CreateEffectSync(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 1);
			}
		...
		else if (o->SubType == 3)
		{
			...
			if (o->LifeTime > 30 && rand() % value == 0)
			{
				...
				CreateParticleSync(BITMAP_FLARE_BLUE, Position, o->Angle, o->Light, 0);
				if (o->LifeTime > 40)
				{
					Position[2] += (600.f);
					Angle[2] = 45.f;
					CreateJointSync(BITMAP_FLARE_BLUE, Position, Position, Angle, 19, NULL, 40);
				}
			}
	...
	case MODEL_STONE2:
		...
		else if (o->SubType == 11)
		{
			if (o->LifeTime < 10)
			{
				o->Alpha *= (timepow(0.8f));
			}
			break;
		}
		else if (o->SubType == 13 || o->SubType == 14)
		{
			o->HeadAngle[2] -= (timefac(o->Gravity));
			...
			Height = RequestTerrainHeight(o->Position[0], o->Position[1]);
			o->Angle[0] += (0.5f * o->LifeTime._runvalue);
			o->Angle[1] += (0.5f * o->LifeTime._runvalue);
			if (o->Position[2] + o->Direction[2] <= Height)
			{
				o->Position[2] = Height;
				o->HeadAngle[0] *= (timepow(0.6f));
				o->HeadAngle[1] *= (timepow(0.6f));
				o->HeadAngle[2] += (o->LifeTime._runvalue);
	...
	case MODEL_EFFECT_SAPITRES_ATTACK_2:
		...
		else if (o->Type == MODEL_EFFECT_SAPITRES_ATTACK_2 && o->SubType == 14)
		{
			...
			o->Position[0] -= timefac((o->Direction[0] * fMoveSpeed));
			o->Position[1] -= timefac((o->Direction[1] * fMoveSpeed));
			o->Position[2] -= timefac((o->Direction[2] * fMoveSpeed));
		...
		if (o->SubType == 0 || o->SubType == 10 || o->SubType == 12 || o->SubType == 13)
		{
			...
			if ((o->SubType == 0 || o->SubType == 12 || o->SubType == 13) && rand() % 10 == 0)
			{
				...
				{
					CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, o->Light, 1 + rand() % 3);
				}
			}
		}
		else if (o->SubType == 1)
			o->Gravity += timefac(0.5f);
	...
	case MODEL_NARCONDRA_STONE3:
		...
		else if (o->SubType == 1)
		{
			...
			if (bSuccess)
			{
				...
				CreateEffectSync(MODEL_SKILL_INFERNO, o->Position, Angle, Light, 2, o);
				...
				for (int i = 0; i < 8; ++i)
				{
					...
					CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, Light, 11, (float)(rand() % 40 + 60) * 0.025f);
				}

				for (int j = 0; j < 6; j++)
				{
					CreateEffectSync(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, vPos, o->Angle, Light, 0);
				}
				...
				CreateParticleSync(BITMAP_EXPLOTION_MONO, vPos, o->Angle, Light, 1, 1.5f);
		...
		else if (o->SubType == 2)
		{
			...
			if (o->Position[2] < fHeight)
			{
				CreateEffectSync(o->Type, o->Position, o->Angle, o->Light, 0);
				o->Live = false;
			}
		}
		break;
	case MODEL_SNOW1:
		o->Direction[2] -= timefac(2.5f);
		CheckTargetRange(o);
		break;
	case MODEL_WOOSISTONE:
		if (o->SubType == 1)
		{
			...
			if (o->Position[2] + o->Direction[2] <= Height)
			{
				...
				o->HeadAngle[0] *= timepow(0.6f);
				o->HeadAngle[1] *= timepow(0.6f);
				o->HeadAngle[2] += (1.0f * o->LifeTime._runvalue);
				if (o->HeadAngle[2] < 0.5f)
					o->HeadAngle[2] = 0;
	...
	case MODEL_SKULL:
		if (o->SubType == 1 && o->Owner != NULL)
		{
			...
			o->HeadAngle[0] += ((float)(rand() % 32 - 16) * timepow(0.2f));
			o->HeadAngle[2] += ((float)(rand() % 32 - 16) * timepow(0.8f));
			o->Angle[0] += (o->HeadAngle[0]);
			o->Angle[2] += (o->HeadAngle[2]);
			o->HeadAngle[0] *= (timepow(0.6f));
			o->HeadAngle[2] *= (timepow(0.8f));

			if (o->LifeTime < 10)
			{
				o->Alpha /= (timepow(1.1f));
				o->BlendMeshLight /= (timepow(1.2f));
			...
			CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 5, 1.2f);
				...
				else
				{
					...
					if (bUseEarthQuake == TRUE)
					{
						...
						for (int i = 0; i < 100; ++i)
						{
							...
							VectorCopy(o->Angle, Angle);
							Angle[2] += (rand() % 180);
							CreateParticleSync(BITMAP_WATERFALL_5, Position, Angle, Light, 2);
						}
					}
					VectorCopy(o->Position, Position);
					...
					Position[2] -= (50.f);
					CreateParticleSync(BITMAP_BUBBLE, Position, o->Angle, o->Light, 4);
			...
			else
			{
				if (o->SubType != 2 && o->LifeTime > 200 + 50 + 135)
				{
					...
					if (rand() % 2 == 0)
					{
						...
						if (rand() % 2 == 0)
							CreateJointSync(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 8, NULL, 50.f, 0, 0);
					}
					...
					if (rand() % 3 == 0)
					{
						vec3_t Light;
						Vector(0.5f, 0.5f, 0.5f, Light);
						CreateParticleSync(BITMAP_SMOKE + 3, o->Position, o->Angle, Light, 2, (float)(rand() % 32 + 48) * 0.02f);
					}
				}
			}
			break;
	...
	case MODEL_CURSEDTEMPLE_STATUE_PART2:
		...
		if (o->Position[2] > 290)
		{
			o->Direction[2] -= timefac(o->Gravity);
			o->Angle[2] -= timefac((float)(rand() % 10));
		}
	...
	case MODEL_XMAS2008_SNOWMAN_HEAD:
	{
		if (o->LifeTime < 45)
		{
			o->Position[0] += (o->Direction[0] * timepow(1.2f));
			o->Position[1] += (o->Direction[1] * timepow(1.2f));
			o->Position[2] += ((o->Gravity * 1.5f));
			o->Gravity -= timefac(1.5f);
	...
	case MODEL_NARCONDRA_NECK:
		...
		o->Position[0] += timefac(o->HeadAngle[0]);
		o->Position[1] += timefac(o->HeadAngle[1]);
		o->Position[2] += timefac(o->HeadAngle[2]);
		...
		{
			o->Position[2] = Height;
			o->HeadAngle[0] *= timepow(0.8f);
			o->HeadAngle[1] *= timepow(0.8f);
			o->HeadAngle[2] += (0.6f * o->LifeTime._runvalue);
		...
		else
		{
			if (o->SubType == 0)
			{
				o->Angle[0] += (0.15f * o->LifeTime._runvalue);
				o->Angle[1] += (0.15f * o->LifeTime._runvalue);
			}
			else
			{
				o->Angle[0] -= (0.15f * o->LifeTime._runvalue);
	...
	case MODEL_DOPPELGANGER_SLIME_CHIP:
		...
		if (o->Position[2] + o->Direction[2] <= Height)
		{
			...
			o->HeadAngle[0] *= timepow(0.8f);
			o->HeadAngle[1] *= timepow(0.8f);
			o->HeadAngle[2] += (1.6f * o->LifeTime._runvalue);
			if (o->HeadAngle[2] < 5.0f)
				o->HeadAngle[2] = 0;
		}
		else
		...
		if (o->SubType == 0)
		{
			o->Angle[0] += (0.35f * o->LifeTime._runvalue);
			o->Angle[1] += (0.35f * o->LifeTime._runvalue);
		}
		else
		{
			o->Angle[0] -= (0.35f * o->LifeTime._runvalue);
	...
	case MODEL_ICE_GIANT_PART5:				case MODEL_ICE_GIANT_PART6:
		...
		{
			o->Position[2] = Height;
			o->HeadAngle[0] *= timepow(0.5f);
			o->HeadAngle[1] *= timepow(0.5f);
			o->HeadAngle[2] += (0.6f * o->LifeTime._runvalue);
			if (o->HeadAngle[2] < 5.0f)
				o->HeadAngle[2] = 0;
	...
	case MODEL_WATER_WAVE:
		...
		o->Velocity -= timefac(10.f);
		o->Gravity -= timefac(20.f);
		if (o->Gravity < 60)
			o->Gravity = 60;
		...
		for (int j = 0; j < 4; j++)
		{
			CreateParticleSync(BITMAP_WATERFALL_5, o->Position, o->Angle, Light, 1);
		}
	...
	case MODEL_STAFF + 8:
		...
		if (o->Position[2] < Height)
		{
			VectorCopy(o->Position, Position);
			Position[2] += (80);
			CreateParticleSync(BITMAP_EXPLOTION, Position, o->Angle, Light);
	...
	case MODEL_PIERCING:
		...
		if (o->SubType == 1)
		{
			...
			CreateJointSync(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, 20.f, 7); //  ���� 
	...
	case MODEL_PIERCING + 1:
		if ((rand() % 2) == 0)
		{
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 0);
		}
		CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 5);
		CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 5);
	...
	case MODEL_ARROW_BEST_CROSSBOW:
		if (o->LifeTime > 25)
		{
			o->BlendMeshLight = (30 - o->LifeTime._runvalue) / 40.f;
		}
		else
		{
			o->BlendMeshLight = 1.f;
		}
		o->Angle[1] += timefac(rand() % 60 + 30.f);
		VectorCopy(o->Position, o->EyeLeft);
	...
	case MODEL_ARROW_HOLY:
		if (o->SubType == 1)
		{
			o->Angle[1] += timefac(60.f);

			if (o->life_time_work == standlimit(13))
		...
		else
		{
			...
			if (o->SubType != 0)
			{
				...
				else if (o->life_time_work == standlimit(30))
				{
					o->AttackPoint[0] = 0;
					o->Kind = 1;
				}
	...
	case MODEL_ARROW:
		...
		else if (o->SubType == 5)
		{
			...
			CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light);
			CheckClientArrow(o);
		}
		else
		{
	...
	case MODEL_LACEARROW:
		if (o->Type == MODEL_ARROW_NATURE)
		{
			if (o->SubType == 1)
			{
				...
				if ((size_t)o->life_time_work > standlimit(25) && o->life_time_work % standlimit(2) == 0)
				{
					for (int j = 0; j < 2; j++)
					{
			...
			else
			{
				...
				for (int j = 0; j < 4; j++)
				{
					Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
					VectorAdd(Position, o->Position, Position);
					CreateParticleSync(BITMAP_FLOWER01 + rand() % 3, Position, o->Angle, o->Light);
				}
				o->Angle[1] += timefac(30.f);
		...
		else if (o->Type == MODEL_LACEARROW)
		{
			...
			if (o->life_time_work % standlimit(2) == 0)
			{
				for (int j = 0; j < 3; j++)
				{
					Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
			...
			VectorCopy(o->Position, o->EyeLeft);
			CreateEffectSync(MODEL_WAVES, o->Position, o->Angle, o->Light, 3, NULL, 0);
		...
		else if (o->Type == MODEL_ARROW_WING)
		{
			...
			for (int j = 0; j < 4; j++)
			{
				Vector((float)(rand() % 16 - 8), (float)(rand() % 16 - 8), (float)(rand() % 16 - 8), Position);
				VectorAdd(Position, o->Position, Position);
				CreateParticleSync(BITMAP_BUBBLE, Position, o->Angle, o->Light, 1);
			}
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 0);
	...
	case MODEL_DARK_SCREAM:
	{
		...
		CreateParticleSync(BITMAP_FLAME, o->Position, o->Angle, o->Light, 8, (o->Scale - 0.4f) * 3.5f);
		if (o->Type == MODEL_DARK_SCREAM)
		{
			CheckClientArrow(o);
		}
	...
	case MODEL_CURSEDTEMPLE_PRODECTION_SKILL:
	{
		...
		BMD* herobmd = gmClientModels->GetModel(o->Owner->Type);
		if (herobmd)
		{
		...
		BMD* effbmd = gmClientModels->GetModel(o->Type);
		if (effbmd)
		{
			...
			CreateEffectSync(BITMAP_SHOCK_WAVE, o->Owner->Position, o->Angle, o->Light, 10, o->Owner);
		}
	}
	break;
	case MODEL_CURSEDTEMPLE_RESTRAINT_SKILL:
	...
	case MODEL_ARROW_RING:
	{
		...
		CreateEffectSync(MODEL_WAVES, o->Position, o->Angle, o->Light, 4, NULL, 0);
	...
	case MODEL_ARROW_GAMBLE:
	{
		...
		for (int j = 0; j < 2; j++)
		{
			Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
			VectorAdd(Position, o->Position, Position);
			CreateParticleSync(BITMAP_SPARK + 1, Position, o->Angle, vLight, 30, 1.f);
		}

		VectorCopy(o->Position, o->EyeLeft);
		CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 24, 1.0f);
	...
	case MODEL_FEATHER_FOREIGN:
	{
		...
		case 4:
		{
			o->Light[0] *= timepow(0.97f);
			o->Light[1] *= timepow(0.97f);
			o->Light[2] *= timepow(0.97f);

			o->Alpha *= timepow(0.97f);
		...
		case 2:
			if (o->Owner != NULL)
			{
				...
				if (o->SubType == 1)
				{
					...
					Vector(1.f, 1.f, 1.f, Light);
					CreateParticleSync(BITMAP_FIRE + 2, Position, o->Angle, Light, 10, 2.f);
				}
			}
			else
			{
	...
	case MODEL_FIRE:
		...
		{
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Direction, 36, 1.0f + o->Scale, o);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Direction, 37, 2.0f + o->Scale, o);
		...
		if (o->SubType == 5)
		{
			...
			if ((rand() % 5) != 0)
			{
				CreateParticleSync(BITMAP_POUNDING_BALL, o->Position, o->Angle, Light);
			}
		}
		else
		{
			...
			if (o->SubType == 6 || o->SubType == 7)
			{
				for (int j = 0; j < 2; j++)
				{
					...
					Position[1] = o->Position[1];
					Position[2] = o->Position[2];
					CreateParticleSync(BITMAP_FIRE, Position, o->Angle, Light, 5);
				}
			}
			else if (o->SubType != 3)
			{
			...
			else
			{
				...
				VectorScale(vRot, timepow(50.f), vRot);
				VectorCopy(o->Position, Pos);
				VectorAdd(Pos, vRot, Pos);

				Vector(0.0f, 0.0f, 0.0f, Angle);
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticleSync(BITMAP_FIRE, Pos, Angle, Light, 8, o->Scale * 2.5f);
	...
	case MODEL_WARCRAFT:
		...
		break;
	case BITMAP_FIRECRACKERRISE:
		if (o->LifeTime.duration(5) && (0 == (rand() % 3)))
		{
	...
	case BITMAP_FIRECRACKER:
		...
		CreateParticleSync(BITMAP_FIRECRACKER, o->Position, o->Angle, o->Light, -1);
	...
	case BITMAP_FIRECRACKER0002:
	{
		if (rand() % 5 == 0)
		{
			...
			CreateEffectSync(BITMAP_FIRECRACKER0003, Position, o->Angle, vLight, 0, o, -1, 0, 0, 0, fScale);
		}
	}
	break;
	case BITMAP_FIRECRACKER0003:
	{
		...
		if (o->LifeTime > 15 - 8)
		{
			iFrame = (15 - o->LifeTime._runvalue) / 8.0f * 7;
			if (iFrame >= 7)
				iFrame = 6;
	...
	case BITMAP_BLIZZARD:
		if (o->LifeTime <= 15)
		{
			if (o->SubType == 0)
			{
				...
				o->StartPosition[0] -= timefac(10.f);

				CreateParticleSync(BITMAP_FIRE + 2, o->Position, o->Angle, Light, 7, o->Scale);

				o->Light[0] += timefac(0.1f);
				o->Light[1] = o->Light[0];
				o->Light[2] = o->Light[0];
			...
			else if (o->SubType == 1)
			{
				...
				CreateParticleSync(BITMAP_FIRE + 2, o->Position, o->Angle, Light, 11, o->Scale);
				...
				if (o->Position[2] < Height)
				{
					...
					if ((rand() % 5) == 0)
						CreateEffectSync(MODEL_ICE_SMALL, Position, o->Angle, o->Light);
				}
			}
		}
		break;
	case BITMAP_SHOTGUN:
	{
		...
		o->Scale = ((15 - o->LifeTime._runvalue) / 20.f) * (rand() % 3 + 2);

		AngleMatrix(Angle, Matrix);
		Vector(0.f, 20.f, 0.f, p);
		VectorRotate(p, Matrix, Position);
		VectorAdd(o->Position, Position, o->StartPosition);
		CreateParticleSync(BITMAP_FIRE + 2, o->StartPosition, o->Angle, Light, 10, o->Scale);
	...
	case MODEL_GATE + 1:
		VectorAdd(o->Position, o->Direction, o->Position);
		VectorScale(o->Direction, timepow(0.9f), o->Direction);
		o->Position[2] += timefac(o->Gravity);

		if (o->SubType == 1)
		{
			VectorScale(o->Direction, timepow(1.1f), o->Direction);
		}
		...
		if (o->Position[2] < Height)
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * timepow(0.5f);
			o->LifeTime -= timefac(5);
			o->Angle[0] -= timefac(o->Scale * 128.f);
		}
		else
			o->Angle[0] -= timefac(o->Scale * 16.f);
	...
	case MODEL_STONE_COFFIN + 1:
		...
		{
			o->Position[2] = Height;
			o->Gravity = -o->Gravity * timepow(0.2f);
			o->LifeTime -= timefac(5);
			o->Angle[0] -= timefac(o->Scale * 128.f);
		}
		else
		...
		o->Alpha = o->LifeTime._runvalue / 10.f;
		if ((rand() % 10) == 0)
		{
			CreateParticleSync(BITMAP_SMOKE + 1, Position, o->Angle, Light);
		}
	...
	case MODEL_SHINE:
		if (o->SubType == 0 && o->Owner != NULL && o->LifeTime > 10)
		{
			...
			CreateParticleSync(BITMAP_SHINY, Position, o->Angle, o->Light, 2, Scale);
		}
		break;
	case MODEL_BLIZZARD:
		if (o->SubType == 0)
		{
			...
			if ((rand() % 2) == 0)
			{
				CreateParticleSync(BITMAP_ENERGY, o->Position, o->Angle, Light, 1, 0.5f);
			}
			else
			{
				CreateParticleSync(BITMAP_FIRE + 2, o->Position, o->Angle, Light, 7, o->Scale);
	...
	case MODEL_ARROW_DRILL:
		if (o->SubType == 0 || o->SubType == 2)
		{
			...
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);
			CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 13);

			if (o->life_time_work == standlimit(1))
			{
				CreateBomb(o->Position, true);
			}
	...
	case MODEL_COMBO:
		if (o->SubType == 0)
		{
			if (o->LifeTime > 4)
			{
				o->Scale += timefac(o->Gravity);
				o->Gravity += timefac(0.1f);
			}
			o->BlendMeshLight /= timepow(1.4f);
		}
		break;
	case MODEL_WAVES:
		if (o->SubType == 0)
		...
		{
			o->Scale += timefac(o->Gravity);
			o->Gravity += timefac(0.07f);
			if (o->Scale > 2.f)
				o->Scale = 2.f;
			o->BlendMeshLight /= timepow(1.5f);
		}
		else if (o->SubType == 2)
		{
			o->Scale += timefac(o->Gravity);
			o->Gravity += timefac(0.01f);
			if (o->Scale > 1.5f)
				o->Scale = 1.5f;
			o->BlendMeshLight /= timepow(1.5f);
		}
		...
		{
			o->Scale += timefac(o->Gravity);
			o->Gravity += timefac(0.005f);
			if (o->Scale > 1.5f)
				o->Scale = 1.5f;
			o->BlendMeshLight /= timepow(1.3f);
			o->Angle[1] += timefac(45.f);
		}
		else if (o->SubType == 4)
		{
			o->Scale += timefac(o->Gravity);
			o->Gravity += timefac(0.002f);
			if (o->Scale > 2.5f)
				o->Scale = 2.5f;
			o->BlendMeshLight /= timepow(1.2f);
			o->Angle[1] += timefac(45.f);
	...
	case MODEL_AIR_FORCE:
		...
		if (o->SubType == 1)
		{
			o->BlendMeshLight = o->LifeTime._runvalue / 10.f;
	...
	case MODEL_PIERCING2:
		...
		if (o->SubType == 1)
		{
			if (o->LifeTime > 1)
			{
				o->BlendMeshLight /= timepow(1.6f);
				CreateEffectSync(MODEL_WAVES, o->Position, o->Angle, o->Light, 2, NULL, (int)o->LifeTime);
			}
		}
		else if (o->SubType == 2)
		{
	...
	case MODEL_DEASULER:
	{
		...
		float	fCurrentRate = 1.0f - ((float)o->LifeTime._runvalue / (float)o->ExtState);
		...
		if (iTYPESWORDFORCE == 1)
		{
			if (fCurrentRate > fRateBlurStart && fCurrentRate < fRateBlurEnd)
			{
				...
				float	fPreRate = 1.0f - (float)((o->LifeTime._runvalue) + 1) / (float)(o->ExtState);
				...
				if (fPreRate < fCurrentRate)
				{
					for (int Loop_bk = 0; Loop_bk < 2; Loop_bk++)
					{
						...
						fStartRate = 1.0f - (float)((o->LifeTime._runvalue) + 2) / (float)(o->ExtState);
						fEndRate = 1.0f - (float)((o->LifeTime._runvalue) + 1) / (float)(o->ExtState);
						...
						for (int i = 0; i < iAccess; i++)
						{
							...
							CreateObjectBlur(o, arrEachBonePos[iBone01], arrEachBonePos[iBone02], vLightBlur, iTypeBlur, false, iBlurIdentity, 20);
		...
		if (o->SubType == 0)
		{
			if (o->Owner != NULL)
			{
				...
				for (int i = 1; i < o->Gravity; ++i)
				{
					...
					{
						if (o->Angle[0] < -90)
							o->Angle[0] += timefac(20.f);
						else
							o->Angle[0] -= timefac(20.f);
					}
					Distance = MoveHumming(o->Position, o->Angle, p, o->Velocity);
					o->Velocity += timefac(0.4f);

					if (o->LifeTime < 10)
					{
						o->Velocity += timefac(0.1f);
						CreateEffectSync(BITMAP_MAGIC + 1, Position, o->Angle, o->Light, 1, o);
					}

					AngleMatrix(o->Angle, Matrix);
					VectorRotate(o->Direction, Matrix, Position);
					VectorAdd(o->Position, Position, o->Position);
					CreateEffectSync(MODEL_TAIL, o->Position, o->Angle, o->Light, 0, o);
				}
	...
	case MODEL_PIER_PART:
		if (o->SubType == 0)
		{
			if (o->Owner != NULL)
			{
				...
				for (int i = 1; i < o->Gravity; i++)
				{
					...
					VectorRotate(o->Direction, Matrix, Position);
					interpolatesum2(o->Position, Position, o->Position);
					CreateEffectSync(MODEL_PIER_PART, o->Position, o->Angle, o->Light, 1, o);
				}
				if (Distance < 40 && o->life_time_work == standlimit(5))
				{
					VectorCopy(o->Position, Position);
					Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
				}

				o->Gravity = 2.f + 0.1f * (20.f - (int)o->LifeTime);
		...
		else if (o->SubType == 2)
		{
			if (o->Owner != NULL)
			{
				...
				if ((o->life_time_work % standlimit(3)) == 0)
				{
					Vector(-90.f, 0.f, o->Angle[2], Angle);
	...
	case MODEL_DARKLORD_SKILL:
		if (o->SubType <= 1)
		{
			o->Scale += timefac(o->Velocity);
			o->Velocity += timefac(0.02f);
			if (o->LifeTime < 7)
				o->BlendMeshLight /= (timepow(1.8f));
		}
		else if (o->SubType == 2)
		{
			o->Scale += timefac(o->Velocity);
			o->Velocity -= timefac(0.06f);
			if (o->LifeTime < 8)
				o->BlendMeshLight /= (timepow(1.5f));
		}
		else
	...
	case MODEL_GROUND_STONE2:
		if (o->LifeTime > 32 && o->LifeTime < 37)
		{
			...
			Vector(1.f, 0.8f, 0.6f, Light);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 11, 2.f, o);
			CreateEffectSync(MODEL_STONE1 + rand() % 2, Position, o->Angle, o->Light, 10);
		}

		if (o->LifeTime < 8)
		{
			o->Alpha /= (timepow(1.3f));
			o->BlendMeshLight /= (timepow(2.f));
		}
	...
	case BITMAP_TWLIGHT:
	{
		...
		{
			o->Scale -= timefac(0.1f);
			o->Angle[2] += timefac(5.f);
		}
		else if (o->SubType == 2)
		{
			o->Scale -= timefac(0.1f);
			o->Angle[2] += timefac(15.f);
		}
		else if (o->SubType == 3)
		{
			VectorCopy(o->Owner->Position, o->Position);
			o->Scale -= timefac(0.15f);
			o->Angle[2] += timefac(10.f);
			if (o->LifeTime >= 20)
			{
				o->PKKey++;
				o->Alpha += timefac(0.1f);
				o->Light[0] = o->EyeRight[0] * (o->PKKey * timepow(0.1f));
				o->Light[1] = o->EyeRight[1] * (o->PKKey * timepow(0.1f));
				o->Light[2] = o->EyeRight[2] * (o->PKKey * timepow(0.1f));
			}
			else if (o->LifeTime <= 10)
			{
				o->PKKey--;
		...
		{
			o->Angle[2] -= timefac(60.0);
			o->Scale += timefac(0.12f);
			o->Position[2] = 130.0;

			if (o->LifeTime <= 18)
			{
				o->Light[0] *= timepow(0.75);
				o->Light[1] *= timepow(0.75);
				o->Light[2] *= timepow(0.75);
			}
			else
			{
				o->Light[0] *= timepow(1.05);
				o->Light[1] *= timepow(1.05);
				o->Light[2] *= timepow(1.05);
			}
		}
		else if (o->SubType == 5)
		{
	...
	case BITMAP_SHOCK_WAVE:
		...
		{
			o->Scale += (timefac((rand() % 5) / 10.f));

			o->Position[0] += timefac((rand() % 8 - 4.f));
			o->Position[1] += timefac((rand() % 8 - 4.f));
		}
		else if (o->SubType == 2)
		{
			o->Scale += (timefac((rand() % 5) / 40.f));
		...
		{
			if (o->LifeTime > 4.0f)
				o->Scale += (timefac((o->LifeTime._runvalue - 4.0f) * 0.25f));
		}
		else if (o->SubType == 13)
		{
			o->Scale += (timefac(0.08f));
		...
		if (o->SubType >= 0 && o->SubType <= 3)
		{
			if (o->LifeTime <= 20)
			{
				Luminosity = o->LifeTime._runvalue / 20.f;
				Vector(Luminosity, Luminosity, Luminosity, o->Light);
			}
			else
			{
				Luminosity = (40 - o->LifeTime._runvalue) / 20.f;
				Vector(Luminosity, Luminosity, Luminosity, o->Light);
			}
		}
		else
		{
			if (o->LifeTime < 6)
			{
				o->Light[0] /= (timepow(1.3f));
				o->Light[1] /= (timepow(1.3f));
				o->Light[2] /= (timepow(1.3f));
			}
		}
		break;
	case BITMAP_DAMAGE_01_MONO:
	...
	case BITMAP_FLARE:
		if (o->SubType == 1)
		{
			CreateParticleSync(BITMAP_FLARE, o->Position, o->Angle, Light, 11, 1.0f);
		}
		else if (o->SubType == 2)
		{
			CreateParticleSync(BITMAP_FLARE_BLUE, o->Position, o->Angle, Light, 1, 1.0f);
		}
		else if (o->SubType == 3)
		{
			Vector(0.9f, 0.4f, 0.1f, Light);
			float fRandom = 3.5f + ((float)(rand() % 20 - 10) * 0.1f);
			CreateParticleSync(BITMAP_LIGHT, o->Position, o->Angle, Light, 5, fRandom);
		}
		else
			if (o->LifeTime.duration(2))
			{
		...
		break;
	case BITMAP_JOINT_THUNDER + 1:
		if (o->LifeTime < 5 && o->LifeTime.duration(2) && o->SubType == 0)
		{
			VectorCopy(o->Position, Position);
	...
	case MODEL_CUNDUN_DRAGON_HEAD:
		...
		if (o->Position[2] > 300 && o->Position[2] < 600)
		{
			...
			if (rand() % 2 == 0)
			{
				...
				CreateJointSync(BITMAP_JOINT_SPIRIT2, Position, Position, Angle, 14, NULL, 100.f, 0, 0);
			}
		}
		break;
	case MODEL_CUNDUN_PHOENIX:
		if (o->LifeTime < 5)
		{
			o->Alpha = o->LifeTime._runvalue * 0.2f;
		}
		break;
	case MODEL_CUNDUN_GHOST:
		if (o->Owner != NULL)
		...
		{
			o->PKKey += (1);
			o->Position[2] += ((o->PKKey * timepow(0.8f)));
			...
			Vector(1, 1, 1, Light);
			for (int i = 0; i < 3; ++i)
				CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 20, 10.f);
		}
		else
		{
			o->Skill += (1);
			if (o->Angle[2] < 45.0f)
				o->Angle[2] += (o->Skill * timepow(0.3f));
			if (o->Angle[2] > 45.0f)
				o->Angle[2] -= (o->Skill * timepow(0.3f));

			if (o->Position[0] < Hero->Object.Position[0] + 300.0f)
				o->Position[0] += (o->Skill * timepow(0.2f));
			if (o->Position[0] > Hero->Object.Position[0] + 300.0f)
				o->Position[0] -= (o->Skill * timepow(0.2f));
			if (o->Position[1] < Hero->Object.Position[1] - 300.0f)
				o->Position[1] += (o->Skill * timepow(0.2f));
			if (o->Position[1] > Hero->Object.Position[1] - 300.0f)
				o->Position[1] -= (o->Skill * timepow(0.2f));
			...
			CreateEffectSync(MODEL_FIRE, Position, Angle, Light, 0, NULL, 0);
	...
	case MODEL_ARROW_TANKER_HIT:
	{
		...
		if (o->HiddenMesh == 99)
		{
			o->Position[0] += timefac((float)(rand() % 100 - 50));
			o->Position[1] += timefac((float)(rand() % 100 - 50));
			o->Position[2] += timefac((float)(rand() % 100 - 50));
			o->Scale = 2.2f;
		}
	...
	case MODEL_FLY_BIG_STONE1:
	{
		...
		{
			o->Position[2] -= (o->Gravity);
			o->Gravity += timefac(1.9f);

			o->Angle[0] += timefac(rand() % 20 + 20.f);
			o->Angle[1] += timefac(rand() % 5);
			...
			if (o->LifeTime < 10)
			{
				o->Alpha /= timepow(1.5f);
			}

			if (o->LifeTime.duration(3))
			{
				Vector(1.f, 0.6f, 0.2f, Light);
		...
		else
		{
			if (o->SubType <= 1)
			{
				o->Position[0] += timefac(o->Direction[0] * o->Velocity);
				o->Position[1] += timefac(o->Direction[1] * o->Velocity);
				o->StartPosition[2] += timefac(o->Direction[2]);
				...
				else
				{
					...
					CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 5, 2.f);
				}

				o->Velocity -= timefac(0.45f);
				if (o->Velocity < 5.f)
					o->Velocity = 5.f;
			...
			if (o->SubType == 88 || o->SubType == 99)
			{
				...
				if (o->LifeTime > o->DamageTime)
				{
					Vector(1.f, 0.6f, 0.2f, Light);
					CreateParticleSync(BITMAP_ADV_SMOKE + 1, o->Position, o->Angle, Light);
					CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, Light, 2, 2.f);

					if (o->SubType == 88 && o->Owner != NULL)
					{
						battleCastle::CollisionHeroCharacter(o->Position, ((15 - o->LifeTime._runvalue) * 20) + 350.f, PLAYER_HIGH_SHOCK);
					}
					else if (o->SubType == 99)
					{
						battleCastle::CollisionTempCharacter(o->Position, ((40 - o->LifeTime._runvalue) * 7) + 350.f, PLAYER_HIGH_SHOCK);
					}
				}
			}
			else
			{
				...
				if (o->Position[2] < Height)
				{
					...
					{
						CreateEffectSync(BITMAP_CRATER, o->Position, o->Angle, o->Light);
						...
						else if (o->SubType == 99 || (o->SubType == 88 && o->Owner != NULL))
						{
							...
							CreateEffectSync(BITMAP_SHOCK_WAVE, o->Position, o->Angle, Light, 7);
						}
					}
					o->HiddenMesh = 99;
	...
	case MODEL_FLY_BIG_STONE2:
	{
		if (o->SubType <= 1)
		{
			...
			else
			{
				...
				if ((rand() % 2) == 0)
				{
					CreateParticleSync(BITMAP_ADV_SMOKE + 1, o->Position, o->Angle, Light, 1, 2.f);
				}
			}
	...
	case MODEL_GOLEM_STONE:
		if (o->Type == MODEL_BIG_STONE_PART2 && o->SubType == 3)
		{
			...
			if (o->Position[2] < Height)
			{
				...
				if (rand() % 10 == 0)
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 24, 1.25f * o->Scale);
			}
			else
			{
				if (rand() % 2 == 0)
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 24, 0.2f);
			}
			break;
		}
		VectorAdd(o->Position, o->Direction, o->Position);
		...
		if (o->Type == MODEL_GOLEM_STONE)
		{
			if (rand() % 4 == 0)
			{
				CreateParticleSync(BITMAP_TRUE_FIRE, o->Position, o->Angle, Light, 5, 2.8f);
				CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 21, 1.8f);
			}
			if ((rand() % 10) == 0)
			{
				CreateParticleSync(BITMAP_SMOKE + 1, o->Position, o->Angle, Light);
			}
		}
	...
	case MODEL_FENRIR_THUNDER:
		if (o->SubType == 0)
		{
			if (o->Live)
			{
				...
				o->Angle[0] += timefac(0.15f);
				o->Angle[1] += timefac(0.15f);
				o->Angle[2] += timefac(0.15f);
			}
		}
		else if (o->SubType == 1)
		{
	...
	case MODEL_FALL_STONE_EFFECT:
	{
		if (o->SubType == 0 || o->SubType == 1)
		{
			o->Position[2] -= (o->Gravity);
			o->Gravity += timefac(0.1f);
			o->Angle[0] += timefac(0.5f);
			o->Angle[1] += timefac(0.5f);
			o->Angle[2] += timefac(0.5f);
		}
		else if (o->SubType == 2)
		{
			o->Position[2] -= (o->Gravity);
			...
			if (o->Position[2] < fHeight)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, vLight, 11, (float)(rand() % 20 + 30) * 0.020f);

				int iRand = rand() % 2 + 2;
				for (int i = 0; i < iRand; ++i)
				{
					float fScale = 0.03f + (rand() % 10) / 40.0f + o->Scale * 0.3f;
					CreateEffectSync(MODEL_FALL_STONE_EFFECT, o->Position, o->Angle, o->Light, 3, NULL, -1, 0, 0, 0, fScale);
				}
	...
	case BITMAP_CRATER:
		if (o->LifeTime < 10)
		{
			o->Light[0] = o->LifeTime._runvalue / 10.f;
			o->Light[1] = o->Light[0];
			o->Light[2] = o->Light[0];
			o->Alpha = o->Light[0];
		}
	...
	case MODEL_STUN_STONE:
		if (o->SubType == 0)
		{
			...
			if (o->Position[2] <= Height)
			{
				...
				if (o->ExtState == 0)
				{
					CreateEffectSync(BITMAP_CRATER, o->Position, o->Angle, o->Light, 1);
					o->ExtState = 1;
				}
			}
			else
			{
				...
				VectorScale(o->StartPosition, timepow(0.9f), o->StartPosition);

				o->HeadAngle[0] -= timefac(o->Scale * 32.f);
	...
	case MODEL_MANA_RUNE:
		if (o->SubType == 0)
		{
			...
			{
				o->Scale += timefac(o->Gravity);
				o->Gravity += timefac(0.15f);
				if (o->Scale > 1.f)
				{
					o->Scale = 1.f;
					o->Gravity = 0.01f;

					CreateEffectSync(MODEL_MANA_RUNE, o->Position, o->Angle, o->Light, 1);
				}
			}
			else if (o->LifeTime < 15)
			{
				o->Scale -= timefac(o->Gravity);
				o->Gravity += timefac(0.2f);
				if (o->Scale < 0.f)
				{
					o->Scale = 0.f;
				}
				o->Alpha = (o->LifeTime._runvalue / 20.f);
				o->Position[2] -= timefac(10.f);
			}
		}
		else if (o->SubType == 1)
	...
	case MODEL_SKILL_JAVELIN:
		...
		{
			VectorCopy(o->Owner->Position, o->StartPosition);
			o->StartPosition[2] += timefac(150.f);
		}
		...
		if (o->LifeTime < 25)
		{
			...
			if (o->LifeTime < 20)
			{
				...
				{
					CreateParticleSync(BITMAP_POUNDING_BALL, o->Position, o->Angle, o->Light, 1);
				}
			}
			o->Velocity += timefac(1.5f);
			...
			{
				o->Direction[1] -= timefac(8.f);
			}
		}
		else
		{
			o->Gravity += timefac(10.f);
			AngleMatrix(o->HeadAngle, Matrix);
			VectorRotate(o->Direction, Matrix, Position);
			interpolatesum2(o->Position, Position, o->Position);
		}
		...
		Height = sinf(o->LifeTime._runvalue * 0.1f) * 30.f;
		if (o->SubType == 1)
		{
	...
	case MODEL_ARROW_IMPACT:
		...
		if (o->LifeTime < 2 && o->SubType == 0 && o->Owner != NULL)
		{
			...
			Position[2] += (1200.f);
			CreateJointSync(BITMAP_FLASH, Position, Position, o->Angle, 2, o, 50.f);
			CreateJointSync(BITMAP_FLASH, Position, Position, o->Angle, 3, o, 50.f);
		}
		else if (o->SubType == 1)
		{
			o->Live = false;
	...
	case BITMAP_JOINT_FORCE:
		if (o->SubType == 0)
		{
			if (o->LifeTime < 11 && o->LifeTime.duration(2))
			{
				Vector(90.f, 0.f, 0.f, o->Angle);
				o->HeadAngle[2] += timefac(72.f);
				AngleMatrix(o->HeadAngle, Matrix);
				VectorRotate(o->Direction, Matrix, Position);
	...
	case MODEL_SWORD_FORCE:
		if (o->SubType == 0 || o->SubType == 2)
		{
			if (o->LifeTime > 12)
			{
				o->Scale += timefac(0.9f);
				o->Direction[1] -= timefac(2.f);
				if (o->SubType == 2)
				{
					CreateEffectSync(MODEL_SWORD_FORCE, o->Position, o->Angle, o->Light, 3, o);
				}
				else
					CreateEffectSync(MODEL_SWORD_FORCE, o->Position, o->Angle, o->Light, 1, o);
			}
			else
			{
				o->Scale -= timefac(0.05f);
				o->BlendMeshLight = (float)o->LifeTime._runvalue / 18.f;
				o->Alpha = o->BlendMeshLight;
				o->Light[0] = o->Alpha;
				o->Light[1] = o->Alpha;
		...
		{
			o->BlendMeshLight = (float)o->LifeTime._runvalue / 10.f;
			o->Alpha = o->BlendMeshLight;
			o->Light[0] = o->Alpha;
			o->Light[1] = o->Alpha;
			o->Light[2] = o->Alpha;
	...
	case MODEL_PROTECTGUILD:
	{
		...
		if (o->LifeTime > 120)
		{
			...
			{
				for (int i = 0; i < 5; ++i)
					CreateParticleSync(BITMAP_SPARK, o->Position, o->Angle, o->Light, 6, 1.5f, o);
			}
		}
			...
			if (o->LifeTime < 60 && o->LifeTime > 39)
			{
				...
				for (int i = 0; i < 1; ++i)
				{
					...
					Vector(o->Position[0] + (rand() % 26 - 13) * sinf(fAngle),
						...
						o->Position[2] + 48 - o->LifeTime._runvalue + (rand() % 5), p);
					CreateParticleSync(BITMAP_SPARK, p, o->Angle, o->Light, 5, 2.0f, o);
				}
			}
		}
	}
	...
	case MODEL_MOVE_TARGETPOSITION_EFFECT:
	{
		if (o->SubType == 0)
		{
			...
			CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 24, 1.0f, o);

			if (o->LifeTime.duration(15))
			{
			...
			{
				o->BlendMeshLight -= (timefac(0.05f));
				o->Light[0] *= (timepow(o->BlendMeshLight));
				o->Light[1] *= (timepow(o->BlendMeshLight));
				o->Light[2] *= (timepow(o->BlendMeshLight));
			}
		}
	}
	break;
	case BITMAP_TARGET_POSITION_EFFECT1:
	{
		if (o->SubType == 0)
		{
			...
			{
				o->Alpha -= timefac(0.05f);
				o->Light[0] *= timepow(o->Alpha);
				o->Light[1] *= timepow(o->Alpha);
				o->Light[2] *= timepow(o->Alpha);
			}
		}
	}
	break;
	...
	case MODEL_EFFECT_SAPITRES_ATTACK:
	{
		if (o->LifeTime.duration(6))
		{
			o->Position[0] += timefac((float)((rand() % 120) - 60));
			o->Position[1] += timefac((float)((rand() % 120) - 60));
	...
	case MODEL_EFFECT_THUNDER_NAPIN_ATTACK_1:
	{
		if (o->SubType == 0)
		{
			...
			if (o->LifeTime >= 4)
			{
				...
				CreateParticleSync(BITMAP_LIGHTNING_MEGA1 + rand() % 3, vPos, o->Angle, vLight, 0, o->Scale);
			}
	...
	case MODEL_EFFECT_SKURA_ITEM:
	{
		if ((o->SubType == 0) || (o->SubType == 1))
		{
			if (o->Owner->Live && o->Live)
			{
				...
				BMD* b = gmClientModels->GetModel(MODEL_EFFECT_SKURA_ITEM);
				...
				for (int i = 0; i < 7; ++i)
				{
					CreateParticleSync(BITMAP_SHINY + 1, vtaWorldPos, o->Angle, o->Light, 5, 0.8f, o);
					CreateParticleSync(BITMAP_CHERRYBLOSSOM_EVENT_PETAL, vtaWorldPos, o->Angle, rand() % 7 == 3 ? vLight2 : vLight1, 0, 0.5f);
				}
				...
				if (o->LifeTime == (30) || o->LifeTime == (15) || o->LifeTime == (4))
				{
					...
					BMD* characterBMD = gmClientModels->GetModel(o->Owner->Type);
			...
			if (o->LifeTime <= 24)
			{
				if (o->LifeTime >= 15)
				{
					...
					for (int i = 0; i < 15; ++i)
					{
						...
						CreateParticleSync(index, vPos, o->Angle, vLight, 8, fScale);
						Vector(1.0f, 1.0f, 1.0f, vLight);
						if (rand() % 2 == 0)
						{
							CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 55, 1.f);
						}
					}
				}
	...
	case MODEL_EFFECT_FLAME_STRIKE:
	{
		if (o->SubType == 0)
		{
			...
			else if (pOwner->CurrentAction == PLAYER_SKILL_FLAMESTRIKE)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					pOwnerModel->Animation(BoneTransform, fOwnerAnimationFrame, (int)fOwnerAnimationFrame - 1, pOwner->PriorAction, pOwner->Angle, pOwner->HeadAngle, false, false);
					pOwnerModel->RotationPosition(BoneTransform[33], p, p);	// ParentMatrix

					pModel->Animation(BoneTransform, fAnimationFrame, (int)fAnimationFrame - 1, pObject->PriorAction, pObject->Angle, pObject->HeadAngle, true, true);	// BoneTransform
	...
	case MODEL_SWELL_OF_MAGICPOWER:
	{
		if (o->SubType == 0)
		{
			...
			{
				vec3_t vDLight;
				float fDynamicLightVal = o->LifeTime._runvalue * 0.05f;
			...
			if (o->LifeTime >= 30)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					CreateJointSync(BITMAP_2LINE_GHOST, o->Position, o->Position, o->Angle, 1, o, 20.0f + rand() % 10, -1, 0, 0, -1, vLight);
				}
			}
		}
	}break;
	...
	case MODEL_SWELL_OF_MAGICPOWER_BUFF_EFF:
	{
		if (o->SubType == 0)
		{
			...
			if (iPastTime >= 6000)
			{
				...
				// Bip01 R Hand
				CreateEffectSync(MODEL_ARROWSRE06, o->Position, o->Angle, vLight, 1, o->Owner, 28);

				// Bip01 L Hand
				CreateEffectSync(MODEL_ARROWSRE06, o->Position, o->Angle, vLight, 1, o->Owner, 37);
		...
		if (0 == o->SubType)
		{
			if (o->LifeTime > 1)
			{
				for (INT i_ = 0; i_ < 4; ++i_)
				{
					...
					CreateParticleSync(BITMAP_RAKLION_CLOUDS, v3ResultPos, v3ResultAngle, v3Light_, 1, fResultScale);
				}
			}
		}
	}break;
	case MODEL_1_STREAMBREATHFIRE:
	{
		...
		CreateParticleSync(BITMAP_WATERFALL_3, v3Pos, o->Angle, o->Light, 11, 0.6f);
		CreateParticleSync(BITMAP_SMOKE, v3Pos, o->Angle, o->Light, 52, 0.6f);
	...
	case MODEL_PKFIELD_ASSASSIN_EFFECT_RED_HEAD:
	{
		...
		{
			CreateEffectSync(BITMAP_FIRE_CURSEDLICH, vPosition, o->Angle, vLight, 4, o);
		}
		else
		{
			CreateEffectSync(BITMAP_FIRE_CURSEDLICH, vPosition, o->Angle, vLight, 3, o);
		}
	}
	break;
	case MODEL_PKFIELD_ASSASSIN_EFFECT_GREEN_BODY:
	case MODEL_PKFIELD_ASSASSIN_EFFECT_RED_BODY:
	{
		...
		if (o->LifeTime < 20)
		{
			o->Alpha = 0.1f * o->LifeTime._runvalue;
		}
	}
	break;
	case MODEL_LAVAGIANT_FOOTPRINT_R:
	case MODEL_LAVAGIANT_FOOTPRINT_V:
	{
		...
		o->Light[0] *= timepow(0.93f);
		o->Light[1] *= timepow(0.93f);
		o->Light[2] *= timepow(0.93f);
	}
	break;
	case MODEL_PROJECTILE:
	{
	...
	case MODEL_STATUE_CRUSH_EFFECT_PIECE03:
	{
		...
		if (rand() % 3 == 0)
		{
			CreateParticleSync(BITMAP_SMOKE + 1, o->Position, o->Angle, o->Light, 6);
		}
	}
	break;
	case MODEL_STATUE_CRUSH_EFFECT_PIECE04:
		...
		else if (o->SubType == 1)
		{
						...
						case MODEL_SWORDLEFT01_EMPIREGUARDIAN_BOSS_GAION_:
						{
							for (int i = 0; i < iAccess; i++)
							{
								...
								if (rand() % 2 == 0)
								{
									...
									CreateParticleSync(BITMAP_FIRE, vPosition, o->Angle, o->Light, 5, 1.35f);

									Vector((float)(rand() % 360), 0.f, (float)(rand() % 360), vAngle);
									AngleMatrix(vAngle, matRandomRotation);
									VectorRotate(vRandomDir, matRandomRotation, vRandomDirPosition);
									VectorAdd(vPosition, vRandomDirPosition, vResultRandomPosition);
									CreateJointSync(BITMAP_JOINT_THUNDER, vResultRandomPosition, vPosition, vAngle, 3, NULL, 10.f, 10, 10);
								}
							}
						}
						break;
			...
			case MODEL_SWORDMAIN01_EMPIREGUARDIAN_BOSS_GAION_:
			{
				...
				if (fCurrentRate >= 0.0f && fCurrentRate <= 0.6f)
				{
					...
					BMD* pBMDSwordModel = gmClientModels->GetModel(o->Type);
					BMD* pOwnerModel = gmClientModels->GetModel(o->Owner->Type);
			...
			case MODEL_EMPIREGUARDIANBOSS_FRAMESTRIKE:
			{
				if (o->LifeTime == (10))
				{
					CreateEffect(MODEL_SWORDMAIN01_EMPIREGUARDIAN_BOSS_GAION_, o->Position, o->Angle, o->Light, 11, o->Owner, -1, 0, 0, 0, o->Scale);
				}
			}
			break;
			case MODEL_SWORDMAIN01_EMPIREGUARDIAN_BOSS_GAION_:
			{
				if (o->LifeTime == (1))
				{
					CreateEffect(MODEL_EMPIREGUARDIANBOSS_FRAMESTRIKE, o->Position, o->Angle, o->Light, 1, o->Owner, -1, 0, 0, 0, o->Scale);
				}
			}
			break;
			}
			...
			if (o->LifeTime <= 24)
			{
				if (o->LifeTime >= 15)
				{
					...
					for (int i = 0; i < 15; ++i)
					{
						...
						if (rand() % 2 == 0)
						{
							CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 55, o->Scale * 0.9f);
						}
					}
				}
	...
	case MODEL_EFFECT_EG_GUARDIANDEFENDER_ATTACK2:
	{
		if (o->LifeTime <= 18)
		{
			o->Light[0] /= timepow(1.4f);
			o->Light[1] /= timepow(1.4f);
			o->Light[2] /= timepow(1.4f);
			o->Scale *= timepow(1.1f);
		}
		CreateSprite(BITMAP_SHOCK_WAVE, o->Position, o->Scale, o->Light, o->Owner);
		CreateParticleSync(BITMAP_FIRE + 2, o->Position, o->Angle, o->Light, 16, 2.5f);
	...
	case BITMAP_WATERFALL_4:
	{
		...
		o->Distance += timefac(2.1f);
		o->Angle[0] += timefac(0.7f);
	...
	case MODEL_WOLF_HEAD_EFFECT:
	{
		if (o->SubType == 0)
		{
			o->BlendMeshLight = (float)o->LifeTime._runvalue / 20.0f;
			o->Alpha = o->BlendMeshLight;
			vec3_t Angle;
			if (o->LifeTime >= 3)
			{
				...
				CreateEffectSync(MODEL_WOLF_HEAD_EFFECT, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 1, o->Owner);
				CreateEffectSync(MODEL_WOLF_HEAD_EFFECT, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 2, o->Owner);
				o->Owner->Angle[2] = Angle[2];
		...
		{
			o->Owner->Velocity += timefac(2.5f);
		}
		else if (o->SubType == 2)
		{
			o->Owner->Velocity += timefac(5.5f);
		}
	}
	break;
	case BITMAP_SBUMB:
	{
		if (o->LifeTime < 17)
		{
			...
			if (pCharacter)
			{
				VectorCopy(pCharacter->Object.Position, vPosition);
				Vector(vPosition[0] + rand() % 80 - 40, vPosition[1] + rand() % 80 - 40, vPosition[2] + rand() % 120 + 30, vPosition);
				float Scale = 1.5f + (rand() % 10 * 0.02f);
				CreateParticleSync(BITMAP_SBUMB, vPosition, o->Angle, vLight, 0, Scale * o->Scale, o->Owner);
			}
		}
		if (o->LifeTime == 15
	...
	case MODEL_DOWN_ATTACK_DUMMY_L:
	{
		if (o->Owner->Live)
		{
			...
			if (o->AnimationFrame >= 5.0f && o->LifeTime > 50)
			{
				...
				if (pCharacter)
				{
					CreateBomb3(pCharacter->Object.Position, 3, 0.9f);
				}
			...
			CreateJointSync(BITMAP_FORCEPILLAR, _StartPos, _StartPos, o->Angle, 0, o, 50.0f);
	...
	case MODEL_DOWN_ATTACK_DUMMY_R:
	{
		if (o->Owner->Live)
		{
			...
			{
				o->Live = false;
				CreateEffectSync(MODEL_DOWN_ATTACK_DUMMY_L, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 0, o->Owner);
			}
			...
			vec3_t _StartPos, _EndPos;
			pModel->TransformByObjectBone(_StartPos, o, 0);
			CreateJointSync(BITMAP_FORCEPILLAR, _StartPos, _StartPos, o->Angle, 1, o, 50.0f);
	...
	case BITMAP_SWORDEFF:
	{
		if (o->Owner->AnimationFrame > gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].NumAnimationKeys)
			o->Live = false;
		...
		CreateJointSync(BITMAP_SWORDEFF, _StartPos, _EndPos, o->Owner->Angle, 0, o->Owner, 100.0f);
		...
		if (o->Owner->AnimationFrame > 6 && o->LifeTime > 50)
		{
			...
			CreateEffectSync(MODEL_SHOCKWAVE01, position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.0f);
			CreateEffectSync(MODEL_WOLF_HEAD_EFFECT2, o->Position, o->Angle, o->Light, 3, o->Owner);
			...
			if (pCharacter)
			{
				CreateEffectSync(BITMAP_DAMAGE1, pCharacter->Object.Position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.4f);
			}
#ifdef PBG_MOD_RAGEFIGHTERSOUND
		...
		if (o->Owner->AnimationFrame > 11 && o->LifeTime > 20)
		{
			...
			CreateEffectSync(MODEL_SHOCKWAVE02, position, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 0.5f);
			CreateEffectSync(MODEL_WOLF_HEAD_EFFECT2, o->Position, o->Angle, o->Light, 5, o->Owner);

			Vector(1.0f, 1.0f, 1.0f, Light);
			Vector(-80.0f, 20.0f, 0.0f, vPosition);
			b->TransformByObjectBone(vPosition, o->Owner, 4, vPosition);
			Vector(90.0f, Angle[1], Angle[2], Angle);
			CreateEffectSync(MODEL_SHOCKWAVE_SPIN01, vPosition, Angle, Light, 0, o->Owner, -1, 0, 0, 0, 0.5f);
			CreateEffectSync(MODEL_SHOCKWAVE_SPIN01, vPosition, Angle, Light, 1, o->Owner, -1, 0, 0, 0, 0.9f);
			...
			{
				VectorCopy(pCharacter->Object.Position, vPosition);
				CreateEffectSync(BITMAP_SHINY + 4, vPosition, o->Angle, Light, 0, o->Owner, -1, 0, 0, 0, 1.9f);
			}
#ifdef PBG_MOD_RAGEFIGHTERSOUND
			StopBuffer(SOUND_RAGESKILL_GIANTSWING_ATTACK, true);
#endif //PBG_MOD_RAGEFIGHTERSOUND
	...
	case MODEL_SHOCKWAVE01:
	{
		...
		{
			o->Alpha = 1.0f;
			o->Scale *= timepow(1.24f);
			o->Position[2] *= timepow(1.19f);
			VectorScale(o->Light, timepow(0.45f), o->Light);
			if (o->Light[0] < 0.001f)
			{
		...
		{
			o->Scale *= timepow(1.15f);
			VectorScale(o->StartPosition, timepow(1.04f), o->StartPosition);
			VectorSubtract(o->Position, o->StartPosition, o->Position);
			VectorScale(o->Light, timepow(1.5f), o->Light);
			if (o->Light[0] > 0.6f)
			{
				o->SubType = 5;
			}
		...
		else
		{
			o->Alpha = 1.0f;
			o->Scale += timefac(0.2f);
			VectorScale(o->StartPosition, timepow(0.99f), o->StartPosition);
			VectorSubtract(o->Position, o->StartPosition, o->Position);
			VectorScale(o->Light, timepow(0.65f), o->Light);
	...
	case MODEL_SHOCKWAVE02:
	{
		...
		o->Scale += timefac(0.1f);
		VectorScale(o->StartPosition, timepow(0.95f), o->StartPosition);
		VectorSubtract(o->Position, o->StartPosition, o->Position);
	}
	break;
	case BITMAP_DAMAGE1:
	...
	case MODEL_SHOCKWAVE_SPIN01:
	{
		o->Scale *= timepow(1.01f);
		o->Angle[1] -= (10.0f);
		if (o->SubType == 1)
		{
			o->Angle[1] += timefac(20.0f);
		}
		VectorScale(o->Light, timepow(0.9f), o->Light);
		o->Alpha = 1.0f;
	}
	break;
	case BITMAP_EVENT_CLOUD:
	...
	case MODEL_WINDFOCE:
	{
		...
		else if (o->SubType == 1)		//�������ΰ� 
		{
			...
			float fFadeInOut = (float)(sinf(WorldTime * 0.001f) + 1.0f) * 0.25f + 0.2f;
			VectorScale(o->Light, timepow(fFadeInOut), o->Light);
			o->Scale = 1.0f;
			VectorCopy(o->Owner->Position, o->Position);
		}
		else if (o->SubType == 2)
		{
			...
			o->Alpha *= timepow(0.92f);
			VectorScale(o->Light, timepow(0.85f), o->Light);
	...
	case BITMAP_SHINY + 4:
	{
		...
		CreateParticleSync(BITMAP_SHINY + 4, vPosition, o->Angle, Light, 2, o->Scale);
	}
	break;
	case BITMAP_LIGHT_RED:
	{
			...
			if (g_isCharacterBuff(o->Owner, eBuff_Def_up_Ourforces))
			{
				if (rand() % 70 == 7)
				{
					if (!SearchEffect(MODEL_WINDFOCE_MIRROR, o->Owner))
					{
						...
						CreateEffectSync(MODEL_WINDFOCE_MIRROR, o->Owner->Position, o->Angle, vLight, 0, o->Owner, -1, 0, 0, 0, 1.0f);
					}
				}
			}
		}
	...
	case BITMAP_SWORD_EFFECT_MONO:
	{
		...
		if (o->LifeTime.duration(3) == false)
	...
	case MODEL_WOLF_HEAD_EFFECT2:
	{
		...
		{
			o->BlendMeshLight = o->LifeTime._runvalue / 20.0f;
			o->Alpha = o->BlendMeshLight;
			...
			if (o->LifeTime >= 3)
			{
				...
				CreateEffectSync(MODEL_WOLF_HEAD_EFFECT2, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 4, o->Owner);
				CreateEffectSync(MODEL_WOLF_HEAD_EFFECT2, o->Owner->Position, o->Owner->Angle, o->Owner->Light, 6, o->Owner);
				o->Owner->Angle[2] = Angle[2];
				o->Owner->Angle[0] = Angle[0];
			}
		}
	...
	case MODEL_SHOCKWAVE_GROUND01:
	{
		...
		{
			o->Scale *= timepow(1.2f);
			VectorScale(o->Light, timepow(0.7f), o->Light);
		}
		else
		{
			o->Scale *= timepow(1.2f);
			VectorScale(o->Light, timepow(0.8f), o->Light);
		}
	}
	break;
	case MODEL_DRAGON_KICK_DUMMY:
	{
		if (o->Owner->AnimationFrame > gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONKICK].NumAnimationKeys
		...
		if (o->Owner->Live)
		{
			...
			if (o->Owner->AnimationFrame > 4 && o->Owner->AnimationFrame < 5)
			{
				...
				pModel = gmClientModels->GetModel(o->Owner->Type);
				for (int i = 0; i < fDelay; i++)
				{
					...
					CreateJointSync(BITMAP_GROUND_WIND, _StartPos, _EndPos, o->Angle, i, o, 150.0f);
					fAnimationFrame += (fSpeedPerFrame);
				}
			}
			...
			if (o->Owner->AnimationFrame > 5)
			{
				for (int j = 0; j < 2; ++j)
				{
					...
					for (int i = 0; i < 4; ++i)
					{
						CreateParticleSync(BITMAP_ENERGY, vPos, o->Angle, o->Light, 7, fScale);
						CreateParticleSync(BITMAP_LIGHTNING_MEGA1, vPos, o->Angle, o->Light, 0, fScale * 0.9f);

						CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, o->Light, 3, 1.0f + (fLuminosity * 0.05f));
						CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, o->Light, 8, 1.0f + (fLuminosity * 0.05f));

						if (i % 2)
						{
							CreateParticleSync(BITMAP_SPARK, vPos, o->Angle, o->Light, 13);
						}
	...
	case BITMAP_LAVA:
	{
		if (o->Owner->AnimationFrame > gmClientModels->GetModel(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONLORE].NumAnimationKeys
		...
		if (o->Owner->AnimationFrame > 2 && o->Owner->AnimationFrame < 8)
		{
			...
			for (int i = 0; i < fDelay; i++)
			{
				...
				CreateJointSync(BITMAP_LAVA, _StartPos, _StartPos, o->Owner->Angle, i, o->Owner, 20.0f);
				CreateJointSync(BITMAP_LAVA, _EndPos, _EndPos, o->Owner->Angle, 7 + i, o->Owner, 20.0f);
				fAnimationFrame += (fSpeedPerFrame);
			}
		}
		...
		{
			Vector(1.0f, 0.12f, 0.0f, vLight);
			CreateEffectSync(MODEL_SHOCKWAVE01, o->Owner->Position, vAngle, vLight, 1, o->Owner, -1, 0, 0, 0, 1.0f);
			o->LifeTime = 95;
		}
		...
		if (o->Owner->AnimationFrame > 5 && o->LifeTime > 80)
		{
			...
			CreateEffectSync(MODEL_SHOCKWAVE_GROUND01, o->Position, o->Angle, vLight, 1, o, -1, 0, 0, 0, 1.0f);
			o->LifeTime = 80;
		}
		...
		if (o->Owner->AnimationFrame > 5)
		{
			...
			VectorCopy(o->Position, vPos);
			vPos[2] += ((80 - o->LifeTime._runvalue) * 1.0f * 10.0f - (80 - o->LifeTime._runvalue) * 3.0f);
			...
			for (int i = 0; i < 8; ++i)
			{
				...
				vPos[2] = tempPos[2];
				CreateParticleSync(BITMAP_FIRE_HIK1, vPos, o->Angle, vLight, 5, fScale);

				vec3_t TempPos;
				VectorCopy(vPos, TempPos);
				TempPos[2] = 250.0f;
				CreateParticleSync(BITMAP_SPARK, TempPos, o->Angle, o->Light, 12, 1.5f);
			}
		}
	}
	break;
	case MODEL_DRAGON_LOWER_DUMMY:
	{
		...
		if (o->AnimationFrame > 1 && o->AnimationFrame < 6)
		{
			...
			temp[2] = 250.0f;
			CreateParticleSync(BITMAP_SPARK, temp, o->Angle, o->Light, 12, 1.5f);
		}
		...
		if (o->AnimationFrame > 1 && o->LifeTime > 80)
		{
			vec3_t vLight;
			Vector(1.0f, 1.0f, 1.0f, vLight);
			CreateEffectSync(BITMAP_LIGHT_RED, o->Position, o->Angle, vLight, 1, o, -1, 0, 0, 0, 15.0f);
			o->LifeTime = 80;
		}
		o->Alpha *= timepow(0.96f);
	}
	break;
	case MODEL_TARGETMON_EFFECT:
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			{
			case 0:
				CreateParticleSync(BITMAP_FIRE_HIK1, o->Position, o->Angle, vLight, 0, fScale);
				break;
			case 1:
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, o->Position, o->Angle, vLight, 4, fScale);
				break;
			case 2:
				CreateParticleSync(BITMAP_FIRE_HIK3, o->Position, o->Angle, vLight, 0, fScale);
				break;
			}
		}
	}
	...
	case MODEL_VOLCANO_OF_MONK:
	{
		if (o->SubType == 1)
		{
			...
			Vector(1.0f, 1.0f, 1.0f, vLight);
			VectorCopy(o->Position, vPos);
			vPos[2] += ((80 - o->LifeTime._runvalue) * (0.9f + rand() % 5 * 0.1f));
			...
			{
			case 0:
				CreateParticleSync(BITMAP_FIRE_HIK1, vPos, o->Angle, vLight, 6, fScale);
				break;
			case 1:
				CreateParticleSync(BITMAP_FIRE_CURSEDLICH, vPos, o->Angle, vLight, 9, fScale);
				break;
			case 2:
				CreateParticleSync(BITMAP_FIRE_HIK3, vPos, o->Angle, vLight, 6, fScale);
				break;
			}
			...
			if (o->LifeTime < 30)
			{
				o->BlendMeshLight = o->LifeTime._runvalue * 0.03f;
			}
	...
	case MODEL_VOLCANO_STONE:
	{
		...
		Height = RequestTerrainHeight(o->Position[0], o->Position[1]);
		o->Angle[0] += (0.3f * o->LifeTime._runvalue);
		o->Angle[1] += (0.3f * o->LifeTime._runvalue);
	...
	case MODEL_PHOENIX_SHOT_EFFECT:
	{
		...
		VectorCopy(o->Light, Light);
		ow = o->Owner;
		if (!ow || !ow->Live || ow->m_sTargetIndex == -1)
			goto LABEL_3646;

		if (o->LifeTime == 70)
		{
			CHARACTER* pCharacter = gmCharacters->GetCharacter(ow->m_sTargetIndex);

			if (o->m_sTargetIndex == ow->m_sTargetIndex && pCharacter)
			{
				VectorCopy(pCharacter->Object.Position, Position);
				Angle[0] = 0;
				Angle[1] = 0;
				Angle[2] = CreateAngle(ow->Position[0], ow->Position[1], Position[0], Position[1]);
				CreateJoint(BITMAP_SWORDEFFOR2_BLUR, ow->Position, ow->Position, Angle, 0, o, 60.0, -1, 0, 0, -1, Light, -1);
				CreateJoint(BITMAP_SWORDEFFOR2_BLUR, ow->Position, ow->Position, Angle, 1, o, 60.0, -1, 0, 0, -1, Light, -1);
			}
			else
			{
				o->Live = 0;
		...
		else
		{
			if (o->LifeTime == 63)
			{
				Light[0] = 1.0;
				Light[1] = 1.0;
				Light[2] = o->m_sTargetIndex;
				Angle[0] = ow->Angle[0];
				Angle[1] = ow->Angle[1];
				Angle[2] = ow->Angle[2];
				for (int i85 = 0; i85 < 3; ++i85)
				{
					CreateEffect(MODEL_WIND_SPIN03, ow->Position, Angle, Light, i85, ow, -1, 0, 0, 0, 0.8, -1);
				}
			}
		}
	}
	...
	case MODEL_WIND_SPIN03:
	{
		...
		vec3_t Direction, vTemp1, vTemp2;

		OBJECT* ow = o->Owner;
		if (o->LifeTime <= 30)
		{
			o->Scale += timefac(0.04);
			o->Angle[2] -= timefac(10.0);
			o->Position[2] = ow->Position[2] + (100.0);
			o->Light[0] *= timepow(0.85);
			o->Light[1] *= timepow(0.85);
			o->Light[2] *= timepow(0.85);
			if (o->LifeTime == 25 || o->LifeTime == 20 || o->LifeTime == 15)
			{
				Vector(0.9f, 0.4f, 0.2f, Light);
				for (int i86 = 0; i86 < 3; ++i86)
				{
					CreateEffect(MODEL_WIND_SPIN01, o->Position, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
				}
			}
			goto LABEL_3646;
		}
		if (ow->m_sTargetIndex == -1 || !ow || !ow->Owner->Live)
		{
			o->LifeTime = 0;
			o->Live = 0;
			goto LABEL_3646;
		...
		Direction[0] *= ((50.0 - (double)o->LifeTime) * 2.0f + 25.0f);
		Direction[1] *= ((50.0 - (double)o->LifeTime) * 2.0f + 25.0f);
		//v524 = (double)o->LifeTime;
		Direction[2] = 0.0;
		interpolatesum(o->Position, Direction);
		o->Angle[2] -= timefac(70.0);
		VectorScale(o->Light, timepow(0.95), o->Light);

		pCharacter = gmCharacters->GetCharacter(ow->m_sTargetIndex);
		if (!pCharacter)
			goto LABEL_3646;

		VectorCopy(pCharacter->Object.Position, vTemp1);
		VectorCopy(o->Position, vTemp2);
		vTemp2[2] = 0.0f;

		VectorSubtract(ow->Position, vTemp2, vTemp2);
		VectorSubtract(vTemp1, ow->Position, vTemp1);

		int v754 = (int)(VectorLength(vTemp2));
		int v756 = (int)(VectorLength(vTemp1));

		if (v754 <= v756 || o->LifeTime <= 30)
			goto LABEL_3611;
		...
		if (pCharacter)
		{
			//vTemp1[0] = pCharacter->Object.Position[0];
			//vTemp1[1] = pCharacter->Object.Position[1];
			//vTemp1[2] = pCharacter->Object.Position[2];
			VectorCopy(pCharacter->Object.Position, vTemp1);

			vTemp1[2] = o->Position[2] + 100.0;

			Position[0] = (double)(rand() % 150) + vTemp1[0] - 75.0;
			Position[1] = (double)(rand() % 150) + vTemp1[1] - 75.0;
			Position[2] = (double)(rand() % 130) + vTemp1[2] + 30.0;

			//Angle[0] = Direction[0];
			//Angle[1] = Direction[1];
			//Angle[2] = Direction[2];
			VectorCopy(Direction, Angle);
			for (int i87 = 0; i87 < 7; ++i87)
			{
				CreateParticleSync(BITMAP_SPARK, Position, Angle, Light, 14, 10.0, 0);
			}

			Vector(1.0f, 0.7f, 0.2f, Light);
			CreateEffectSync(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
			CreateEffectSync(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);
			CreateEffectSync(MODEL_WIND_SPIN01, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.1, -1);

			//Vector(1.0f, 1.0f, 1.0f, Light);
			//CreateEffectSync(MODEL_WIND_SPIN02, vTemp1, ow->Angle, Light, 0, ow, -1, 0, 0, 0, 0.25, -1);
			Vector(0.2f, 0.12f, 0.04f, Light);
			...
			for (int i88 = 0; i88 < 8; ++i88)
			{
				Angle[2] += 30.0;
				vTemp1[2] = 130.0;
				CreateEffectSync(BITMAP_TWLIGHT, vTemp1, Angle, Light, 4, ow, -1, 0, 0, 0, 2.5, -1);
			}
			StopBuffer(sound_CombatMaster_attack3, 1);
			PlayBuffer(sound_CombatMaster_attack3, 0, 0);
			o->LifeTime = 30;
		LABEL_3611:
			if (o->LifeTime <= 30)
			{
				...
				VectorCopy(pCharacter->Object.Position, vTemp1);
				VectorCopy(vTemp1, o->Position);
				o->Scale += timefac(0.08);

				VectorScale(o->Light, timepow(0.97f), o->Light);
			}
			...
			if (o->Live && !o->SubType)
			{
				...
				if (o->Scale > 1.0)
				{
					VectorScale(Light, 0.97f, Light);
				}

				VectorCopy(o->Position, vTemp1);
				vTemp1[2] = 130.0;

				o->Angle[2] = (double)o->LifeTime * 40.0f + 90.0f;
				CreateEffectSync(BITMAP_TWLIGHT, vTemp1, o->Angle, Light, 5, o, -1, 0, 0, 0, 3.5f, -1);

				o->Angle[2] = (double)o->LifeTime * 20.0f + 90.0f;
				CreateEffectSync(BITMAP_TWLIGHT, vTemp1, o->Angle, Light, 5, o, -1, 0, 0, 0, 3.5f, -1);
			}
		}
	}
	...
	case MODEL_WIND_SPIN02:
	{
		...
		if (!o->Owner || o->Owner->m_sTargetIndex < 0 || o->Type < 0)
		{
			o->LifeTime = 0;
			o->Live = false;
			goto LABEL_3646;
		...
		if (o->SubType == 1)
			...
			o->Position[2] = o->Owner->Position[2] + 100.0;

		o->Scale += timefac(0.02f);
		VectorScale(o->Light, timepow(0.8f), o->Light);
		pModel->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, o->Velocity / 5.0, o->Position, o->Angle);

	}
	break;
	case MODEL_WIND_SPIN01:
	{
		if (o->Owner != NULL && o->Owner->m_sTargetIndex >= 0)
		{
			VectorCopy(o->Position, Position);
			Position[2] = o->Owner->Position[2] + 100;
			o->Scale += timefac(0.4f);
			o->Angle[2] += timefac(o->Scale * 10.f + 10.f);
			//o->Light[0] *= timepow(0.78f);
			//o->Light[1] *= timepow(0.78f);
			//o->Light[2] *= timepow(0.78f);
			VectorScale(o->Light, timepow(0.78f), o->Light);
			VectorCopy(Position, o->Position);
		}
		else
		{
		...
		|| o->Type == MODEL_CHANGE_UP_EFF))
	{
		if (o->Type >= MODEL_BIRD01 && o->Type < MODEL_SKILL_END)
		{
			...
			if (pModel)
			{
				...
				if (o->CurrentAction != 0xff && pModel->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, o->Velocity, o->Position, o->Angle) == false)
				{
					Action_t* pAction = pModel->GetAction(pModel->CurrentAction);
					if (pAction)
					{
						o->AnimationFrame = pAction->NumAnimationKeys;
						o->CurrentAction = 0xff;
					}
				}
			}
		}
...
void MoveEffects()
{
	...
	// Effects disabled: purge main pool + skill-effect pool (skip heavy MoveEffect sim).
	if (!EffectBudget::Enabled())
	{
		EffectBudget::ClearAll(Effects, MAX_EFFECTS);
...
void RenderWheelWeapon(OBJECT* o)
{
	...
	if (pModel)
	{
		...
		pModel->Skin = gCharacterManager.GetBaseClass(Hero->Class);
		pModel->CurrentAction = o->CurrentAction;
...
{
	BMD* b = gmClientModels->GetModel(MODEL_SPEARSKILL);
	if (b)
	{
		...
		vec3_t Light;
		for (int i = 0; i < 3; ++i)
			Light[i] = o->Light[i] * o->LifeTime._runvalue * 0.05f;
...
void RenderEffects(bool bRenderBlendMesh)
{
	...
	int iRenderLevel = 0;
	if (g_pOption)
		iRenderLevel = g_pOption->GetRenderLevel();
	...
	for (int i = 0; i < iEffectSize; ++i)
	{
		...
		if (o->Live)
		{
			...
			if (o->Visible)
			{
				// Effect Level: skip non-essential particles on Low/Medium
				if (iRenderLevel <= 1) // Low or Medium
				{
					if (o->Type >= BITMAP_SPARK_BEGIN && o->Type <= BITMAP_SPARK_END)
						continue;
				}
				if (iRenderLevel == 0) // Lowest
				{
					if (o->Type >= BITMAP_WATER_BEGIN && o->Type <= BITMAP_WATER_END)
						continue;
				}
				if (bRenderBlendMesh)
				{
				...
				case MODEL_MULTI_SHOT3:
				{
					...
					if (pModel)
					{
						...
						float fSpeed = (o->Velocity / 6.f);
						pModel->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, fSpeed, o->Position, o->Angle);
						RenderObject(o);
					}
				}
				break;
				...
				case MODEL_SUMMONER_SUMMON_LAGUL:
					if (o->SubType == 1)
					{
						...
						if (pModel)
						{
							...
							if (rand() % 5 == 0)
							{
								int nBones = pModel->NumBones;
								pModel->TransformByObjectBone(vPos, o, rand() % nBones);
								Vector(0.6f, 0.5f, 0.9f, vLight);
								CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 57, 2.0f);
								Vector(0.7f, 0.7f, 1.f, vLight);
								CreateParticleSync(BITMAP_TWINTAIL_WATER, vPos, o->Angle, vLight, 1);
							}
							RenderObject(o);
				...
				case BITMAP_LIGHT_MARKS:
				{
					...
					if (o->LifeTime >= 35)
					{
						o->Light[0] /= timepow(fLight);
						o->Light[1] /= timepow(fLight);
						o->Light[2] /= timepow(fLight);
					}
					else
					{
						o->Light[0] *= timepow(fLight);
				...
				case MODEL_SWORDMAIN01_EMPIREGUARDIAN_BOSS_GAION_:
				{
					...
					if (o->SubType != 20)
					{
						...
						int		arrBoneIdxs_SwordEffectRed01[] = { 2,	9,	1,	3,	10 };
						int		arrBoneIdxs_SwordEffectRed02[] = { 3,	10, 2,	10, 11 };
						int		arrBoneIdxs_SwordEffectEdge01[] = { 13, 13, 3,	4,	1 };
						int		arrBoneIdxs_SwordEffectEdge02[] = { 12, 6,	4,	12, 2 };
						int		arrBoneIdxs_SwordEffectEdge03[] = { 5,	1,	5,	2,	3 };
						int		arrBoneIdxs_SwordEffectEdge04[] = { 6,	7,	6,	9,	4 };
						int		arrBoneIdxs_SwordEffectEdge05[] = { 1,	2,	12,	5,	5 };
						int		arrBoneIdxs_SwordEffectEdge06[] = { 8,	4,	8,	7,	6 };
						int		arrBoneIdxs_SwordEffectEdge07[] = { 9,	3,	9,	6,	7 };
						int		arrBoneIdxs_SwordEffectEdge08[] = { 10, 11, 10, 8,	8 };
						int		arrBoneIdxs_SwordEffectEdge09[] = { 4,	8,	7,	1,	9 };
				...
				case BITMAP_LIGHT_RED:
				{
					...
					{
						fScale = o->Scale;
						fFadeInOut = o->LifeTime._runvalue * 0.01f;
					}
...
void RenderAfterEffects(bool bRenderBlendMesh)
{
	...
	for (int i = 0; i < iEffectSize; ++i)
	{
		...
		if (o->Live && o->m_bRenderAfterCharacter)
		{
			...
			if (World == WD_39KANTURU_3RD && (o->Type == MODEL_STORM3 || o->Type == MODEL_MAYASTAR))
				o->Visible = true;
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				...
				case BITMAP_MAGIC + 1:
					...
					if (o->LifeTime < 5)
					{
						Luminosity -= ((float)(5 - o->LifeTime._runvalue) * 0.2f);
					}
					else if (o->SubType == 7)
					{
						if (o->LifeTime > 30)
							Luminosity = ((float)(40 - o->LifeTime._runvalue) * 0.1f);
					}
					...
					if (o->SubType == 4 || o->SubType == 10)
					{
						...
						if (Luminosity == 1.f)
						{
							Luminosity = sinf((60 - o->LifeTime._runvalue) * 0.05f) * 1.f + 0.5f;
						}
					}
					else if (o->SubType == 6)
					{
						Scale = ((80 - o->LifeTime._runvalue) * 0.05f);
					}
					else if (o->SubType == 7)
					{
						Scale = ((double)o->LifeTime * 0.07f);
					}
					else if (o->SubType == 9)
					{
						Scale = o->Scale;
					...
					{
						Scale = ((20 - o->LifeTime._runvalue) * 0.15f);
					}
					else if (o->SubType == 12)
					{
						Scale = ((20 - o->LifeTime._runvalue) * 0.15f);
					...
					else if (o->SubType == 6 && o->LifeTime.duration(25))
					{
						CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 7, o->Owner);
					}
					else if (o->SubType == 8 && o->LifeTime.duration(2))
					{
				...
				case BITMAP_MAGIC + 2:
					...
					if (o->SubType != 2)
					{
						...
						if (o->LifeTime < 5)
							Luminosity -= ((float)(5 - (float)o->LifeTime) * 0.2f);
						Scale = (20 - (float)o->LifeTime) * 0.15f;
					}
					else if (o->SubType == 2)
					{
						if (o->LifeTime > 10)
						{
							Scale = (20 - (float)o->LifeTime) * 0.55f;
						}
						else
						{
							Luminosity -= ((float)(10 - (float)o->LifeTime) * 0.1f);
						}
					}
					Vector(Luminosity * 1.f, Luminosity * 0.4f, Luminosity * 0.2f, Light);
					RenderTerrainAlphaBitmap(BITMAP_MAGIC + 1, o->Position[0], o->Position[1], Scale, Scale, Light, -o->Angle[2]);
				...
				case BITMAP_LIGHTNING + 1:
					o->Scale += timefac(0.2f);
					Luminosity = (float)(o->LifeTime._runvalue) * 0.1f;
					Vector(Luminosity, Luminosity, Luminosity, Light);
