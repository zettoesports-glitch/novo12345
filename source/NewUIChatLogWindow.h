...
namespace SEASON3B 
{
	...
	class TMessageText
	{
		...
		DWORD m_dwCreateTime;	// when the line was added (for auto-rise / expire)
	...
	public:
#ifdef ITEM_CHAT_SELL_SYSTEM
		TMessageText() : m_MsgType(TYPE_UNKNOWN), m_dwIndentSize(0), m_dwCreateTime(0), m_dwIndentItem(0) {}
...
#else
		TMessageText() : m_MsgType(TYPE_UNKNOWN), m_dwIndentSize(0), m_dwCreateTime(0) {}
		~TMessageText() { Release(); }
	...
	private:
		enum 
		{
			// EXP / system / error: 3 minutes on screen
			SYSTEM_LINE_LIFETIME_MS = 180000,
			// Conversation (chat, whisper, party, guild...): no time limit (0 = never expire by age)
			CHAT_LINE_LIFETIME_MS = 0,
			// Sum EXP kills into one chat line every N ms
			EXP_AGGREGATE_MS = 750,
		...
		// EXP aggregate (farm: one line instead of one per kill)
		DWORD				m_dwPendingExp;
		DWORD				m_dwPendingExpKills;
		DWORD				m_dwPendingExpTick;
		bool				m_bPendingMasterExp;
		...
		// Remove old lines so chat keeps rising without needing new messages
		void ProcessMessageTimeout();
		int ExpireMessagesInVector(type_vector_msgs* pvecMsgs, DWORD now);
		void FlushPendingExperience(bool force);
...
#endif // ITEM_CHAT_SELL_SYSTEM
		// Prefer this for monster EXP (aggregates then posts one system line)
		void AddExperience(DWORD dwExp, bool bMasterLevel);
