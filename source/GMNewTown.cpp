...
{
	return (gMapManager->currentMap == WD_51HOME_6TH_CHAR ||
		gMapManager->currentMap == WD_73NEW_LOGIN_SCENE || gMapManager->currentMap == WD_74NEW_CHARACTER_SCENE
		|| gMapManager->currentMap == WD_77NEW_LOGIN_SCENE || gMapManager->currentMap == WD_78NEW_CHARACTER_SCENE
		);
}

bool GMNewTown::IsNewMap73_74()
{
	return (gMapManager->currentMap == WD_73NEW_LOGIN_SCENE || gMapManager->currentMap == WD_74NEW_CHARACTER_SCENE);
}
...
void GMNewTown::CreateObject(OBJECT* pObject)
{
	...
	if (gMapManager->currentMap == WD_77NEW_LOGIN_SCENE || gMapManager->currentMap == WD_78NEW_CHARACTER_SCENE)
	{
		if (pObject->Type == 62)
		{
			pObject->SubType = 0;
...
bool GMNewTown::MoveObject(OBJECT* pObject)
{
	...
	if (gMapManager->currentMap == WD_51HOME_6TH_CHAR)
	...
	case 62:
		pObject->HiddenMesh = -2;
		{
			...
			if (!pBoid->Live)
			{
				...
				if (gMapManager->currentMap == WD_74NEW_CHARACTER_SCENE
					|| gMapManager->currentMap == WD_77NEW_LOGIN_SCENE
...
bool GMNewTown::RenderObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 54:
		...
		if (rand() % 4 == 0)
		{
			CreateParticleSync(BITMAP_WATERFALL_2, pObject->Position, pObject->Angle, Light, 4, pObject->Scale);
		}
		break;
	case 58:
		CreateParticleSync(BITMAP_WATERFALL_5, pObject->Position, pObject->Angle, Light, 0);
		break;
	case 59:
		Vector(1.f, 1.f, 1.f, Light);
		CreateParticleSync(BITMAP_WATERFALL_3, pObject->Position, pObject->Angle, Light, 8, pObject->Scale);
		break;
	case 60:
		if (pObject->HiddenMesh != -2)
		{
			...
			for (int i = 0; i < 10; ++i)
			{
				if (gMapManager->currentMap == WD_77NEW_LOGIN_SCENE)
					CreateParticleSync(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 3, pObject->Scale * 2, pObject);
				else
					CreateParticleSync(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 3, pObject->Scale, pObject);
			}
			pObject->HiddenMesh = -2;
		}
		break;
	case 61:
		...
		{
			Vector(1.f, 1.f, 1.f, Light);
			CreateParticleSync(BITMAP_TRUE_BLUE, pObject->Position, pObject->Angle, Light, 0, pObject->Scale);
		}
		break;
	case 63:
	{
	...
	case 110:
		...
		if (rand() % 2 == 0)
		{
			...
			CreateParticleSync(BITMAP_LIGHT, Position, pObject->Angle, Light, 11, 0.6f, pObject);
		}
		break;
	case 121:
	{
	...
	case 140:	case 141:	case 142:	case 143:	case 144:	case 145:	case 146:	case 147:
		if (pObject->HiddenMesh != -2)
		{
			...
			CHARACTER* pCharacter = gmCharacters->GetCharacter(icntIndex);

			OBJECT* pNewObject = &pCharacter->Object;
			...
			case 136:
				...
				gGoboidManager->CreateBug(MODEL_FENRIR_BLUE, pNewObject->Position, pNewObject);
				break;
				// 			case 137:
			case 138:
			...
			case 146:
				...
				gGoboidManager->CreateBug(MODEL_DARK_HORSE, pNewObject->Position, pNewObject);
			...
			pNewObject->PriorAnimationFrame = pNewObject->AnimationFrame = rand() % gmClientModels->GetModel(pNewObject->Type)->Actions[pNewObject->CurrentAction].NumAnimationKeys;
	...
	case 155:
		if (pObject->HiddenMesh != -2)
		{
			...
			if (gmClientModels->GetModel(pNewObject->Type)->Actions[MONSTER01_ATTACK1].NumAnimationKeys > 0)
			{
				pNewObject->PriorAnimationFrame = pNewObject->AnimationFrame =
					rand() % gmClientModels->GetModel(pNewObject->Type)->Actions[MONSTER01_ATTACK1].NumAnimationKeys;
			}
			else
			{
				assert(!"°ø°Ýµ¿ÀÛÀÌ ¾ø´?");
	...
	if (pObject->Type >= 133 && pObject->Type <= 147)
	{
		...
		{
			CHARACTER* pCharacter = gmCharacters->GetCharacter(pObject->SubType);
			if (pObject->Owner->CurrentAction < PLAYER_WALK_MALE)
			{
				if (pObject->Type == 133)
				{
					...
					{
						pCharacter->Helper.Type = -1;
						SetAction(pObject->Owner, PLAYER_HIT_SKILL_WHEEL);
				...
				else if (pObject->Type == 134)
				{
					...
					{
						SetAction(pObject->Owner, PLAYER_HIT_ONETOONE);
...
bool GMNewTown::RenderObject(OBJECT* pObject, BMD* pModel, bool ExtraMon)
{
	...
	else if (gMapManager->currentMap == WD_77NEW_LOGIN_SCENE)
	{
		Mesh_t* m = NULL;
		for (int i = 0; i < pModel->NumMeshs; i++)
		{
	...
	else if (gMapManager->currentMap == WD_78NEW_CHARACTER_SCENE && pObject->Type == 4)
	{
...
CHARACTER* GMNewTown::CreateNewTownMonster(int iType, int PosX, int PosY, int Key)
{
	...
	case 416:
		...
		gmClientModels->GetModel(MODEL_ELBELAND_RHEA)->Actions[0].PlaySpeed = 0.2f;
		gmClientModels->GetModel(MODEL_ELBELAND_RHEA)->Actions[1].PlaySpeed = 0.4f;
		break;
	case 417:
