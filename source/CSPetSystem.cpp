...
bool CSPetSystem::PlayAnimation(OBJECT* o)
{
	...
	if (pModel)
	{
		...
		return pModel->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, playSpeed, o->Position, o->Angle);
	}
	return false;
}
...
void CSPetSystem::SetCommand(int Key, BYTE cmd)
{
	...
	if (cmd == PET_CMD_TARGET)
	{
		...
		m_PetTarget = gmCharacters->GetCharacter(Index);
...
void CSPetDarkSpirit::MovePet(void)
{
	...
	if (o->m_bActionStart == true && m_PetTarget)
	{
	...
	if (o->AI == PET_FLY || o->AI == PET_FLYING)
	{
		...
		if (m_PetOwner->Helper.Type == ITEM_HELPER + 4 || gMapManager->currentMap == WD_55LOGINSCENE)
		{
			CharacterHeight = 350;
		}
		...
		if (rand() % speedRandom == 0)
		{
			...
			else
			{
				...
				o->Angle[2] += timefac((float)(rand() % 60));
			}
	...
	else if (o->AI == PET_ATTACK || o->AI == PET_ESCAPE)
	{
		...
		interpolatesum2(o->Position, Pos, o->Position);
...
void CSPetDarkSpirit::AttackEffect(CHARACTER* c, OBJECT* o)
{
	...
	case PET_ATTACK:
		if (c->AttackTime >= 0 && c->AttackTime <= 2 && m_PetTarget != NULL)
		{
			for (int i = 0; i < 10; i++)
			{
				CreateJointSync(BITMAP_LIGHT, o->Position, o->Position, o->Angle, 1, NULL, (float)(rand() % 40 + 20));
			}
			if (c->AttackTime == 1)
			{
				vec3_t Angle, Light;
		...
		if (c->AttackTime > 3 && c->AttackTime.duration(2))
		{
			...
			if (o->BoneTransform != NULL)
			{
				...
				for (int i = rand() % 2; i < 66; i += 2)
				{
					if (!b->Bones[i].Dummy && i < b->NumBones)
					{
						...
						CreateParticleSync(BITMAP_LIGHT, Pos, o->Angle, Light, 6, 1.3f);
					}
				}
			}
		}
