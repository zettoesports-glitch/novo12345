...
// Desliga UI pedida (brincos custom / setas / titulo / expansao lateral)
namespace
{
	const bool kForceDisableEarring = true;      // aneis/brincos custom (ERING)
	const bool kForceDisableInvNavArrows = true; // setas Main/Sub do inventario
	const bool kForceDisableInvTitle = true;     // texto "Inventario"
	const bool kForceDisableInvExpansion = true; // expansao lateral (bags extras)
}
#include "TextClien.h"
...
SEASON3B::CNewUIMyInventory::CNewUIMyInventory()
{
	...
	for (int i = 0; i < MAX_NEW_EQUIPMENT; i++)
	{
		...
		if (((i == EQUIPMENT_ERING_L || i == EQUIPMENT_ERING_R) && (!gmProtect->shutdown_item_earring || kForceDisableEarring))
			|| (i == EQUIPMENT_WING_CORE && !gmProtect->shutdown_item_core)
			|| (i == EQUIPMENT_FLAG_NAT  && !gmProtect->shutdown_item_flag)
			|| (i == EQUIPMENT_MUUN && !gmProtect->shutdown_item_muun)
			|| (i == EQUIPMENT_EAGLE && !gmProtect->shutdown_item_eagle)
			)
		{
...
void SEASON3B::CNewUIMyInventory::SetPos(int x, int y)
{
	...
	if (GMProtect->IsInvExt() && !kForceDisableInvExpansion)
	{
	...
	{
		if (GMProtect->IsInvExt() == false || kForceDisableInvExpansion)
		{
			X = m_Pos.x + 13;
		}
...
bool SEASON3B::CNewUIMyInventory::UpdateKeyEvent()
{
	...
	if (CanOpenMyShopInterface() != true || !SEASON3B::IsPress('S'))
	{
		...
		if (SEASON3B::IsRepeat(VK_CONTROL))
		{
			...
			else if (SEASON3B::IsPress('R') && !gmProtect->checkold_school())
			{
				iHotKey = HOTKEY_R;
			}
...
void SEASON3B::CNewUIMyInventory::SetEquipmentSlotInfo()
{
	...
	/* if (gmProtect->shutdown_item_earring && !kForceDisableEarring)
	{
...
void SEASON3B::CNewUIMyInventory::RenderButtons()
{
	...
	// Setas Main/Sub do inventario (pagina flag/core)
	if (!kForceDisableInvNavArrows && (gmProtect->shutdown_item_flag || gmProtect->shutdown_item_core))
	{
#if MAIN_UPDATE > 603
...
bool SEASON3B::CNewUIMyInventory::BtnProcess()
{
	...
	{
		if (!kForceDisableInvExpansion
			&& m_BtnExpansion.UpdateMouseEvent() && !CNewUIInventoryCtrl::GetPickedItem()
			&& GMProtect->IsInvExt())
		{
