...
public:
	...
	// 0..1 = restante / max (0 se sem timer)
	float GetBuffRemainingRatio( eBuffState bufftype );
...
private:
	struct BuffTimeInfo 
	{
		...
		DWORD    s_CurBuffTime;  // ms restante (decai)
		DWORD    s_MaxBuffTime;  // ms total no register (barra verde)
		DWORD    s_EventBuffTime;

		BuffTimeInfo() : s_BuffType(eBuffNone), s_CurBuffTime(0), s_MaxBuffTime(0), s_EventBuffTime(0) {}
	};
