...
void CUIGuildMaster::DoCreateInfoAction()
{
	if (m_NextButton.DoMouseAction())
	{
		...
		{
			SendRequestCreateGuild(0, GuildMark[MARK_EDIT].GuildName, Mark);
		}
...
void CUIGuildMaster::RenderCreateInfo()
{
	...
	SEASON3B::RenderGoldRect(ptOrigin.x, ptOrigin.y, 140, 20.f);
	...
	ptOrigin.x += 44;
	ptOrigin.y += 33;
	SEASON3B::RenderGoldRect(ptOrigin.x, ptOrigin.y, 53.f, 53.f);
	CreateGuildMark(MARK_EDIT);
