...
typedef struct 
{
	...
	// 1=S1, 2=S2/S3 (Look-2), 6=Look-2.5 (mesmo layout que Look-2, pasta HUD/Look-2.5)
	bool checkold_school() {
		return this->LookAndFeel == 1 || this->LookAndFeel == 2 || this->LookAndFeel == 6;
	};
	bool checknew_school() {
		return this->LookAndFeel == 3 || this->LookAndFeel == 4;
	};
	// Comportamento/layout idêntico ao Look-2 (S2/S3)
	bool IsLook2Style() {
		return this->LookAndFeel == 2 || this->LookAndFeel == 6;
	};
	// Pasta de texturas: Look-2 ou Look-2.5
	const char* GetLook2Folder() {
		return (this->LookAndFeel == 6) ? "Look-2.5" : "Look-2";
	};
} MAIN_FILE_INFO;
