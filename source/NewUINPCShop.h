...
namespace SEASON3B
{
	...
	private:
		enum
		{
			NPCSHOP_WIDTH = WIN_WINDOW_SIZEX,
			NPCSHOP_HEIGHT = WIN_WINDOW_SIZEY,
		};
	...
	public:
		...
		ITEM* GetStandbyItem();
		ITEM* GetStandbyBuyItem();

		void SetSellingItem(bool bFlag);
		bool IsSellingItem();

		void SetStandbyBuyItemKey(BYTE dwItemKey);
		BYTE GetStandbyBuyItemKey() const;
	private:
		void Init();
		void SetButtonInfo();
