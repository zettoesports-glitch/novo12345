...
namespace SEASON4A
{
	...
	{
	/*+000*/ int m_iOptionID;
	/*+004*/ int m_iOptionCategory;
	/*+008*/ char m_szOptionName[MAX_SOCKET_OPTION_NAME_LENGTH];
	/*+072*/ char m_bOptionType;
	/*+076*/ int m_iOptionValue[5];
	/*+096*/ BYTE m_bySocketCheckInfo[6];
	} SOCKET_OPTION_INFO;
	...
	public:
		...
		void GetSocketOptionNameText(char* szOptionName, int SOCKET, int iSocketSeedID);
		void GetSocketOptionValueText(char* szOptionValueText, int SOCKET, int iSocketSeedID, int Type);

	/*+00004*/ SOCKET_OPTION_STATUS_BONUS m_StatusBonus;
	protected:
	/*+00056*/ SOCKET_OPTION_INFO m_SocketOptionInfo[MAX_SOCKET_OPTION_TYPES][MAX_SOCKET_OPTION];
	/*+15656*/ int m_iNumEquitSetBonusOptions;
	/*+15660*/ std::deque<DWORD> m_EquipSetBonusList;
