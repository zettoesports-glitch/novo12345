...
void CreateFlags(CHARACTER* c)
{
	//if (c->m_pFlagParts == NULL && c->Flag.Type > 0)
	//{
	//	c->m_pFlagParts = (CSIPartsMDL*)new CSParts(c->Flag.Type, 20, 0, -45.0, -5.0, 0.0, 0.0, 0.0, 1.0);
	//}
}

void DeleteFlags(CHARACTER* c)
{
	//if (c->m_pFlagParts != NULL)
	//{
	//	SAFE_DELETE(c->m_pFlagParts);
	//}
}
...
{
	if (FindText(c->ID, "webzen") || g_isCharacterBuff((&c->Object), eBuff_GMEffect) || ((c->CtlCode == CTLCODE_20OPERATOR) || (c->CtlCode == CTLCODE_08OPERATOR)))
	{
...
void RenderParts(CHARACTER* c)
{
	...
	//if (c->m_pFlagParts != NULL)
	//{
	//	CSIPartsMDL* pTempParts = (CSIPartsMDL*)c->m_pFlagParts;
	//	pTempParts->IRender(c);
	//}
#endif // SYSTEM_FLAG_NAT
...
void CSParts::IRender(CHARACTER* c)
{
	...
	if (b)
	{
		...
		b = gmClientModels->GetModel(m_pObj.Type);
		...
		if (b)
		{
			...
			b->PlayAnimation(&m_pObj.AnimationFrame, &m_pObj.PriorAnimationFrame, &m_pObj.PriorAction, (m_pObj.Velocity * gsteady_clock->GetNormalizerFps()), m_pObj.Position, m_pObj.Angle);
#else
			...
			GMItemEffect->RenderLinkEffect((m_pObj.Type - MODEL_ITEM), b, &m_pObj);
		}
	}
}
...
{
	BMD* b = gmClientModels->GetModel(m_pObj.Type);
