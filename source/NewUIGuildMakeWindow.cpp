...
bool CNewUIGuildMakeWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_EditBox->Init(gwinhandle->GethWnd(), 200, 14, MAXGUILDNAME);
...
bool CNewUIGuildMakeWindow::UpdateGMMark()
{
	...
	{
		char tempText[MAX_PATH] = { 0, };
		wchar_t szReceivedChat[MAX_PATH] = { 0, };

		m_EditBox->GetText(tempText);
		m_EditBox->GetText(szReceivedChat);

		if(CheckSpecialText(szReceivedChat) == true)
		{
