...
bool CPartyManager::IsPartyMember(int index)
{
	...
	return IsPartyMemberChar(pCharacter);
}

bool CPartyManager::IsPartyMemberChar(CHARACTER* pCharacter)
{
	if (pCharacter)
	{
		for (int i = 0; i < PartyNumber; ++i)
		{
			...
			int length = max(1, strlen(pCharacter->ID));

			if (!strncmp(p->Name, pCharacter->ID, length))
				return true;
		}
	}
