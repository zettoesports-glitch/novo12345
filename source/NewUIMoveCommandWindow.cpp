...
void SEASON3B::CNewUIMoveCommandWindow::SetPos(int x, int y)
{
	...
	m_iRealFontHeight = (FontHeight * (gwinhandle->GetScreenX() / (float)WindowWidth)) + 2;
...
void SEASON3B::CNewUIMoveCommandWindow::SettingCanMoveMap()
{
	...
	for (int i = 0; i < m_iRenderEndTextIndex; i++, li++)
	{
		...
		if ((gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_DARK || gCharacterManager.GetBaseClass(CharacterAttribute->Class) == CLASS_DARK_LORD
			...
			&& (iReqLevel != CharacterAttribute->MaxCharacterLevel))
		{
		...
		if (iLevel >= iReqLevel && (int)iZen >= iReqZen && (int)Hero->PK < PVP_MURDERER1)
		{
			...
			else if (strncmp((*li)->_ReqInfo.szMainMapName, GlobalText[37], strlen(GlobalText[37])) == 0)
			{
		...
		for (int i = 0; i < m_iRenderEndTextIndex; i++, li++)
		{
			...
			if (CheckMouseIn(iX, iY, m_MapNameUISize.x - 22, m_iRealFontHeight))
			{
				if ((*li)->_bCanMove == true)
				{
					...
					{
						if (IsTheMapInDifferentServer(gMapManager->currentMap, (*li)->_ReqInfo.index))
						{
							SaveOptions();
						}
...
void SEASON3B::CNewUIMoveCommandWindow::RenderFrame()
{
	...
	RenderImage(IMAGE_MOVECOMMAND_SCROLL_TOP, m_ScrollBarPos.x, m_ScrollBarPos.y, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_TOP_HEIGHT);

	float RenderFrameY = (m_ScrollBarPos.y + MOVECOMMAND_SCROLLBAR_TOP_HEIGHT);

	float RenderSizeY = ((m_ScrollBarPos.y + m_iScrollBarHeightPixel - MOVECOMMAND_SCROLLBAR_TOP_HEIGHT) - RenderFrameY);

	int RenderNumber = RenderSizeY / MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT;

	for (int i = 0; i < RenderNumber; i++)
	{
		RenderImage(IMAGE_MOVECOMMAND_SCROLL_MIDDLE, m_ScrollBarPos.x, RenderFrameY, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT);
		RenderFrameY += MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT;
	}

	float render_spare = RenderSizeY - (int)(MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT * RenderNumber);

	if (render_spare > 0)
	{
		RenderImage(IMAGE_MOVECOMMAND_SCROLL_MIDDLE, m_ScrollBarPos.x, RenderFrameY, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, render_spare);
	}

	RenderImage(IMAGE_MOVECOMMAND_SCROLL_BOTTOM, m_ScrollBarPos.x, m_ScrollBarPos.y + m_iScrollBarHeightPixel - MOVECOMMAND_SCROLLBAR_TOP_HEIGHT, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_TOP_HEIGHT);
