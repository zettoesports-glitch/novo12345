...
bool CNewUIQuestProgressByEtc::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_btnComplete.ChangeButtonImgState(true, BITMAP_msgbox_btn_empty, true, false, false, 0.0, 0.0, 411.f, 96.0);
#else
...
void CNewUIQuestProgressByEtc::SetPos(int x, int y)
{
	...
	m_RequestRewardListBox.SetPosition(m_Pos.x + 9 + ((QPE_WIDTH - 190) / 2), m_Pos.y + 360);
#else
...
bool CNewUIQuestProgressByEtc::UpdateSelTextMouseEvent()
{
	...
	{
		if (MouseX < m_Pos.x + 18 || MouseX > m_Pos.x + 154)
			return false;
	}
	else
	{
...
void CNewUIQuestProgressByEtc::RenderSelTextBlock()
{
	...
	for (int i = 0; i < m_nSelAnswer - 1; ++i)
	{
	...
	{
		glColor4f(0.5f, 0.7f, 0.3f, 0.5f);
		RenderColor(m_Pos.x + 18, nBlockPosY, 154.f, QPE_TEXT_GAP * m_anAnswerLine[m_nSelAnswer - 1]);
		EndRenderColor();
	}
	else
	{
...
void CNewUIQuestProgressByEtc::RenderText()
{
	...
	{
		g_pRenderText->SetTextColor(255, 230, 210, 255);
		for (int i = 0; i < QPE_PLAYER_LINE_MAX; ++i)
		{
	...
	if (gmProtect->checkold_school())
	{
		for (int i = 0; i < QPE_NPC_MAX_LINE_PER_PAGE; ++i)
		{
			g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 59 + (QPE_TEXT_GAP * i), m_aszNPCWords[i + QPE_NPC_MAX_LINE_PER_PAGE * m_nSelNPCPage], 0, 0, RT3_SORT_LEFT);
		}
		...
		if (PLAYER_WORDS_MODE == m_eLowerView)
		{
			...
			for (int i = 0; i < QPE_PLAYER_LINE_MAX; ++i)
			{
				g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 207 + (QPE_TEXT_GAP * i), m_aszPlayerWords[i], 0, 0, RT3_SORT_LEFT);
			}
		}
}
	else
...
bool CNewUIQuestProgressByEtc::ClosingProcess()
{
...
void CNewUIQuestProgressByEtc::SetCurNPCWords()
{
	...
	nLine = ::DivideStringByPixel(&m_aszNPCWords[0][0], QPE_NPC_LINE_MAX, QPE_WORDS_ROW_MAX, g_QuestMng.GetNPCWords(m_dwCurQuestIndex), 160);
...
void CNewUIQuestProgressByEtc::SetCurPlayerWords()
{
	...
	memset(&m_anAnswerLine[0], 0, sizeof(int) * QM_MAX_ANSWER);
	...
	for (i = 0; i < QM_MAX_ANSWER; ++i)
	{
		...
		//if (gmProtect->checkold_school())
		//{
		//	m_anAnswerLine[i] = ::DivideStringByPixel(&m_aszPlayerWords[nPlayerWordsRow][0], 2, QPE_WORDS_ROW_MAX, szAnswer, 150, false);
		//}
		//else
		//{
