...
{
	CGMAcheronPtr pMapAcheron = NULL;
	CGMDeventerPtr pMapDeventer = NULL;
	CGMUrukMountainPtr pMapUruk = NULL;
	CGMZoneNarsPtr pMapZoneNars = NULL;
	CGMFereaPtr pMapZoneFerea = NULL;
	CGMNixiesLakePtr pMapNixiesLake = NULL;
	...
	CGMCharacterScenePtr CharacterEx = CGMCharacterScene::Make();
	CharacterEx->AddMapIndex(WD_93NEW_CHARACTER_SCENE);
	Register(CharacterEx);

	CGMLogInScenePtr LogInEx = CGMLogInScene::Make();
	LogInEx->AddMapIndex(WD_94NEW_LOGIN_SCENE);
	Register(LogInEx);



	std::vector<CUSTOM_MAPE_INFO> mapList = gMapManager->GetMapList();

	if (mapList.size() > 0)
	{
		for (CUSTOM_MAPE_INFO& mapInfo : mapList)
		{
			if ((&mapInfo)->mapIndex == -1 || (&mapInfo)->checkIndex == -1)
				continue;

			if ((&mapInfo)->checkIndex == WD_91GMACHERON1 || (&mapInfo)->checkIndex == WD_92GMACHERON2)
			{
				if (!pMapAcheron)
				{
					pMapAcheron = CGMAcheron::Make();
				}
				pMapAcheron->AddMapIndex((&mapInfo)->mapIndex);
			}
			if ((&mapInfo)->checkIndex == WD_95DEBENTER1 || (&mapInfo)->checkIndex == WD_96DEBENTER2)
			{
				if (!pMapDeventer)
				{
					pMapDeventer = CGMDeventer::Make();
				}
				pMapDeventer->AddMapIndex((&mapInfo)->mapIndex);
			}
			if ((&mapInfo)->checkIndex == WD_100URUK_MOUNTAIN || (&mapInfo)->checkIndex == WD_101URUK_MOUNTAIN)
			{
				if (!pMapUruk)
				{
					pMapUruk = CGMUrukMountain::Make();
				}
				pMapUruk->AddMapIndex((&mapInfo)->mapIndex);
			}
			if ((&mapInfo)->checkIndex == WD_110NARS1 || (&mapInfo)->checkIndex == WD_111NARS2)
			{
				if (!pMapZoneNars)
				{
					pMapZoneNars = CGMZoneNars::Make();
				}
				pMapZoneNars->AddMapIndex((&mapInfo)->mapIndex);
			}
			if ((&mapInfo)->checkIndex == WD_112FEREA)
			{
				if (!pMapZoneFerea)
				{
					pMapZoneFerea = CGMFerea::Make();
				}
				pMapZoneFerea->AddMapIndex((&mapInfo)->mapIndex);
			}
			if ((&mapInfo)->checkIndex == WD_113NIXIESLAKE)
			{
				if (!pMapNixiesLake)
				{
					pMapNixiesLake = CGMNixiesLake::Make();
				}
				pMapNixiesLake->AddMapIndex((&mapInfo)->mapIndex);
			}
		}

		Register(pMapAcheron);

		Register(pMapDeventer);

		Register(pMapUruk);

		Register(pMapZoneNars);

		Register(pMapNixiesLake);
	}
}
...
bool MapProcess::LoadMapData()
{
	...
	for (MapList::iterator iter = m_MapList.begin(); iter != m_MapList.end();)
	{
		...
		if (basemap.expired() == false)
		{
			if (basemap.lock()->IsCurrentMap(mapIndex))
			{
