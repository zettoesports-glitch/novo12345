...
{
	m_iPortalWorld = gMapManager->currentMap;
...
{
	m_iReviveWorld = gMapManager->currentMap;
...
int CPortalMgr::GetReviveWorld()
{
	return m_iReviveWorld;
}

int CPortalMgr::GetRevivePosX()
{
	return m_iRevivePosition_x;
}

int CPortalMgr::GetRevivePosY()
{
	return m_iRevivePosition_y;
}
...
void CPortalMgr::GetPortalPositionText(char * pszOut)
{
	...
	if (m_iPortalWorld == -1)
	{
		assert(!"The move location is not specified.");
	}
	else
	{
