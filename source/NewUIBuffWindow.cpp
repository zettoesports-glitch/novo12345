...
void SEASON3B::CNewUIBuffWindow::RenderBuffIcon(eBuffState& eBuffType, float x, float y, float width, float height)
{
	...
	if (eBuffType > eBuffNone && eBuffType < eBuff_Count) // eBuff_Berserker
	{
		...
		// Linha verde embaixo = tempo restante (estilo MuMobile)
		const float ratio = g_BuffRemainingRatio(eBuffType);
		if (ratio > 0.001f)
		{
			const float barH = 3.f;
			const float barY = y + height - barH;
			const float barW = width * ratio;

			EnableAlphaTest(true);
			// fundo escuro da barra
			glColor4f(0.05f, 0.05f, 0.05f, 0.85f);
			RenderColor(x, barY, width, barH, 0.0f, 0);
			// preenchimento verde (cheio no inicio, encolhe com o tempo)
			// <20% fica amarelo/vermelho para urgencia
			if (ratio > 0.35f)
				glColor4f(0.15f, 0.90f, 0.25f, 0.95f);
			else if (ratio > 0.15f)
				glColor4f(0.95f, 0.80f, 0.15f, 0.95f);
			else
				glColor4f(0.95f, 0.25f, 0.15f, 0.95f);
			RenderColor(x, barY, barW, barH, 0.0f, 0);
			EndRenderColor();
			glColor4f(1.f, 1.f, 1.f, 1.f);
		}
	}
...
void SEASON3B::CNewUIBuffWindow::RenderBuffStatus(float x, float y, OBJECT* pHeroObject)
{
	...
	for (std::list<eBuffState>::iterator iter = buffstate.begin(); iter != buffstate.end(); )
	{
		...
		if (buffwidthcount < 4)
		{
			RenderFrameX = x - (buffwidthcount * RenderSizeX);
		}
		else
		{
			RenderFrameX = x + ((buffwidthcount - 4) * RenderSizeX) + RenderSizeX;
		}

		RenderFrameY = y - (buffheightcount * (RenderSizeY + 2));

		// Icon + barra verde de tempo (se houver timer registrado — Hero)
