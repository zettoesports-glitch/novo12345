...
void SEASON3B::CNewUICommandWindow::InitButtons()
{
	...
	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	{
		...
		m_BtnCommand[i].ChangeButtonInfo(m_Pos.x + ((WIN_WINDOW_SIZEX - 108) / 2), (m_Pos.y + 45) + (i * (29 + COMMAND_BTN_INTERVAL_SIZE)), 108, 29);
	}
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
		float add_x = m_Pos.x + 45.0;
		float add_y = m_Pos.y + 50.0;
		for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
		{
			SetButtonImagen(&m_BtnCommand[i], Bitmap_Message_box, Bitmap_Message_box, 0.0, 0.0, 213.0, 64.0);
			m_BtnCommand[i].ChangeButtonInfo(add_x, add_y, 100.0, 25.0);
			add_y += 28.0;
		}
		SetButtonInterface(&m_BtnExit, m_Pos.x, m_Pos.y, 1, false);
		m_BtnExit.ChangeButtonInfo(m_Pos.x + 20, m_Pos.y + 395, 24, 24);
	}
	else
	{
		float add_x = m_Pos.x + ((WIN_WINDOW_SIZEX - 108) / 2);
		float add_y = m_Pos.y + 33.f;
		for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
		{
			m_BtnCommand[i].ChangeButtonImgState(true, IMAGE_COMMAND_BTN, true);
			m_BtnCommand[i].ChangeButtonInfo(add_x, add_y, 108, 29);
			add_y += (29 + COMMAND_BTN_INTERVAL_SIZE);
		}
	...
	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	{
		SetButtonImagen(&m_BtnCommand[i], Bitmap_Message_box, Bitmap_Message_box, 0.0, 0.0, 213.0, 64.0);
		m_BtnCommand[i].ChangeButtonInfo(add_x, add_y, 100.0, 25.0);
		//m_BtnCommand[i].CurSize(213.0, 64.0);
		add_y += 28.0;
	}
...
void SEASON3B::CNewUICommandWindow::RenderFrame()
{
	SEASON3B::window_nomal((float)m_Pos.x, (float)m_Pos.y);
}

void SEASON3B::CNewUICommandWindow::RenderText()
{
...
#if MAIN_UPDATE > 303
#if MAIN_UPDATE > 603
	g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 84) / 2), m_Pos.y + 12, GlobalText[938], 84, 0, RT3_SORT_CENTER);
#else
	if (gmProtect->checkold_school())
	{
		g_pRenderText->SetBgColor(20, 20, 20, 255);
		g_pRenderText->RenderText(m_Pos.x + 35, m_Pos.y + 12, GlobalText[938], 120, 0, RT3_SORT_CENTER);
		g_pRenderText->SetBgColor(0, 0, 0, 0);
	}
	else
	{
...
void SEASON3B::CNewUICommandWindow::RenderButtons()
{
#if MAIN_UPDATE > 303
#if MAIN_UPDATE > 603
	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	...
	if (gmProtect->checkold_school())
	{
		for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
		{
			...
			RenderCheckBox(m_Pos.x + 25, m_Pos.y + i * 28 + 58.0, (BOOL)(i == m_iCurSelectCommand));
		}
	}
	else
	{
...
void SEASON3B::CNewUICommandWindow::SetPos(int x, int y)
{
...
#if MAIN_UPDATE > 603
	SEASON3B::LoadPos(&m_BtnExit, 0, m_Pos.x, m_Pos.y);

	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	{
		m_BtnCommand[i].SetPos(m_Pos.x + ((WIN_WINDOW_SIZEX - 108) / 2), (m_Pos.y + 45) + (i * (29 + COMMAND_BTN_INTERVAL_SIZE)));
	}
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
		...
		for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
		{
			m_BtnCommand[i].SetPos(add_x, add_y);
			add_y += 28.0;
		}
	...
	{
		float add_y = m_Pos.y + 33.f;
		float add_x = m_Pos.x + ((COMMAND_WINDOW_WIDTH - 108) / 2);
	...
	for (int i = COMMAND_TRADE; i < COMMAND_END; i++)
	{
		m_BtnCommand[i].SetPos(m_Pos.x + add_x, add_y);
		add_y += 28.0;
	}
#endif
}
...
void SEASON3B::CNewUICommandWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
...
void SEASON3B::CNewUICommandWindow::RunCommand()
{
	...
	{
		pSelectedCha = gmCharacters->GetCharacter(SelectedCharacter);
...
void SEASON3B::CNewUICommandWindow::SetBtnState(int iBtnType, bool bStateDown)
{
	...
	if (bStateDown)
	{
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_UP, 2);
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_OVER, 2);
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_DOWN, 2);
	}
	else
	{
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_UP, 0);
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_OVER, 1);
		m_BtnCommand[iBtnType].ChangeButtonState(BUTTON_STATE_DOWN, 2);
	}
	m_BtnCommand[iBtnType].ChangeFrame(BUTTON_STATE_UP);
	...
	if (gmProtect->checkold_school())
	{
		if (bStateDown)
		{
			m_BtnCommand[iBtnType].Lock();
			m_BtnCommand[iBtnType].ChangeImgColor(BUTTON_STATE_UP, RGBA(100, 100, 100, 255));
			m_BtnCommand[iBtnType].ChangeTextColor(RGBA(100, 100, 100, 255));
		}
		else
		{
			m_BtnCommand[iBtnType].UnLock();
			m_BtnCommand[iBtnType].ChangeImgColor(BUTTON_STATE_UP, RGBA(255, 255, 255, 255));
			m_BtnCommand[iBtnType].ChangeTextColor(RGBA(255, 255, 255, 255));
		}
	}
	else
	{
