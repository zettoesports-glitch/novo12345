...
bool MoveMainCamera()
{
	...
	else
	{
		...
		{
			g_pCatapultWindow->GetCameraPos(Position);
			// GM /watch: smooth camera focus on target instead of own body
			gSpectatorMode.ApplyCameraFocus(Position);
		}
		...
		{
			// /watch: use focus height (target), not GM body
			if (gSpectatorMode.IsActive())
...
void MoveMainScene()
{
	...
	{
		PERF_SCOPE(PERF_MOVE_CHARS);
		MoveHero();
		MoveCharactersClient();
	}
	// MoveMainCamera so no RenderMainScene (1x/frame). Antes: 2x/frame (Move+Render).
	ThePetProcess().UpdatePets();
	gGoboidManager->MoveBugs();


	{
		PERF_SCOPE(PERF_EFFECTS);
		MovePoints();
		MovePlanes();
		MoveEffects();
		MoveJoints();
...
bool RenderMainScene()
{
	...
	if (gwinhandle->CheckPerformance())
	{
		if (IsWaterTerrain() == false)
		{
			if (World == WD_39KANTURU_3RD)
			{
				...
				{
				PERF_SCOPE(PERF_TERRAIN);
				PerfGpuBeginSection(PERF_GPU_TERRAIN);
				RenderTerrain(false);
				PerfGpuEndSection(PERF_GPU_TERRAIN);
				}
			}
		...
		{
			PERF_SCOPE(PERF_CHARS);
			PerfGpuBeginSection(PERF_GPU_CHARS);
			RenderCharactersClient();
			PerfGpuEndSection(PERF_GPU_CHARS);
		}
		...
		{
			PERF_SCOPE(PERF_EFFECTS);
			PerfGpuBeginSection(PERF_GPU_EFFECTS);
			RenderJoints(byWaterMap);
			RenderEffects();
			RenderBlurs();
			PerfGpuEndSection(PERF_GPU_EFFECTS);
			CheckSprites();
			BeginSprite();
	...
	if (gwinhandle->CheckPerformance())
	{
		{
			PERF_SCOPE(PERF_UI);
			PerfGpuBeginSection(PERF_GPU_UI);
			if (CameraTopViewEnable == false)
			{
...
void MainScene(HDC hDC)
{
	...
	//while (accumulatedTime >= fixedDeltaTime)
	{
		...
		// F11 = profiler on/off | Ctrl+F11 = clear peaks + log
		if (SEASON3B::IsPress(VK_F11))
		{
			if (SEASON3B::IsRepeat(VK_CONTROL) || (GetKeyState(VK_CONTROL) & 0x8000))
				PerfClear();
			else
				PerfToggle();
		}

		// Auto HP (potions) — independent of Mu Helper
		if (SceneFlag == MAIN_SCENE)
			gAutoCombat.Update();

		// Discord Rich Presence — atualiza status no Discord
		DiscordPresence_Update();
		...
		case MAIN_SCENE:
			{
				PERF_SCOPE(PERF_MOVE);
				MoveMainScene();
			}
			break;
		}

		// Scale physics with FPS normalizer (stable at 30/60/120)
		g_PhysicsManager.Move(static_cast<float>(0.005 * FPS_ANIMATION_FACTOR));
		...
		if (MacroTime > 0)
		{
			// Fractional tick so MacroTime still advances at 60/120 FPS
			static double s_macroAccum = 0.0;
			s_macroAccum += timefac(1.0);
			const int macroDec = static_cast<int>(s_macroAccum);
			if (macroDec > 0)
			{
				MacroTime -= macroDec;
				s_macroAccum -= macroDec;
				if (MacroTime < 0)
					MacroTime = 0;
			}
		}
	...
	// Screenshot QoL: Screenshots\13-07-2026_22h_(Nick).jpg
	{
		SYSTEMTIME st;
		GetLocalTime(&st);
		...
		if (Hero && Hero->ID[0])
		{
			...
			nick[j] = 0;
			if (j == 0)
				strcpy_s(nick, "Player");
		}

		// Preferred: 13-07-2026_22h_(Nick).jpg
		// GrabFileName is extern char[] — need explicit size for sprintf_s
		sprintf_s(GrabFileName, MAX_PATH, "Screenshots\\%02d-%02d-%04d_%02dh_(%s).jpg",
			st.wDay, st.wMonth, st.wYear, st.wHour, nick);

		// Avoid overwrite if multiple shots in the same hour
		if (GetFileAttributesA(GrabFileName) != INVALID_FILE_ATTRIBUTES)
		{
			sprintf_s(GrabFileName, MAX_PATH, "Screenshots\\%02d-%02d-%04d_%02dh%02d_(%s).jpg",
				st.wDay, st.wMonth, st.wYear, st.wHour, st.wMinute, nick);
			if (GetFileAttributesA(GrabFileName) != INVALID_FILE_ATTRIBUTES)
			{
				sprintf_s(GrabFileName, MAX_PATH, "Screenshots\\%02d-%02d-%04d_%02dh%02d%02d_(%s)_%d.jpg",
					st.wDay, st.wMonth, st.wYear, st.wHour, st.wMinute, st.wSecond, nick, GrabScreen % 10000);
			}
		}
	}
	...
	char lpszTemp[64];
	wsprintf(lpszTemp, " [%s / %s]",
		g_ServerListManager ? g_ServerListManager->GetSelectServerName() : "",
		(Hero && Hero->ID[0]) ? Hero->ID : "");
	...
	if (Success)
	{
		...
		if (now - lastTitleUpdate > 500) {
			RenderFPSOverlay(gwinhandle->GethWnd());
			lastTitleUpdate = now;
		}

		{
			PERF_SCOPE(PERF_SWAP);
			SwapBuffers(hDC);
		}
	}
