...
void CPhysicsVertex::UpdateForce(unsigned int iKey, DWORD dwType, float fWind)
{
	...
	{
	case PCT_RUBBER:
		m_vForce[2] += (fRand * (fWind + 0.1f) * 1.f) * timefac();
		break;
	case PCT_RUBBER2:
		m_vForce[2] += (fRand * fWind) * timefac();
		break;
	}
	...
	case PCT_ELASTIC_HALLOWEEN:
		m_vForce[0] += -(fRand * fWind * 0.5f) * timefac();
		m_vForce[2] += (fRand * (fWind + 0.1f) * 0.5f * (float)sinf(WorldTime * 0.003f) * 5.0f) * timefac();
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 50.0f) * timefac();
		break;
	case PCT_ELASTIC_RAGE_L:
		m_vForce[0] += -((fRand * fWind * 0.8f) * timefac());
		break;
	...
	{
	case PCT_HEAVY:
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 180.0f) * timefac();
		break;
	default:
		m_vForce[2] -= (s_Gravity * fGravityRate * s_fMass * 100.0f) * timefac();
		break;
	}
}
...
void CPhysicsVertex::DoOneTimeMove(void)
{
	if (PVS_FIXEDPOS & m_byState)
	{
		m_vOneTimeMove[0] = 0.0;
		m_vOneTimeMove[1] = 0.0;
		m_vOneTimeMove[2] = 0.0f;
		return;
	}
...
BOOL CPhysicsCloth::Create(OBJECT* o, int iBone, float fxPos, float fyPos, float fzPos, int iNumHor, int iNumVer, float fWidth, float fHeight, int iTexFront, int iTexBack, DWORD dwType)
{
	...
	for (int j = 0; j < m_iNumVer; ++j)
	{
		...
		for (int i = 0; i < m_iNumHor; ++i)
		{
			...
			if (m_oOwner->BoneTransform)
			{
				gmClientModels->GetModel(m_oOwner->Type)->TransformPosition(Matrix, vPos, vPos2, true);
			}
			else
			{
...
void CPhysicsCloth::InitForces()
{
	int iSeed = (static_cast<int>(WorldTime / 400.f) * 101) % m_iNumVertices;

	for (int iVertex = 0; iVertex < m_iNumVertices; ++iVertex)
	{
		m_pVertices[iVertex].UpdateForce(abs(iSeed % m_iNumHor - iVertex % m_iNumHor) + abs(iSeed / m_iNumHor - iVertex / m_iNumHor), m_dwType, (m_fWind) * timefac(1.0));
	}
}
...
void CPhysicsCloth::MoveVertices(float fTime)
{
		...
		if (pLink->m_byStyle & PLS_SPRING)
		{
			...
			if (fDistance > pLink->m_fDistance[1] + 0.01f)
			{
				...
				for (int i = 0; i < 3; ++i)
				{
					...
					{
						vForce[i] *= timepow((pLink->m_fDistance[1] / 32.0f));
					}
					switch (PCT_MASK_ELASTIC & m_dwType)
					{
					case PCT_RUBBER:
						vForce[i] *= timepow(3.0f);
						break;
					}
					...
					{
					case PCT_ELASTIC_HALLOWEEN:
						vForce[i] *= timepow(2.0f);
						break;
					case PCT_ELASTIC_RAGE_L:
					case PCT_ELASTIC_RAGE_R:
					{
						vForce[i] *= timepow(1.0f);
					}
					break;
					}
				}
	...
	for (int iVertex = 0; iVertex < m_iNumVertices; ++iVertex)
	{
		m_pVertices[iVertex].Move(timefac(fTime));
	}
}
...
BOOL CPhysicsCloth::PreventFromStretching(void)
{
	...
	for (; pHead; pHead = m_lstCollision.GetNext(pHead))
	{
		...
		vec3_t vPos2;
		gmClientModels->GetModel(m_oOwner->Type)->TransformPosition(m_oOwner->BoneTransform[pCollision->GetBone()], vPos, vPos2, true);
...
void CPhysicsCloth::Render(vec3_t* pvColor, int iLevel)
{
	...
	DWORD dwType = m_dwType & 0x3000;

	if (dwType)
	{
		if (dwType == PCT_MASK_ALPHA)
		{
			EnableAlphaTest(1);
		}
		else if (dwType == PCT_MASK_BLEND)
		{
			EnableAlphaBlend();
		}
	}
	...
	if (PCT_MASK_LIGHT & m_dwType)
	{
		...
		for (int i = 0; i < m_iNumHor; i++)
		{
			...
			{
				fScale = i * 0.2f + 0.4f;
				Lum2 = (m_iNumHor - i) * Lum / 5.f;

				Vector(Lum2, Lum2, Lum2, Light);
				if (iOffset < m_iNumVertices)
				{
...
void CPhysicsCloth::RenderFace(BOOL bFront, int iTexture, vec3_t* pvRenderPos)
{
	...
	{
		float fLuminosity = 1.0;
		
		if (m_fLuminosity[0] > 0.0)
		{
			float _Alpha = m_fLuminosity[2] - m_fLuminosity[1];
			fLuminosity = Render22(-m_fLuminosity[0], 0.0)* _Alpha + m_fLuminosity[1];
		}

		if (iTexture == m_iTexBack)
		{
			EnableAlphaBlend();
			glColor3f(fLuminosity, fLuminosity, fLuminosity);
		}
	}
...
void CPhysicsManager::Move(float fTime)
{
	...
	CPhysicsManager::s_fWind += (fPlus) * timefac(1.0);
