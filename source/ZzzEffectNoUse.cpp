...
void MovePlanes()
{
	for (int i = 0; i < MAX_PLANES; i++)
	{
		...
		if (o->Live)
		{
			...
			if (o->LifeTime < 10)
			{
				float Light = (float)o->LifeTime._runvalue * 0.1f;
				Vector(Light * 0.6f, Light, Light * 0.8f, o->Light);
			}
			o->Angle[2] -= o->LifeTime._runvalue;
		}
	}
}
...
void MagicBitmap(int Texture)
{
	...
	if (b->Buffer.size() == 0)
	{
		b->Width = 128.f;
		b->Height = 128.f;
		b->Buffer.resize((int)b->Width * (int)b->Height * 3, 0);
	}
	int Index = 0;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, 3, (int)b->Width, (int)b->Height, 0, GL_RGB, GL_UNSIGNED_BYTE, b->Buffer.data());
}
