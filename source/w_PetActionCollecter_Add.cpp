...
bool PetActionCollecterAdd::Move(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	double Distance = sqrtf(Range[0] * Range[0] + Range[1] * Range[1]);
	...
	case eAction_Stand:
		{
			...
			double Speed = (FlyRange >= Distance) ? 0 : (float)log(Distance) * 2.3f;
...
bool PetActionCollecterAdd::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	if (rand() % 3 == 0)
	{
		...
		CreateParticleSync(BITMAP_LIGHT + 3, Position, obj->Angle, Light, 1, 1.f);
	}
...
bool PetActionCollecterSkeleton::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	m_bIsMoving = !(
		...
		|| (Owner->Object.CurrentAction >= PLAYER_FENRIR_STAND && Owner->Object.CurrentAction <= PLAYER_FENRIR_STAND_WEAPON_LEFT)
	...
	if (m_bIsMoving == TRUE)
	{
		...
		for (int i = 0; i < 2; ++i)
		{
			...
			{
			case 0:
				CreateParticleSync(BITMAP_FIRE_HIK1_MONO, vPosition, vAngle, vLight, 4, obj->Scale, obj);
				break;
			case 1:
				CreateParticleSync(BITMAP_FIRE_HIK2_MONO, vPosition, vAngle, vLight, 8, obj->Scale, obj);
				break;
			case 2:
				CreateParticleSync(BITMAP_FIRE_HIK3_MONO, vPosition, vAngle, vLight, 5, obj->Scale, obj);
				break;
			}
		}
