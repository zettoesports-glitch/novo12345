...
bool CNewUIInGameShop::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_StorageItemListBox.SetPosition(x + 490, y + 360);
...
void CNewUIInGameShop::SetPos(int x, int y)
{
	...
	m_CloseButton.SetPos(m_Pos.x + IMAGE_IGS_EXIT_BTN_POS_X, m_Pos.y + IMAGE_IGS_EXIT_BTN_POS_Y);

	for (int i = 0; i < INGAMESHOP_DISPLAY_ITEMLIST_SIZE; i++)
	{
		m_ViewDetailButton[i].SetPos(m_Pos.x + IMAGE_IGS_VIEWDETAIL_BTN_POS_X + ((i % IGS_NUM_ITEMS_WIDTH) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X), m_Pos.y + IMAGE_IGS_VIEWDETAIL_BTN_POS_Y + ((i / IGS_NUM_ITEMS_HEIGHT) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y));
	}

	m_CashGiftButton.SetPos(m_Pos.x + IMAGE_IGS_ITEMGIFT_BTN_POS_X, m_Pos.y + IMAGE_IGS_ICON_BTN_POS_Y);
	m_CashChargeButton.SetPos(m_Pos.x + IMAGE_IGS_CASHGIFT_BTN_POS_X, m_Pos.y + IMAGE_IGS_ICON_BTN_POS_Y);
	m_CashRefreshButton.SetPos(m_Pos.x + IMAGE_IGS_REFRESH_BTN_POS_X, m_Pos.y + IMAGE_IGS_ICON_BTN_POS_Y);
	m_UseButton.SetPos(m_Pos.x + IMAGE_IGS_USE_BTN_POS_X, m_Pos.y + IMAGE_IGS_USE_BTN_POS_Y);
	m_PrevButton.SetPos(m_Pos.x + IMAGE_IGS_PAGE_LEFT_POS_X, m_Pos.y + IMAGE_IGS_PAGE_BUTTON_POS_Y);

	// next
	m_NextButton.SetPos(m_Pos.x + IMAGE_IGS_PAGE_RIGHT_POS_X, m_Pos.y + IMAGE_IGS_PAGE_BUTTON_POS_Y);
	// Storage Page prev
...
bool CNewUIInGameShop::IsInGameShopRect(float _x, float _y)
{
	...
	if (_x >= _TempRT.left && _x < _TempRT.left + _TempRT.right && _y < _TempRT.top + _TempRT.bottom && _y >= _TempRT.top)
		return true;
	else
		return false;
...
{
	Script_Item* pItemAttr = GMItemMng->find(_ItemType);
...
void CNewUIInGameShop::RenderDisplayItems()
{
	...
	for (int i = 0; i < g_InGameShopSystem->GetSizePackageAsDisplayPackage(); i++)
	{
		...
		RenderItem3D(m_Pos.x + iPosX, m_Pos.y + iPosY, IGS_ITEMRENDER_POS_WIDTH, IGS_ITEMRENDER_POS_HEIGHT, g_InGameShopSystem->GetPackageItemCode(i), 0, 0, 0, true);
	}
...
void CNewUIInGameShop::SetBtnInfo()
{
	...
	for (int i = 0; i < INGAMESHOP_DISPLAY_ITEMLIST_SIZE; i++)
	{
		...
		m_ViewDetailButton[i].ChangeButtonInfo(m_Pos.x + IMAGE_IGS_VIEWDETAIL_BTN_POS_X + ((i % IGS_NUM_ITEMS_WIDTH) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_X), m_Pos.y + IMAGE_IGS_VIEWDETAIL_BTN_POS_Y + ((i / IGS_NUM_ITEMS_HEIGHT) * IMAGE_IGS_VIEWDETAIL_BTN_DISTANCE_Y), IMAGE_IGS_VIEWDETAIL_BTN_WIDTH, IMAGE_IGS_VIEWDETAIL_BTN_HEIGHT);
...
void SEASON3B::CNewUIInGameShop::EventOrderWindows(double WindowsX, double WindowsY)
{
	this->SetPos(PositionX_The_Mid(640), PositionY_In_The_Mid(0));
}
...
bool CNewUIInGameShop::IsInGameShopOpen()
{
	...
	if (!(Hero->SafeZone) && (WD_0LORENCIA == gMapManager->currentMap || WD_3NORIA == gMapManager->currentMap || WD_2DEVIAS == gMapManager->currentMap || WD_51HOME_6TH_CHAR == gMapManager->currentMap))
	{
		CMsgBoxIGSCommon* pMsgBox = NULL;
		CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout), &pMsgBox);
...
void CNewUIInGameShop::InitBanner(unicode::t_char* pszFileName, unicode::t_char* pszBannerURL)
{
	...
	if (Bitmaps.ConvertFormat(pszFileName) == false)
		return;
...
void CNewUIInGameShop::RenderBanner()
{
	...
	RenderImage(IMAGE_IGS_BANNER, m_Pos.x + IMAGE_IGS_BANNER_POS_X, m_Pos.y + IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT);
}
...
bool CNewUIInGameShop::UpdateBanner()
{
	...
	if ((SEASON3B::IsPress(VK_LBUTTON))
		&& (SEASON3B::CheckMouseIn(m_Pos.x + IMAGE_IGS_BANNER_POS_X, m_Pos.y + IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH, IMAGE_IGS_BANNER_HEIGHT)))
	{
...
void CNewUIInGameShop::InitZoneBtn()
{
	...
	m_ZoneButton.UnRegisterRadioButton();
	m_ZoneButton.CreateRadioGroup(g_InGameShopSystem->GetSizeZones() > 1 ? 1:1, IMAGE_IGS_ZONE_BTN);
	m_ZoneButton.ChangeRadioButtonInfo(true, m_Pos.x + IMAGE_IGS_ZONE_BTN_POS_X, m_Pos.y + IMAGE_IGS_ZONE_BTN_POS_Y, IMAGE_IGS_ZONE_BTN_WIDTH, IMAGE_IGS_ZONE_BTN_HEIGHT);
	m_ZoneButton.SetFont(g_hFontBold);
	m_ZoneButton.ChangeRadioText(g_InGameShopSystem->GetZoneName());
	m_ZoneButton.ChangeFrame(0);
