...
bool CNewUIGoldBowmanWindow::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	m_EditBox->Init(gwinhandle->GethWnd(), 200, 14, MAXGOLDBOWMANSESERIAL);
...
void CNewUIGoldBowmanWindow::OpenningProcess()
{
	ZeroMemory ( g_strGiftName, sizeof( char )*64 );
...
bool CNewUIGoldBowmanWindow::UpdateMouseEvent()
{
	...
	{
		if (g_pMyInventory->FindEmptySlot(2, 4) == -1)
		{
...
bool CNewUIGoldBowmanWindow::Update()
{
	...
	if(m_EditBox)
	{
		...
		if(!m_EditBox->HaveFocus() && GetRelatedWnd() != gwinhandle->GethWnd())
		{
