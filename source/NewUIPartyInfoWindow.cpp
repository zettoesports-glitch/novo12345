...
#if MAIN_UPDATE > 603
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 12, GlobalText[190], WIN_WINDOW_SIZEX, 0, RT3_SORT_CENTER);
...
void CNewUIPartyInfoWindow::SetPos(int x, int y)
{
	...
	float RenderPosX = m_Pos.x;
	float RenderPosY = m_Pos.y;
	m_BtnExit.SetPos(RenderPosX + 22.f, RenderPosY + 384.f);
#else
	if (gmProtect->checkold_school())
	{
	...
	{
		int iVal = i * 71;
		m_BtnPartyExit[i].SetPos(m_Pos.x + 159, m_Pos.y + 63 + iVal);
	}
}
...
void SEASON3B::CNewUIPartyInfoWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
	float RenderFrameX = pos_right(640.f);
