...
namespace battleCastle
{
	...
	void SetBattleCastleStart(bool bResult)
	{
		...
		if ( /*g_bBattleCastleStart!=g_bBattleCastleStartBackup && */ gMapManager->currentMap != -1 && gMapManager->InBattleCastle())
		{
			char FileName[64];
			char WorldName[32];
	...
	bool    RenderBattleCastleMonsterObjectMesh(OBJECT* o, BMD* b)
	{
		...
		case MODEL_NPC_CAPATULT_ATT:
		{
			...
			BMD* linkBmd = gmClientModels->GetModel(MODEL_FLY_BIG_STONE1);
