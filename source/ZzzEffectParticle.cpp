...
int CreateParticleSync(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, float Scale, OBJECT* Owner)
{
	if (checkNormalizer)
	{
...
{
	// Upgrade 1: Options Low / Effect off must not spawn (was render-only skip).
	if (!EffectBudget::Enabled())
		return false;

	// Effect Level: skip cosmetic particles on Low/Medium to save FPS
	if (g_pOption)
	{
		int lv = g_pOption->GetRenderLevel();
		if (lv <= 1)
		{
			if (Type == BITMAP_SMOKE || Type == BITMAP_SPARK || Type == BITMAP_WATER)
				return false;
		}
		if (lv == 0)
		{
			if (Type == BITMAP_FIRE || Type == BITMAP_LIGHT)
				return false;
		}
	}

	if (EffectBudget::IsFull(Particles, MAX_PARTICLES, EffectBudget::CapParticles()))
		return false;
		...
		if (!o->Live)
		{
			...
			case BITMAP_FLAME:
				...
				case 2:
				{
					...
					MovePosition(o->Position, o->Angle, o->Velocity, false);
			...
			case BITMAP_RAIN_03:
				...
				if (o->Type == BITMAP_RAIN_03 || o->SubType == 1)
				{
			...
			case BITMAP_FLARE:  //
				...
				if (o->SubType == 10)
				{
					float count = (o->Velocity[0] + o->LifeTime._runvalue) * 0.1f;
			...
			case BITMAP_CLUD64:
			{
				...
				else if (o->SubType == 10)	// BITMAP_FIRE_CURSEDLICH o->SubType == 1�� ���.
				{
					o->Position[0] += ((rand() % 10 - 5) * 0.2f);
					o->Position[1] += ((rand() % 10 - 5) * 0.2f);
					o->Position[2] += ((rand() % 10 - 5) * 0.2f);
			...
			case BITMAP_SMOKELINE3:
				...
				{
					if (gMapManager->currentMap == WD_63PK_FIELD)
					{
						o->LifeTime = 25;
					}
					else
			...
			case BITMAP_SPARK + 1:
				...
				case 7:
					o->Alpha = 1.0f;
					o->Position[0] += (-30.0f + (float)(rand() % 60));
					o->Position[1] += (-30.0f + (float)(rand() % 60));
			...
			case BITMAP_CHERRYBLOSSOM_EVENT_FLOWER:
			{
				if (o->SubType == 0)
				{
					...
					o->Alpha = 1.0f;
					//o->Scale = (float)(rand()%20)/20.0f+1.0f;	//(1~2 20�ܰ�)
			...
			case BITMAP_SWORD_FORCE:
				...
				break;
			case BITMAP_CLOUD:
				if (gMapManager->currentMap == WD_77NEW_LOGIN_SCENE && o->SubType == 3)
				{
					o->LifeTime = 30;
					o->Gravity = (float)(rand() % 1000);
...
void MoveParticles()
{
	// Effects disabled: purge pool so slots free up and CPU skips the big sim loop.
	if (!EffectBudget::Enabled())
	{
		EffectBudget::ClearAll(Particles, MAX_PARTICLES);
		return;
	}

	// Wind integration scaled by frame time (was FPS-dependent → particles “sped up” at 120)
	const float windStep = static_cast<float>(timefac(1.0));
	for (int i = 0; i < 2; ++i)
	{
		g_vParticleWindVelo[i] += ((rand() % 2001 - 1000) * (0.001f * 0.6f)) * windStep;
		g_vParticleWindVelo[i] = max(-.6f, min(g_vParticleWindVelo[i], .6f));
	}

	for (int i = 0; i < 2; ++i)
	{
		g_vParticleWind[i] += g_vParticleWindVelo[i] * windStep;
		...
		if (o->Live)
		{
			...
			case BITMAP_EFFECT:
				if (o->LifeTime >= 10)
				{
					...
					{
						o->Light[0] *= timepow(1.16f);
						o->Light[1] *= timepow(1.16f);
						o->Light[2] *= timepow(1.16f);
					}
				}
				else
				{
			...
			case BITMAP_FLOWER01 + 2:
				if (o->Frame == 0)
				{
					if (o->SubType == 0)
					{
						o->Velocity[0] += ((float)(rand() % 32 - 16) * timepow(0.1f));
						o->Velocity[1] += ((float)(rand() % 32 - 16) * timepow(0.1f));
						o->Velocity[2] += ((float)(rand() % 32 - 16) * timepow(0.1f));
					}
					else if (o->SubType == 1)
					{
						o->Velocity[0] += ((float)(rand() % 8 - 4) * timepow(0.05f));
						o->Velocity[1] += ((float)(rand() % 8 - 4) * timepow(0.05f));
						o->Velocity[2] -= ((float)(rand() % 8) * timepow(0.05f));
					}
			...
			case BITMAP_BLUE_BLUR:
				...
				case 0:
					...
					if (o->SubType == 1)
					{
						o->Scale += timefac(0.19f);
						o->Position[2] += timefac(5.00f);
					}
					else
			...
			case BITMAP_LIGHT + 2:
				...
				{
					o->TurningForce[2] -= timefac(o->Gravity);

					o->Position[0] += timefac(o->TurningForce[0]);
					o->Position[1] += timefac(o->TurningForce[1]);
					o->Position[2] += timefac(o->TurningForce[2]);
					...
					if (o->LifeTime < 20)
					{
						o->TurningForce[0] *= timepow(0.6f);
						o->TurningForce[1] *= timepow(0.6f);
					}
					...
					VectorCopy(o->Target->Position, o->StartPosition);
					VectorAdd(o->Position, o->StartPosition, o->Position);
					o->Velocity[0] += timefac(0.05f);
					o->Velocity[1] += timefac(0.05f);

					o->Rotation += timefac(1.0f);
				}
				else if (o->SubType == 6)
				{
					o->TurningForce[2] -= timefac(o->Gravity);
					...
					o->Angle[0] += timefac(0.5f * o->LifeTime._runvalue);
					o->Angle[1] += timefac(0.5f * o->LifeTime._runvalue);
					o->Scale += timefac(0.04f);
					if (o->LifeTime < 10)
					{
						o->Alpha -= timefac(0.1f);
				...
				else if (o->SubType == 7)
				{
					...
					o->Light[0] += timefac(Luminosity);
					o->Light[1] += timefac(Luminosity);
					o->Light[2] += timefac(Luminosity);
				}
				else
				{
					Luminosity = (float)(o->LifeTime) / 8.f * 0.02f;
			...
			case BITMAP_BUBBLE:
				...
				case 0:
					o->Frame++;
					o->Position[0] += timefac((float)(rand() % 20 - 10) * 2.5f * o->Scale);
					o->Position[1] += timefac((float)(rand() % 20 - 10) * 2.5f * o->Scale);
					o->Position[2] += timefac((float)(rand() % 20 + 10) * 2.5f * o->Scale);
					break;
				case 1:
					o->Frame++;
					o->Position[0] += timefac((float)(rand() % 30 - 15) * 1.f * o->Scale);
					o->Position[1] += timefac((float)(rand() % 30 - 15) * 1.f * o->Scale);
					o->Position[2] += timefac((float)(rand() % 20 + 10) * 0.3f * o->Scale);
					o->Scale += timefac(0.002f);
					break;
				case 2:
					o->Position[0];
					o->Position[1];
					o->Position[2] += timefac((float)(rand() % 20 + 10) * 0.3f);
					o->Scale += timefac(0.005f);
					break;

				case 3:
					o->Frame++;
					o->Position[0] += timefac((float)(rand() % 20 - 10) * 2.5f * o->Gravity);
					o->Position[1] += timefac((float)(rand() % 20 - 10) * 2.5f * o->Gravity);
					o->Position[2] += timefac((float)(rand() % 20 + 10) * 2.5f * o->Gravity);
					break;
				case 4:
					o->Position[0];
					o->Position[1];
			...
			case BITMAP_EXPLOTION_MONO:
			case BITMAP_EXPLOTION:
				o->Frame = (20 - o->LifeTime._runvalue) / 2;

				if (o->SubType == 2)
				{
					int Life = 0;
					if (o->LifeTime > 10)
						Life = o->LifeTime._runvalue - 10;
					else
						Life = 1;
					o->Frame = (20 - Life) / 2;
					vec3_t  p, Angle;
				...
				break;
			case BITMAP_SUMMON_SAHAMUTT_EXPLOSION:
				o->Frame = (16 - o->LifeTime._runvalue);
				break;
			case BITMAP_SPOT_WATER:
				o->Frame = (32 - o->LifeTime._runvalue) / 4;
				break;
			case BITMAP_EXPLOTION + 1:
				o->Frame = (12 - o->LifeTime._runvalue) / 3;
				break;
			case BITMAP_LIGHTNING + 1:
			...
			case BITMAP_CHROME_ENERGY2:
				...
				o->Scale -= timefac(0.04f);
				o->Rotation += timefac(5);
				o->Frame = (23 - o->LifeTime._runvalue) / 6;
				break;
			case BITMAP_FIRE_CURSEDLICH:
			case BITMAP_FIRE_HIK2_MONO:
				if (o->SubType == 0)
				{
					o->Scale -= timefac((rand() % 20 + 10) * 0.001f);
					Luminosity -= timefac(0.05f);
					o->Position[2] += timefac((o->Gravity * 10.0));
				}
				else if (o->SubType == 1)
				{
					o->Scale -= timefac((rand() % 5 + 15) * 0.0016f);
					o->Position[2] += timefac((o->Gravity * 10.0));
				...
				{
					o->Scale -= timefac((rand() % 20 + 10) * 0.004f);
					Luminosity -= timefac(0.05f);
					o->Position[2] += timefac((o->Gravity * 25.f));
				}
				else if (o->SubType == 3)
				{
					o->Scale -= timefac(0.03f);
				...
				else if (o->SubType == 4 || o->SubType == 9)
				{
					if (o->LifeTime < 10)
					{
						o->Alpha -= timefac(0.2f);
					}
					else if (o->Alpha < 1.0f)
					{
						o->Alpha += (rand() % 2 + 2) * (0.1f) * timefac(1.0);
					}
					else
					{
						o->Alpha = 1.0f;
					...
					if (o->Scale > 0)
					{
						o->Scale -= ((rand() % 3 + 6) * 0.01f) * timefac(1.0);
					}
					else
					{
						o->Live = false;
					...
					o->Position[2] += timefac(o->Gravity);
					o->Rotation += timefac(3.0f);

					if (o->SubType == 9)
					{
						o->Rotation = 0.0f;
						o->Position[2] += timefac(o->Gravity * 1.2f);
					}
				}
				else if (o->SubType == 5)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= ((rand() % 2 + 5) * 0.01f) * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 6)
				{
					...
					o->Position[0] += timefac(o->Velocity[0]);
					o->Position[1] += timefac(o->Velocity[1]);
					o->Position[2] += timefac(o->Gravity);
					o->Rotation += timefac(3.0f);
				}
				else if (o->SubType == 7)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 3 + 3) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 8)
				{
					...
					o->Scale += timefac((rand() % 3 + 8) * 0.01f);

					if (o->LifeTime < 3)
					{
						o->Scale += ((10 - o->LifeTime._runvalue) * 0.02f);
					}
					...
					interpolatesum2(o->Position, vMoveVec, o->Position);

					VectorSubtract(o->Position, o->StartPosition, o->Position);
					VectorCopy(o->Target->Position, o->StartPosition);
					VectorAdd(o->Position, o->Target->Position, o->Position);

					o->Rotation += timefac(5.0f);
				}
				break;
			case BITMAP_LEAF_TOTEMGOLEM:
			{
				o->Velocity[2] += timefac(o->Gravity);
			...
			case BITMAP_FIRE + 3:
				...
				case 7:
				{
					o->Gravity += (timefac(0.004f));
					Luminosity = (float)(o->LifeTime) / 24.f;
					o->Frame = (15 - o->LifeTime._runvalue) / 6;
					o->Scale -= (timefac(0.04f));
					o->Frame = (23 - o->LifeTime._runvalue) / 6;
					o->Position[2] += (o->Gravity * 10.f);
				}
				...
				case 9:
				{
					...
					o->Gravity += (timefac(((rand() % 60) + 60) / 100));
				...
				case 11:
				{
					...
					if (o->LifeTime > 12)
					{
						o->Frame = (24 - o->LifeTime._runvalue) / 3;
					}
					o->Scale += (timefac(0.04f));
					o->Frame = (23 - o->LifeTime._runvalue) / 6;
					o->Position[2] += (o->Gravity * 10.f);
				...
				case 12:
				{
					o->Gravity += (timefac(0.004f));
					Luminosity = (float)(o->LifeTime) / 24.f;
					o->Rotation += (timefac((float)(rand() % 10 + 10.f)));
					o->Scale -= (timefac(0.04f));
				...
				case 16:
				{
					o->Frame = (3 - o->LifeTime._runvalue);
					o->Light[0] /= (timepow(1.7f));
					o->Light[1] /= (timepow(1.7f));
					o->Light[2] /= (timepow(1.7f));
				...
				case 18:
				{
					...
					Luminosity = (float)(o->LifeTime) / 24.f;
					o->Scale += (o->Gravity);
					VectorScale(o->Velocity, timepow(0.98f), o->Velocity);
					o->Frame = (23 - o->LifeTime._runvalue) / 6;
					o->Position[2] += (o->Gravity * 10.f);
			...
			case BITMAP_FIRE + 1:
				if (o->SubType == 1)
				{
					VectorScale(o->Velocity, timepow(1.05f), o->Velocity);
					o->Position[2] += (timefac(o->Gravity * 20.f));
				...
				else if (o->SubType == 5)
				{
					...
					o->Position[2] += (timefac(o->Gravity));
					o->Rotation += (timefac(1));
					//o->Scale += (0.003f);
				}
				else if (o->SubType == 6)
				{
					o->Position[2] += (timefac(o->Gravity));
					o->Rotation += (timefac(4));
				}
				else if (o->SubType == 7)
				{
					VectorScale(o->Velocity, timepow(1.05f), o->Velocity);
				...
				{
					o->Scale += (timefac(0.13f));
					o->Rotation += (timefac(o->Gravity));
					o->Light[0] /= (timepow(1.13f));
					o->Light[1] /= (timepow(1.13f));
					o->Light[2] /= (timepow(1.13f));
				}
				else if (o->SubType == 0)
				{
					o->Gravity += timefac(0.02f);
					o->Scale += timefac(o->Gravity);
					o->Velocity[0] *= (timepow(1.05));
					o->Velocity[1] *= (timepow(1.05));
					o->Velocity[2] *= (timepow(1.05));
			...
			case BITMAP_FLAME:
				...
				case 8:
				{
					...
					o->Position[2] += timefac(o->Gravity / 2.f);
					o->Scale -= timefac(o->Gravity / 95.f);
					if (o->Scale <= 0.0f)
						o->Live = false;

					o->Light[0] /= timepow(1.007f);
					o->Light[1] /= timepow(1.007f);
					o->Light[2] /= timepow(1.007f);
				}
				break;
				case 7:
				{
				...
				{
					if (o->StartPosition[0] < o->Position[0])
						o->Rotation += timefac(0.5f);
					else
						o->Rotation -= timefac(0.5f);

					o->Position[2] += timefac(o->Gravity * 1.2f);
					o->Scale -= timefac(o->Gravity / 98.f);
					if (o->Scale <= 0.0f)
						o->Live = false;

					o->Light[0] /= timepow(1.008f);
					o->Light[1] /= timepow(1.008f);
					o->Light[2] /= timepow(1.008f);
				}
				break;
				case 10:
					VectorAdd(o->Position, o->Velocity, o->Position);
					o->Velocity[0] *= timepow(0.95f);
					o->Velocity[1] *= timepow(0.95f);
					o->Light[0] = (float)(o->LifeTime) / 10.f;
					o->Light[1] = o->Light[0];
					o->Light[2] = o->Light[0];
					o->Velocity[2] += timefac(0.3f);
					o->Scale += timefac(0.07f);
					break;
				case 11:
					o->Light[0] /= timepow(1.008f);
					o->Light[1] /= timepow(1.008f);
			...
			case BITMAP_ENERGY:
			{
				...
				if (o->SubType == 1)
				{
					o->Light[1] = o->Light[2] = o->Light[0] = o->LifeTime._runvalue / 15.f;
					o->Scale += timefac(0.1f);
				}
				else if (o->SubType == 2)
				{
					o->Light[0] -= timefac(0.01f);
					o->Light[1] -= timefac(0.01f);
					o->Light[2] -= timefac(0.01f);
				}
				else if (o->SubType == 3 || o->SubType == 4 || o->SubType == 5)
				{
					BMD* pModel = gmClientModels->GetModel(o->Target->Type);
			...
			case BITMAP_FLARE:  //
				...
				else if (o->SubType == 2)
				{
					o->Position[0] = o->StartPosition[0];
					o->Position[1] = o->StartPosition[1];
					o->Position[2] += timefac(o->Gravity * ((60 - o->LifeTime._runvalue) / 10));
					o->Scale -= timefac(0.002f);
				}
				else if (o->SubType == 5)
				{
					...
					o->StartPosition[0] += timefac(2.5f);
				}
				...
				else if (o->SubType == 12)
				{
					...
					o->Position[2] += timefac((o->Gravity + 0.1f));
				...
				break;
			case BITMAP_FLARE_RED:
				o->Scale = sinf(o->LifeTime._runvalue * 10.f * (Q_PI / 180.f)) * 3.f;
				HandPosition(o);
				break;
			case BITMAP_CLUD64:
			{
				...
				else if (o->SubType == 6)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= timefac((rand() % 5 + 55) * 0.001f);
					}
				...
				else if (o->SubType == 7)
				{
					...
					{
						o->Alpha += ((float)(rand() % 20) * timepow(0.005f));
					}

					VectorAdd(o->Position, o->Velocity, o->Position);
					o->Scale += timefac(((float)(rand() % 10) * 0.01f));
				}
				else if (o->SubType == 8)
				{
					if (o->LifeTime < 10)
				...
				{
					o->Scale *= timepow(0.98f + ((float)(rand() % 20 - 10) * 0.002f));

					o->Light[0] *= timepow(0.95f + ((float)(rand() % 20 - 10) * 0.002f));
					o->Light[1] *= timepow(0.95f + ((float)(rand() % 20 - 10) * 0.002f));
					o->Light[2] *= timepow(0.95f + ((float)(rand() % 20 - 10) * 0.002f));
					o->Alpha *= timepow(0.95f + ((float)(rand() % 20 - 10) * 0.002f));
				}
				else if (o->SubType == 10)
				{
					o->Scale -= timefac((rand() % 5 + 15) * 0.0016f);
			...
			case BITMAP_LIGHT + 3:
			{
				if (o->SubType == 0)
				{
					float _Angle = (o->Velocity[0] + o->LifeTime._runvalue) * 0.1f;
					o->Position[0] = o->StartPosition[0] + sinf(_Angle) * 35.f;
					o->Position[1] = o->StartPosition[1] - cosf(_Angle) * 35.f;
					o->Position[2] += timefac(o->Gravity * 0.1f);
					o->Scale -= timefac(0.001f);
				}
#ifdef PJH_ADD_PANDA_PET
				else if (o->SubType == 1)
				{
					o->Scale = sinf(o->LifeTime._runvalue * 2.f * (Q_PI / 180.f));
				}
			...
			case BITMAP_TWINTAIL_WATER:
			{
				...
				{
					o->Scale -= timefac(0.013f);
				}
				else if (o->SubType == 2)
				{
					o->Scale -= timefac(0.026f);
				}
			...
			case BITMAP_SMOKE:
				...
				case 36:
					if (o->LifeTime < 23)
					{
						o->Gravity += timefac(0.1f * o->Scale);
						o->Position[2] += timefac(o->Gravity);
						o->Position[1] += timefac(o->Gravity);
						o->Scale -= timefac(0.08f);
					}
					else
					{
					...
					o->Rotation += (timefac(0.01f));
					break;
				case 37:
					o->Position[2] -= timefac(o->TurningForce[2] * 0.8f);
					o->Position[1] -= timefac(o->TurningForce[1] * 0.8f);
					o->Scale += timefac(0.08f);
					o->Rotation += timefac(0.01f);
					o->Light[0] /= timepow(1.03f);
					o->Light[1] /= timepow(1.03f);
				...
				case 33:
					...
					VectorScale(o->Velocity, timepow(0.001f), o->Velocity);
					o->Scale += timefac(0.01f * 2);
					o->Position[2] += timefac(o->Scale);
					break;
				case 32:
					Luminosity = (float)(o->LifeTime) / 8.f;
					Vector(Luminosity * 0.8f, Luminosity * 0.8f, Luminosity, o->Light);
					VectorScale(o->Velocity, timepow(0.4f), o->Velocity);
					o->Scale += timefac(0.1f);
					break;
				case 23:
					Luminosity = (float)(o->LifeTime) / 8.f;
				...
				case 15:
					...
					o->Gravity += timefac((rand() % 20) / (500.f));
					o->Position[0] += timefac((rand() % 100 - 20) / (100.f));
					o->Position[1] += timefac((rand() % 100 - 20) / (100.f));
					o->Position[2] += timefac(o->Gravity);
					o->Scale += timefac((rand() % 10) / (1000.f));
					o->Rotation = o->Scale;
					break;
				case 1:
					Luminosity = (float)(o->LifeTime) / 50.f;
				...
				case 2:
					...
					Vector(Luminosity, Luminosity, Luminosity, o->Light);
					o->Gravity -= timefac(0.1f);
					o->Position[0] -= timefac(o->Gravity * 0.2f);
					o->Position[2] += timefac(o->Gravity);
					o->Scale -= timefac(0.01f);
					break;
				case 16:
				...
				case 5:
					...
					Vector(Luminosity, Luminosity, Luminosity, o->Light);
					o->Gravity -= timefac(0.1f);
					o->Position[0] -= timefac(o->Gravity * ((0.2f) * (rand() % 2 + 1)));
					o->Position[2] += timefac(o->Gravity);
					o->Scale -= timefac(0.01f);
					break;
				case 6:
				...
				case 7:
					Luminosity = 1.0f;
					o->Scale += timefac(0.03f);
					o->Gravity += timefac(1.0f);
					o->Velocity[1] -= timefac(0.1f);
					o->Position[2] -= timefac(o->Gravity);
					if (o->LifeTime < 5)
					{
						Luminosity = (float)(o->LifeTime) / 8.f;
				...
				case 22:
					...
					o->Gravity -= timefac(0.1f);
					o->Position[0] += timefac(o->Gravity * sinf(WorldTime) * 0.05f);
					o->Position[2] += timefac(o->Gravity);
					o->Scale += timefac(0.04f);
					break;
				case 24:
				...
				case 44:
				{
					if (o->LifeTime >= 30)
					{
						o->Light[0] *= (timepow(1.07f));
						o->Light[1] *= (timepow(1.07f));
						o->Light[2] *= (timepow(1.07f));
					}
					else
					{
						o->Light[0] /= (timepow(1.07f));
					...
					o->Position[2] += (timefac(0.8f));
					o->Rotation += timefac(o->Gravity / (50.0f));
				}
				break;
				case 45:
				{
				...
				case 46:
				{
					...
					o->Gravity -= timefac(0.05f);
					o->Position[2] += timefac((o->Scale + o->Gravity) * (1.5f));
					o->Scale += timefac(o->Scale / (100.0f));
				}
				break;
				case 47:
				{
				...
				case 49:
				{
					...
					VectorScale(o->Velocity, timepow(0.001f), o->Velocity);
					o->Scale += timefac((rand() % 15 + 4) * (0.001f));
					o->Position[2] += timefac(o->Scale * (6.0f));
					o->Rotation += timefac((rand() % 10 + 10) * (0.1f) * o->Angle[0]);
				}
				break;
				case 50:
					if (o->LifeTime < 10)
				...
				case 52:
				{
					o->Position[0] += timefac(o->Velocity[0]);
					o->Position[1] += timefac(o->Velocity[1]);
					o->Position[2] += timefac(o->Velocity[2]);
				...
				case 58:
					Luminosity = (float)(o->LifeTime) / 50.f;
					Vector(Luminosity * o->StartPosition[0], Luminosity * o->StartPosition[1], Luminosity * o->StartPosition[2], o->Light);
					VectorScale(o->Velocity, timepow(0.4f), o->Velocity);
					o->Scale += (timefac(0.05f));
					break;
				case 60:
				...
				case 64:
				{
					...
					o->Rotation += (timefac(0.05f));//(rand()%100)/100.f;

					o->Gravity -= (timefac(0.1f));
					o->Position[0] += timefac(o->Gravity * sinf(WorldTime) * (0.05f));
					o->Position[2] += timefac(o->Gravity);

					o->Scale += timefac((float)(rand() % 15 + 15) * (0.01f));
				...
				case 66:
				{
					...
					o->Gravity -= (timefac(0.05f));
					o->Position[0] += timefac(o->TurningForce[0] * (o->Scale + o->Gravity) * (1.0f));
					o->Position[1] += timefac(o->TurningForce[1] * (o->Scale + o->Gravity) * (1.0f));
					o->Scale += timefac(o->Scale / (100.0f));
				}
				break;
				case 67:
				...
				case 68:
				{
					Luminosity = (float)(o->LifeTime) / 40.f;
					Vector(Luminosity * 0.4f, Luminosity * 0.4f, Luminosity * 0.4f, o->Light);
					o->Velocity[0] += timefac((0.03f * o->Velocity[0]));
					o->Velocity[1] += timefac((0.03f * o->Velocity[1]));
					o->Velocity[2] += timefac((0.03f * o->Velocity[2]));
			...
			case BITMAP_SMOKE + 2:
				Luminosity = 1.f;
				if (o->LifeTime < 10)
					Luminosity -= ((float)(10 - o->LifeTime._runvalue) * (0.1f));
				Vector(Luminosity, Luminosity, Luminosity, o->Light);
				AngleMatrix(o->Angle, Matrix);
			...
			case BITMAP_SMOKE + 3:
				...
				case 0:
					if (o->TurningForce[0] > 0.1f)
						o->TurningForce[0] -= (timefac(0.015f));
					if (o->TurningForce[1] > 0.1f)
						o->TurningForce[0] -= (timefac(0.005f));
				...
				case 2:
					if (o->TurningForce[1] > 0.1f)
						o->TurningForce[1] -= (timefac(0.005f));
					if (o->TurningForce[2] > 0.1f)
						o->TurningForce[2] -= (timefac(0.015f));
				...
				case 3:
					if (o->Light[0] <= 0.05f)
						o->Live = false;
					o->Light[0] /= (timepow(1.012f));
					o->Light[1] /= (timepow(1.012f));
					o->Light[2] /= (timepow(1.012f));
					o->Scale += (timefac(0.01f));
					o->Rotation += (o->Gravity / timepow(2.0f));
			...
			case BITMAP_SMOKELINE3:
				if (o->SubType == 0 || o->SubType == 5)
				{
					...
					Luminosity = o->Alpha;
					Vector(o->TurningForce[0] * Luminosity, o->TurningForce[1] * Luminosity, o->TurningForce[2] * Luminosity, o->Light);
					o->Scale += timefac((rand() % 3 + 6) * (0.01f));//0.07f;
					o->Position[2] += (timefac(o->Gravity));
				...
				else if (o->SubType == 1)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= timefac((rand() % 10 + 10) * (0.001f));//0.07f;
					}
					else
					{
						o->Live = false;
				...
				else if ((o->SubType == 2) || (o->SubType == 3))
				{
					...
					Luminosity = o->Alpha;
					Vector(o->TurningForce[0] * Luminosity, o->TurningForce[1] * Luminosity, o->TurningForce[2] * Luminosity, o->Light);
					o->Scale += timefac((rand() % 3 + 6) * (0.01f));
					o->Position[2] += (timefac(o->Gravity));
				}
				else if (o->SubType == 4)
				{
					o->Position[2] += (timefac(o->Gravity));
					o->Rotation += (timefac(0.8f));
					o->Scale += (timefac(0.01f));
			...
			case BITMAP_FIRE_HIK1_MONO:
				if (o->SubType == 0 || o->SubType == 6)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 3 + 5) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 1)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 2 + 4) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 2)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 3 + 7) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 3)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= ((rand() % 3 + 2) * 0.01f) * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 4)
				{
					...
					VectorRotate(vMoveDir, Matrix, vMoveVec);
					VectorScale(vMoveVec, 8.0f, vMoveVec);
					interpolatesum(o->Position, vMoveVec);
			...
			case BITMAP_FIRE_HIK3_MONO:
				...
				else if (o->SubType == 1)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 2 + 6) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 4)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= (rand() % 4 + 5) * 0.01f * timefac(1.0);
					}
					else
					{
						o->Live = false;
				...
				else if (o->SubType == 5)
				{
					...
					if (o->LifeTime < 3)
					{
						o->Scale += timefac((10 - o->LifeTime._runvalue) * 0.02f);
					}
			...
			case BITMAP_LIGHT + 1:
				if (o->SubType == 2)
				{
					o->Scale *= timepow(0.92f);
				}
				else if (o->SubType == 3)
				{
					o->Scale *= timepow(1.3f);
					o->Light[0] *= timepow(0.9f);
					o->Light[1] *= timepow(0.9f);
					o->Light[2] *= timepow(0.9f);
				}
			...
			case BITMAP_BLOOD + 1:
				o->Frame = (12 - o->LifeTime._runvalue) / 3;
				VectorScale(o->Velocity, timepow(0.95f), o->Velocity);
			...
			case BITMAP_SPARK:
				...
				if (o->SubType == 7)
					o->Gravity -= timefac(0.6f);
				else if (o->SubType == 9)
					o->Gravity -= timefac(0.4f);
				else
				...
				{
					o->Position[2] = Height;
					o->Gravity = -o->Gravity * timepow(0.6f);
			...
			case BITMAP_SPARK + 1:
				...
				case 2:
					...
					o->Light[0] = o->LifeTime._runvalue / 5.f;
					o->Light[1] = o->LifeTime._runvalue / 5.f;
					o->Light[2] = o->LifeTime._runvalue / 5.f;
				...
				case 9:
				{
					o->Velocity[0] -= timefac(1.2f);
					o->Velocity[2] -= timefac(1.0f);
					o->Scale -= timefac(0.08f);
					if (o->LifeTime <= 0)
						o->Live = false;
					if (o->Scale < 0.2f)
				...
				case 18:
				{
					...
					if (o->Frame == 77)
					{
						o->Gravity *= timepow(1.03f);
						o->Position[2] -= timefac(o->Gravity);
					}
					else if (o->Frame == 88)
					{
						o->Gravity /= timepow(1.2f);
						if (o->Gravity <= 0.1f)
						{
							o->Gravity = 0.1f;
							o->Frame = 99;
				...
				case 20:
				{
					...
					o->Position[2] += timefac(o->Gravity * (0.5f));
				...
				case 24:
				{
					if (o->LifeTime < 10 && o->Rotation == 0)
					{
						VectorScale(o->Velocity, timepow(-1), o->Velocity);
						o->Rotation = 1;
					}
				...
				case 25:
				{
					if (o->LifeTime == 19)
					{
						o->Velocity[0] -= timefac(1.f);
						o->Velocity[1] -= timefac(1.f);
						o->Velocity[2] -= timefac(1.f);
					}

					o->Light[0] /= timepow(1.05f);
					o->Light[1] /= timepow(1.05f);
					o->Light[2] /= timepow(1.05f);
					o->Scale = sinf(o->LifeTime._runvalue * (Q_PI / 40.f));
					o->Alpha -= timefac(0.001f);
					if (o->Alpha <= 0.0f)
						o->Live = false;
				}
				...
				case 30:
				{
					if (o->LifeTime <= 45)
					{
						o->Velocity[0] += timefac(sinf(Q_PI / 180.0f * float(rand() % 360)));
						o->Velocity[1] += timefac(cosf(Q_PI / 180.0f * float(rand() % 360)));
					}
			...
			case BITMAP_SPARK + 2:
			{
				if (o->SubType == 0)
				{
					o->Frame = (16 - o->LifeTime._runvalue) / 4;
				}
				else if (o->SubType == 1)
				{
			...
			case BITMAP_SMOKE + 4:
				...
				//o->Angle[0] += (4.f);
				interpolatesum2(o->Position, o->Velocity, o->Position);
				VectorScale(o->Velocity, timepow(0.9f), o->Velocity);

				if (o->SubType == 6)
				{
					o->Scale -= timefac(0.05f);
					o->Position[2] += timefac(4.0f);
				}
				else if (o->SubType != 5)
				{
			...
			case BITMAP_SHINY:
				if (o->SubType == 2)
				{
					o->Rotation -= timefac(12.f);

					o->Velocity[2] -= timefac((float)(rand() % 8) * 0.05f);
					VectorAdd(o->Position, o->Velocity, o->Position);
				...
				else if (o->SubType == 6 || o->SubType == 8 || o->SubType == 9)
				{
					o->Light[0] /= timepow(1.01f);
					o->Light[1] /= timepow(1.01f);
					o->Light[2] /= timepow(1.01f);
					...
					o->StartPosition[0] /= timepow(1.01f);
					...
					if (o->LifeTime < 60)
					{
						o->Velocity[0] /= timepow(1.04f);
						o->Velocity[1] /= timepow(1.04f);
						o->Velocity[2] /= timepow(1.04f);

						o->Position[2] -= timefac((o->Gravity * 0.1f));
						o->Gravity += timefac(0.5f);

						if (o->SubType == 9 && rand() % 5 == 0)
						{
				...
				{
					o->Scale = sinf(o->LifeTime._runvalue * 10.f * (Q_PI / 180.f));
					if (o->SubType == 1)
					{
						o->Scale *= timepow(0.75f);
						o->Rotation -= timefac(12.f);
					}
				}
				break;
			case BITMAP_CHERRYBLOSSOM_EVENT_PETAL:
			{
				...
				{
					o->Rotation = (float)(rand() % 360);
					o->Scale = sinf(o->LifeTime._runvalue * (Q_PI / 180.f));
					o->Alpha -= timefac(0.001f);
					if (o->Alpha <= 0.0f)
						o->Live = false;
				}
			...
			case BITMAP_SHINY + 1:
				...
				else
				{
					...
					{
						o->Scale = sinf(o->LifeTime._runvalue * 5.f * (Q_PI / 180.f)) * 5.f;
			...
			case BITMAP_SHINY + 4:
				if (o->SubType == 0)
				{
					o->Scale = sinf(o->LifeTime._runvalue * 10.f * (Q_PI / 180.f)) * 3.f + 2.f;
					Luminosity = (float)(o->LifeTime) / 5.f;
					Vector(Luminosity, Luminosity, Luminosity, o->Light);
				}
				else if (o->SubType == 1)
				{
					...
					o->Gravity += ((15 - o->LifeTime._runvalue) * 6.f);
					if (o->Gravity > 90.f)
					{
						o->Gravity = 90.f;
					}
			...
			case BITMAP_SHINY + 6:
				if (o->SubType == 0)
				{
					...
					o->Light[0] += timefac(fLight);
					o->Light[1] += timefac(fLight);
					o->Light[2] += timefac(fLight);
				}
				else if (o->SubType == 1)
				{
					o->Scale -= timefac(0.01f);
			...
			case BITMAP_GHOST_CLOUD2:
			{
				if (o->SubType == 0)
				{
					...
					if (fSin > 0.f)
						o->Rotation += timefac(0.2f + (rand() % 4 * 0.1f));
					else
						o->Rotation -= timefac(0.2f + (rand() % 4 * 0.1f));
					...
					if (fTemp <= 20.f)
					{
						for (int i = 0; i < 3; ++i)
						{
							o->Light[i] = o->TurningForce[i] * 0.1f * (500 - o->LifeTime._runvalue);
			...
			case BITMAP_CLOUD:
				if (o->SubType == 6)
				{
					...
					o->Velocity[0] *= timepow(0.95f);
					o->Velocity[1] *= timepow(0.95f);
					o->Velocity[2] += timefac(0.6f);
					o->Position[0] += timefac((float)(rand() % 4 - 2));
					o->Position[1] += timefac((float)(rand() % 4 - 2));
					o->Position[2] += timefac((float)(rand() % 4 - 2) * 0.8f);
					o->Scale += timefac(0.05f);
					o->Light[0] = (float)(o->LifeTime) / 50.f;
					o->Light[1] = o->Light[0];
					o->Light[2] = o->Light[0];
					o->Rotation += timefac((1.f + rand() % 2));
				}
				else if (o->SubType == 8)
				{
					if (o->LifeTime <= 0)
					...
					{
						if (o->Alpha < 1.0f)
							o->Alpha += timefac(0.04);
						o->Position[2] += timefac(1.f);
					}
					else
					{
						if (o->Alpha > 0.1f)
							o->Alpha -= timefac(0.025f);
				...
				else if (o->SubType == 11)
				{
					...
					o->Rotation += timefac((0.5f + rand() % 2));
				...
				{
					o->Rotation += timefac(o->Gravity * 1.5f);
				}
				else if (o->SubType == 14)
				{
					interpolatesum(o->Position, o->Velocity);
				...
				else if (o->SubType == 17)
				{
					...
					o->Rotation += timefac((0.08f + rand() % 2 * 0.15f));

					if (o->LifeTime >= 400.f)
					{
						for (int i = 0; i < 3; ++i)
						{
							o->Light[i] = o->TurningForce[i] * 0.02f * (500 - o->LifeTime._runvalue);
							if (o->Light[i] > o->TurningForce[i])
							{
								o->Light[i] = o->TurningForce[i];
							}
				...
				else if (o->SubType == 18)
				{
					...
					if (o->LifeTime > 90)
					{
						o->Light[0] += timefac(0.0020f);
						o->Light[1] += timefac(0.0020f);
						o->Light[2] += timefac(0.0017f);
					}
					else if (o->LifeTime < 30)
					{
						o->Light[0] -= timefac(0.0040f);
						o->Light[1] -= timefac(0.0040f);
						o->Light[2] -= timefac(0.0033f);
					}
					if (o->Scale > 0)
						o->Scale -= timefac(0.001f);
					if (o->TurningForce[1] > 0)
						o->TurningForce[1] -= timefac(0.5f);
					...
					o->Position[2] += timefac(((rand() % 10) / 10.0f));
				...
				else if (o->SubType == 19)
				{
					...
					o->Position[1] += timefac((8.0f + (rand() % 200) * 0.1f));
				...
				else if (o->SubType == 21)
				{
					if (o->LifeTime <= 0) {
						...
						if (o->Alpha > 0.1f)
							o->Alpha -= timefac(0.005f);
						o->Position[2] -= timefac(1.0f);
					}
				}
				else if (o->SubType == 22)
				{
				...
				else if (o->SubType == 23)
				{
					...
					{
						o->Velocity[0] *= timepow(0.7f);
						o->Velocity[1] *= timepow(0.7f);

						if (o->Alpha > 0.0f)
							o->Alpha -= timefac((rand() % 10 * 0.01f));
						else
							o->Live = false;
						o->Position[2] += timefac(0.2f);

						//o->Scale += (0.0005f);
					}
					o->Rotation += (o->TurningForce[0] * timepow(5.0f));
				}
				else
				{
					Luminosity = 0.6f;
			...
			case BITMAP_LIGHT:
				if (0 == o->SubType || 2 == o->SubType || o->SubType == 7 || o->SubType == 8)
				{
					...
					if (o->SubType == 7)
					{
						fScale = sinf(WorldTime + o->LifeTime._runvalue) * 5.f + 10.f;
						o->Position[2] += timefac(fScale);
					}
					else
					{
						...
						interpolatesum(o->Position, vRandom);
						o->Position[2] += timefac((5.f * fScale));
					}
					...
					{
						o->Rotation += timefac(30.f);
					}
					else if (1 == o->SubType)
					{
						o->Scale *= timepow(0.98f);
					}
					else if (5 == o->SubType)
					{
						o->Scale *= timepow(0.95f);
				...
				{
					VectorScale(o->Light, timepow(0.9f), o->Light);
					o->Scale *= timepow(0.85f);
					o->Gravity += timefac(5.f);

					VectorCopy(o->Target->Position, o->Position);

					o->Position[0] += timefac(2.f);
					o->Position[1] -= timefac(2.f);
					o->Position[2] += timefac(o->Gravity);
				}
				else if (o->SubType == 4)
				{
					...
					o->Gravity += timefac(((rand() % 40 + 60) / 100.f * 9.5f));
					o->Scale -= timefac(((rand() % 400 + 400) / 10000.f));
					o->Position[2] += (o->Gravity);

					o->Light[0] /= timepow(1.35);
					o->Light[1] /= timepow(1.35);
					o->Light[2] /= timepow(1.35);
				}
				else if (o->SubType == 9)
				{
					o->Scale -= timefac(0.011f);
					if (!o->Live)
						o->LifeTime = 0;
					if (o->Scale <= 0.0f)
						o->LifeTime = 0;
				...
				{
					VectorScale(o->Light, timepow(0.98f), o->Light);
					o->Scale *= timepow(0.95f);
				}
				else if (o->SubType == 15)
				{
					o->StartPosition[2] += timefac(((rand() % 10 + 5) * 0.01f));
			...
			case BITMAP_POUNDING_BALL:
				...
				else if (o->SubType == 3)
				{
					...
					if (o->Scale > 0)
					{
						o->Scale -= timefac(((rand() % 3 + 5) * 0.01f));
					}
					else
					{
						o->Live = false;
			...
			case BITMAP_ADV_SMOKE + 1:
				...
				o->Velocity[1] *= timepow(0.95f);
				o->Velocity[2] += timefac(0.6f);
				o->Position[0] += timefac((float)(rand() % 4 - 2.0));
				o->Position[1] += timefac((float)(rand() % 4 - 2.0));
				o->Position[2] += timefac((float)(rand() % 4 - 2.0) * 0.8);
				o->Scale += timefac(0.05f);
				if (o->SubType == 2)
				{
			...
			case BITMAP_TRUE_BLUE:
				...
				if (o->SubType >= 3 && o->SubType < 5 && o->Target != NULL)
				{
					...
					{
					case 3:
						name = "Monster82_LHand";
						break;
					case 4:
						name = "Monster82_RHand";
						break;
					}
					if (BoneManager::GetBonePosition(o->Target, name, TargetPosition)) {
						...
						o->Scale -= timefac(length * (0.003f));
						o->Light[0] -= timefac(length * (0.08f));
						o->Light[1] -= timefac(length * (0.08f));
						o->Light[2] -= timefac(length * (0.08f));
					}
				}
			...
			case BITMAP_WATERFALL_5:
				...
				{
					o->Scale -= (timefac(0.005f));
					o->Rotation += (timefac(4.f));
					o->Velocity[2] += (timefac(0.1f));
				}
				else if (o->SubType == 4)
				{
					o->Position[0] += timefac((float)(cos(o->Angle[2]) * 20.0f));
					o->Position[1] += timefac((float)(sin(o->Angle[2]) * 20.0f));
					o->Position[2] += (timefac(o->Gravity));
					o->Rotation += (timefac(1));
				}
				else if (o->SubType == 5)
			...
			case BITMAP_PLUS:
				o->Scale -= (timefac(0.01f));
				o->Position[0] += (timefac(rand() % 2 - 1));
				o->Position[1] += (timefac(rand() % 2 - 1));
				o->Position[2] += (timefac(2.f));

				o->Light[0] = o->LifeTime._runvalue / 20.f;
				o->Light[1] = o->LifeTime._runvalue / 20.f;
				o->Light[2] = o->LifeTime._runvalue / 20.f;
				break;
			...
			case BITMAP_WATERFALL_2:
				if (o->SubType == 5)
				{
					...
					o->Position[0] += timefac(o->Velocity[0]);
					o->Position[1] += timefac(o->Velocity[1]);
					o->Position[2] += timefac(o->Gravity + o->Velocity[2]);
				...
				o->Scale += (timefac(0.03f));
				o->Velocity[0] = timefac((rand() % 20 - 10) * 0.1);
				o->Velocity[1] = timefac((rand() % 20 - 10) * 0.1);
				o->Velocity[2] += (timefac(0.1f));
				if (o->LifeTime < 10)
				{
					o->Light[0] /= (timepow(1.1f));
				...
				if (o->SubType == 3)
				{
					o->Scale -= (timefac(0.038f));
					o->Velocity[2] -= (timefac(0.02f));
					o->Light[0] /= (timepow(1.02f));
					o->Light[1] /= (timepow(1.02f));
					o->Light[2] /= (timepow(1.02f));
				}
				if (o->SubType == 4)
				{
					o->Rotation -= (timefac(1.1f));
				}
				if (o->SubType == 6)
				{
					o->Gravity += (timefac(rand() % 5 + 1.5f * (o->LifeTime._runvalue / 5.0f)));

					o->Velocity[0] += (timefac(1));

					o->Velocity[0] -= (timefac(100 * (20.0f / o->LifeTime._runvalue)));
					o->Rotation += (timefac(50.f));
					AngleMatrix(o->Angle, Matrix);
					VectorRotate(o->Velocity, Matrix, TargetPosition);
					VectorAdd(o->StartPosition, TargetPosition, o->Position);
					o->Scale += (timefac((1.0 / o->LifeTime._runvalue) * 2.0f));

					o->Light[0] *= (timepow(0.80f));
					o->Light[1] *= (timepow(0.77f));
					o->Light[2] *= (timepow(0.77f));
			...
			case BITMAP_WATERFALL_4:
				...
				else if (o->SubType == 12)
				{
					...
					o->Scale *= (timepow(0.92f));
					o->Light[0] *= (timepow(0.92f));
					o->Light[1] *= (timepow(0.92f));
					o->Light[2] *= (timepow(0.92f));
			...
			case BITMAP_SHOCK_WAVE:
			{
				...
				if (o->SubType == 4)
				{
					...
					o->Light[1] /= timepow(1.03f);
					o->Light[2] /= timepow(1.03f);
					o->Scale += timefac(o->Gravity * 0.015f);
					o->Gravity += timefac(2.4f);
				}
			}
			break;
			case BITMAP_CURSEDTEMPLE_EFFECT_MASKER:
			{
				...
				if (o->SubType == 0)
				{
					...
					if (rand() % 2 == 1)
						CreateParticleSync(BITMAP_CURSEDTEMPLE_EFFECT_MASKER, o->StartPosition, o->Angle, Light, 1, 1.3f);
				}
			}
			break;
			case BITMAP_RAKLION_CLOUDS:
			{
				...
				interpolatesum(o->Position, vForceVec);

				o->Scale += timefac(SCALEFACTOR);
				o->Light[0] /= timepow(LIGHTDIVIDEDFACTOR);
				o->Light[1] /= timepow(LIGHTDIVIDEDFACTOR);
				o->Light[2] /= timepow(LIGHTDIVIDEDFACTOR);
				...
				if (rand() % 10 == 0)
				{
					...
					CreateParticleSync(BITMAP_SHINY + 6, v3Pos_ShinyStar, o->Angle, o->Light, 0, 0.5f);
				}
			}
			break;
			case BITMAP_CHROME2:
			...
			case BITMAP_AG_ADDITION_EFFECT:
			{
				...
				if (b)
				{
					...
					if (!o->Live && g_isCharacterBuff(o->Target, eBuff_AG_Addition))
					{
						...
						if (o->SubType == 0)
							CreateParticleSync(BITMAP_AG_ADDITION_EFFECT, Temp_Pos, o->Angle, o->Light, 0, 1.0f, o->Target);
						else if (o->SubType == 1)
							CreateParticleSync(BITMAP_AG_ADDITION_EFFECT, Temp_Pos, o->Angle, o->Light, 1, 1.0f, o->Target);
						else if (o->SubType == 2)
							CreateParticleSync(BITMAP_AG_ADDITION_EFFECT, Temp_Pos, o->Angle, o->Light, 2, 1.0f, o->Target);
					}
				}
			}
			break;
			case BITMAP_SBUMB:
			{
				o->Frame = 4 - o->LifeTime._runvalue;
			}
			break;
			case BITMAP_DAMAGE1:
			{
			...
			case BITMAP_SWORD_EFFECT_MONO:
			{
				...
				VectorScale(o->Light, timepow(0.94f), o->Light);
			}
			break;
			case BITMAP_DAMAGE2:
			{
				o->Scale *= timepow(1.2f);
				VectorScale(o->Light, timepow(0.55f), o->Light);
			}
			break;
			}
		}
	...
	for (int i = 0; i < MAX_PARTICLES; i++)
	{
		...
		if (o->Live)
		{
			...
			case BITMAP_FIRECRACKER:
			{
				...
				vec3_t Position;
				vec3_t Light;
				int iTemp = o->LifeTime._runvalue / 4 + o->SubType;
				//int iTemp = 0;
				int iColor = iTemp / 10;
