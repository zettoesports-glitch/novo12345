...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort = RT3_SORT_CENTER, int iRenderPoint = STRP_NONE, BOOL bUseBG = TRUE, bool bRender3d = false);
...
void RenderItem3D(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp = false, float _renderposx=0.0, float _renderposy = 0.0, float _rendersizex = 0.0, float _rendersizey = 0.0, float Scale = 0.0);
...
void RequireClass(Script_Item* p);
...
namespace SEASON3B
{
	extern void RenderGoldRect(float fPos_x, float fPos_y, float fWidth, float fHeight, int iFillType = 0);
	extern void RenderInventoryInterface(int StartX, int StartY, int Flag);
	extern void SetButtonImagen(CNewUIButton* button, int imgindex1, int imgindex2, float su = 0.f, float sv = 0.f, float uw = 0.f, float vh = 0.f);
	extern void SetButtonPosition(CNewUIButton* button, int StartX, int StartY, int type);
	extern void SetButtonInterface(CNewUIButton* button, int StartX, int StartY, int type, bool move = true);
}
