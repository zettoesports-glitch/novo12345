...
void SEASON3B::CNewUIChatLogWindow::Init()
{
	...
	m_dwPendingExp = 0;
	m_dwPendingExpKills = 0;
	m_dwPendingExpTick = 0;
	m_bPendingMasterExp = false;

	SetAllMsgEnable();
	// EXP (system) + conversation must both be visible and age out together
	m_msgEnable[TYPE_SYSTEM_MESSAGE] = true;
	m_msgEnable[TYPE_CHAT_MESSAGE] = true;
}
...
void SEASON3B::CNewUIChatLogWindow::ProcessAddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, MESSAGE_TYPE ErrMsgType, void* MemoryData)
{
	...
	// Always stick to bottom when following live chat (clean rise with new lines)
	if (nScrollLines > 0)
		StickScrollToBottom();
}
...
void SEASON3B::CNewUIChatLogWindow::ProcessAddText(const type_string& strID, const type_string& strText, MESSAGE_TYPE MsgType, MESSAGE_TYPE ErrMsgType)
{
	...
	// Always stick to bottom when following live chat
	if (nScrollLines > 0)
		StickScrollToBottom();
}
#endif // ITEM_CHAT_SELL_SYSTEM

void SEASON3B::CNewUIChatLogWindow::StickScrollToBottom()
{
	type_vector_msgs* pvecMsgs = GetMsgs(GetCurrentMsgType());
	if (pvecMsgs == NULL || pvecMsgs->empty())
	{
		m_iCurrentRenderEndLine = -1;
		return;
	}
	m_iCurrentRenderEndLine = (int)pvecMsgs->size() - 1;
	UpdateScrollPos();
}

int SEASON3B::CNewUIChatLogWindow::ExpireMessagesInVector(CNewUIChatLogWindow::type_vector_msgs* pvecMsgs, DWORD now)
{
	if (pvecMsgs == NULL)
		return 0;

	int removed = 0;
	for (type_vector_msgs::iterator it = pvecMsgs->begin(); it != pvecMsgs->end(); )
	{
		CMessageText* pMsg = *it;
		if (pMsg == NULL)
		{
			it = pvecMsgs->erase(it);
			removed++;
			continue;
		}

		const MESSAGE_TYPE t = pMsg->GetType();

		// Conversation: no time limit
		if (t == TYPE_CHAT_MESSAGE
			|| t == TYPE_WHISPER_MESSAGE
			|| t == TYPE_PARTY_MESSAGE
			|| t == TYPE_GUILD_MESSAGE
			|| t == TYPE_UNION_MESSAGE
			|| t == TYPE_GM_MESSAGE
			|| t == TYPE_GENS_MESSAGE)
		{
			++it;
		...
		// EXP / system / error: expire after SYSTEM_LINE_LIFETIME_MS
		if ((now - pMsg->GetCreateTime()) < (DWORD)SYSTEM_LINE_LIFETIME_MS)
		{
			++it;
			continue;
		}
...
void SEASON3B::CNewUIChatLogWindow::ProcessMessageTimeout()
{
	const DWORD now = GetTickCount();
	int removed = 0;

	removed += ExpireMessagesInVector(&m_vecAllMsgs, now);
	removed += ExpireMessagesInVector(&m_VecChatMsgs, now);
	removed += ExpireMessagesInVector(&m_vecWhisperMsgs, now);
	removed += ExpireMessagesInVector(&m_vecPartyMsgs, now);
	removed += ExpireMessagesInVector(&m_vecGuildMsgs, now);
	removed += ExpireMessagesInVector(&m_vecUnionMsgs, now);
	removed += ExpireMessagesInVector(&m_VecSystemMsgs, now);
	removed += ExpireMessagesInVector(&m_vecErrorMsgs, now);
	removed += ExpireMessagesInVector(&m_vecGMMsgs, now);
	removed += ExpireMessagesInVector(&m_vecGensMsgs, now);
...
void SEASON3B::CNewUIChatLogWindow::AddExperience(DWORD dwExp, bool bMasterLevel)
{
	if (dwExp == 0)
		return;
	...
	// Different kind (normal vs master): flush previous bucket first
	if (m_dwPendingExpKills > 0 && m_bPendingMasterExp != bMasterLevel)
		FlushPendingExperience(true);

	if (m_dwPendingExpKills == 0)
		m_dwPendingExpTick = now;

	m_dwPendingExp += dwExp;
	m_dwPendingExpKills++;
	m_bPendingMasterExp = bMasterLevel;

	if ((now - m_dwPendingExpTick) >= (DWORD)EXP_AGGREGATE_MS)
		FlushPendingExperience(true);
}

void SEASON3B::CNewUIChatLogWindow::FlushPendingExperience(bool force)
{
	if (m_dwPendingExpKills == 0 || m_dwPendingExp == 0)
	{
		m_dwPendingExp = 0;
		m_dwPendingExpKills = 0;
		m_dwPendingExpTick = 0;
	...
	if (!force && (GetTickCount() - m_dwPendingExpTick) < (DWORD)EXP_AGGREGATE_MS)
		return;

	char Text[128];
	if (m_bPendingMasterExp)
		sprintf_s(Text, GlobalText[1750], m_dwPendingExp);
	else
		sprintf_s(Text, GlobalText[486], m_dwPendingExp);

	// Optional kill count when aggregated (cleaner farm feedback)
	if (m_dwPendingExpKills > 1)
	{
		char Extra[48];
		sprintf_s(Extra, " (%u)", (unsigned)m_dwPendingExpKills);
		// append if space
		if (strlen(Text) + strlen(Extra) < sizeof(Text) - 1)
			strcat_s(Text, Extra);
	}
	...
	AddText("", Text, TYPE_SYSTEM_MESSAGE);
}
...
{
	// Post aggregated EXP line on interval (even if no more kills for a moment)
	FlushPendingExperience(false);

	// Old lines expire → chat keeps rising/clearing without needing new XP
