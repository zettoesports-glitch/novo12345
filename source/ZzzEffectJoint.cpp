...
{
	// Upgrade 1: no joint spawn when effects off / over soft cap (trails, ghost lines, skill FX).
	if (!EffectBudget::Enabled())
		return;
	if (EffectBudget::IsFull(Joints, MAX_JOINTS, EffectBudget::CapJoints()))
		return;
	...
	for (int i = 0; i < MAX_JOINTS; i++)
	{
		...
		if (!o->Live)
		{
			...
			else if (Type == BITMAP_SWORDEFFOR2_BLUR)
			{
				bCreateStartTail = false;
				o->NumTails = -1;
			}
			...
			case BITMAP_2LINE_GHOST:
			{
				if (o->SubType == 0)
				{
					...
					o->Direction[0] = 0;
					// Guarda yaw base (skill reta) — update so faz onda leve, nao gira livre
					o->Direction[2] = o->Angle[2];
					o->m_sTargetIndex = 2;
			...
			case BITMAP_JOINT_SPIRIT2:
				...
				case 24:
				{
					...
					if (0.9f <= fRargleScale)
					{
						...
						CreateEffect(MODEL_SUMMONER_SUMMON_LAGUL, o->Position, o->Angle, vLight, 1, (OBJECT*)o, -1, 0, 0, 0, fRargleScale);
					}
				}
				break;
				case 25:
			...
			case BITMAP_JOINT_THUNDER:
				...
				case 15:
					...
					{
						CHARACTER* tc = gmCharacters->GetCharacter(j);
						OBJECT* to = &tc->Object;
			...
			case BITMAP_LAVA:
			{
				...
				o->m_bCreateTails = true;
				Vector(1.0f, 1.0f, 1.0f, o->Light);
				o->Velocity = gmClientModels->GetModel(o->Target->Type)->Actions[o->Target->CurrentAction].PlaySpeed;
			}
			break;
			case BITMAP_SWORDEFFOR2_BLUR:
				o->Scale = Scale;
				o->RenderType = 1;
				o->RenderFace = 2;
				o->LifeTime = 9;
				o->Target->Velocity = 1.5;
				o->MaxTails = 4;
				o->Target->AnimationFrame = 0.0;
				o->Target->PriorAnimationFrame = 0.0;
				o->m_sTargetIndex = (int)o->LifeTime;
				o->m_bCreateTails = false;
				o->TargetPosition[0] = Position[0];
				o->TargetPosition[1] = Position[1];
				o->TargetPosition[2] = Position[2];
				o->Target = Target;
				break;
...
void MoveJoint(JOINT* o, int iIndex)
{
	...
	if ((o->Type != BITMAP_JOINT_FORCE || o->SubType)
		&& (o->Type != BITMAP_JOINT_THUNDER || o->SubType != 15)
		&& o->Type != BITMAP_LIGHT
		&& o->Type != BITMAP_PIERCING
		&& o->Type != BITMAP_FLARE_FORCE
		...
		&& (o->Type != BITMAP_FLASH || o->SubType != 6))
	{
		...
		if (o->Velocity != 0.0f)
		{
			...
			if ((o->Type == BITMAP_JOINT_FORCE && o->SubType >= 2 && o->SubType <= 6) || (o->Type == BITMAP_FLARE && o->SubType == 7))
			{
				VectorAdd(o->Position, Position, o->Position);
			}
			else
	...
	case BITMAP_JOINT_LASER:
		...
		if (!o->Collision && Distance <= o->Velocity * 2.f)
		{
			o->Collision = true;
			o->LifeTime = 5;
			o->life_time_work = standlimit(5);
		}
		//light
		Luminosity = o->LifeTime._runvalue * 0.1f;
	...
	case BITMAP_JOINT_ENERGY:
		...
		case 57:
			...
			case 57:
			{
				...
				if (pModel)
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, o->PKKey);
				}
			}
			break;
			case 48:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 24);
				}
			}
			break;
			case 49:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 28);
				}
			}
			break;
			case 50:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 32);
				}
			}
			break;
			case 51:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 44);
				}
			}
			break;
			case 52:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 48);
				}
			}
			break;
			case 53:
			{
				...
				{
					pModel->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);
					pModel->TransformByObjectBone(o->Position, o->Target, 52);
				}
			}
			break;
			}
			...
			case 3:
				...
				CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 0, 1.0, NULL);
				break;
			case 14:
			case 15:
				Luminosity = sinf(WorldTime * 0.002f) * 0.3f + 0.8f;
				Vector(Luminosity, Luminosity * 0.1f, Luminosity * 0.1f, Light);
				CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 4, 1.0, NULL);
				break;
			case 22:
			case 23:
		...
		case 46:
		{
			if (o->LifeTime >= 100 && o->SubType != 44)
			{
				...
				else if (o->SubType == 12)
				{
					...
					o->Position[0] += timefac((int)o->MultiUse * (rand() % 5));
					o->Position[1] += timefac((int)o->MultiUse * (rand() % 5));
					o->Position[2] += (timefac(6.f));
				}
				else if (o->SubType == 16 || o->SubType == 46)
				{
					o->Angle[2] -= timefac(o->MultiUse * 20.f);
					o->Position[0] += timefac(o->MultiUse * (rand() % 5 + 3));
					o->Position[1] += timefac(o->MultiUse * (rand() % 5 + 3));
					o->Position[2] += (timefac(8.f));
				}
				else if (o->SubType == 13)
				{
				...
				{
					o->Angle[2] -= (timefac(20.f));
					o->Position[0] += timefac((int)o->MultiUse * rand() % 5 + cosf(Q_PI / (float)(rand() % 180) * 20.0f));
					o->Position[1] += timefac((int)o->MultiUse * rand() % 5 + sinf(Q_PI / (float)(rand() % 180) * 20.0f));
					o->Position[2] += timefac(6.f);
				}
				else
				{
					o->Angle[2] -= timefac(10.f);
					o->Position[2] += timefac(5.f);
				}
			}
			else if (o->SubType == 44 && o->LifeTime <= 120 && o->LifeTime >= 10)
			{
				...
				{
					o->Velocity += timefac(12.f);
					if (o->Velocity >= 20.f)
						o->Velocity = 20.f;

					o->Position[0] += timefac(cosf(Q_PI / (float)(rand() % 180 + 150)));
					o->Position[1] += timefac(sinf(Q_PI / (float)(rand() % 180 + 150)));
			...
			else
			{
				...
				{
					o->Velocity += timefac(7.f);
					if (o->Velocity >= 40.f)
						o->Velocity = 40.f;
				}
				else
				{
					o->Velocity += timefac(5.f);
					if (o->Velocity >= 30.f)
						o->Velocity = 30.f;
				}
				...
				{
					o->Position[2] += timefac(o->Velocity);
				}
				else if (o->SubType == 44)
				{
					VectorCopy(o->Target->Position, o->TargetPosition);
			...
			else if (o->SubType == 16)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, Light, 31, 0.8f);
			}
		...
		case 42:
		{
			if (o->LifeTime >= 60)
			{
				o->Position[2] += timefac(1.f);
				o->Light[0] += timefac(0.002f);
				o->Light[1] += timefac(0.0015f);
				o->Light[2] += timefac(0.011f);
			}
			else
			{
		...
		case 43:
		{
			...
			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 31, 1.0f);
		}
		break;
		}
		break;
	case BITMAP_JOINT_HEALING:
		if (o->SubType == 4)
		{
			...
			o->Position[2] += timefac(o->Velocity);
			o->Velocity += (timefac(10.f));
			Luminosity = (12 - o->LifeTime._runvalue) * (0.1f);
		...
		else if (o->SubType == 9)
		{
			...
			o->Position[2] = o->TargetPosition[2] + 10 + (90 - o->LifeTime._runvalue);

			o->MultiUse += timefac(2.f);
			o->Direction[0] -= timefac(1.f);
			...
			{
				VectorScale(o->Light, timepow(1.25f), o->Light);
			}
		}
		else if (o->SubType == 10)
		{
			...
			o->Position[0] = o->TargetPosition[0] + sinf(o->MultiUse * 0.1f) * o->Direction[0];
			o->Position[1] = o->TargetPosition[1] + cosf(o->MultiUse * 0.1f) * o->Direction[0];
			o->Position[2] = o->TargetPosition[2] + (300 * (o->LifeTime._runvalue / 80.f));

			if (o->Collision)
				o->MultiUse += timefac(2);
			else
				o->MultiUse -= timefac(2);
			...
			if (o->LifeTime < 40)
			{
				o->Light[0] /= timepow(1.1);
				o->Light[1] /= timepow(1.1);
				o->Light[2] /= timepow(1.1);
			}
			else if (o->LifeTime > 68)
			{
				o->Light[0] *= timepow(1.2);
				o->Light[1] *= timepow(1.2);
				o->Light[2] *= timepow(1.2);
			}
		}
		else if (o->SubType == 14)
		{
			if (o->LifeTime < 5)
			{
				float fLumi = o->LifeTime._runvalue / 5.f;
		...
		else
		{
			...
			else
			{
				if (o->SubType == 8 || o->SubType == 13 || o->SubType == 17)
				{
					...
					{
						o->Light[0] /= (timepow(1.8f));
						o->Light[1] /= (timepow(1.8f));
						o->Light[2] /= (timepow(1.8f));
					}
			...
			Luminosity = ((12 - (double)o->LifeTime) * 0.1f);
			switch (o->SubType)
			{
			case 1:
				Vector(Luminosity * 0.4f, Luminosity * 0.6f, Luminosity * 1.f, o->Light);
			...
			if (o->SubType == 6)
			{
				...
				{
					vec3_t Light;
					float fAlpha = ((float)(6 - abs((double)o->LifeTime - 6)) * 0.15f);
	...
	case BITMAP_2LINE_GHOST:
	{
		if (o->SubType == 0)
		{
			// Skill reta: yaw = base (Direction[2]) + onda leve ±8 (nao acumula giro em bola)
			float fWave = sinf((float)o->life_time_work * 0.25f) * 8.f;
			o->Angle[2] = o->Direction[2] + fWave;
		}
		else if (o->SubType == 1)
		{
			float TargetAngleH;
	...
	case BITMAP_JOINT_SPIRIT:
	case BITMAP_JOINT_SPIRIT2:
		if (o->SubType == 0 || o->SubType == 5 || o->SubType == 19)
		{
			if (o->Scale == 80.f)
			{
				if (o->SubType == 5)
					CreateEffectSync(MODEL_LASER, o->Position, o->Angle, o->Light, 3);
				else
					CreateEffectSync(MODEL_LASER, o->Position, o->Angle, o->Light, 0);
				...
				if (battleCastle::IsBattleCastleStart())
				{
					...
					{
						o->Velocity = 0.f;
						o->LifeTime /= timepow(5.f);
						o->life_time_work /= 5;
						break;
					}
				}

				if (o->life_time_work % standlimit(15) == 0 && o->Target == &Hero->Object)
				{
			...
			{
				CreateEffectSync(MODEL_SKULL, o->Position, o->Angle, o->Light);
			}
			VectorCopy(o->Target->Position, o->TargetPosition);
			...
			if (o->SubType != 5)
			{
				float randuin = (float)(rand() % 32 - 16);

				o->Direction[0] += randuin * (0.2f) * timefac(1.0);
				o->Direction[2] += randuin * (0.8f) * timefac(1.0);
				o->Angle[0] += timefac(o->Direction[0]);
				o->Angle[2] += timefac(o->Direction[2]);
				o->Direction[0] *= timepow(0.6f);
			...
			if (o->SubType != 19)
			{
				Luminosity = (double)o->LifeTime * 0.1f;
				Vector(Luminosity, Luminosity, Luminosity, o->Light);
				Luminosity = -(float)(rand() % 4 + 4) * 0.01f;
				Vector(Luminosity, Luminosity, Luminosity, Light);
				AddTerrainLight(o->Position[0], o->Position[1], Light, 4, PrimaryTerrainLight);
		...
		else if (o->SubType == 3 || o->SubType == 13)
		{
			...
			if (o->SubType != 13 || o->LifeTime > 28)
			{
				...
				interpolatesum(o->Position, vRot);
			...
			else if (o->Target != NULL)
			{
				...
				if (o->SubType == 13)
				{
					...
					Position[0] += (timefac(rand() % 150 - 70.f));
					Position[1] += (timefac(rand() % 150 - 70.f));
					Position[2] += (timefac(50.f));
				...
				{
					VectorCopy(o->Light, Light);
					Scale = 2.f * (o->LifeTime._runvalue / 10) + 2.f;
				}
			}
		...
		else if (o->SubType == 2 || o->SubType == 6 || o->SubType == 7 || o->SubType == 21 || o->SubType == 22 || o->SubType == 23)
		{
			vec3_t Angle;
			...
			{
				CreateSprite(BITMAP_FLAME, o->Position, (o->Scale + (20 - o->LifeTime._runvalue) / 5), Light, o->Target, (float)(rand() % 360), 0);
			}
			else
			{
				CreateSprite(BITMAP_LIGHT, o->Position, (4.0f + (20 - o->LifeTime._runvalue) / 5), Light, o->Target, (float)(rand() % 360), 0);
			}
			o->Velocity += timefac(5.f);
		}
		else if (8 == o->SubType)
		...
		{
			o->Velocity = 50.f;
			o->Angle[2] -= timefac(50.0f);
		}
		else if (o->SubType == 9)
		{
			o->Position[0] += timefac(rand() % 10 - 5.f);
			o->Position[1] += timefac(rand() % 10 - 5.f);
			o->Position[2] += timefac(o->Velocity);
			o->Velocity += timefac(1.f);

			if (o->LifeTime < 10)
			{
				o->Light[0] /= timepow(1.15f);
				o->Light[1] /= timepow(1.15f);
				o->Light[2] /= timepow(1.15f);
			}
		}
		else if (o->SubType == 11)
		{
			...
			CreateParticleSync(BITMAP_FIRE + 1, Position, o->Angle, Light, 5, 0.9f);
			if (rand() % 200 == 0)
			{
				CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, o->Angle, 12, NULL, 0.9f);
			}
			o->Position[0] += timefac(cosf(o->Angle[2]) * 20.0f);
			o->Position[1] += timefac(sinf(o->Angle[2]) * 20.0f);
		}
		else if (o->SubType == 12)
		{
			Vector(1, 1, 1, Light);
			...
			CreateParticleSync(BITMAP_SMOKE + 3, Position, o->Angle, Light, 0, (float)(rand() % 32 + 48) * 0.01f);
			Vector(1, 1, 1, Light);
			CreateParticleSync(BITMAP_FIRE + 1, Position, o->Angle, Light, 6, 0.6f);
		}
		else if (o->SubType == 14)
		{
			Vector(1, 1, 1, Light);
			...
			int r = rand() % 5;
			if (r == 0)
				CreateParticleSync(BITMAP_WATERFALL_3, o->Position, o->Angle, o->Light, 2);
			else if (r <= 2)
				CreateParticleSync(BITMAP_WATERFALL_4, o->Position, o->Angle, o->Light, 2);
			else
				CreateParticleSync(BITMAP_WATERFALL_5, o->Position, o->Angle, o->Light, 4);
		}
		else if (o->SubType == 15)
		{
			Vector(1, 1, 1, Light);
			...
			if (o->LifeTime < 35)
			{
				CreateParticleSync(BITMAP_FIRE, Position, o->Angle, Light, 7, 1.0f);

				if (rand() % 2 == 0)
				{
					Position[0] -= (timefac(sinf(o->Angle[2]) * (40.0f - o->PKKey * 0.1f)));
					Position[1] += (timefac(cosf(o->Angle[2]) * (40.0f - o->PKKey * 0.1f)));
					Position[2] = 350;
					vec3_t Angle;
					...
					for (int i = 0; i < 2; ++i)
					{
						...
						CreateJointSync(BITMAP_JOINT_SPIRIT, Position, Position, Angle, 14, NULL, 0.9f);
					}
				}
				if (o->Target != NULL)
				{
					...
					if (o->Target->LifeTime._runvalue > 5)
						o->Target->Alpha = 1.0f;

					VectorCopy(o->Position, Position);
					Position[0] += timefac(sinf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
					Position[1] -= timefac(cosf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
					//Position[2] = 350;
			...
			o->Position[0] += timefac(sinf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
			o->Position[1] -= timefac(cosf(o->Angle[2]) * (30.0f + o->PKKey * 0.1f));
		}
		else if (o->SubType == 16)
		{
			Vector(1, 1, 1, Light);
			...
			o->Position[0] += timefac(cosf(o->Angle[2]) * 30.0f);
			o->Position[1] += timefac(sinf(o->Angle[2]) * 30.0f);
		}
		else if (o->SubType == 17)
		{
			Vector(1, 1, 1, Light);
			...
			if (o->life_time_work == standlimit(100))
			{
				for (int i = 0; i < 120; ++i)
				{
					Vector(0.f, 0.f, i * 3.f, Angle);
		...
		{
			o->Velocity += timefac(3.0f);
			o->Angle[0] = -90.0f;

			size_t add = (size_t)(o->life_time_work % standlimit(12));
			...
			o->Angle[0] -= timefac(o->PKKey);
			o->Angle[2] += timefac(10);
			...
			VectorCopy(o->Position, Position);
			Vector(1, 1, 1, Light);
			CreateParticleSync(BITMAP_SMOKE, Position, o->Angle, Light, 19, 7.f);
		}
		else if (o->SubType == 20)
		{
			if (o->LifeTime < 42)
		...
		else if (o->SubType == 24)
		{
			...
			o->Direction[0] += timefac((float)(rand() % 32 - 16) * (0.2f));
			o->Direction[2] += timefac((float)(rand() % 32 - 16) * (0.8f));
			o->Angle[0] += timefac(o->Direction[0]);
			o->Angle[2] += timefac(o->Direction[2]);
			o->Direction[0] *= timepow(0.6f);
			o->Direction[2] *= timepow(0.8f);
			...
			if (o->LifeTime < 30)
				Luminosity = o->LifeTime._runvalue / 30.f;
			else if (o->LifeTime > 144)
				Luminosity = (160 - o->LifeTime._runvalue) / 15.f;
			else
				Luminosity = 1.0f;
		...
		else if (o->SubType == 25)
		{
			if (o->LifeTime < 10)
			{
				o->Light[0] /= (timepow(1.45f));
				o->Light[1] /= (timepow(1.45f));
				o->Light[2] /= (timepow(1.45f));
			...
			{
				o->Position[0] += timefac(rand() % 16 - 8.f);
				o->Position[1] += timefac(rand() % 16 - 8.f);
				o->Position[2] += timefac(o->Velocity);
				o->Scale += (timefac(3.0f));
			}
	...
	case MODEL_SPEARSKILL:
		if (o->m_iChaIndex != -1)
		{
			CHARACTER* pCharacter = gmCharacters->GetCharacter(o->m_iChaIndex);

			if (pCharacter)
			{
				if (o->SubType == 4)
				{
					if (pCharacter->Helper.Type == MODEL_HELPER + 37 && !pCharacter->SafeZone)
					{
						o->SubType = 9;
					}
				}
				...
				{
					if ((pCharacter->Helper.Type == MODEL_HELPER + 2
						|| pCharacter->Helper.Type == MODEL_HELPER + 3
						|| pCharacter->Helper.Type == MODEL_HELPER + 4
						|| pCharacter->Helper.Type == MODEL_HELPER + 37) && pCharacter->SafeZone == false)
					{
						o->SubType = 11;
					}
				}
				else if (o->SubType == 11)
				{
					if ((pCharacter->Helper.Type != MODEL_HELPER + 2
						&& pCharacter->Helper.Type != MODEL_HELPER + 3
						&& pCharacter->Helper.Type != MODEL_HELPER + 4
						&& pCharacter->Helper.Type != MODEL_HELPER + 37) || pCharacter->SafeZone == true)
					{
						o->SubType = 10;
					}
				}
		...
		else if (o->SubType == 3)
		{
			...
			o->Direction[0] = (sinf(((double)o->LifeTime - o->Direction[1]) * 0.05f) * 3.f);
			o->Position[0] += timefac(o->Direction[0]);

			if (o->Target != NULL)
			{
				float fAlpha;
				if (o->Target->LifeTime._runvalue > 50)
				{
					fAlpha = (100 - o->Target->LifeTime._runvalue) / 40.f;
				}
				else
				{
					fAlpha = o->Target->LifeTime._runvalue / 10.f;
				}
		...
		else if (o->SubType == 5 || o->SubType == 6 || o->SubType == 7)
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				if (o->Weapon == 0)
				{
					o->Direction[1] *= timepow(0.95f);
					if (o->Direction[1] < 10.f)
						o->Direction[1] = -10.f;
				}
				if (o->Weapon > 40)
				{
					o->Direction[1] -= timefac(20.f);
					o->Angle[2] -= timefac(5.f);
					o->Scale += timefac(15.f);
				}
				if (o->Direction[1] < 0)
				{
		...
		else
		{
			...
			if (o->SubType == 10 || o->SubType == 11)
			{
				...
				if (rand() % 10 == 0)
				{
					...
					CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, Light, 19);
				}
			}
			...
			if (o->SubType == 0 || o->SubType == 4 || o->SubType == 9
			...
			case 0:
			case 4:
				o->Position[0] = o->TargetPosition[0] + vDir[0] * 80.0f;
				o->Position[1] = o->TargetPosition[1] + vDir[1] * 80.0f;
			...
			case 9:
				...
				break;
			case 10:
				o->Position[0] = o->TargetPosition[0] + vDir[0] * 60.0f;
				o->Position[1] = o->TargetPosition[1] + vDir[1] * 60.0f;
			...
			case 11:
				...
				break;
			case 1:
				o->Position[0] = o->TargetPosition[0] + vDir[0] * 70.0f;
				o->Position[1] = o->TargetPosition[1] + vDir[1] * 70.0f;
			...
			case 16:
			{
				if (o->Target != NULL)
				{
					o->Position[0] = o->TargetPosition[0] + vDir[0] * 20.0f;
					o->Position[1] = o->TargetPosition[1] + vDir[1] * 20.0f;
	...
	case MODEL_FENRIR_SKILL_THUNDER:
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 50.f, false);
			...
			if (Distance < o->Velocity * 1.5f)
			{
				if (o->Scale == 50.f)
				{
					CreateParticleSync(BITMAP_ENERGY, o->Position, o->Angle, o->Light);
					vec3_t Position;
					if (rand() % 8 == 0)
					{
	...
	case BITMAP_JOINT_LASER + 1:
		...
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			o->Direction[0] += timefac((float)(rand() % 256 - 128) / o->Scale);
			o->Direction[2] += timefac((float)(rand() % 256 - 128) / o->Scale);
			...
			if (Distance <= o->Velocity * 2.f)
			{
				...
				if (rand() % 2 == 0)
				{
					Vector((float)(rand() % 64 - 32), (float)(rand() % 64 - 32), (float)(rand() % 64 - 32), Position);
					VectorAdd(Position, o->TargetPosition, Position);
					CreateParticleSync(BITMAP_FIRE, o->Position, o->Angle, o->Light);
				}
				break;
			}
			//light
	...
	case BITMAP_JOINT_THUNDER:
		...
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			case 4:
				...
				Distance = MoveHumming(o->Position, o->Angle, Position, -10, false);//-25.f);
				break;
			case 5:
				Position[0] = o->TargetPosition[0];
				Position[1] = o->TargetPosition[1];
				Position[2] = o->TargetPosition[2] + 600.f;
				Distance = MoveHumming(Position, o->Angle, o->Position, -10, false);
				break;
			case 6:
				VectorCopy(o->Target->Position, o->TargetPosition);
				...
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, (float)(rand() % 100 + 50), false);
				break;
			case 7:
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, (float)(rand() % 100 + 50), false);
				break;
			case 9:
				Distance = MoveHumming(o->Position, o->Angle, Position, (float)(rand() % 80 + 60.f), false);
				break;
			case 12:
			{
				BMD* b = gmClientModels->GetModel(o->Target->Type);
			...
			break;
			case 18:
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 110.f, false);
				break;
			case 19:
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 25.f + o->Scale, false);
				break;
			case 20:
				o->TargetPosition[2] += (100.f);
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 100.f, false);
				break;
			case 22:
			case 23:
			case 24:
			{
				if (o->Target)
				{
					if (j == 0)
					{
						...
						if (pTargetChar)
						{
							...
							if (pModel)
							{
								...
								if (o->SubType == 22)
								{
									pModel->TransformPosition(pSourceObj->GetBoneTransform(37), vRelativePos, vPos, true);
								}
								else if (o->SubType == 23)
								{
									pModel->TransformPosition(pSourceObj->GetBoneTransform(28), vRelativePos, vPos, true);
								}
								else if (o->SubType == 24)
								{
					...
					Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, o->Velocity, false);
				}
				else
				{
					assert(!"�����");
			...
			case 25:
			{
				...
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 40.f, false);
			}
			break;
			default:
				Distance = MoveHumming(o->Position, o->Angle, o->TargetPosition, 25.f, false);
				break;
			}
			...
			else if (Distance < o->Velocity * 1.5f && (o->SubType != 6 && o->SubType != 9 && o->SubType != 7 && o->SubType != 18 && o->SubType != 19))
			{
				if (o->Scale == 50.f)
				{
					CreateParticleSync(BITMAP_ENERGY, o->Position, o->Angle, o->Light);
		...
		else if (o->SubType == 15 && o->LifeTime.duration(2))
		{
			...
			for (int i = 0; i < o->MultiUse; ++i)
			{
				if ((i % 15) == 0)
				{
					CHARACTER* tc = gmCharacters->GetCharacter(o->TargetIndex[i / 15]);
					OBJECT* to = &tc->Object;
	...
	case BITMAP_JOINT_THUNDER + 1:
		...
		else if (o->SubType == 1 || o->SubType == 2 || o->SubType == 3 || o->SubType == 5 || o->SubType == 6 || o->SubType == 7) //  ������ �Ʒ��� �������� ����.

		{
			...
			if (o->SubType == 2)
			{
				...
				CreateSprite(BITMAP_SHINY + 1, o->TargetPosition, 1.5f, Light, NULL, (float)((rand() % 360)));
				CreateParticleSync(BITMAP_TRUE_BLUE, o->TargetPosition, o->Angle, Light);
			}
			else if (o->SubType == 3)
			{
				Vector(o->Light[0] * 0.5f, o->Light[1] * 0.6f, o->Light[2], Light);
				CreateEffectSync(MODEL_ICE, o->TargetPosition, o->Angle, Light);
			}
			else if (o->SubType == 6)
			{
				o->TargetPosition[2] = RequestTerrainHeight(o->TargetPosition[0], o->TargetPosition[1]) + 30.f;
				CreateParticleSync(BITMAP_TRUE_BLUE, o->TargetPosition, o->Angle, Light, 0, 2.0f);
			}
		}
		else if (o->SubType == 4)
		{
	...
	case BITMAP_FLARE_BLUE:
		if (o->SubType == 0 || o->SubType == 10 || o->SubType == 18)
		{
			...
			if (o->PKKey != -1)
			{
				count = (o->Direction[1] + (double)o->LifeTime) / (float)(o->PKKey);
			}
			else if (o->SubType == 18)
			{
				count = (o->Direction[1] + ((double)o->LifeTime * 2)) * 0.1f;
				if (o->Skill == 1)
				{
					count *= timepow(-1);
				}
			...
			{
				count = (o->Direction[1] + (double)o->LifeTime) * 0.1f;
			}
			...
			{
				o->Position[2] += (timefac(o->Direction[2] * 1.1f));
				o->Velocity += (timefac(0.1f));
		...
		else if (o->SubType == 14 || o->SubType == 15)
		{
			if (o->Target != NULL)
			{
				...
				o->Scale += (timefac(1));
				o->Direction[1] += (timefac(4));
				o->Direction[2] += (timefac((rand() % 200 + 200) / 100.f));
			}
		}
		else if (o->SubType == 2 || o->SubType == 24 || o->SubType == 50 || o->SubType == 51)
		{
		...
		{
			static int lMax = (int)o->LifeTime;
			int radious = 50.f;
			vec3_t vPos;
			vec3_t backPos;
			o->m_bCreateTails = false;
			...
			for (int i = 0; i < MAX_TAILS; ++i)
			{
				...
				vPos[2] = rSin;
				o->TargetPosition[0] += (o->Direction[0] * (lMax - o->LifeTime._runvalue));
				o->TargetPosition[1] += (o->Direction[1] * (lMax - o->LifeTime._runvalue));
				VectorAdd(o->TargetPosition, vPos, o->Position);
				AngleMatrix(o->Angle, Matrix);
				CreateTail(o, Matrix);
			}
			VectorCopy(backPos, o->Position);
			o->Position[0] += (timefac(o->Direction[0] * 12));
			o->Position[1] += (timefac(o->Direction[1] * 12));
			VectorCopy(o->Position, o->TargetPosition);
		...
		else if (o->SubType == 4 || o->SubType == 6 || o->SubType == 12)
		{
			...
			if (o->SubType == 6)
			{
				if (o->Type == BITMAP_FLARE_BLUE)
				{
					count = (o->Direction[0] + (o->LifeTime._runvalue * 0.8f));
				}
				else
				{
					count = (o->Direction[0] + (o->LifeTime._runvalue * 0.5f));
				}
			...
			{
				count = (o->Direction[0] + (double)o->LifeTime) * 0.1f * gsteady_clock->GetNormalizerFps();
				vPos[0] = -cosf(count) * (40.0f * .65f);
				vPos[1] = sinf(count) * (40.0f * .65f) - (float)(90 - (double)o->LifeTime) * 0.3f;
			}
			else
			{
				count = (o->Direction[0] + (double)o->LifeTime) * 0.1f;
				vPos[0] = -cosf(count) * ((float)max((double)o->LifeTime + 40, 10) * .65f);
				vPos[1] = sinf(count) * ((float)max((double)o->LifeTime + 40, 10) * .65f);
			}
			vPos[2] = 0.0f;
			o->TargetPosition[0] += timefac(o->Direction[1]);
			o->TargetPosition[1] += timefac(o->Direction[2]);
		...
		else if (o->SubType == 7 || o->SubType == 11 || o->SubType == 25 || o->SubType == 45 || o->SubType == 46 || o->SubType == 47)
		{
			...
			vec3_t vDir, vDirTemp;
			float fSpeed[3] = { 0.048, 0.0613, 0.1113 };
			...
			vDirTemp[0] = cosf((float)iFrame * fSpeed[1]) * sinf((float)(iFrame + 55555) * fSpeed[0]);
			vDirTemp[1] = sinf((float)iFrame * fSpeed[1]) * sinf((float)(iFrame + 55555) * fSpeed[0]);
			vDirTemp[2] = cosf((float)(iFrame + 55555) * fSpeed[0]);
			...
			{
				CreateEffectSync(MODEL_CHANGE_UP_EFF, o->Target->Position, o->Target->Angle, o->Target->Light, 1, o->Target);
				CreateEffectSync(MODEL_CHANGE_UP_CYLINDER, o->Target->Position, o->Target->Angle, o->Target->Light, 1, o->Target);
			}
			...
			if (o->SubType != 11 && o->LifeTime == 1 && o->SubType != 45 && o->SubType != 46 && o->SubType != 47)
			{
		...
		else if (o->SubType == 8)
		{
			...
			o->TargetPosition[2] += timefac(10.f);
		}
		else if (o->SubType == 9)
		{
			AngleMatrix(o->Angle, Matrix);
		...
		else if (o->SubType == 17)
		{
			if (((o->Target->CurrentAction >= PLAYER_WALK_MALE) ||
				(o->Target->CurrentAction >= PLAYER_HIT_SKILL_SWORD1 && o->Target->CurrentAction <= PLAYER_HIT_SKILL_SWORD5)
				&& (o->Target->CurrentAction != PLAYER_RAGE_UNI_STOP_ONE_RIGHT)
				))
			{
				o->LifeTime -= (timefac(10));
			}
			else
			{
				o->Direction[0] += timefac(sinf(rand() % 360 * 0.1f) * 4.f);
				o->Direction[1] += timefac(cosf(rand() % 360 * 0.1f) * 4.f);
				o->Direction[2] += timefac(o->Velocity);
		...
		else if (o->SubType == 23)
		{
			if (o->LifeTime < 20)
			{
				...
				else
				{
					...
					{
						o->Direction[1] += timefac(1.5f);
						switch ((int)o->MultiUse)
						{
						case 0:
							o->HeadAngle[0] += timefac(3.f);
							o->HeadAngle[2] -= timefac(o->Direction[1]);
							break;
						case 1:
							o->HeadAngle[0] -= timefac(3.f);
							o->HeadAngle[2] -= timefac(o->Direction[1]);
						...
						case 4:
							...
							break;
						case 5:
							o->HeadAngle[0] -= timefac(o->Direction[1]);
							break;
						}
		...
		else if (o->SubType == 43)
		{
			if (o->Target->Live)
			{
				...
				if (rand() % 3 == 0)
				{
					...
					o->Position[0] += timefac((rand() % 60 - 30));
					o->Position[1] += timefac((rand() % 60 - 30));
					CreateJointSync(BITMAP_FLARE, o->Position, o->Position, o->Angle, 44, o->Target, o->Scale, 0, 0, 0, 0, o->Light);
				}
			}
			else
			{
		...
		if ((o->SubType == 0 || o->SubType == 10 || o->SubType == 18) && o->Scale < 40.f)
		{
				...
				))
			{
				o->SubType = 1;
				o->m_bCreateTails = false;
				o->LifeTime = (double)min(10, (double)o->LifeTime);
				o->life_time_work = standlimit((int)o->LifeTime);
			}
		}
		break;
	case BITMAP_FLARE + 1:
		...
		{
			o->Position[0] = o->StartPosition[0] + sinf(o->LifeTime._runvalue / 2.5f) * o->Direction[0];
			o->Position[1] = o->StartPosition[1];
			o->Position[2] += timefac(o->Velocity);
			o->Velocity -= timefac(0.1f);
			o->Direction[0] -= timefac(0.1f);
		...
		else if (o->SubType == 8 || o->SubType == 9)
		{
			...
			{
				o->Position[2] += timefac(o->MultiUse);
			}
		}
		else if (o->SubType == 12 || o->SubType == 13 || o->SubType == 14 || o->SubType == 17 || o->SubType == 15 || o->SubType == 18)
		{
			if (o->Target != NULL)
			{
				...
				if (o->SubType == 14 && o->Live == true)
				{
					...
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 34, 1.0f);
					CreateParticleSync(BITMAP_SMOKE, o->Position, o->Angle, o->Light, 35, 1.0f);
					//CreateParticle(BITMAP_SPARK+1, o->Position, o->Angle, o->Light, 12, 1.0f);
					Vector(0.7f, 0.3f, 0.3f, o->Light);
					CreateParticleSync(BITMAP_SPARK + 1, o->Position, o->Angle, o->Light, 10, 4.0f);
					Vector(1.0f, 1.0f, 0.2f, o->Light);
				}
				else if (o->SubType == 15 && o->Live == true)
				{
					CreateParticleSync(BITMAP_SHINY, o->Position, o->Angle, o->Light, 3, 0.5f);
					CreateParticleSync(BITMAP_SHINY, o->Position, o->Angle, o->Light, 3, 0.5f);
				}
		...
		else
		{
			...
			if (o->Target->Live)
			{
				...
				if (o->LifeTime < 15)
				{
					o->Light[0] /= timepow(1.5f);//o->LifeTime/60.f;
					o->Light[1] /= timepow(1.5f);
					o->Light[2] /= timepow(1.5f);
				}
			}
			else
			{
	...
	case BITMAP_JOINT_FORCE:
			...
			for (int j = 0; j < 8; ++j)
			{
				if (o->NumTails < o->MaxTails - 1)
				{
					...
					if (o->SubType == 10)
					{
						...
						CreateJointSync(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 10 + 5.f, 5, 10);
						CreateJointSync(BITMAP_JOINT_THUNDER, Light, o->Position, o->Angle, 3, NULL, rand() % 8 + 4.f, 5, 10);
					}
					else if (o->SubType == 8)
					{
						CreateJointSync(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, rand() % 10 + 5.f, 5, 10);
						CreateJointSync(BITMAP_JOINT_THUNDER, o->Position, o->Position, o->Angle, 3, NULL, rand() % 8 + 4.f, 5, 10);
					}
				}
				if (o->SubType == 10)
				{
		...
		else if (o->SubType >= 2 && o->SubType <= 6)
		{
			...
			if (o->SubType != 4 && o->NumTails >= o->MaxTails - 1)
			{
				o->m_bCreateTails = false;
			}
			...
			if ((o->life_time_work % standlimit(5)) || o->SubType != 2)
			{
				if (o->SubType == 4)
				{
					Vector(0.1f, 0.6f, 1.f, Light);

					CreateJointSync(BITMAP_FLARE_BLUE, o->Position, o->Position, o->Angle, 6, NULL, 30.0f);
		...
		if (o->SubType == 0)
		{
			if (o->LifeTime > 16)
			{
				...
				for (int i = 0; i < 10; ++i)
				{
					...
					interpolatesum(o->Position, vPos);
				}
			}
		}
		else if (o->SubType == 1)
	...
	case BITMAP_FLARE_FORCE:
		...
		else
		{
			if (o->Target)
			{
				...
				vec4_t* bone01 = o->Target->GetBoneTransform((int)o->MultiUse);

				if (!bone01)
					break;

				pModel->TransformPosition(bone01, p, o->StartPosition, true);
				VectorCopy(o->StartPosition, o->Position);
				...
				{
					bone01 = o->Target->GetBoneTransform((int)o->MultiUse);

					if (!bone01)
						break;

					VectorRotate(Direction, bone01, Position);
					VectorAdd(o->StartPosition, Position, o->StartPosition);
	...
	case BITMAP_FLASH:
		if (o->SubType <= 3 || o->SubType == 5)
		{
			if (o->SubType == 2)
			{
				o->Angle[2] += timefac(rand() % 10 + 10.f);
			}
			...
			Vector(0.f, -o->Velocity, 0.f, p);
			VectorRotate(p, Matrix, Position);
			interpolatesum(o->Position, Position);
			...
			if (o->Position[2] < Height)
			{
				...
				if (o->m_bCreateTails && o->PKKey != 1)
				{
					if (o->SubType < 2)
					{
						if (o->Target != NULL)
						{
							BMD* b = gmClientModels->GetModel(MODEL_SHADOW_BODY);
							if (b)
							{
		...
		else if (o->SubType == 6)
		{
			...
			if ((o->Weapon - (double)o->LifeTime) < 10)
			{
				CreateParticleSync(BITMAP_EXPLOTION + 1, o->StartPosition, o->Angle, o->Light, 0, 3.f);
				o->Scale = 100.f;
			}
			else
			{
	...
	case BITMAP_DRAIN_LIFE_GHOST:
	{
		...
		case 0:
		{
			...
			VectorCopy(o->Light, Light);
			float Scale = 2.f * (o->LifeTime._runvalue / 10) + 2.f;
		}
		break;
		}
	}break;
	...
	case BITMAP_FORCEPILLAR:
	{
		...
		gmClientModels->GetModel(o->Target->Type)->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame,
			o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);

		gmClientModels->GetModel(o->Target->Type)->TransformByObjectBone(o->Position, o->Target, 0);
	}
	break;
	case BITMAP_SWORDEFF:
	{
		...
		gmClientModels->GetModel(o->Target->Type)->TransformByObjectBone(o->Position, o->Target, 5);
	}
	break;
	case BITMAP_GROUND_WIND:
	{
		...
		if (pModel)
		{
			...
			pModel->Animation(BoneTransform, TargetAniFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);

			pModel->TransformByObjectBone(o->Position, o->Target, 2);

			o->Position[2] += timefac(20.0f);
	...
	case BITMAP_LAVA:
	{
		...
		if (pModel)
		{
			...
			{
				pModel->TransformByObjectBone(o->Position, o->Target, 36);
			}
	...
	case BITMAP_SWORDEFFOR2_BLUR:
	{
		...
		if (!owner)
			break;// goto LABEL_1548;
		//result = (float*)a4;

		if (!owner->Live || owner->Type != MODEL_PHOENIX_SHOT_EFFECT)
		{
			o->Live = false;
			return;
		}
		BMD* pModel = gmClientModels->GetModel(owner->Type);
		...
		if (pModel)
		{
			...
			if (owner->AnimationFrame < 18.0 && owner->AnimationFrame >= 3.0 && checkNormalizer)
			{
				...
				int v213 = (o->SubType != 1);

				Action_t* mtAction = gmClientModels->GetModelAction(owner->Type, owner->CurrentAction);

				if (mtAction)
				{
					float PlaySpeed = mtAction->PlaySpeed;

					Velocity = (owner->Velocity + 0.5);

					float AnimationFrame1 = (owner->AnimationFrame - PlaySpeed);
					float AnimationFrame2 = (owner->AnimationFrame - Velocity);

					vec3_t StartRelative, EndRelative;

					Vector(0.0, 0.0, 0.0, EndRelative);
					Vector(0.0, 0.0, 0.0, StartRelative);

					for (int i4 = 0; i4 < 10; ++i4)
					{
						pModel = gmClientModels->GetModel(owner->Type);
						if (!pModel)
							break;
						pModel->Animation(BoneTransform, AnimationFrame2, ((int)(AnimationFrame2)-1), owner->PriorAction, owner->Angle, owner->HeadAngle, true, true);

						pModel->TransformByObjectBone(o->Position, owner, iBoneNumber1[v213], 0);

						StartRelative[0] = o->Position[0];
						StartRelative[1] = o->Position[1];
						StartRelative[2] = o->Position[2];
						pModel->TransformByObjectBone(o->Position, owner, iBoneNumber2[v213], 0);


						EndRelative[0] = o->Position[0];
						EndRelative[1] = o->Position[1];
						EndRelative[2] = o->Position[2];
						CreateObjectBlur(owner, StartRelative, EndRelative, o->Light, o->TexType, 0, v213 + 1, -1);

						AnimationFrame1 += (PlaySpeed / 10.0);
						AnimationFrame2 += (Velocity / 10.0);
					}
				}
			}
		}
	...
	else if (((o->Type == BITMAP_FLARE || o->Type == BITMAP_FLARE_BLUE) && (o->SubType == 4 || o->SubType == 12)))
	{
		if (o->life_time_work % standlimit(12))
		{
			MoveJoint(o, iIndex);
		}
	}
...
void MoveJoints()
{
	// Effects disabled: clear joints (MoveJoint is very heavy per live slot).
	if (!EffectBudget::Enabled())
	{
		EffectBudget::ClearAll(Joints, MAX_JOINTS);
		return;
	}
	...
	for (int i = 0; i < MAX_JOINTS; i++)
	{
		...
		if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
		{
			...
			for (int j = 0; j < o->NumTails; j++)
			{
				...
				if (o->Type == BITMAP_FLARE_FORCE && o->SubType >= 0 && o->SubType <= 4 || (o->SubType >= 11 && o->SubType <= 13))
				{
				...
				if (o->Type == BITMAP_JOINT_FORCE && o->SubType == 0)
				{
					...
					int vertex_index = 0;
					TEXCOORD(textCoords[vertex_index], Light1, 0.f);
					VectorCopy(o->Tails[j][0], vertices[vertex_index]);
					vertex_index++;
					TEXCOORD(textCoords[vertex_index], Light1, 1.f);
					VectorCopy(o->Tails[j][1], vertices[vertex_index]);
					vertex_index++;
					TEXCOORD(textCoords[vertex_index], Light2, 1.f);
					VectorCopy(o->Tails[j + 1][1], vertices[vertex_index]);
					vertex_index++;
					TEXCOORD(textCoords[vertex_index], Light2, 0.f);
				...
				else
				{
					...
					else if (o->Type == BITMAP_FLARE_FORCE && (o->SubType >= 0 && o->SubType <= 4) || (o->SubType >= 11 && o->SubType <= 13))
					{
...
{
	double iFrame = gsteady_clock->GetDeltAccumulated() + iParam;
