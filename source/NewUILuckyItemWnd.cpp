...
void CNewUILuckyItemWnd::GetResult( BYTE _byResult, int _nIndex, BYTE* _pbyItemPacket )
{
	...
	if( nMessage > nDefault )
		g_pChatListBox->AddText("", GlobalText[nMessage], SEASON3B::TYPE_ERROR_MESSAGE);	
	if( nPlaySound > nDefault )
		PlayBuffer(nPlaySound);
	if( bInitInven )
		g_pLuckyItemWnd->Process_InventoryCtrl_DeleteItem(-1);
	if( nAddInven > nDefault )
		Process_InventoryCtrl_InsertItem( nAddInven, _pbyItemPacket );
...
#if MAIN_UPDATE > 603
	m_BtnMix.ChangeButtonInfo(m_ptPos.x + (m_fSizeX - fWidth) * 0.5f, m_ptPos.y + 370, fWidth, fHeight);
#else
