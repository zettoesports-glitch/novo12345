	...
	for( i=0 ; i<m_icntServer ; i++ )
	{
		if (m_aServerBtn[i].IsClick())
		{
			...
			if (pServerInfo->m_iPercent < 100)
			{
				...
				{
					g_ServerListManager->SetSelectServerInfo(pServerInfo->m_bName, pServerInfo->m_iIndex, iCensorshipIndex, pServerInfo->m_byNonPvP, bTestServer);
				}
...
void CServerSelWin::RenderControls()
{
	...
	int x = m_winDescription.GetXPos();
	int y = m_winDescription.GetYPos();

	for (int i = 0; i < 2; i++)
	{
		m_aBtnDeco[i].Render();
		m_aArrowDeco[i].Render();
		g_pRenderText->RenderText((int)((x + 11) / g_fScreenRate_x), (int)((y + 12 + i * 20) / g_fScreenRate_y), m_szDescription[i]);
	}
