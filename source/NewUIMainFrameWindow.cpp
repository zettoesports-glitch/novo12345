...
void SEASON3B::CNewUIMainFrameWindow::LoadImages()
{
	...
	{
		// Look-2 (2) ou Look-2.5 (6) — mesma lógica, pastas diferentes
		char path[160];
		const char* look = gmProtect->GetLook2Folder();

		sprintf(path, "Interface\\HUD\\%s\\newui_exbar.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_EXBAR, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Exbar_Master.jpg", look);
		LoadBitmap(path, IMAGE_MASTER_GAUGE_BAR, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu01_new.jpg", look);
		LoadBitmap(path, IMAGE_MENU_1, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu02.jpg", look);
		LoadBitmap(path, IMAGE_MENU_2, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\helper_buttons.tga", look);
		LoadBitmap(path, IMAGE_MENU_2_1, GL_LINEAR);

		// IMAGE_MENU_3 (Menu03)
		// Look-2.5: só pasta Look-2.5\Menu03_new (não mexe no Look-2 / Look-1)
		if (gmProtect->LookAndFeel == 6)
		{
			sprintf(path, "Interface\\HUD\\%s\\Menu03_new.jpg", look);
			LoadBitmap(path, IMAGE_MENU_3, GL_LINEAR);
		}
		else if (gmProtect->MuHelperOficial)
			LoadBitmap("Interface\\HUD\\Look-1\\Menu03_new_helper.jpg", IMAGE_MENU_3, GL_LINEAR);
		else
			LoadBitmap("Interface\\HUD\\Look-1\\Menu03_new.jpg", IMAGE_MENU_3, GL_LINEAR);

		sprintf(path, "Interface\\HUD\\%s\\Menu_blue.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_BLUE, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu_green.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_GREEN, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu_red.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_RED, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu03_new_ag.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_AG, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\Menu_character.jpg", look);
		LoadBitmap(path, IMAGE_MENU_BTN_CHAINFO, GL_LINEAR, GL_CLAMP_TO_EDGE);
		sprintf(path, "Interface\\HUD\\%s\\Menu_inventory.jpg", look);
		LoadBitmap(path, IMAGE_MENU_BTN_MYINVEN, GL_LINEAR, GL_CLAMP_TO_EDGE);
		sprintf(path, "Interface\\HUD\\%s\\Guild.jpg", look);
		LoadBitmap(path, IMAGE_MENU_BTN_GUILD, GL_LINEAR, GL_CLAMP_TO_EDGE);
		sprintf(path, "Interface\\HUD\\%s\\Menu_party.jpg", look);
		LoadBitmap(path, IMAGE_MENU_BTN_PARTY, GL_LINEAR, GL_CLAMP_TO_EDGE);

		sprintf(path, "Interface\\HUD\\%s\\menu01_new2.jpg", look);
		LoadBitmap(path, IMAGE_MENU_BTN_WINDOW, GL_LINEAR);
		sprintf(path, "Interface\\HUD\\%s\\menu01_new2_SD.jpg", look);
		LoadBitmap(path, IMAGE_GAUGE_SD, GL_LINEAR);

		if (WindowWidth > 800 && gmProtect->ScreenType != 0)
		{
			sprintf(path, "Interface\\HUD\\%s\\newui_menu04.tga", look);
			LoadBitmap(path, IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		}
		else
		{
			sprintf(path, "Interface\\HUD\\%s\\Menu04.tga", look);
			LoadBitmap(path, IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		}
	}
...
void SEASON3B::CNewUIMainFrameWindow::UnloadImages()
{
	...
	if ((gmProtect->LookAndFeel >= 1 && gmProtect->LookAndFeel <= 4) || gmProtect->LookAndFeel == 6)
	{
...
void SEASON3B::CNewUIMainFrameWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
	...
	if (gmProtect->checkold_school())
	{
		...
		// Recarrega asas no resize — Look-2.5 usa a própria pasta (não Look-1)
		if (gmProtect->LookAndFeel == 6)
		{
			if (WindowWidth > 800 && gmProtect->ScreenType != 0)
				LoadBitmap("Interface\\HUD\\Look-2.5\\newui_menu04.tga", IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
			else
				LoadBitmap("Interface\\HUD\\Look-2.5\\Menu04.tga", IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		}
		else if (WindowWidth > 800 && gmProtect->ScreenType != 0)
			LoadBitmap("Interface\\HUD\\Look-1\\newui_menu04.tga", IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
		else
			LoadBitmap("Interface\\HUD\\Look-1\\Menu04.tga", IMAGE_MENU_4, GL_LINEAR, GL_CLAMP_TO_EDGE, true, false);
	}
...
void SEASON3B::CNewUIMainFrameWindow::RenderFrame()
{
	if (gmProtect->checkold_school())
	{
		...
		RenderFrameX += 128.0;
		// Menu03: imagem 256x48 → GPU 256x64 → UV altura 48/64 = 0.75 = 100% da imagem
		// (UV 1.0 puxa padding preto embaixo — não usar)
		// Look-2.5 carrega Look-2.5\Menu03_new; Look-2 não é alterado no load
		if (gmProtect->LookAndFeel == 6)
			RenderBitmap(IMAGE_MENU_3, RenderFrameX, RenderFrameY, 256.0, 48.0, 0, 0, 1.0, 0.75f);
		else
			RenderBitmap(IMAGE_MENU_3, RenderFrameX, RenderFrameY, 256.0, 48.0, 0, 0, 1.0, 0.75);

		// newui_menu04 (IMAGE_MENU_4) — Look-2.5: pasta Look-2.5, sempre as duas asas (esq+dir)
		const float menu4Open = (gmProtect->LookAndFeel == 6) ? 52.f : 0.f;
		const float menu4Down = (gmProtect->LookAndFeel == 6) ? 2.f : 0.f; // Look-2.5: desce 2 (era 3, subiu 1)

		if (GMProtect->CheckWideScreen())
		{
			RenderFrameX = PosX;
			// Look-2.5: topo = Menu02 (+ menu4Down); outros: original
			const float menu4Y = (gmProtect->LookAndFeel == 6)
				? (RenderFrameY + menu4Down)
				: (RenderFrameY - 16.f);
			const float menu4YShop = (gmProtect->LookAndFeel == 6)
				? (RenderFrameY + menu4Down)
				: (RenderFrameY - 3.f);

			if (gmProtect->LookAndFeel == 6)
			{
				// Look-2.5: sempre mesma asa (não troca UV/altura ao abrir C / janelas)
				SEASON3B::RenderImageF(IMAGE_MENU_4, RenderFrameX - 52.0 - menu4Open, menu4Y, 128.f, 64.f, 0.0, 0.0, 128.f, 64.f);
				RenderFrameX += 564.0 + menu4Open;
				SEASON3B::RenderImageF(IMAGE_MENU_4, RenderFrameX, menu4Y, 128.f, 64.f, 128.f, 0.0, -128.f, 64.f);
			}
			else if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INGAMESHOP) || g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MASTER_LEVEL))
			{
				SEASON3B::RenderImageF(IMAGE_MENU_4, RenderFrameX - 52.0 - menu4Open, menu4YShop, 128.f, 51.f, 0.0, 13.f, 128.f, 51.f);
				RenderFrameX += 564.0 + menu4Open;
				SEASON3B::RenderImageF(IMAGE_MENU_4, RenderFrameX, menu4YShop, 128.f, 51.f, 128.f, 13.f, -128.f, 51.f);
			}
			else
			{
				SEASON3B::RenderImageF(IMAGE_MENU_4, RenderFrameX - 52.0 - menu4Open, menu4Y, 128.f, 64.f, 0.0, 0.0, 128.f, 64.f);
		...
		else
		{
			...
			{
				// clássico 108x45 — Look-2.5: topo Menu02 + down; sempre desenha as duas asas
				RenderFrameX = PosX - menu4Open;
				RenderFrameY = (gmProtect->LookAndFeel == 6) ? (PosY - 48.f + menu4Down) : (PosY - 93.f);
				RenderBitmap(IMAGE_MENU_4, RenderFrameX, RenderFrameY, 108.0, 45.0, 0, 0, 0.84375, 0.703125);

				if (gmProtect->LookAndFeel == 6 || GetScreenWidth() == (int)GetWindowsX)
				{
					RenderFrameX = PosX + 532.0 + menu4Open;
...
void SEASON3B::CNewUIMainFrameWindow::RenderFrame()
{
	...
	{
		// Look-2.5: corrige ícone inventário invertido no estado ativo
		if (gmProtect->LookAndFeel == 6)
			RenderBitmap(IMAGE_MENU_BTN_MYINVEN, RenderFrameX + 378.0, RenderFrameY + 450.5, 23.0, 22.0, 0.0f, 0.75f, 0.75f, -0.75f, true, true, 0.0);
		else
...
void SEASON3B::CNewUIMainFrameWindow::RenderPosition()
{
	...
	// currentFPS: contador QPC (alta precisao) — inteiro no canto (ex: 86)
	unicode::_sprintf(szDebugText, "FPS:%.0f | %s", g_FpsCounter.currentFPS, g_FpsCounter.m_vendor == GPUVendor::NVIDIA ? "NV" : g_FpsCounter.m_vendor == GPUVendor::AMD ? "AMD" : g_FpsCounter.m_vendor == GPUVendor::INTEL ? "Intel" : "GL");
...
void SEASON3B::CNewUIMainFrameWindow::RenderButtons()
{
	if (gmProtect->checkold_school())
	{
		...
		{
			// Look-2.5: textura 24x24 — UV normal deixava o ícone de ponta-cabeça no estado ativo
			if (gmProtect->LookAndFeel == 6)
				RenderBitmap(IMAGE_MENU_BTN_MYINVEN, RenderFrameX + 410.0, RenderFrameY + 452.0, 24.0, 24.0, 0.0f, 0.75f, 0.75f, -0.75f, true, true, 0.0);
			else
...
bool SEASON3B::CNewUIMainFrameWindow::BtnProcess()
{
	...
	else if (g_pNewUIHotKey->CanUpdateKeyEvent() == true)
	{
		...
		else if (m_BtnCShop.UpdateMouseEvent() == true && !gmProtect->checkold_school())
		{
...
bool SEASON3B::CNewUIItemHotKey::UpdateKeyEvent()
{
	...
	else if (SEASON3B::IsPress('R') == true && !gmProtect->checkold_school())
	{
...
int SEASON3B::CNewUIItemHotKey::GetHotKeyItemIndex(int iType, bool bItemCount)
{
	...
	case HOTKEY_E:
		...
		break;
	case HOTKEY_R:
		if (GetHotKeyCommonItem(iType, iStartItemType, iEndItemType) == false && !gmProtect->checkold_school())
		{
			if (m_iHotKey[iType].Type >= ITEM_POTION + 0 && m_iHotKey[iType].Type <= ITEM_POTION + 3)
			{
				iStartItemType = ITEM_POTION + 3; iEndItemType = ITEM_POTION + 0;
...
void SEASON3B::CNewUISkillList::LoadImages()
{
	...
	else if (gmProtect->IsLook2Style())
	{
		char path[160];
		const char* look = gmProtect->GetLook2Folder();
		sprintf(path, "Interface\\HUD\\%s\\Skill.jpg", look);
		LoadBitmap(path, IMAGE_SKILL1, GL_LINEAR); //-- 31310
		sprintf(path, "Interface\\HUD\\%s\\Skill2.jpg", look);
		LoadBitmap(path, IMAGE_SKILL2, GL_LINEAR); //-- 31311
		sprintf(path, "Interface\\HUD\\%s\\Skill.jpg", look);
		LoadBitmap(path, IMAGE_NON_SKILL1, GL_LINEAR); //-- 31316
		sprintf(path, "Interface\\HUD\\%s\\Skill2.jpg", look);
		LoadBitmap(path, IMAGE_NON_SKILL2, GL_LINEAR); //-- 31317
		sprintf(path, "Interface\\HUD\\%s\\skillbox.jpg", look);
		LoadBitmap(path, IMAGE_SKILLBOX, GL_LINEAR); //-- 31314
		sprintf(path, "Interface\\HUD\\%s\\skillbox2.jpg", look);
		LoadBitmap(path, IMAGE_SKILLBOX_USE, GL_LINEAR); //-- 31315
		sprintf(path, "Interface\\HUD\\%s\\command.jpg", look);
		LoadBitmap(path, IMAGE_COMMAND, GL_LINEAR); //-- 31312
		sprintf(path, "Interface\\HUD\\%s\\command.jpg", look);
		LoadBitmap(path, IMAGE_NON_COMMAND, GL_LINEAR); //-- 31318
	}
	else
	{
		LoadBitmap("Interface\\newui_skill.jpg", IMAGE_SKILL1, GL_LINEAR); //-- 31310
...
#if MAIN_UPDATE > 303
	if (!gmProtect->checkold_school() && Hero->m_pPet != NULL)
	{
		width = 32; height = 38;
