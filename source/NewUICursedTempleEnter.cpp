...
namespace
{
	...
	int EnterMinLevel[EnterLevelCount] = { 220, 271, 321, 351, 381 };
	int EnterMaxLevel[EnterLevelCount] = { 270, 320, 350, 380, 400 };
...
void SEASON3B::CNewUICursedTempleEnter::CreateEntrace()
{
	EnterMinLevel[0] = 220;
	EnterMaxLevel[0] = CharacterMachine->ConvertLevel(271) - 1;

	EnterMinLevel[1] = CharacterMachine->ConvertLevel(271);
	EnterMaxLevel[1] = CharacterMachine->ConvertLevel(321) - 1;

	EnterMinLevel[2] = CharacterMachine->ConvertLevel(321);
	EnterMaxLevel[2] = CharacterMachine->ConvertLevel(351) - 1;

	EnterMinLevel[3] = CharacterMachine->ConvertLevel(351);
	EnterMaxLevel[3] = CharacterMachine->ConvertLevel(381) - 1;

	EnterMinLevel[4] = CharacterMachine->ConvertLevel(381);
	EnterMaxLevel[4] = CharacterAttribute->MaxCharacterLevel;
}

bool SEASON3B::CNewUICursedTempleEnter::CheckEnterLevel(int& enterlevel)
{
	if (gCharacterManager.IsMasterLevel(Hero->Class) && gmProtect->shutdown_master_skill)
	{
		enterlevel = 6;
		return true;
	}
...
bool SEASON3B::CNewUICursedTempleEnter::CheckInventory(BYTE& itempos, int enterlevel)
{
	...
	pos = g_pMyInventory->FindItemIndex(ITEM_HELPER + 51, enterlevel);
	if (pos != -1)
	{
		itempos = pos;
		return true;
	...
	pos = g_pMyInventory->FindItemIndex(ITEM_HELPER + 61, -1);
	if (pos != -1)
	{
		itempos = pos;
		return true;
