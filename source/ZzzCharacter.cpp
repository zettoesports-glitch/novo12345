...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	// Luz por normal nas armas linkadas (cetro/espada). LightEnable=false zerava volume 3D.
			...
			for (int i = MAX_BODYPART - 1; i >= 0; i--)
			{
				...
				if (p->Type != -1)
				{
					...
					else
					{
						...
						{
							// Login/char select: SEMPRE desenha o set (option Equipment off so no jogo, nao na cena).
							const bool hideEquip = (g_pOption->GetRenderEquipment() == false)
								&& !(SceneFlag == LOG_IN_SCENE || SceneFlag == CHARACTER_SCENE);
							if (i > BODYPART_HEAD && o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && hideEquip)
							{
