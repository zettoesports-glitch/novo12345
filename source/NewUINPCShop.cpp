...
void SEASON3B::CNewUINPCShop::SetPos(int x, int y)
{
	...
	{
		m_BtnRepair.SetPos(m_Pos.x + 85.0, m_Pos.y + 365.0);
		m_BtnRepairAll.SetPos(m_Pos.x + 115.0, m_Pos.y + 365.0);
	}
	else
	{
		m_BtnRepair.SetPos(m_Pos.x + 54, m_Pos.y + 390);
		m_BtnRepairAll.SetPos(m_Pos.x + 98, m_Pos.y + 390);
	}
}
...
#if MAIN_UPDATE > 603
	g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 84) / 2), m_Pos.y + 12, GlobalText[230], 84, 0, RT3_SORT_CENTER);

	unicode::t_char strText[256];
	unicode::_sprintf(strText, GlobalText[1623], m_iTaxRate);
	g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 36, strText, WIN_WINDOW_SIZEX, 0, RT3_SORT_CENTER);
#else
...
void SEASON3B::CNewUINPCShop::RenderRepairMoney()
{
	if (m_bRepairShop)
	{
		...
		if (gmProtect->checkold_school())
		{
			RenderImage(BITMAP_INVENTORY + 11, m_Pos.x + 55, m_Pos.y + 395, 113.0, 18.0, 0, 0, 0.8828125, 0.5625);
			g_pRenderText->SetBgColor(255, 255, 255, 0);
			g_pRenderText->SetTextColor(255, 220, 150, 255);

			g_pRenderText->SetFont(g_hFontBold);
			g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 395, GlobalText[239], 0, 18);
			g_pRenderText->SetTextColor(getGoldColor(AllRepairGold));
			g_pRenderText->RenderText(m_Pos.x + 65, m_Pos.y + 395, strText, 0, 18);
		}
		else
		{
...
void SEASON3B::CNewUINPCShop::EventOrderWindows(double WindowsX, double WindowsY)
{
	...
	if (m_pNewInventoryCtrl)
	{
		m_pNewInventoryCtrl->SetPos((int)RenderFrameX + 15, (int)RenderFrameY + 50);
	}
}

CNewUIInventoryCtrl* SEASON3B::CNewUINPCShop::GetInventoryCtrl() const
{
...
bool SEASON3B::CNewUINPCShop::InventoryProcess()
{
	...
	if (SEASON3B::IsRelease(VK_LBUTTON) == true && m_pNewInventoryCtrl->CheckPtInRect(MouseX, MouseY) == true && m_bSellingItem == false)
	{
		...
		if (InvenIndex != -1)
		{
			...
			iSourceIndex = g_pMyInventory->SlotCntSendSrcInvenIndex(iSourceIndex, InvenIndex) + MAX_EQUIPMENT_INDEX;
...
void SEASON3B::CNewUINPCShop::SetButtonInfo()
{
	...
	if (gmProtect->checkold_school())
	{
		...
		SetButtonInterface(&m_BtnRepairAll, m_Pos.x, m_Pos.y, 3);
...
ITEM* SEASON3B::CNewUINPCShop::GetStandbyBuyItem()
{
	if (m_pNewInventoryCtrl)
		return m_pNewInventoryCtrl->FindItem(m_dwStandbyBuyItemKey);
	return NULL;
}
...
void SEASON3B::CNewUINPCShop::SetStandbyBuyItemKey(BYTE dwItemKey)
{
	m_dwStandbyBuyItemKey = dwItemKey;
}

BYTE SEASON3B::CNewUINPCShop::GetStandbyBuyItemKey() const
{
	return m_dwStandbyBuyItemKey;
}