...
protected:
	...
	BYTE    m_byWindMax;
	BYTE    m_byWindMin;
	float   m_fLuminosity[3];
public:
	OBJECT* GetOwner(void) {
		return (m_oOwner);
	}

	bool CompareTexture(int iTexFront) {
		return (m_iTexFront == iTexFront && m_iTexBack == iTexFront);
	}

	void SetTextureLuminosity(WORD iTexFront, float arg1, float arg2, float arg3)
	{
		if (iTexFront != 0xffff)
		{
			m_iTexBack = iTexFront;
			m_fLuminosity[0] = arg1;
			m_fLuminosity[1] = arg2;
			m_fLuminosity[2] = arg3;
		}
	}
protected:
