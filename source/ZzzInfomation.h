...
//extern Script_Item *ItemAttribute;
...
public:
	CHARACTER_MACHINE() {
		...
		Lock = 0;
		AccountLevel = 0;
		memset(AccountExpireDate, 0, sizeof(AccountExpireDate));
	}
	//-- input
	CHARACTER_ATTRIBUTE Character;
	ITEM				Equipment[MAX_NEW_EQUIPMENT];
	DWORD				Gold;
	DWORD				Coin1;
	DWORD				Coin2;
	DWORD				Coin3;
	...
	bool CheckEquipment(int index);
	ITEM* GetOnlyEquipment(int index);
	int ConvertLevel(int level);

	float GetRealLife();
	float GetRealMana();
	float GetRealShield();
	float GetRealSkillMP();
