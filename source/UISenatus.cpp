...
CSenatusInfo::CSenatusInfo()
{
	...
	GATELEVEL_HP[0] = 1900000;
	GATELEVEL_HP[1] = 2500000;
	GATELEVEL_HP[2] = 3500000;
	GATELEVEL_HP[3] = 5200000;

	STATUELEVEL_HP[0] = 1500000;
	STATUELEVEL_HP[1] = 2200000;
	STATUELEVEL_HP[2] = 3400000;
	STATUELEVEL_HP[3] = 5000000;

	GATELEVEL_DEFENSE[0] = 100;
	GATELEVEL_DEFENSE[1] = 180;
	GATELEVEL_DEFENSE[2] = 300;
	GATELEVEL_DEFENSE[3] = 520;

	STATUELEVEL_DEFENSE[0] = 80;
	STATUELEVEL_DEFENSE[1] = 180;
	STATUELEVEL_DEFENSE[2] = 340;
	STATUELEVEL_DEFENSE[3] = 550;

	STATUELEVEL_RECOVER[0] = 0;
	STATUELEVEL_RECOVER[1] = 1;
	STATUELEVEL_RECOVER[2] = 2;
	STATUELEVEL_RECOVER[3] = 3;
}
...
int CSenatusInfo::GetHP(int nType, int nLevel)
{
	...
	{
		if (nLevel == 0)
			return GATELEVEL_HP[0];
		else if (nLevel == 1)
			return GATELEVEL_HP[1];
		else if (nLevel == 2)
			return GATELEVEL_HP[2];
		else
			return GATELEVEL_HP[3];
	}
	else if (nType == STATUENPC_NUMBER)
	{
		if (nLevel == 0)
			return STATUELEVEL_HP[0];
		else if (nLevel == 1)
			return STATUELEVEL_HP[1];
		else if (nLevel == 2)
			return STATUELEVEL_HP[2];
		else
			return STATUELEVEL_HP[3];
	}
...
int CSenatusInfo::GetHPLevel(LPPMSG_NPCDBLIST pInfo)
{
	if (pInfo->iNpcNumber == GATENPC_NUMBER)
	{
		if (pInfo->iNpcMaxHp <= GATELEVEL_HP[0])
			return 0;
		else if (GATELEVEL_HP[0] < pInfo->iNpcMaxHp && pInfo->iNpcMaxHp <= GATELEVEL_HP[1])
			return 1;
		else if (GATELEVEL_HP[1] < pInfo->iNpcMaxHp && pInfo->iNpcMaxHp <= GATELEVEL_HP[2])
			return 2;
		else
			return 3;
	}
	...
	{
		if (pInfo->iNpcMaxHp <= STATUELEVEL_HP[0])
			return 0;
		else if (STATUELEVEL_HP[0] < pInfo->iNpcMaxHp && pInfo->iNpcMaxHp <= STATUELEVEL_HP[1])
			return 1;
		else if (STATUELEVEL_HP[1] < pInfo->iNpcMaxHp && pInfo->iNpcMaxHp <= STATUELEVEL_HP[2])
			return 2;
		else
			return 3;
	}
...
int CSenatusInfo::GetNextAddHP(LPPMSG_NPCDBLIST pInfo)
{
	...
	{
		if (pInfo->iNpcMaxHp == GATELEVEL_HP[0])
			return GATELEVEL_HP[1] - GATELEVEL_HP[0];
		if (pInfo->iNpcMaxHp == GATELEVEL_HP[1])
			return GATELEVEL_HP[2] - GATELEVEL_HP[1];
		else if (pInfo->iNpcMaxHp == GATELEVEL_HP[2])
			return GATELEVEL_HP[3] - GATELEVEL_HP[2];
		else
			return 0;
	}
	else if (pInfo->iNpcNumber == STATUENPC_NUMBER)
	{
		if (pInfo->iNpcMaxHp == STATUELEVEL_HP[0])
			return STATUELEVEL_HP[1] - STATUELEVEL_HP[0];
		if (pInfo->iNpcMaxHp == STATUELEVEL_HP[1])
			return STATUELEVEL_HP[2] - STATUELEVEL_HP[1];
		else if (pInfo->iNpcMaxHp == STATUELEVEL_HP[2])
			return STATUELEVEL_HP[3] - STATUELEVEL_HP[2];
		else
			return 0;
	}
...
int CSenatusInfo::GetDefense(int nType, int nLevel)
{
	...
	{
		if (nLevel == 0)
			return GATELEVEL_DEFENSE[0];
		else if (nLevel == 1)
			return GATELEVEL_DEFENSE[1];
		else if (nLevel == 2)
			return GATELEVEL_DEFENSE[2];
		else
			return GATELEVEL_DEFENSE[3];
	}
	else if (nType == STATUENPC_NUMBER)
	{
		if (nLevel == 0)
			return STATUELEVEL_DEFENSE[0];
		else if (nLevel == 1)
			return STATUELEVEL_DEFENSE[1];
		else if (nLevel == 2)
			return STATUELEVEL_DEFENSE[2];
		else
			return STATUELEVEL_DEFENSE[3];
	}
	return -1;
}
...
int CSenatusInfo::GetNextAddDefense(LPPMSG_NPCDBLIST pInfo)
{
	if (pInfo->iNpcNumber == GATENPC_NUMBER)
	{
		if (pInfo->iNpcDfLevel == 0)
			return GATELEVEL_DEFENSE[1] - GATELEVEL_DEFENSE[0];
		if (pInfo->iNpcDfLevel == 1)
			return GATELEVEL_DEFENSE[2] - GATELEVEL_DEFENSE[1];
		else if (pInfo->iNpcDfLevel == 2)
			return GATELEVEL_DEFENSE[3] - GATELEVEL_DEFENSE[2];
		else
			return 0;
	}
	else if (pInfo->iNpcNumber == STATUENPC_NUMBER)
	{
		if (pInfo->iNpcDfLevel == 0)
			return STATUELEVEL_DEFENSE[1] - STATUELEVEL_DEFENSE[0];
		if (pInfo->iNpcDfLevel == 1)
			return STATUELEVEL_DEFENSE[2] - STATUELEVEL_DEFENSE[1];
		else if (pInfo->iNpcDfLevel == 2)
			return STATUELEVEL_DEFENSE[3] - STATUELEVEL_DEFENSE[2];
		else
			return 0;
	}
	else
...
int CSenatusInfo::GetRecover(int nType, int nLevel)
{
	...
	{
		if (nLevel == 0)
			return STATUELEVEL_RECOVER[0];
		else if (nLevel == 1)
			return STATUELEVEL_RECOVER[1];
		else if (nLevel == 2)
			return STATUELEVEL_RECOVER[2];
		else
			return STATUELEVEL_RECOVER[3];
	}
	else
		return -1;
}
...
int CSenatusInfo::GetNextAddRecover(LPPMSG_NPCDBLIST pInfo)
{
	...
	{
		if (pInfo->iNpcRgLevel == STATUELEVEL_RECOVER[0])
			return STATUELEVEL_RECOVER[1] - STATUELEVEL_RECOVER[0];
		if (pInfo->iNpcRgLevel == STATUELEVEL_RECOVER[1])
			return STATUELEVEL_RECOVER[2] - STATUELEVEL_RECOVER[1];
		else if (pInfo->iNpcRgLevel == STATUELEVEL_RECOVER[2])
			return STATUELEVEL_RECOVER[3] - STATUELEVEL_RECOVER[2];
		else
			return 0;
	}
	else
...
void CSenatusInfo::BuyNewNPC(int iNpcNumber, int iNpcIndex)
{
	...
	if (iNpcNumber == GATENPC_NUMBER)	// gate
		pNPCInfo->iNpcHp = pNPCInfo->iNpcMaxHp = GATELEVEL_HP[0];
	else
		pNPCInfo->iNpcHp = pNPCInfo->iNpcMaxHp = STATUELEVEL_HP[0];
