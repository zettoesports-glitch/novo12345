...
void CNewUIQuestProgress::SetPos(int x, int y)
{
	...
	m_btnProgressL.SetPos(x + 131, y + 168);
	m_btnProgressR.SetPos(x + 153, y + 168);
	m_btnComplete.SetPos(x + (QP_WIDTH - 108) / 2, y + 362);
...
{
	SEASON3B::window_nomal((float)m_Pos.x, (float)m_Pos.y, 0, 181.f);
}
...
void CNewUIQuestProgress::RenderSelTextBlock()
{
	...
	if (gmProtect->checkold_school())
	{
		...
		RenderColor(m_Pos.x + 18, nBlockPosY, 154.f, QP_TEXT_GAP * m_anAnswerLine[m_nSelAnswer - 1]);
		EndRenderColor();
	}
	else
	{
...
void CNewUIQuestProgress::RenderText()
{
	...
	g_pRenderText->RenderText(m_Pos.x + textleft, m_Pos.y + 51, g_QuestMng.GetNPCName(), 0, 0, RT3_SORT_LEFT);

	if (REQUEST_REWARD_MODE != m_eLowerView)
	{
		g_pRenderText->SetTextColor(255, 185, 10, 255);
		g_pRenderText->RenderText(m_Pos.x + textleft, m_Pos.y + 207, CharacterAttribute->Name, 0, 0, RT3_SORT_LEFT);
	}
	...
	{
		g_pRenderText->RenderText(m_Pos.x + textleft, m_Pos.y + 66 + (QP_TEXT_GAP * i), m_aszNPCWords[i + QP_NPC_MAX_LINE_PER_PAGE * m_nSelNPCPage], 0, 0, RT3_SORT_LEFT);
	}
	...
	if (PLAYER_WORDS_MODE == m_eLowerView)
	{
		...
		for (i = 0; i < 2; ++i)
		{
			g_pRenderText->RenderText(m_Pos.x + textleft, m_Pos.y + 222 + (QP_TEXT_GAP * i), m_aszPlayerWords[i], 0, 0, RT3_SORT_LEFT);
		}
		for (; i < QP_PLAYER_LINE_MAX; ++i)
		{
			g_pRenderText->RenderText(m_Pos.x + textleft, m_Pos.y + 225 + (QP_TEXT_GAP * i), m_aszPlayerWords[i], 0, 0, RT3_SORT_LEFT);
		}
	}
}
...
void SEASON3B::CNewUIQuestProgress::EventOrderWindows(double WindowsX, double WindowsY)
{
...
void CNewUIQuestProgress::OpenningProcess()
{
	::PlayBuffer(SOUND_INTERFACE01);
}

bool CNewUIQuestProgress::ClosingProcess()
{
	m_dwCurQuestIndex = 0;
	SendExitInventory();
	::PlayBuffer(SOUND_CLICK01);
...
void CNewUIQuestProgress::SetCurNPCWords()
{
	...
	::memset(m_aszNPCWords, 0, sizeof(char) * QP_NPC_LINE_MAX * QP_WORDS_ROW_MAX);
	...
	if (gmProtect->checkold_school())
	{
		nLine = ::DivideStringByPixel(&m_aszNPCWords[0][0], QP_NPC_LINE_MAX, QP_WORDS_ROW_MAX, g_QuestMng.GetNPCWords(m_dwCurQuestIndex), 150);
	}
	else
	{
		nLine = ::DivideStringByPixel(&m_aszNPCWords[0][0], QP_NPC_LINE_MAX, QP_WORDS_ROW_MAX, g_QuestMng.GetNPCWords(m_dwCurQuestIndex), 160);
	}
...
void CNewUIQuestProgress::SetCurPlayerWords()
{
	...
	::memset(m_aszPlayerWords, 0, sizeof(char) * QP_PLAYER_LINE_MAX * QP_WORDS_ROW_MAX);
