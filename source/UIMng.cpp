...
void CUIMng::CreateTitleSceneUI()
{
	...
	const float screenW = (float)rInput.GetScreenWidth();
	const float screenH = (float)rInput.GetScreenHeight();

	// Base UI 800x600 (barra de loading)
	const float fScaleX = screenW / 800.0f;
	const float fScaleY = screenH / 600.0f;

	// Arte principal 1280 x 735 (512+223) — preenche a tela SEM faixas pretas cima/baixo
	const float designW = 1280.0f;
	const float designH = 735.0f; // BACK4-6 (512) + BACK7-9 (223)
	const float imgScaleX = screenW / designW;
	const float imgScaleY = screenH / designH;

	m_asprTitle = new CSprite[UIM_TS_MAX];

	// --- Faixas pretas antigas (cima/baixo): cria mas NAO mostra ---
	...
	// --- Imagem central (unica arte visivel), Y=0 e escala full screen ---
	m_asprTitle[UIM_TS_BACK4].Create(512, 512, BITMAP_TITLE + 8, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK4].SetPosition(0, 0);
	m_asprTitle[UIM_TS_BACK5].Create(512, 512, BITMAP_TITLE + 9, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK5].SetPosition(512, 0);
	m_asprTitle[UIM_TS_BACK6].Create(256, 512, BITMAP_TITLE + 10, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK6].SetPosition(1024, 0);
	m_asprTitle[UIM_TS_BACK7].Create(512, 223, BITMAP_TITLE + 11, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK7].SetPosition(0, 512);
	m_asprTitle[UIM_TS_BACK8].Create(512, 223, BITMAP_TITLE + 12, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK8].SetPosition(512, 512);
	m_asprTitle[UIM_TS_BACK9].Create(256, 223, BITMAP_TITLE + 13, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleX, imgScaleY);
	m_asprTitle[UIM_TS_BACK9].SetPosition(1024, 512);

	// Logos (escondidos — so imagem + barra)
	m_asprTitle[UIM_TS_MU].Create(216, 138, BITMAP_TITLE + 2, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, imgScaleY, imgScaleY);
	m_asprTitle[UIM_TS_MU].SetPosition(0, 0);
	...
	// Linha / barra de carregamento (embaixo)
	m_pgbLoding = new CGaugeBar;
	RECT rc = { 0, 0, 656, 15 };
	m_pgbLoding->Create(4, 15, BITMAP_TITLE + 5, &rc, 0, 0, -1, true, fScaleX, fScaleY);
	m_pgbLoding->SetPosition(72, 560); // perto da base da tela (coord 800x600)

	// So mostra arte + barra
	for (int i = 0; i < UIM_TS_MAX; ++i)
		m_asprTitle[i].Show(false);

	m_asprTitle[UIM_TS_BACK4].Show(true);
	m_asprTitle[UIM_TS_BACK5].Show(true);
	m_asprTitle[UIM_TS_BACK6].Show(true);
	m_asprTitle[UIM_TS_BACK7].Show(true);
	m_asprTitle[UIM_TS_BACK8].Show(true);
	m_asprTitle[UIM_TS_BACK9].Show(true);
...
void CUIMng::RenderTitleSceneUI(HDC hDC, DWORD dwNow, DWORD dwTotal)
{
	...
	// Fundo preto limpo (sem faixas extras)
	::glColor4f(0.f, 0.f, 0.f, 1.f);

	// So sprites visiveis (imagem central — Show=false nao desenha se CSprite respeitar)
	for (int i = 0; i < UIM_TS_MAX; ++i)
	{
		// Pula faixas pretas (BACK0-3) e logos (MU / 121518 / WEBZEN)
		if (i == UIM_TS_BACK0 || i == UIM_TS_BACK1 || i == UIM_TS_BACK2 || i == UIM_TS_BACK3)
			continue;
		if (i == UIM_TS_MU || i == UIM_TS_121518 || i == UIM_TS_WEBZEN)
			continue;
