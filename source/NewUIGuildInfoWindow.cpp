...
void SEASON3B::CNewUIGuildInfoWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
...
void SEASON3B::CNewUIGuildInfoWindow::ReceiveGuildRelationShip( BYTE byRelationShipType, BYTE byRequestType, BYTE  byTargetUserIndexH, BYTE byTargetUserIndexL)
{
	...
	else
	{
		...
		if( nIndex < 0 || nIndex >= MAX_CHARACTERS_CLIENT )
			return;
		CHARACTER* pPlayer = gmCharacters->GetCharacter(nIndex);
