...
void ChangeCharacterExt(int Key, DWORD* Equipment, CHARACTER* pCharacter = NULL, OBJECT* pHelper = NULL);
...
extern void CreateJoint(int Type, vec3_t Position, vec3_t TargetPosition, vec3_t Angle, int SubType = 0, OBJECT* Target = NULL, float Scale = 10.f, short PK = -1, WORD SkillIndex = 0, WORD SkillSerialNum = 0, int iChaIndex = -1, const float* vColor = NULL, short int sTargetIndex = -1);
extern void CreateJointSync(int Type, vec3_t Position, vec3_t TargetPosition, vec3_t Angle, int SubType = 0, OBJECT* Target = NULL, float Scale = 10.f, short PK = -1, WORD SkillIndex = 0, WORD SkillSerialNum = 0, int iChaIndex = -1, const float* vColor = NULL, short int sTargetIndex = -1);


extern bool RenderCharacterBackItem(CHARACTER* c, OBJECT* o, bool bTranslate);
extern bool IsBackItem(CHARACTER* c, int iType);
extern void runtime_physical_cloth(CHARACTER* pCharacter, OBJECT* o, vec3_t CloakLight);
#endif