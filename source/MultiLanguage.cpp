...
CMultiLanguage::CMultiLanguage(std::string strSelectedML)
{
	...
	else if (_stricmp(strSelectedML.c_str(), "CHS") == 0)
	{
		byLanguage = 3;
		iCodePage = 1252;
		iNumByteForOneCharUTF8 = 2;
	...
	{
		byLanguage = 3;
		iCodePage = CP_GBK;  // CP_GBK
		iNumByteForOneCharUTF8 = 2; // GBK puede tener 1 o 2 bytes por carácter
	}
	else if (_stricmp(strSelectedML.c_str(), "KOR") == 0)
	{
		byLanguage = 4;
		iCodePage = CP_KOR;  // CP_GBK
		iNumByteForOneCharUTF8 = 2; // GBK puede tener 1 o 2 bytes por carácter
	}
	else
	{
...
#ifdef CHINESE_LANGUAGE
	int iDestType = CHINESE_CP;
#else
	int iDestType = (IsCharUTF8(lpString)) ? this->GetCodePage() : CP_UTF8;
#endif

	//CP_UTF8 : this->GetCodePage();
