...
bool SEASON3B::CNewUIHotKey::UpdateKeyEvent()
{
	...
	{
		// Modo Foto: ESC restores the HUD
		if (SEASON3B::IsPhotoModeHideHud())
		{
			if (g_pOption)
				g_pOption->SetPhotoMode(false);
			PlayBuffer(SOUND_CLICK01);
			return false;
		}

		// Event board (CustomEventTime) is not a NewUI window — close it like Rank/VIP/Jewel
		if (gInterface && gInterface->Data[eWindowEventTime].OnShow)
		{
			gInterface->Data[eWindowEventTime].OnShow = false;
			PlayBuffer(SOUND_CLICK01);
			return false;
		}
		// Security panel has its own UpdateKeyEvent; keep HotKey from opening system menu first
		if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_ACCOUNT_SECURITY))
		{
			g_pNewUISystem->Hide(SEASON3B::INTERFACE_ACCOUNT_SECURITY);
			PlayBuffer(SOUND_CLICK01);
			return false;
		}
	...
	// F10 = camera toggle | Ctrl+F10 = camera backup
	// F11 reserved for FrameProfiler (ZzzScene)
	...
	if (CanUpdateKeyEventRelatedMyInventory() != true)
	{
		...
		// F key (Friend / send message window) disabled by request
		...
		else
		{
			...
			// Y-key CEffect color editor disabled (players must not change item mesh colors in-game).
			// Effects still load from MainInfo / av-code45.pak.
