...
CCharInfoBalloon::CCharInfoBalloon() : m_pCharInfo(NULL)
{
	...
	memset(m_szRese, 0, sizeof(char) * 64);
	memset(m_szMaster, 0, sizeof(char) * 64);
}
...
void CCharInfoBalloon::Create(CHARACTER* pCharInfo)
{
	...
	if (GMProtect->CharInfoBalloonType() == 1)
		CSprite::Create(118, 70, BITMAP_LOG_IN + 7, 0, NULL, 59, 54);
	if (GMProtect->CharInfoBalloonType() == 2)
		CSprite::Create(118, 86, BITMAP_LOG_IN + 7, 0, NULL, 59, 54);
...
void CCharInfoBalloon::Render()
{
	...
	int PosX = CSprite::GetXPos();
	int PosY = CSprite::GetYPos();

	g_pRenderText->SetFont(g_hFixFont);

	int nTextPosX = (int)(PosX / g_fScreenRate_x);
	g_pRenderText->SetBgColor(0);
	g_pRenderText->SetTextColor(m_dwNameColor);
	g_pRenderText->RenderText(nTextPosX, (int)((PosY + 7) / g_fScreenRate_y), m_szName, (CSprite::GetWidth() / g_fScreenRate_x), 0, RT3_SORT_CENTER);

	g_pRenderText->SetTextColor(CLRDW_WHITE);
	g_pRenderText->RenderText(nTextPosX, (int)((PosY + 23) / g_fScreenRate_y), m_szGuild, (CSprite::GetWidth() / g_fScreenRate_x), 0, RT3_SORT_CENTER);

	g_pRenderText->SetTextColor(CLRDW_BR_ORANGE);
	g_pRenderText->RenderText(nTextPosX - 10, (int)((PosY + 39) / g_fScreenRate_y), m_szClass, ((CSprite::GetWidth() + 30) / g_fScreenRate_x), 0, RT3_SORT_CENTER);

	if (GMProtect->CharInfoBalloonType() >= 1)
		g_pRenderText->RenderText(nTextPosX, (int)((PosY + 55) / g_fScreenRate_y), m_szRese, (CSprite::GetWidth() / g_fScreenRate_x), 0, RT3_SORT_CENTER);

	if (GMProtect->CharInfoBalloonType() == 2)
		g_pRenderText->RenderText(nTextPosX, (int)((PosY + 71) / g_fScreenRate_y), m_szMaster, (CSprite::GetWidth() / g_fScreenRate_x), 0, RT3_SORT_CENTER);
	...
	if (m_pCharInfo->CtlCode & CTLCODE_80MANAGER_MOVE_CHAR)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			g_pRenderText->RenderText(nTextPosX, (int)((PosY - 46 + i * 16) / g_fScreenRate_y), GlobalText[1241 + i], CSprite::GetWidth() / g_fScreenRate_x, 0, RT3_SORT_CENTER);
		}
	}
}
...
void CCharInfoBalloon::SetInfo()
{
	...
	sprintf(this->m_szRese, "Reset: %d", m_pCharInfo->m_Reset);

	sprintf(this->m_szMaster, "MasterReset: %d", m_pCharInfo->m_MasterReset);

	sprintf(m_szClass, "%s %d", gCharacterManager.GetCharacterClassText(m_pCharInfo->Class), m_pCharInfo->Level);

	sprintf(m_szOnClass, "%s", gCharacterManager.GetCharacterClassText(m_pCharInfo->Class));

	sprintf(m_szOnLevel, "Lv. %d", m_pCharInfo->Level);
}