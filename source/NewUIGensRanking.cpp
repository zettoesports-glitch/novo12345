...
CNewUIGensRanking::CNewUIGensRanking() : m_fBooleanSize(0.5f)
{
	Init();
}
...
void SEASON3B::CNewUIGensRanking::EventOrderWindows(double WindowsX, double WindowsY)
{
...
void CNewUIGensRanking::SetBtnInfo(float _PosX, float _PosY)
{
	...
	m_BtnExit.ChangeButtonInfo(_PosX + 13, _PosY + 392, 36, 29);
...
void SEASON3B::CNewUIGensRanking::RanderMark2(float _x, float _y, BYTE _GensInfluence, BYTE _GensRankInfo, int _ImageArea, float _RenderY)
{
	if (!_GensInfluence)
		return;
	...
	RenderBitmap(_Image_Type, _x, _y, _width, _height, u, v, _width / _BitmapWidth, _height / _BitmapHeight, true, false);
}
