	...
	else if (pCharacter->Object.Kind == KIND_PLAYER)
	{
		...
		if (pCharacter->GuildRelationShip != GR_RIVAL && pCharacter->GuildRelationShip != GR_RIVALUNION)
		{
				...
				|| strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
			{
				...
				else if (pCharacter->PK < PVP_MURDERER2
					&& !gAutoCombat.AllowPlayerTargetWithoutCtrl()
					&& (SEASON3B::IsRepeat(VK_CONTROL) != true || pCharacter == Hero))
				{
					success = false;
		...
		if (c == Hero && sc)
		{
				...
				|| strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[sc->GuildMarkIndex].GuildName))
			{
				...
				else if (sc->GuildRelationShip != GR_RIVAL && sc->GuildRelationShip != GR_RIVALUNION)
				{
					...
					{
						if ((signed int)sc->PK < PVP_MURDERER2
							&& !gAutoCombat.AllowPlayerTargetWithoutCtrl()
							&& (SEASON3B::IsRepeat(VK_CONTROL) != true || sc == Hero))
						{
							if (gMapManager->IsCursedTemple())
							{
			...
			if (Skill > Skill_Twisting_Slash_Strengthener)
			{
				...
				LABEL_120:
					...
					if (CheckTile(c, TargetX, TargetY, Distance))
					{
						...
						// Chaotic Diseier — 5 shiny + 8 rastros (contagem MuMobile)
						// Spawn em ESPACO LOCAL (lado/frente do yaw), nao eixo X do MUNDO
						// (eixo mundo = espalha em volta; local = faixa reta na direcao da skill)
						for (int i = 0; i < 5; ++i)
						{
						...
						{
							float matGhost[3][4];
							vec3_t vLocal, vWorld;
							AngleMatrix(o->Angle, matGhost);
							for (int i = 0; i < 8; i++)
							{
								// lateral ~±119 (mesmo range Mobile), leve avanco em -Y local (frente do joint)
								float fSide = (float)(rand() % 240) - 119.f;
								float fFront = (float)(rand() % 50); // 0..50 a frente, nao atras
								Vector(fSide, -fFront, 0.f, vLocal);
								VectorRotate(vLocal, matGhost, vWorld);
								Position[0] = o->Position[0] + vWorld[0];
								Position[1] = o->Position[1] + vWorld[1];
...
void MoveHero()
{
	...
	// GM /watch: block movement only (chat + /unwatch + ESC/END still work)
	if (gSpectatorMode.IsInputBlocked())
	{
		gSpectatorMode.Update(); // hotkeys while standing still
		SetPlayerStop(c);
		return;
	}
...
void RenderPartyHP()
{
	// Floating HP bars above characters (players + party heads) DISABLED.
	// - Party list UI (side window) still shows HP.
	// - Own life bar on main HUD still shows.
	// - Monster floating HP bars remain (optional hunt aid).

	// --- Party floating bars over heads: OFF ---
	// (was PartyHpBarType 0/1 rendering above party members)
	...
	for (size_t i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		// Only monsters keep floating HP; players (personagens) do not.
		if (o->Live && o->Kind == KIND_MONSTER && pCharacter->Dead == 0 && pCharacter != Hero)
		{
			vec3_t      Position;
