...
namespace SEASON3B
{
	...
	public:
		...
		void MoveTextTipPos(CNewUIButton* Button, int iX, int iY);
		void SetButtonInfo(CNewUIButton* Button, int imgindex, int x, int y, int sx, int sy, bool overflg, bool isimgwidth, bool bClickEffect, bool MoveTxt, unicode::t_string btname, unicode::t_string tooltiptext, bool istoppos);
	private:
		void LoadImages();
		void UnloadImages();
	};
