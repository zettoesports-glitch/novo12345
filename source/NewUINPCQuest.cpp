...
void CNewUINPCQuest::SetPos(int x, int y)
{
	...
	m_btnComplete.SetPos(x + 41, y + 355);
...
bool CNewUINPCQuest::Render()
{
	...
	RenderBackImage(byCurQuestState);
...
void CNewUINPCQuest::RenderBackImage(BYTE byCurQuestState)
{
	...
	if (gmProtect->checkold_school())
	{
		if (QUEST_ING == byCurQuestState)
		{
			RenderImage(BITMAP_INVENTORY + 19, m_Pos.x, m_Pos.y + 335, 190, 10);
		}
	}
	else
	{
...
void CNewUINPCQuest::RenderText()
{
	...
	{
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 16, g_csQuest.GetNPCName(2), NPCQUEST_WIDTH, 0, RT3_SORT_CENTER);
	}
	else
	{
		g_pRenderText->RenderText(m_Pos.x, m_Pos.y + 16, g_csQuest.GetNPCName(byCurQuestIndex), NPCQUEST_WIDTH, 0, RT3_SORT_CENTER);
	}
...
void SEASON3B::CNewUINPCQuest::EventOrderWindows(double WindowsX, double WindowsY)
{
...
void CNewUINPCQuest::OpenningProcess()
{
	g_csQuest.ShowQuestNpcWindow();
}

bool CNewUINPCQuest::ClosingProcess()
{
	SendExitInventory();
	return true;
}
