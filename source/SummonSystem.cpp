...
extern float RequestTerrainHeight(float xf, float yf);
...
{
	int Weapon02Kind2 = GMItemMng->GetKind2(iItemType - MODEL_ITEM);
...
{
	int sc_type_helper = pCharacter->Helper.Type;

	if (sc_type_helper == MODEL_HELPER + 37 || pObject->movementType == Movement::Fenrir)
	{
		SetAction(pObject, PLAYER_SKILL_SUMMON_FENRIR, true);
	}
	else if (sc_type_helper == MODEL_HELPER + 2 || pObject->movementType == Movement::Uniria)
	{
		SetAction(pObject, PLAYER_SKILL_SUMMON_UNI, true);
	}
	else if (sc_type_helper == MODEL_HELPER + 3 || pObject->movementType == Movement::Dinorant)
	{
		SetAction(pObject, PLAYER_SKILL_SUMMON_DINO, true);
	}
	else if (sc_type_helper == MODEL_HELPER + 4 || pObject->movementType == Movement::Horse)
	{
		SetAction(pObject, PLAYER_SKILL_SUMMON_UNI, true);
	}
	else
	{
		SetAction(pObject, PLAYER_SKILL_SUMMON, true);
	}
}
...
BOOL CSummonSystem::SendRequestSummonSkill(int iSkill, CHARACTER* pCharacter, OBJECT* pObject)
{
	...
	//if (iTargetKey != -1)
	//{
	//	CHARACTER * pTargetCharacter = gmCharacters->GetCharacter(SelectedCharacter);
	//	if (pTargetCharacter)
	//	{
	...
	CHARACTER* pTargetCharacter = gmCharacters->GetCharacter(SelectedCharacter);
	if (pTargetCharacter)
		g_MovementSkill.m_iKey = pTargetCharacter->Key;
	else
		g_MovementSkill.m_iKey = 0;

	float fDistance = gSkillManager.GetSkillDistance(iSkill, pCharacter);

	if (CheckTile(pCharacter, TargetX, TargetY, fDistance))
	{
		if (iTargetKey != -1)
		{
			SendRequestMagicContinue(iSkill, (BYTE)(pCharacter->TargetPosition[0] / TERRAIN_SCALE), (BYTE)(pCharacter->TargetPosition[1] / TERRAIN_SCALE), (BYTE)(pObject->Angle[2] / 360.f * 256.f), 0, 0, (WORD)iTargetKey, 0);
		}
		else
		{
			SendRequestMagicContinue(iSkill, (BYTE)TargetX, (BYTE)TargetY, (BYTE)(pObject->Angle[2] / 360.f * 256.f), 0, 0, 0, 0);
		}
...
void CSummonSystem::CreateCastingEffect(vec3_t vPosition, vec3_t vAngle, int iSubType)
{
	...
	case AT_SKILL_SUMMON_POLLUTION:
		...
		break;
	default: Vector(0.7f, 0.7f, 1.0f, vLight);
		break;
	}
