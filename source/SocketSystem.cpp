...
int CSocketItemMgr::CalcSocketOptionValue(int iOptionType, float fOptionValue)
{
	...
	{
	case 1:
		return static_cast<int>(fOptionValue);
	case 2:
		return static_cast<int>(fOptionValue);
	case 3:
	{
		...
		{
			wLevel += Master_Level_Data.nMLevel;
		}
...
void CSocketItemMgr::CheckSocketSetOption()
{
	...
	for (int i = 0; i < m_iNumEquitSetBonusOptions; ++i)
	{
		...
		int icnt = 0;
		for (int j = 0; j < 6 && iSeedSum[j] >= pbySetTest[j]; ++j, ++icnt)
		{
		}
...
void CSocketItemMgr::OpenSocketItemScript(const unicode::t_char* szFileName)
{
	...
	if (fp != NULL)
	{
		...
		int k = 0;
		for (k = 0; k < MAX_SOCKET_OPTION; ++k)
		{
			BYTE* pbySetTest = m_SocketOptionInfo[SOT_EQUIP_SET_BONUS_OPTIONS][k].m_bySocketCheckInfo;
			if (pbySetTest[0] + pbySetTest[1] + pbySetTest[2] + pbySetTest[3] + pbySetTest[4] + pbySetTest[5] == 0)
			{
				break;
			}
		}
		m_iNumEquitSetBonusOptions = k;
	}
	else
	{
...
void SEASON4A::CSocketItemMgr::GetSocketOptionNameText(char* szOptionName, int SOCKET, int iSocketSeedID)
{
#if MAIN_UPDATE > 303
	SOCKET_OPTION_INFO* pInfo = &m_SocketOptionInfo[SOCKET][iSocketSeedID];
	sprintf(szOptionName, pInfo->m_szOptionName);
#endif
}

void SEASON4A::CSocketItemMgr::GetSocketOptionValueText(char* szOptionValueText, int SOCKET, int iSocketSeedID, int Type)
{
#if MAIN_UPDATE > 303
	SOCKET_OPTION_INFO* pInfo;

	pInfo = &m_SocketOptionInfo[SOCKET][iSocketSeedID];

	float fOptionValue = (float)pInfo->m_iOptionValue[(Type - (ITEM_WING + 100)) / 6];
