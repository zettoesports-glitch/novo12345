...
void CUIMapName::InitImgPathMap()
{
	...
	std::string strFolderName = "Local\\ImgsMapName\\";
#else
	...
	{
		if ((&mapInfo)->mapIndex == -1)
			continue;

		if ((&mapInfo)->ImgName[0] == '\0')
			continue;

		m_mapImgPath[(&mapInfo)->mapIndex] = strFolderName + (&mapInfo)->ImgName;
	}
}
...
void CUIMapName::ShowMapName()
{
	...
	if (m_nOldWorld != gMapManager->currentMap)
	{
		char szImgPath[128];
		strcpy(szImgPath, m_mapImgPath[gMapManager->currentMap].data());

		DeleteBitmap(BITMAP_INTERFACE_EX+45);
		LoadBitmap(szImgPath, BITMAP_INTERFACE_EX+45);
		
		m_nOldWorld = gMapManager->currentMap;
...
#ifdef ASG_ADD_GENS_SYSTEM
	if (m_bStrife)
		::RenderBitmap(BITMAP_INTERFACE_EX+47, UIMN_IMG_POS_X, UIMN_STRIFE_POS_Y, UIMN_IMG_WIDTH, UIMN_STRIFE_HEIGHT, 0.0f, 0.0f,	UIMN_IMG_WIDTH / 256.0f, UIMN_STRIFE_HEIGHT / 32.0f, false, false);
#endif	// ASG_ADD_GENS_SYSTEM

	::RenderBitmap(BITMAP_INTERFACE_EX+45, UIMN_IMG_POS_X, UIMN_IMG_POS_Y, UIMN_IMG_WIDTH, UIMN_IMG_HEIGHT, 0.0f, 0.0f,UIMN_IMG_WIDTH / 256.0f, UIMN_IMG_HEIGHT / 128.0f, false, false);
