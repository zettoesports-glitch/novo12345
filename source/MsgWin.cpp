...
void CMsgWin::SetMsg(MSG_WIN_TYPE eType, LPCTSTR lpszMsg, LPCTSTR lpszMsg2)
{
	...
	if (NULL == lpszMsg2)
	{
		m_nMsgLine = ::SeparateTextIntoLines((char*)lpszMsg, m_aszMsg[0], MW_MSG_LINE_MAX, MW_MSG_ROW_MAX);
	}
	else
	{
...
void CMsgWin::PopUp(int nMsgCode, char* pszMsg)
{
	...
	case MESSAGE_DELETE_CHARACTER_CONFIRM:
		{
			CHARACTER* pCharacter = gmCharacters->GetCharacter(FindCharacterIndex(SelectedHero));

			if (pCharacter)
			{
				sprintf(szTempMsg, GlobalText[1712], pCharacter->ID);
				lpszMsg = szTempMsg;
				eType = MWT_BTN_BOTH;
			}
		}
	...
	case MESSAGE_DELETE_CHARACTER_SUCCESS:
		{
			...
			if (pCharacter)
			{
				...
				gGoboidManager->DeleteBug(&pCharacter->Object);
				SelectedHero = -1;
