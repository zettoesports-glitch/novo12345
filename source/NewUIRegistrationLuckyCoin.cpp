...
namespace SEASON3B
{
...
#if MAIN_UPDATE > 603
		g_pRenderText->RenderText(_x, GetPos().y + 15, szText, LUCKYCOIN_REG_WIDTH, 0, RT3_SORT_CENTER);
#else
	...
	void CNewUIRegistrationLuckyCoin::RenderLuckyCoin()
	{
		...
		x = GetPos().x + (LUCKYCOIN_REG_WIDTH - 190) / 2;
		y = GetPos().y + 50;
...
#if MAIN_UPDATE > 603
		m_RegistButton.ChangeButtonImgState(true, BITMAP_msgbox_btn_empty_very_small, true, false, false, 0.0, 0.0, 304.0, 96.0);
#else
...
#if MAIN_UPDATE > 603
		m_CloseButton.ChangeButtonImgState(true, BITMAP_msgbox_btn_empty_very_small, true, false, false, 0.0, 0.0, 304.0, 96.0);
#else
		m_CloseButton.ChangeButtonImgState(true, IMAGE_CLOSE_REGIST, true);
#endif
		m_CloseButton.ChangeButtonInfo(_x, GetPos().y + 360, m_width, m_height);
	...
	void CNewUIRegistrationLuckyCoin::OpenningProcess()
	{
