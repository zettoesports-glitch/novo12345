...
void CNewUIPetInfoWindow::InitButtons()
{
	...
	m_BtnExit.ChangeButtonInfo(RenderPosX + 22, RenderPosY + 384, 20, 20);
	...
	{
		m_BtnExit.ChangeButtonImgState(true, IMAGE_PETINFO_BTN_EXIT);
...
void CNewUIPetInfoWindow::SetPos(int x, int y)
{
	...
	float RenderPosX = m_Pos.x;
	float RenderPosY = m_Pos.y;
	m_BtnExit.SetPos(RenderPosX + 22, RenderPosY + 384);
#else
...
bool CNewUIPetInfoWindow::Render()
{
...
#if MAIN_UPDATE > 603
	g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 70) / 2), m_Pos.y + 13, GlobalText[1217], 70, 0, RT3_SORT_CENTER);
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
		g_pRenderText->SetBgColor(20, 20, 20, 255);
		g_pRenderText->RenderText(m_Pos.x + 35, m_Pos.y + 12, GlobalText[190], 120, 0, RT3_SORT_CENTER);
		g_pRenderText->SetBgColor(0);
	}
	else
	{
	...
	case TAB_TYPE_DARKHORSE:
	{
		...
		if (pPetInfo->m_dwPetType == PET_TYPE_NONE)
		{
			...
			g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 100, szText, (WIN_WINDOW_SIZEX - 40), 30, RT3_SORT_CENTER);
		}
		else
		{
	...
	case TAB_TYPE_DARKSPIRIT:
	{
		...
		if (pPetInfo->m_dwPetType == PET_TYPE_NONE)
		{
			...
			g_pRenderText->SetTextColor(0xFF0000FF);
			g_pRenderText->SetFont(g_hFontBold);
			g_pRenderText->RenderText(m_Pos.x + 20, m_Pos.y + 100, GlobalText[1169], (WIN_WINDOW_SIZEX - 40), 30, RT3_SORT_CENTER);
		}
		else
		{
...
bool CNewUIPetInfoWindow::RenderDarkHorseInfo(PET_INFO* pPetInfo)
{
	...
	{
		Script_Item* Item_info = GMItemMng->find(pItem->Type);

		if (Item_info)
		{
			g_pRenderText->SetFont(g_hFont);
			g_pRenderText->SetTextColor(255, 255, 255, 255);
			g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 70) / 2), m_Pos.y + 25, Item_info->Name, 70, 0, RT3_SORT_CENTER);
	...
	else
	{
		g_pRenderText->SetFont(g_hFont);
		g_pRenderText->SetTextColor(255, 255, 255, 255);
		g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 70) / 2), m_Pos.y + 25, GlobalText[1187], 70, 0, RT3_SORT_CENTER);
		...
		DWORD _DamageMin = m_aiDamage[0];
		DWORD _DamageMax = m_aiDamage[1];

		CharacterAttribute->GetDarkHorseDamage(&_DamageMin, &_DamageMax);

		unicode::_sprintf(szText, GlobalText[203], _DamageMin, _DamageMax, CharacterAttribute->Work_DarkSpiritAttackSuccessRate);
		g_pRenderText->RenderText(iPosX + 10, iPosY + 72, szText, 0, 0, RT3_SORT_CENTER);

		unicode::_sprintf(szText, GlobalText[64], CharacterAttribute->Work_DarkSpiritAttackSpeed);
...
bool CNewUIPetInfoWindow::RenderDarkSpiritInfo(PET_INFO* pPetInfo)
{
	...
	g_pRenderText->RenderText(m_Pos.x + ((WIN_WINDOW_SIZEX - 70) / 2), m_Pos.y + 25, GlobalText[1214], 70, 0, RT3_SORT_CENTER);
...
void SEASON3B::CNewUIPetInfoWindow::EventOrderWindows(double WindowsX, double WindowsY)
{
