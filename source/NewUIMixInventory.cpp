...
bool CNewUIMixInventory::Create(CNewUIManager* pNewUIMng, int x, int y)
{
	...
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + (INVENTORY_WIDTH - 160) / 2, y + 110, 8, 4))
	{
...
#if MAIN_UPDATE > 603
	SetButtonImagen(&m_BtnMix, IMAGE_MIXINVENTORY_MIXBTN, IMAGE_MIXINVENTORY_MIXBTN, 0.f, 0.f, 187.f, 179.f);
	m_BtnMix.ChangeImgColor(BUTTON_STATE_DOWN, RGBA(100, 100, 100, 255));
	m_BtnMix.ChangeButtonInfo(m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f, m_Pos.y + 370, 44.f, 35.f);
	m_BtnMix.ChangeToolTipText(GlobalText[591], true);
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
		SetButtonImagen(&m_BtnMix, BITMAP_INVENTORY_BUTTON + 12, BITMAP_INVENTORY_BUTTON + 13);
		m_BtnMix.ChangeButtonInfo(m_Pos.x + 75.0, m_Pos.y + 380.0, 44.0, 32.0);
	}
	else
	{
...
{
#if(CB_GETMIXRATE)
	if (gCB_GetMixRate) gCB_GetMixRate->SetInfoMixID(0);
#endif
...
void CNewUIMixInventory::SetPos(int x, int y)
{
	...
	if (gmProtect->checkold_school())
	{
		m_BtnMix.SetPos(m_Pos.x + 75.0, m_Pos.y + 380.0);
	}
	else
	{
		m_BtnMix.SetPos(m_Pos.x + INVENTORY_WIDTH * 0.5f - 22.f, m_Pos.y + 380);
	}

	m_SocketListBox.SetPosition(x + (INVENTORY_WIDTH - 160) / 2, y + 360);
}
...
void SEASON3B::CNewUIMixInventory::EventOrderWindows(double WindowsX, double WindowsY)
{
	...
	if (m_pNewInventoryCtrl)
	{
		m_pNewInventoryCtrl->SetPos(RenderFrameX + 15, RenderFrameY + 110);
	}
}
...
#if MAIN_UPDATE > 603
	LoadBitmap("Interface\\HUD_TEST\\newui_bt_mix.dds", IMAGE_MIXINVENTORY_MIXBTN, GL_LINEAR);
#else
...
{
	float fPos_x = m_Pos.x + (INVENTORY_WIDTH - 160) / 2;
	float fPos_y = m_Pos.y;
	float fLine_y = 13.0f;
...
bool CNewUIMixInventory::Mix()
{
	...
	if (g_MixRecipeMgr.GetMixInventoryType() == SEASON3A::MIXTYPE_ATTACH_SOCKET)
	{
		...
		else if (iSelectedLine > g_MixRecipeMgr.GetFirstItemSocketCount() || g_MixRecipeMgr.GetFirstItemSocketSeedID(iSelectedLine) != SOCKET_EMPTY)
		{
...
bool CNewUIMixInventory::InventoryProcess()
{
	...
	if (m_pNewInventoryCtrl)
	{
		if (pPickedItem)
		{
			...
			if (GetMixState() == MIX_READY && g_MixRecipeMgr.IsMixSource(pPickedItem->GetItem()) && InvenIndex != -1)
			{
				...
				if (SEASON3B::IsPress(VK_LBUTTON))
				{
					...
					{
						if (SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT_INDEX + iSourceIndex, pItemObj, iCurInventory, iTargetIndex))
						{
							return true;
						}
					}
			...
			else if (GetMixState() == MIX_READY && g_MixRecipeMgr.IsMixSource(pPickedItem->GetItem()) && pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
			{
				m_pNewInventoryCtrl->SetSquareColorNormal(m_fInventoryColor[0], m_fInventoryColor[1], m_fInventoryColor[2]);
				if (SEASON3B::IsPress(VK_LBUTTON))
				{
		...
		{
			return g_pMyInventory->MoveItemProcess();
		}
	}
	return false;
}
...
void CNewUIMixInventory::RenderMixEffect()
{
	...
	for (int i = 0; i < (int)m_pNewInventoryCtrl->GetNumberOfItems(); ++i)
	{
		int iWidth = GMItemMng->find(m_pNewInventoryCtrl->GetItem(i)->Type)->Width;
		int iHeight = GMItemMng->find(m_pNewInventoryCtrl->GetItem(i)->Type)->Height;
