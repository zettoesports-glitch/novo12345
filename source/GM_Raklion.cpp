...
bool CGM_Raklion::MoveMonsterVisual(OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 145:
	{
		...
		case MONSTER01_ATTACK2:
		{
			if (o->AnimationFrame > 4.4f && o->AnimationFrame < 4.7f)
			{
				...
				*/
				...
				CreateEffectSync(MODEL_STREAMOFICEBREATH, v3Pos, o->Angle, o->Light, 0, 0, -1, 0, 0, 0, 0.2f, -1);
			}
		}
		break;
		case MONSTER01_DIE:
		{
			...
			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, o->Light, 3, Scale);
			b->TransformByObjectBone(vPos, o, 79);
			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, o->Light, 53, Scale);
		}
		break;
		default:
		{
	...
	case MODEL_MONSTER01 + 146:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			if (o->AnimationFrame >= 8.0f && o->AnimationFrame < (8.0f + fActionSpeed))
			{
				...
				CreateParticleSync(BITMAP_CLUD64, EndPos, o->Angle, Light, 7, 2.0f);
				CreateParticleSync(BITMAP_CLUD64, EndPos, o->Angle, Light, 7, 2.0f);
				Vector(0.9f, 0.9f, 0.9f, Light);
				CreateParticleSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 8);
				CreateParticleSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 13);
				CreateParticleSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 13);

				for (int iu = 0; iu < 60; iu++)
				{
					Vector(0.4f, 0.8f, 0.9f, Light);
					CreateEffectSync(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, EndPos, o->Angle, Light, 0);
				}
			}
		}
		else
	...
	case MODEL_MONSTER01 + 148:
	{
		if (o->AnimationFrame >= 1.7f && o->AnimationFrame <= 2.0f)
		{
			...
			case MONSTER01_ATTACK1:
			{
				...
				CreateEffectSync(MODEL_1_STREAMBREATHFIRE, vPos, o->Angle, vLight, 0, 0, -1, 0, 0, 0, fSize, -1);
			}
			break;
			case MONSTER01_ATTACK2:
			{
		...
		if (o->AnimationFrame <= 8.0f)
		{
			...
			case MONSTER01_DIE:
			{
				...
				if (rand() % 3 != 0)
				{
					for (int i = 0; i < b->NumBones; ++i)
					{
						...
						if (rand() % 5 == 0)
						{
							...
							CreateParticleSync(BITMAP_WATERFALL_5, vPos, o->Angle, vLight, 8, 2.0f);
						}
						if (rand() % 5 == 0)
						{
							Vector(0.1f, 1.f, 0.1f, vLight);
							CreateParticleSync(BITMAP_WATERFALL_3, vPos, o->Angle, vLight, 8, 2.5f);
						}
					}
				}
				...
				if (rand() % 3 == 0)
				{
					...
					CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 1, 0.5f);
					CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 24, 1.25f);

					VectorCopy(o->Position, vPos);
					vPos[0] += (float)(rand() % 250 - 125);
					vPos[1] += (float)(rand() % 250 - 125);
					Vector(0.1f, 1.0f, 0.1f, vLight);
					CreateEffectSync(BITMAP_CLOUD, vPos, o->Angle, vLight, 0, NULL, -1, 0, 0, 0, 1.0f);

					Vector(1.0f, 1.0f, 1.0f, vLight);
					vPos[2] += 50.f;
					CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 5, 0.75f);
				}
	...
	case MODEL_MONSTER01 + 150:
	{
		if (o->CurrentAction == MONSTER01_ATTACK1)
		{
			...
			if (o->AnimationFrame >= 3.7f && o->AnimationFrame <= 4.f)
			{
				...
				VectorCopy(o->Position, vPos);
				vPos[1] += 30.f;
				CreateEffectSync(MODEL_RAKLION_BOSS_MAGIC, vPos, o->Angle, vLight, 0, o, -1, 0, 0, 0, 1.5f);
				...
				for (int i = 0; i < 20; ++i)
				{
					...
					CreateEffectSync(iIndex, vPos, o->Angle, vLight, 1, NULL, -1, 0, 0, 0, fScale);
				}
			}
		}
		else if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_WATERFALL_3, vPos, o->Angle, vLight, 11, 2.f);

			CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 52, 2.f);
			...
			if (o->AnimationFrame >= 6.8f && o->AnimationFrame <= 7.f)
			{
				...
				for (int i = 0; i < 5; ++i)
				{
					...
					CreateEffectSync(MODEL_MOONHARVEST_MOON, vPos, vDirection2, vLight, 1, NULL, -1, 0, 0, 0, 0.4f);
				}
			}
		}
		else if (o->CurrentAction == MONSTER01_ATTACK3)
		{
			...
			{
				CreateEffectSync(BITMAP_DAMAGE_01_MONO, o->Position, o->Angle, vLight, 0);
				...
				for (int i = 0; i < 3; ++i)
				{
					...
					CreateEffectSync(BITMAP_FIRE_HIK2_MONO, vPos2, o->Angle, vLight, 0, o);
				}

				Vector(0.0f, 0.0f, 1.f, vLight);
				CreateEffectSync(BITMAP_FIRE_HIK2_MONO, o->Position, o->Angle, vLight, 1, o);

				Vector(0.1f, 0.2f, 1.f, vLight);
				CreateEffectSync(MODEL_RAKLION_BOSS_CRACKEFFECT, o->Position, o->Angle, vLight, 0, o, -1, 0, 0, 0, 2.f);
			}
		}
		else if (o->CurrentAction == MONSTER01_APEAR)
		{
			...
			if (o->AnimationFrame <= 3.f)
			{
				...
				for (int i = 0; i < 1; ++i)
				{
					...
					CreateEffectSync(iIndex, vPos, o->Angle, vLight, 2, NULL, -1, 0, 0, 0, fScale);
					...
					CreateEffectSync(MODEL_FALL_STONE_EFFECT, vPos, o->Angle, vLight, 2, NULL, -1, 0, 0, 0, fScale);

					Vector(0.7f, 0.7f, 0.8f, vLight);
					vPos[0] += (float)(rand() % 80 - 40);
					vPos[1] += (float)(rand() % 80 - 40);
					CreateParticleSync(BITMAP_WATERFALL_3 + (rand() % 2), vPos, o->Angle, vLight, 2);
				}
			}
		...
		else if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame <= 8.f)
		{
			...
			if (rand() % 3 == 0)
			{
				...
				Vector(1.0f, 1.f, 1.f, vLight);
				CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 1, 1.f);
				CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 24, 2.5f);
				...
				Vector(1.0f, 1.0f, 1.0f, vLight);
				vPos[2] += 50.f;
				CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 5, 1.5f);
			}
		}
		...
		if (m_byDetailState >= BATTLE_OF_SELUPAN_PATTERN_3 && m_byDetailState <= BATTLE_OF_SELUPAN_PATTERN_7)
		{
			...
			for (int k = 0; k < 1; ++k)
			{
				if (rand() % 2 == 0)
				{
					...
					CreateParticleSync(BITMAP_SMOKE, vPos1, o->Angle, vLight, 50, fScale);
					CreateParticleSync(BITMAP_SMOKELINE1 + rand() % 3, vPos1, o->Angle, vLight, 0, fScale);

					b->TransformByObjectBone(vPos2, o, 52);
					CreateParticleSync(BITMAP_SMOKE, vPos2, o->Angle, vLight, 50, fScale);
					CreateParticleSync(BITMAP_SMOKELINE1 + rand() % 3, vPos2, o->Angle, vLight, 0, fScale);

					if (m_byDetailState == BATTLE_OF_SELUPAN_PATTERN_6 || m_byDetailState == BATTLE_OF_SELUPAN_PATTERN_7)
					{
						CreateParticleSync(BITMAP_FIRE_HIK1_MONO, vPos1, o->Angle, vLight, 0, o->Scale);
						CreateParticleSync(BITMAP_FIRE_HIK1_MONO, vPos2, o->Angle, vLight, 0, o->Scale);
					}
				}
			}
		}
	...
	case MODEL_MONSTER01 + 153:
	{
		if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame <= 12.f)
		{
			...
			for (int i = 0; i < 1; ++i)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 11, 1.2f);
	...
	case MODEL_MONSTER01 + 205:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			if (o->AnimationFrame >= 8.0f && o->AnimationFrame < (8.0f + fActionSpeed))
			{
				...
				CreateParticleSync(BITMAP_CLUD64, EndPos, o->Angle, Light, 7, 2.0f);
				Vector(0.9f, 0.9f, 0.9f, Light);
				CreateEffectSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 8);
				CreateEffectSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 13);
				CreateEffectSync(BITMAP_SHOCK_WAVE, EndPos, o->Angle, Light, 13);
...
bool CGM_Raklion::RenderObjectMesh(OBJECT* o, BMD* b, bool ExtraMon)
{
		...
		if (o->CurrentAction == 0)
		{
			...
			if ((o->AnimationFrame >= 2 && o->AnimationFrame <= 5))
			{
				for (int i = 0; i < b->NumBones; ++i)
				{
					...
					CreateParticleSync(BITMAP_WATERFALL_5, vWorldPos, o->Angle, vLight, 7);
				}
			}
			...
			{
				b->TransformPosition(BoneTransform[i], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_3, vWorldPos, o->Angle, vLight, 8, 1.5f);
			}
		}
	...
	else if (o->Type == 20)
	{
		...
		if (o->CurrentAction == 0)
		{
			...
			CreateParticleSync(BITMAP_WATERFALL_3, vWorldPos, o->Angle, vLight, 8, 2.f);
			...
			if (o->AnimationFrame <= 8)
			{
				...
				b->TransformPosition(BoneTransform[6], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_3, vWorldPos, o->Angle, vLight, 8, 1.5f);
				CreateParticleSync(BITMAP_WATERFALL_5, vWorldPos, o->Angle, vLight, 7, 0.1f);
			...
			if (o->AnimationFrame <= 15)
			{
				// 날개
				b->TransformPosition(BoneTransform[8], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_3, vWorldPos, o->Angle, vLight, 9, 2.f);
				CreateParticleSync(BITMAP_WATERFALL_5, vWorldPos, o->Angle, vLight, 7, 0.1f);
	...
	else if (o->Type == 21)
	{
		...
		if (o->CurrentAction == 0)
		{
			if (o->AnimationFrame >= 4 && o->AnimationFrame <= 8)
			{
				...
				b->TransformPosition(BoneTransform[7], vRelativePos, vWorldPos, false);
				// 물 이펙트
				CreateParticleSync(BITMAP_WATERFALL_3, vWorldPos, o->Angle, vLight, 9, 0.5f);
				CreateParticleSync(BITMAP_WATERFALL_5, vWorldPos, o->Angle, vLight, 7);

				// 7, 16, 17, 21, 22
				// 연기 이펙트
				b->TransformPosition(BoneTransform[7], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_2, vWorldPos, o->Angle, vLight, 5, 1.f);
				b->TransformPosition(BoneTransform[16], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_2, vWorldPos, o->Angle, vLight, 5, 0.5f);
				b->TransformPosition(BoneTransform[17], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_2, vWorldPos, o->Angle, vLight, 5, 0.7f);
				b->TransformPosition(BoneTransform[20], vRelativePos, vWorldPos, false);
				CreateParticleSync(BITMAP_WATERFALL_2, vWorldPos, o->Angle, vLight, 5, 0.3f);
		...
		else
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				if (o->Type == MODEL_MONSTER01 + 151)
				{
					...
					if (rand() % 100 == 0)
						CreateParticleSync(BITMAP_SMOKE, vPos, o->Angle, vLight, 33, 1.f);
				}
...
bool CGM_Raklion::RenderObjectVisual(OBJECT* o, BMD* b)
{
	...
	case 70:
	{
		...
		{
		case 0:
			CreateParticleSync(BITMAP_FIRE_HIK1_MONO, o->Position, o->Angle, vLight, 2, o->Scale);
			break;
		case 1:
			CreateParticleSync(BITMAP_FIRE_HIK2_MONO, o->Position, o->Angle, vLight, 6, o->Scale);
			break;
		case 2:
			CreateParticleSync(BITMAP_FIRE_HIK3_MONO, o->Position, o->Angle, vLight, 2, o->Scale);
			break;
		}
		CreateParticleSync(BITMAP_FIRE_HIK1_MONO, o->Position, o->Angle, vLight, 2, o->Scale);
...
bool CGM_Raklion::RenderMonsterVisual(CHARACTER* c, OBJECT* o, BMD* b)
{
	...
	case MODEL_MONSTER01 + 149:
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			CreateParticleSync(BITMAP_SMOKELINE1 + rand() % 3, vPos, o->Angle, vLight, 2, 0.8f);
		}
		...
		if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			if (c->AttackTime > 6 && c->AttackTime <= 12)
			{
				...
				float fDistance = 100.0f + (float)(c->AttackTime._runvalue - 8) * 10.0f;
		...
		else if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame < 1.0f)
		{
			...
			for (int i = 0; i < 4; ++i)
			{
				...
				b->TransformByObjectBone(vPos, o, iBones[i]);
				for (int j = 0; j < 6; ++j)
					CreateEffectSync(MODEL_EFFECT_BROKEN_ICE0 + rand() % 3, vPos, o->Angle, vLight, 0);
			}
		}
	}
	break;
	case MODEL_MONSTER01 + 150:
	{
		...
		CreateParticleSync(BITMAP_SPARK + 1, vPos, o->Angle, vLight, 11, 1.5f);
	...
	case MODEL_MONSTER01 + 208:
	{
		...
		if (o->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			if (c->AttackTime >= 12)
			{
				...
				CreateEffectSync(MODEL_COMBO, o->Position, o->Angle, o->Light);
				//				PlayBuffer(SOUND_COMBO);
			}
		}
		else if (o->CurrentAction == MONSTER01_DIE && o->AnimationFrame < 1.0f)
...
void CGM_Raklion::RenderBaseSmoke()
{
	...
	RenderBitmapUV(BITMAP_CHROME + 3, 0.f, 0.f, gwinhandle->GetScreenX(), gwinhandle->GetScreenY(), WindX2, WindY2, 3.0f, 2.0f);
...
bool IsIceCity()
{
	if (gMapManager->currentMap >= WD_57ICECITY && gMapManager->currentMap <= WD_58ICECITY_BOSS)
	{
		return true;
	}
	else if (gMapManager->currentMap == WD_65DOPPLEGANGER1)
	{
		return true;
	}
...
void CGM_Raklion::SetBossMonsterAction(CHARACTER* c, OBJECT* o)
{
	...
	else if (c->MonsterSkill == 39)
	{
		...
		for (int i = 0; i < 20; ++i)
		{
			...
			vPos[1] += rand() % 400 - 200;
			vPos[2] -= 100.f;
			CreateJointSync(BITMAP_FLARE, vPos, vPos, vAngle, 2, NULL, 40);
		}

		Vector(1.f, 0.5f, 0.1f, vLight);
		CreateEffectSync(BITMAP_MAGIC + 1, o->Position, o->Angle, vLight, 13, o, -1, 0, 0, 0, 8.f);
		CreateEffectSync(BITMAP_MAGIC + 1, o->Position, o->Angle, vLight, 13, o, -1, 0, 0, 0, 8.f);
	}
	...
	else if (c->MonsterSkill == 42)
	{
		...
		CreateEffectSync(MODEL_STORM2, o->Position, o->Angle, vLight, 1, NULL, -1, 0, 0, 0, 1.6f);
		CreateEffectSync(MODEL_SUMMON, o->Position, o->Angle, vLight, 3);
	}
	else
	{
		SetAction(o, MONSTER01_ATTACK4);
...
{
	if (gMapManager->currentMap == WD_57ICECITY)
	{
	...
	if (gMapManager->currentMap == WD_58ICECITY_BOSS && m_bMusicBossMap == true)
	{
