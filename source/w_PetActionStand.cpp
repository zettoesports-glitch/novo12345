...
bool PetActionStand::Move(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	float Speed = (FlyRange * FlyRange >= Distance) ? 0 : (float)log(Distance) + 5.f;
...
bool PetActionStand::Effect(OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender)
{
	...
	float fRad1 = ((3.14f / 3000.0f) * fmodf(tick , 3000));
	float fRad2 = ((3.14f / 3000.0f) * fmodf((tick + 1500) , 3000));
	float tempLight;
	...
	Position[2] -= 25.0f;
	CreateEffectSync(MODEL_FEATHER_FOREIGN, Position, obj->Angle, Light, 4, NULL, -1, 0, 0, 0, 0.3f);
