...
bool SEASON3B::CNewUIPickedItem::Create(CNewUIItemMng* pNewItemMng, CNewUIInventoryCtrl* pSrc, ITEM* pItem)
{
	...
	Script_Item* pItemAttr = GMItemMng->find(m_pPickedItem->Type);
...
int SEASON3B::CNewUIPickedItem::GetSourceLinealPos()
{
	if (m_pSrcInventory)
	{
		return m_pSrcInventory->GetNumberOfColumn() * m_pPickedItem->y + m_pPickedItem->x;
	}
...
void SEASON3B::CNewUIInventoryCtrl::Init()
{
	...
	m_afColorStateNormal[3] = 0.4f;
	m_afColorStateWarning[3] = 0.4f;
...
{
#if MAIN_UPDATE > 603
	LoadBitmap("Interface\\HUD_TEST\\item_box.dds", BITMAP_item_box);
#else
	...
	LoadBitmap("Interface\\newui_item_table03(Up).tga", IMAGE_ITEM_TABLE_TOP_PIXEL, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_item_table03(Dw).tga", IMAGE_ITEM_TABLE_BOTTOM_PIXEL, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_item_table03(L).tga", IMAGE_ITEM_TABLE_LEFT_PIXEL, GL_LINEAR, GL_REPEAT);
	LoadBitmap("Interface\\newui_item_table03(R).tga", IMAGE_ITEM_TABLE_RIGHT_PIXEL, GL_LINEAR, GL_REPEAT);
...
bool SEASON3B::CNewUIInventoryCtrl::Create(CNewUI3DRenderMng* pNew3DRenderMng, CNewUIItemMng* pNewItemMng, CNewUIObj* pOwner, int x, int y, int nColumn, int nRow)
{
	...
	m_pNew3DRenderMng->Add3DRenderObj(this, TOOLTIP_CAMERA_Z_ORDER);
...
bool SEASON3B::CNewUIInventoryCtrl::AddItem(int iColumnX, int iRowY, BYTE* pbyItemPacket)
{
	...
	Script_Item* pItemAttr = GMItemMng->find(pNewItem->Type);
	pNewItem->x = iColumnX;
	pNewItem->y = iRowY;
...
int SEASON3B::CNewUIInventoryCtrl::FindItemReverseKind2(short sType, int iLevel)
{
...
int SEASON3B::CNewUIInventoryCtrl::FindEmptySlot(ITEM* pItem)
{
	Script_Item* pItemAttr = GMItemMng->find(pItem->Type);

	return FindEmptySlot(pItemAttr->Width, pItemAttr->Height);
}
...
bool SEASON3B::CNewUIInventoryCtrl::UpdateMouseEvent()
{
		...
		)
	{
		...
		&& m_iPointedSquareIndex != -1
		&& NULL == GetPickedItem()
		&& (m_pdwItemCheckBox[m_iPointedSquareIndex] > 1))
	{
		if (g_pNewUIMng)
		{
			ITEM* pItem = FindItemByKey(m_pdwItemCheckBox[m_iPointedSquareIndex]);
			...
			if (pItem != m_pToolTipItem)
			{
				...
				if ((pItem->Type == ITEM_HELPER + 4) || (pItem->Type == ITEM_HELPER + 5) || pItem->Kind3 == ItemKind3::PET_GAIN_EXPERIENCE)
				{
					Script_Item* pItemAttr = GMItemMng->find(m_pToolTipItem->Type);

					int iTargetX = m_Pos.x + m_pToolTipItem->x * INVENTORY_SQUARE_WIDTH + pItemAttr->Getwidth() / 2;
...
void SEASON3B::CNewUIInventoryCtrl::Render(bool RendOption)
{
	int x, y;
	for (y = 0; y < m_nRow; y++)
	{
		for (x = 0; x < m_nColumn; x++)
		{
			...
			if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
			{
				...
				Height = GMItemMng->find(pItem->Type)->Height;
			}

			if (pItem && pItem->Durability == 254 && g_pMyInventory->IsInvenItem(pItem->Type))
			{
				if (Height == 1)
				{
					RenderImage(IMAGE_ITEM_SQUARE_FOR_1_BY_1, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21);
				}
				else if (y != m_nRow - 1 && pItem != FindItem(x, y - 1))
				{
					RenderImage(IMAGE_ITEM_SQUARE_TOP_RECT, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21.0, 21.0);
				}
				else
				{
					RenderImage(IMAGE_ITEM_SQUARE_BOTTOM_RECT, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21.0, 21.0);
				}
			}
			else
			{
				RenderImageF(BITMAP_item_box, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 20, 20, 0.0, 0.0, 75.0, 78.0);
			}
			...
			{
				EnableAlphaTest();
				DWORD color = RGBA(255, 255, 255, 255);
				if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
				{

					ITEM* pItem = FindItemByKey(m_pdwItemCheckBox[iCurSquareIndex]);

					RenderImage(BITMAP_INVENTORY + 18, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), INVENTORY_SQUARE_WIDTH, INVENTORY_SQUARE_HEIGHT, 0.0, 0.0, 0.625, 0.66666669, color);
					...
					if (pItem)
					{
						...
						if (pItem->byColorState)
						{
							...
							if (pItem->byColorState >= ITEM_COLOR_DURABILITY_50 && pItem->byColorState <= ITEM_COLOR_TRADE_WARNING)
							{
								RenderColor(m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), INVENTORY_SQUARE_WIDTH, INVENTORY_SQUARE_HEIGHT);
								EndRenderColor();
							}
					...
					//RenderBitmap(BITMAP_INVENTORY + 18, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), 21, 21, 0.0, 0.0, 1.0, 1.0);
				}
				else
				{
					RenderImage(BITMAP_INVENTORY + 17, m_Pos.x + (x * INVENTORY_SQUARE_WIDTH), m_Pos.y + (y * INVENTORY_SQUARE_HEIGHT), INVENTORY_SQUARE_WIDTH, INVENTORY_SQUARE_HEIGHT, 0.0, 0.0, 1.0, 1.0, color);
				}
			}
#else
			EnableAlphaTest();
	...
	if ((gmProtect->LookAndFeel != 1 && gmProtect->LookAndFeel != 2) && RendOption)
	{
		...
		x = m_Pos.x - WND_LEFT_EDGE + 14;
		y = m_Pos.x + m_Size.cx - WND_RIGHT_EDGE;
		RenderImage(IMAGE_ITEM_TABLE_TOP_PIXEL, x, m_Pos.y - WND_TOP_EDGE, (y - x), 14);
		RenderImage(IMAGE_ITEM_TABLE_BOTTOM_PIXEL, x, m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE, (y - x), 14);

		y = m_Pos.y - WND_TOP_EDGE + 14;
		x = m_Pos.y + m_Size.cy - WND_BOTTOM_EDGE;
		RenderImage(IMAGE_ITEM_TABLE_LEFT_PIXEL, m_Pos.x - WND_LEFT_EDGE, y, 14, x - y);
		RenderImage(IMAGE_ITEM_TABLE_RIGHT_PIXEL, m_Pos.x + m_Size.cx - WND_RIGHT_EDGE, y, 14, x - y);
	...
	if (ms_pPickedItem)
	{
		...
		if (pickitemvisible)
		{
			...
			if (IntersectRect(&rcIntersect, &rcPickedItem, &rcInventory))
			{
				...
				Script_Item* pItemAttr = GMItemMng->find(pPickItem->Type);
				int iPickedItemX = MouseX - ((pItemAttr->Width - 1) * INVENTORY_SQUARE_WIDTH / 2);
				int iPickedItemY = MouseY - ((pItemAttr->Height - 1) * INVENTORY_SQUARE_HEIGHT / 2);
						...
						for (int y = 0; y < nItemRow; y++)
						{
							for (int x = 0; x < nItemColumn; x++)
							{
								...
								if (m_pdwItemCheckBox[iCurSquareIndex] > 1)
								{
									...
									if (pTargetItem)
									{
										...
										else if (pPickItem->Type == ITEM_POTION + 42)
										{
											if (pTargetItem->Jewel_Of_Harmony_Option == 0)
											{
												...
												if ((strengthitem != SI_None) && (!g_SocketItemMgr.IsSocketItem(pTargetItem)) && ((pTargetItem->ExtOption % 0x04) != EXT_A_SET_OPTION && (pTargetItem->ExtOption % 0x04) != EXT_B_SET_OPTION))
												{
													bSuccess = true;
												}
											}
										...
										else if (GMJewelOfAction->CheckOfJewelReinforce(pPickItem->Type, pTargetItem))
										{
											bSuccess = true;
										}
									...
									glColor4fv(m_afColorStateWarning);
								}
								else
								{
									glColor4fv(m_afColorStateNormal);
								}
	...
	if (m_pNew3DRenderMng)
	{
		...
		if (m_pToolTipItem && GetPickedItem() == NULL)
		{
			if (tooltipvisible)
			{
				m_pNew3DRenderMng->RenderUI2DEffect(TOOLTIP_CAMERA_Z_ORDER, UI2DEffectCallback, this, RENDER_ITEM_TOOLTIP, 0);
			}
		}
	}
}
...
void SEASON3B::CNewUIInventoryCtrl::RenderItemToolTip()
{
	if (m_pToolTipItem)
	{
		...
		{
			g_pNewItemTooltip->RenderItemTooltip(iTargetX, iTargetY, m_pToolTipItem, false, UNKNOWN_TOOLTIP_TYPE);
		}
		...
		{
			g_pNewItemTooltip->RenderItemTooltip(iTargetX, iTargetY, m_pToolTipItem, true, UNKNOWN_TOOLTIP_TYPE);
		}
		else if (m_ToolTipType == TOOLTIP_TYPE_MY_SHOP)
		{
			g_pNewItemTooltip->RenderItemTooltip(iTargetX, iTargetY, m_pToolTipItem, false, TOOLTIP_TYPE_MY_SHOP);
		}
		else if (m_ToolTipType == TOOLTIP_TYPE_PURCHASE_SHOP)
		{
			g_pNewItemTooltip->RenderItemTooltip(iTargetX, iTargetY, m_pToolTipItem, false, TOOLTIP_TYPE_PURCHASE_SHOP);
		}
	}
}
...
void SEASON3B::CNewUIInventoryCtrl::BackupPickedItem()
{
	if (ms_pPickedItem && EquipmentItem == false)
	{
		...
		{
			ITEM* pEquipmentItemSlot = CharacterMachine->GetOnlyEquipment(pItemObj->lineal_pos);
			memcpy(pEquipmentItemSlot, pItemObj, sizeof(ITEM));

			g_pMyInventory->CreateEquippingEffect(pItemObj->lineal_pos, pEquipmentItemSlot);
			...
			if ((pEquipmentItemSlot->Type == ITEM_HELPER + 4 || pEquipmentItemSlot->Kind3 == ItemKind3::PET_GAIN_EXPERIENCE) && !gMapManager->InChaosCastle())
			{
...
bool SEASON3B::CNewUIInventoryCtrl::IsOverlayItem(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if (iSrcType == iTarType)
	{
		...
		else if ((iSrcType >= ITEM_POTION && iSrcType <= ITEM_POTION + 8 && iSrcType != ITEM_POTION + 7) && (iTarType >= ITEM_POTION && iTarType <= ITEM_POTION + 8 && iTarType != ITEM_POTION + 7) && (iSrcDurability < 255 && iTarDurability < 255))
		{
			return true;
		}
		if ((iSrcType >= ITEM_POTION + 38 && iSrcType <= ITEM_POTION + 40) && (iTarType >= ITEM_POTION + 38 && iTarType <= ITEM_POTION + 40) && (iSrcDurability < 255 && iTarDurability < 255))
		{
			return true;
		}
		...
		{
			CUSTOM_ITEM_STACK* StackItem = GMProtect->GetItemStack(iSrcType, ((iSrcLevel >> 3) & 15));

			if (StackItem != NULL)
			{
				if ((StackItem->Level != -1 && iSrcLevel != iTarLevel) || (iSrcDurability < 1 || iTarDurability >= StackItem->MaxStack))
				{
					return false;
				}
...
bool SEASON3B::CNewUIInventoryCtrl::CanUpgradeItem(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if (((pTargetItem->Type >= ITEM_SWORD && pTargetItem->Type < ITEM_WING)
		...
		|| pTargetItem->Kind1 == ItemKind1::WINGS
		)
	{
		if ((pSourceItem->Type == ITEM_POTION + 13) && (iTargetLevel >= 0 && iTargetLevel <= 5))
		{
			return true;
		}
		else if ((pSourceItem->Type == ITEM_POTION + 14) && (iTargetLevel >= 0 && iTargetLevel <= 8))
		{
			return true;
		}
	}
