...
CNewUIItemEnduranceInfo::CNewUIItemEnduranceInfo()
{
	...
	m_iTooltipIndex = -1;
	// Corner radar disabled — only TAB full minimap (INTERFACE_MINI_MAP) is used
	m_iHideMap = 1;
}
...
{
	// Corner radar is permanently off; keep layout as "hidden"
	(void)success;
	m_iHideMap = 1;
}
...
{
	// Bottom-left HUD: plain text, tight spacing, color by quality
	(void)RenderSizeX;
	unicode::t_char szDebugText[128];

	const float fps = g_FpsCounter.currentFPS;
	const int ping = gsteady_clock->get_ping_time(); // ms round-trip (real probe 0x71)

	// FPS: green >= 55, yellow >= 30, red below
	BYTE fr = 255, fg = 80, fb = 80;
	if (fps >= 55.f) { fr = 80; fg = 220; fb = 100; }
	else if (fps >= 30.f) { fr = 255; fg = 210; fb = 60; }

	// Ping: green <= 80ms, yellow <= 150ms, red above
	BYTE pr = 255, pg = 80, pb = 80;
	if (ping <= 80) { pr = 80; pg = 220; pb = 100; }
	else if (ping <= 150) { pr = 255; pg = 210; pb = 60; }

	g_pRenderText->SetFont(g_hFontBold);
	g_pRenderText->SetBgColor(0);

	// Draw FPS then Ping with a clear gap
	unicode::_sprintf(szDebugText, "FPS:%.0f", fps);
	g_pRenderText->SetTextColor(fr, fg, fb, 255);
	g_pRenderText->RenderFont(x, y, szDebugText, 0, 15.0, RT3_SORT_LEFT);

	unicode::_sprintf(szDebugText, "  Ping:%dms", ping);
	g_pRenderText->SetTextColor(pr, pg, pb, 255);
	// Fixed slot wide enough for "FPS:999" + space (~58px on bold font)
	g_pRenderText->RenderFont(x + 58.f, y, szDebugText, 0, 15.0, RT3_SORT_LEFT);
}

void SEASON3B::CNewUIItemEnduranceInfo::runtime_render_map()
{
	// Bottom-left HUD — use 640x480 UI space (NOT raw pixel Y).
	// Old hardcoded Y=672 was off-screen: RenderFont multiplies by g_fScreenRate_y
	// (WindowHeight/480), so 672 becomes ~1.4x the window height.
	EnableAlphaTest();
	glColor3f(1.f, 1.f, 1.f);
	// X=12, Y near bottom of virtual screen (works at any resolution / ScreenType)
	runtime_render_fps(12.f, pos_botton(458.f), 0.f);
}

bool SEASON3B::CNewUIItemEnduranceInfo::UpdateMouseWorld()
{
	// No corner-map hitbox / zoom / click-to-move — use TAB minimap only
	return false;
}
