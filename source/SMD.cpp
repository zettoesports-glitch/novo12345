...
{ 
	...
	{
#ifdef _DEBUG
		extern HWND gwinhandle->GethWnd();
		char Text[1024];
...
{
	if(gmClientModels->GetModel(ID)->NumMeshs > 0) return false;

	if(OpenSMDFile(FileName1,REFERENCE_FRAME,Flip))
	{
		strcpy(gmClientModels->GetModel(ID)->Name,FileName1);
		gmClientModels->GetModel(ID)->Version = 10;
		FixupSMD();
		//gmClientModels->GetModel(ID)->Version = 11;
		//Triangle2Strip();
...
{
	if(gmClientModels->GetModel(ID)->NumBones > 0)
	{
