...
{
	return (gMapManager->currentMap == WD_33AIDA || gMapManager->currentMap == WD_54CHARACTERSCENE) ? true : false;
}
...
bool M33Aida::RenderAidaObjectVisual(OBJECT* pObject, BMD* pModel)
{
	...
	case 59:
		if (pObject->HiddenMesh != -2)
		{
			...
			for (int i = 0; i < 20; ++i)
			{
				CreateParticleSync(BITMAP_CLOUD, pObject->Position, pObject->Angle, Light, 1, pObject->Scale, pObject);
			}
		}
		break;
	case 60:
	...
	case 67:
		...
		CreateParticleSync(BITMAP_FLAME, pObject->Position, pObject->Angle, pObject->Light, 7, pObject->Scale);
		break;
	case 70:
	{
		int time = timeGetTime() % 1024;
		if (rand() % 5 == 1 && (time >= 0 && time < 30))
		{
			...
			CreateEffectSync(MODEL_GHOST, pObject->Position, pObject->Angle, Light, 0, pObject, -1, 0, pObject->Scale);
		}
	}
	break;
	case 71:
...
bool M33Aida::RenderAidaMonsterVisual(CHARACTER* pCharacter, OBJECT* pObject, BMD* pModel)
{
	...
	case MODEL_MONSTER01 + 100:
	{
		...
		CreateParticleSync(BITMAP_LIGHT + 1, Position, Angle, Light, 4, 4.0f);
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK1 || pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			if (pObject->AnimationFrame >= 4.0f)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, Position, Angle, Light, 27, 2.0f);
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticleSync(BITMAP_LIGHT + 1, Position, Angle, Light, 2, 1.0f);
			}
			if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK1)
			{
				...
				BoneManager::GetBonePosition(pObject, "Monster100_Footstepst", Relative, Position);
				Vector(1.0f, 1.0f, 1.0f, Light);
				CreateParticleSync(BITMAP_LIGHT + 1, Position, Angle, Light, 3, 1.3f);
			}
			if (pCharacter->TargetCharacter >= 0 && pCharacter->TargetCharacter < MAX_CHARACTERS_CLIENT)
			{
				CHARACTER* tc = gmCharacters->GetCharacter(pCharacter->TargetCharacter);
				OBJECT* to = &tc->Object;
				...
				if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK1)
				{
					CreateParticleSync(BITMAP_LIGHT + 1, vTemp, Angle, Light, 3, 1.3f);
				}
				if (pObject->AnimationFrame >= 6.0f && pObject->CurrentAction == MONSTER01_ATTACK2)
				{
					CreateJointSync(BITMAP_JOINT_THUNDER, to->Position, vTemp, pObject->Angle, 16);
				}
			}
		}
	...
	case MODEL_MONSTER01 + 101:
	{
		...
		CreateParticleSync(BITMAP_WATERFALL_2, Position, Angle, Light, 1);
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK1)
		{
			...
			if (pObject->AnimationFrame >= 4.0f)
			{
				...
				CreateParticleSync(BITMAP_SMOKE, Position, Angle, Light, 25);

				if (pObject->AnimationFrame >= 5.0f && pObject->AnimationFrame <= 5.5f)
				{
					Vector(5.0f, 5.0f, 5.0f, Light);
					BoneManager::GetBonePosition(pObject, "Monster101_L_Arm", Relative, Position);
					CreateParticleSync(BITMAP_SHOCK_WAVE, Position, Angle, Light, 3, 0.5f);
				}
			}
			...
			if (pCharacter->TargetCharacter >= 0 && pCharacter->TargetCharacter < MAX_CHARACTERS_CLIENT && pObject->AnimationFrame >= 6.9f)
			{
				...
				CreateJointSync(BITMAP_JOINT_ENERGY, vTemp, pObject->Position, to->Angle, 16, pObject, 20.0f);
			}
		}
	...
	case MODEL_MONSTER01 + 102:
	{
		...
		CreateParticleSync(BITMAP_SPARK + 1, Position, Angle, Light, 7);
	...
	case MODEL_MONSTER01 + 104:
	{
		...
		for (int i = 0; i < 6; i++)
		{
			...
			CreateParticleSync(BITMAP_SPARK + 1, Position, Angle, Light, 6);
		}
		if (pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_SMOKE, Position, pObject->Angle, Light, 26);
		}
	...
	case MODEL_MONSTER01 + 105:
	{
		...
		if (pObject->CurrentAction == MONSTER01_ATTACK2)
		{
			...
			CreateParticleSync(BITMAP_TRUE_FIRE, Position, Angle, Light, 6, 5.0f);
