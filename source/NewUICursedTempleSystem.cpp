...
namespace
{
	...
	{
		float frameX = (gwinhandle->GetScreenX() - 640.0);
		float framey = (gwinhandle->GetScreenY() - 480.0);

		float minmapframeposX = frameX + 464.f;
		float minmapframeposY = framey + 299.f;
...
void SEASON3B::CNewUICursedTempleSystem::SetButtonInfo()
{
	...
	m_Button[CURSEDTEMPLERESULT_ALPH].ChangeButtonInfo(frameX + 513, frameY + 238, 38, 24);
...
bool SEASON3B::CNewUICursedTempleSystem::CheckInventoryHolyItem(CHARACTER* c)
{
	...
	else
	{
		...
		CHARACTER* pc = gmCharacters->GetCharacter(holyitemcharacterindex);
...
void SEASON3B::CNewUICursedTempleSystem::RenderMiniMap()
{
	...
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ALLIED_NPC, npc_x, npc_y, 9.0f, 9.0f, 0.f, 0.f, 9.f / 16.f, 9.f / 16.f);

	npc_x = MiniMapPos(138, 58, m_Scale, AXIS_X);
	npc_y = MiniMapPos(138, 58, m_Scale, AXIS_Y);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ALLIED_HOLYITEM, npc_x, npc_y, 9.0f, 8.0f, 0.f, 0.f, 9.f / 16.f, 8.f / 8.f);

	npc_x = MiniMapPos(192, 113, m_Scale, AXIS_X);
	npc_y = MiniMapPos(192, 113, m_Scale, AXIS_Y);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ILLUSION_HOLYITEM, npc_x, npc_y, 9.0f, 8.0f, 0.f, 0.f, 9.f / 16.f, 8.f / 8.f);

	npc_x = MiniMapPos(193, 126, m_Scale, AXIS_X);
	npc_y = MiniMapPos(193, 126, m_Scale, AXIS_Y);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ILLUSION_NPC, npc_x, npc_y, 9.0f, 9.0f, 0.f, 0.f, 9.f / 16.f, 9.f / 16.f);
	...
	for (int k = 0; k < m_CursedTempleMyTeamCount; ++k)
	{
		...
		if (p->wPartyUserIndex != Hero->Key && p->wPartyUserIndex != m_HolyItemPlayerIndex)
		{
			...
			{
				RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ALLIED_PC, pcX - 3.f, pcY - 3.f, 7.0f, 7.0f, 0.f, 0.f, 7.f / 8.f, 7.f / 8.f);
			}
			else
			{
				RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ILLUSION_PC, pcX - 3.f, pcY - 3.f, 7.0f, 7.0f, 0.f, 0.f, 7.f / 8.f, 7.f / 8.f);
			}
		}
	}
	...
	if (m_HolyItemPlayerIndex != 0xffff && m_HolyItemPlayerIndex != Hero->Key)
	{
		...
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_HOLYITEM_PC, holypcX - 5.f, holypcY - 5.f, 14.0f, 14.0f, 0.f, 0.f, 14.f / 16.f, 14.f / 16.f);
	}
	...
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_HERO, hero_x - 4, hero_y - 4, 11.0f, 11.0f, 0.f, 0.f, 11.f / 16.f, 11.f / 16.f);

	// 알파 값
	RenderNumber2D(frameX + 517.f + 15.f, frameY + 246.f, static_cast<int>(m_Alph * 100), 8, 8);
	// 점수
	RenderNumber2D(frameX + 517.f + 66.f, frameY + 246.f, m_AlliedPoint, 8, 8);
	RenderNumber2D(frameX + 517.f + 110.f, frameY + 246.f, m_IllusionPoint, 8, 8);
...
void SEASON3B::CNewUICursedTempleSystem::RenderScore()
{
	...
	{
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ALLIED_NUMBER + (m_AlliedPoint / 10), frameX + 196, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ALLIED_NUMBER + (m_AlliedPoint % 10), frameX + 253, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
	}
	else
	{
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ALLIED_NUMBER + (m_AlliedPoint % 10), frameX + 224, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
	}

	// :
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_VS1 + (m_IllusionPoint / 10), frameX + 310, frameY + 168.f, 20.0f, 45.0f, 0.f, 0.f, 20.f / 32.f, 45.f / 64.f);

	// 환영교단 점수
	if (m_IllusionPoint / 10 != 0)
	{
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ILLUSION_NUMBER + (m_IllusionPoint / 10), frameX + 331, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ILLUSION_NUMBER + (m_IllusionPoint % 10), frameX + 388, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
	}
	else
	{
		RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ILLUSION_NUMBER + (m_IllusionPoint % 10), frameX + 358, frameY + 160.f, 56.0f, 66.0f, 0.f, 0.f, 56.f / 64.f, 66.f / 128.f);
	}

	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ALLIED_GAAIL, frameX + 232.f, frameY + 115.f, 40.0f, 36.0f, 0.f, 0.f, 40.f / 64.f, 36.f / 64.f);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_VS0, frameX + 292.f, frameY + 123.f, 49.0f, 27.0f, 0.f, 0.f, 49.f / 64.f, 27.f / 32.f);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_ILLUSION_GAAIL, frameX + 367.f, frameY + 115.f, 40.0f, 36.0f, 0.f, 0.f, 40.f / 64.f, 36.f / 64.f);

	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_LEFT, frameX + 133.f, frameY + 115.f, 67.0f, 125.0f, 0.f, 0.f, 67.f / 128.f, 125.f / 128.f);
	RenderBitmap(IMAGE_CURSEDTEMPLESYSTEM_SCORE_RIGHT, frameX + 445.f, frameY + 115.f, 67.0f, 125.0f, 0.f, 0.f, 67.f / 128.f, 125.f / 128.f);
...
void SEASON3B::CNewUICursedTempleSystem::RenderTutorialStep()
{
	...
	float frameX = ((gwinhandle->GetScreenX() - 640.0) / 2.0);
	//float frameY = (gwinhandle->GetScreenY() - 480.0);

	::EnableAlphaTest();
	for (int j = 0; j < TextNum; ++j)
	{
		DrawText(TextList[j], frameX + 140, 50 + (j * 14), TextListColor[j], 0x00000000, RT3_SORT_LEFT, 300, false);
	}
	::DisableAlphaBlend();
}
...
void SEASON3B::CNewUICursedTempleSystem::SetCursedTempleSkill(CHARACTER* c, OBJECT* o, DWORD selectcharacterindex)
{
	...
	if (checktile)
	{
		...
		if (CursedTempleCurSkillType == AT_SKILL_CURSED_TEMPLE_RESTRAINT
			|| CursedTempleCurSkillType == AT_SKILL_CURSED_TEMPLE_SUBLIMATION)
		{
			CHARACTER* pCharacter = gmCharacters->GetCharacter(selectcharacterindex);

			if (pCharacter)
			{
				for (int i = 0; i < m_CursedTempleMyTeamCount; ++i)
				{
					if (m_CursedTempleMyTeam[i].wPartyUserIndex == pCharacter->Key)
					{
						MouseRButtonPush = false;
						return;
					}
			...
			else if (getTargetCharacterKey(c, selectcharacterindex, 0) == -1)
			{
				MouseRButtonPush = false;
				return;
			}

			tc = gmCharacters->GetCharacter(selectcharacterindex);
		}
		else
		{
			tc = Hero;
...
void SEASON3B::CNewUICursedTempleSystem::ReceiveCursedTempRegisterSkill(BYTE* ReceiveBuffer)
{
	...
	CHARACTER* sc = gmCharacters->GetCharacter(sourceobjindex);
	OBJECT* sco = &sc->Object;

	CHARACTER* tc = gmCharacters->GetCharacter(targetobjindex);
	...
	case AT_SKILL_CURSED_TEMPLE_PRODECTION:
	{
		...
		if (sc != Hero) SetAction(sco, PLAYER_HIT_REMOVAL);
